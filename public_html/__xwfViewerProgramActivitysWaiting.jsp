...
//    StringBuffer boql = new StringBuffer("SELECT xwfActivity STATE open OR wait WHERE program = ");
...
//    }
    ...
    wainting.append("&boql=").append(boql.toString());
//    wainting.append("&onclick=").append(onclick);
    wainting.append("&userClick=").append(onclick);
    wainting.append("&userDblClick=window;");
    wainting.append("&canSelectRows=no");
    wainting.append("&lineColorState=wait;open");
    wainting.append("&list_orderby=netgest.bo.runtime.sorter.ActivtiyDatesComparator");
%>
...
<title></title>
<script language="javascript">var objLabel="&nbsp;&nbsp;Lista de Actividades em Espera"</script>
</head>   

<body style='margin:0px' >
<form style='height:100%;' class="objectForm" name="boForm" id="<%=IDX%>">
    <IFRAME src='<%=wainting.toString()%>' frameBorder=0 width='100%' scrolling=no height='100%' ></IFRAME> 
</form>
