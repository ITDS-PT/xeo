...
try {
    ...
    docHTML_treeRuntime  tree=null;
    if((treeKey!=null) && ((treeKeyTemp==null) || ((treeDef!=null) && (treeDef.equals("")) && (mode.equals("CHANGE_DEF")))))
    {
       
       tree = docHTML_treeServer.getTree(treeKey);
        if(tree==null)
        {
          boDefHandler bodef = boDefHandler.getBoDefinition(request.getParameter("vobject"));
          tree=docHTML_treeServer.getTree(bodef, bodef.getViewer("general").getChildNode("forms").getChildNode(request.getParameter("vname")).getChildNode("treeView"), DOC);
        }
       tree = docHTML_treeServer.copyTreeToTemp(tree, DOC);
       treeKeyTemp = tree.p_key;
    }
    else if(treeKeyTemp!=null)
    {
      tree = docHTML_treeServer.getTree( treeKeyTemp );
      if(tree==null || mode.equals("CHANGE_DEF"))
      {
        tree = docHTML_treeServer.loadTree(boctx, Long.parseLong(treeDef), DOC);
        treeKeyTemp = tree.p_key;
      }
    }
    ...
    if ( selectedCols==null || selectedCols.equals(""))
    {
            ...
            {
                cols=new docHTML_treeAttribute[0];
                attrs=new docHTML_treeAttribute[0]; 
...
var objLabel='<%=JSPMessages.getString("ExportData.1")%>';
var objDescription='<%=JSPMessages.getString("ExportData.2")%>';
</script>
...
<TABLE style="WIDTH: 100%; HEIGHT: 100%" cellSpacing=0 cellPadding=8>
  <TBODY>
      ...
      <td width="90%" colSpan=2 align='left'><%=(tree==null)?"":tree.p_filterName%>
				...
				<option <%=(("".equals(treeDef))?"selected":"")%>><%=(tree==null)?JSPMessages.getString("ExportData.3"):JSPMessages.getString("ExportData.4")%></option>
...
<%
    if(tree!=null)
    {
        ...
        {
          out.print("["+tree.p_groups[i].getLabel() + "]&nbsp;&nbsp;&nbsp;");
        }
    }
%>
      </td>
...
<%    
    if(tree!=null)
    {
        ...
        tree.p_bodef.getName()+"&referenceFrame=___chooseExportData&reference="+
      ...
      <TABLE id=tblFind height="100%" cellSpacing=0 cellPadding=0 
        ...
        <TR>
          ...
          <TD align=middle width=300px><BUTTON id=btnAppend 
            ...
            <P><BUTTON id=btnRemove title="<%=JSPMessages.getString("ExportData.13")%>" 
...
<%
        ...
        if(mode.equals("Exportar"))
          out.print("winmain().openDocUrl('fixed,730px,450px','__choseExportList.jsp','?docid="+DOC.getDocIdx()+"&viewer="+tree.p_key+"','lookup');\n");
          
        //Impressão de Listas
        if(mode.equals("Imprimir"))
          out.print("winmain().open('__printTree.jsp?docid="+DOC.getDocIdx()+"&viewer="+tree.p_key+"','_blank');\n");

        //gravar definição
        if(mode.equals("Save"))
          out.print("winmain().openDocUrl('fixed,400px,150px,noresize','__saveTree.jsp','?docid="+DOC.getDocIdx()+"&viewer="+tree.p_key+"','lookup');\n");          
