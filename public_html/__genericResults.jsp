...
<script>
var objLabel='Lista de Objectos';</script>
...
<%}%>
<%if (scriptToRun==null) {%>
<body ondragenter="activethis()" onload="<%=DOC.getActionOnClient()%>">
<%}
else{%>
<body ondragenter="activethis()" onload="<%=scriptToRun%>">
<%
}
%>
<%if ( request.getParameter("boFormSubmitXml")!= null && request.getParameter("actRenderObj") != null &&BOI.exists()){%>
...
<form class="objectForm" name="boForm" id="<%=IDX%>">
   ...
   <%
BOI=( boObject )currObjectList.getObject();grid = DOC.getGRID("gSYSGENERIC");
if(grid==null) { 
  grid = DOC.createGRID("gSYSGENERIC","std");
  grid.setTitle();  
} //endCreategrid
...
<% } %>
<INPUT type='hidden' name='boFormSubmitMode' value="" />
<INPUT type='hidden' name='resultBridge' value="true" />
<INPUT type='hidden' value='1' name='boFormSubmitObjectType' />
<INPUT type='hidden' name='boFormSubmitId' />
<% if (currObjectList!=null && currObjectList.getParent()!=null) { %><INPUT type='hidden' name='BOUI' value='<%=currObjectList.getParent().getBoui()%>'/>
