...
try {
   ...
   // docParameter attr;
    ...
    String clientIDX            = request.getParameter("myIDX");
//    String templateBOUI         = request.getParameter("templateBOUI");
    String valueToSave          = request.getParameter("submitXml");
    String sprotocolObject       = request.getParameter("protocolObject");    
    String objConfigBoui        = request.getParameter("daoconfig");
    
    
  
    
    long boui_in_edit   =   netgest.utils.ClassUtils.convertToLong( objConfigBoui ,0);
    
   
    boObject configObject   = DOC.getObject( boui_in_edit );
        
    boObject OBJ=configObject;
    boObject activityObject = configObject.getParentBridgeRow().getObject();
    boObject protocolObject = DOC.getObject( ClassUtils.convertToLong( sprotocolObject )); 
    
    String objClass;
    if ( activityObject.getName().equalsIgnoreCase("Ebo_Template") )
    {
          
          boObject classReg       = activityObject.getAttribute("masterObjectClass").getObject();
          objClass         =  classReg.getAttribute("name").getValueString();
    }
    else
    {
        objClass         =   activityObject.getName();
    }
    ...
    if ( valueToSave !=null )
    {
          OBJ.getAttribute("conditionFormula").setValueString(valueToSave);
    }
    fTransform=OBJ.getAttribute("conditionFormula").getValueString();
    
        boDefHandler[] fromObjectsDef = new boDefHandler[ 1 ];
    fromObjectsDef[0] = netgest.bo.def.boDefHandler.getBoDefinition( objClass ); 
    %>
...
//formula="<formula><par>(</par><atr external='Solicita栯.Data Inicio'>rs.beginDate</atr><oper>+</oper><dateLiteral>10days</dateLiteral></formula>";
}
...
var objLabel='<span style="color:#FFFFFF">Condições para executar </span><b><%=activityObject.getAttribute("name").getValueString()%>';var objDescription='';
</script>
...
<table style='background-color:#FFFFFF' class="layout" cellpadding=0 cellspacing=0 border="0" width="100%">
		...
		<table class="mnubarFlat" id="mnuBar2" cellSpacing="0" cellPadding="0">
		      ...
		      <tr>
		        ...
		        <td class="icMenu" noWrap>
					<span class="menuFlat" title="Guardar condição" onclick="compoe_formula();" tabIndex="0"><img class="mnuBtn" src="templates/menu/std/16_save.gif" WIDTH="16" HEIGHT="16"> Guardar Condição</span>
							...
							<td>
							   <div id="grid$0$activitylist"  class="gContainerLines_std">
									<table id="g1643_body"  onmouseover="so('g1643');onOver_GridBody_std(event)" onmouseout="so('g1643');onOut_GridBody_std(event)" onclick="so('g1643');addRowAttribute()"  cellpadding="2" cellspacing="0" class="gBodyLines_std">
                                        ...
                                        {
                                            xDef=fromObjectsDef[0];
                                            netgest.bo.def.boDefAttribute[] attrDefs=xDef.getBoAttributes();
                                       ...
                                       // netgest.bo.runtime.AttributeHandler x;
                                        ...
                                        boDefHandler oxDef=fromObjectsDef[0];
                                        netgest.bo.def.boDefAttribute[] oattrDefs=xDef.getBoAttributes();
                                        
                                         // while (xenum.hasMoreElements())
                                          for (int i = 0; i < oattrDefs.length ; i++)
                                          {
                                           //x=(netgest.bo.runtime.AttributeHandler)xenum.nextElement();
                                            ...
                                            // )
                                             %>
										<tr>
												<td class="gCell_std" internalv="<%=oxDef.getName()%>"><%=oattrDefs[i].getLabel()%></td>
												<td class="gCell_std" internalv="<%=oxDef.getName()%>"><%=oxDef.getLabel()%></td>
										</tr>
										<%}%>
                                        
                                        <%
                                        //extendAttribute do template
                                        bridgeHandler bridgeHandler = protocolObject.getBridge("extendAttribute");
										...
										<tr>
												...
												<td class="gCell_std" internalv="<%="extendAttribute." + shortAlias%>"><%=fromObjectsDef[0].getLabel()%></td>
										</tr>
										<%           
                                            if(cadinalidade == 1)
                                            {
		...
		<table width='100%'>
			<tr>
				<td>
					...
					<span>&nbsp;</span>
                    <img style="cursor:hand" onclick="addSinal('=')" height='30' width='30' src="templates/form/std/formulaEditor/igual.gif" />
                    <span>&nbsp;</span>
                    <img style="cursor:hand" onclick="addSinal('>')" height='30' width='30' src="templates/form/std/formulaEditor/maiorque.gif" />
					<span>&nbsp;</span>
                    <img style="cursor:hand" onclick="addSinal('>=')" height='30' width='30' src="templates/form/std/formulaEditor/maiorigualque.gif" />
					<span>&nbsp;</span>
                    <img style="cursor:hand" onclick="addSinal('<')" height='30' width='30' src="templates/form/std/formulaEditor/menorque.gif" />
					<span>&nbsp;</span>
                    <img style="cursor:hand" onclick="addSinal('<=')" height='30' width='30' src="templates/form/std/formulaEditor/menorigualque.gif" />
					<span>&nbsp;</span>
				</td>
			</tr>
		</table>
  ...
  <tr height"55">
	...
	<td ><span class="headerChoice">Parentises e operadores logicos</span>
		<table width='100%'>
			<tr>
				<td>
					...
					<span>&nbsp;</span>
                    <img onclick="addParentises(' e ')" height='30' width='30' src="templates/form/std/formulaEditor/and.gif" />
					<span>&nbsp;</span>
                    <img onclick="addParentises(' ou ')" height='30' width='30' src="templates/form/std/formulaEditor/or.gif" />
					<span>&nbsp;</span>
                    <img onclick="addParentises(' ! ')" height='30' width='30' src="templates/form/std/formulaEditor/not.gif" />
					<span>&nbsp;</span>
				</td>
			</tr>
		</table>
...
<FORM name='boFormSubmit'  method='get'>
    ...
    <INPUT type='hidden' name='myIDX' value='<%=clientIDX%>' />
    
    <INPUT type='hidden' name='daoconfig' value='<%=objConfigBoui%>' />
    ...
    <INPUT type='hidden' value='<%=sprotocolObject%>' name='protocolObject' />
