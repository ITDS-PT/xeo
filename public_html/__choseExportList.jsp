...
try {
    ...
    {
      if(xtree.p_cols[i].p_bridgeInd>-1)
        b_cols.add(xtree.p_cols[i]);
    }
    ...
    if(csvmode!=null)
    {
      ...
      {
        docHTML_treeAttribute att = (docHTML_treeAttribute)b_cols.elementAt(i);
        String valAtt = request.getParameter("ATT_"+att.p_name);
        String onChk = request.getParameter("CHK_"+att.p_name);
        
        if((onChk!=null) && (onChk.equals("ON")))
          continue;
        
        if(valAtt!=null)
          cols.put(((docHTML_treeAttribute)b_cols.elementAt(i)).p_name, new Integer(valAtt));
      }
    }
   ...
   %>
    ...
    <HTML><HEAD><TITLE>Escolher Mode de Exporta袯</TITLE>
...
<script language="javascript">
  ...
  %>
    ...
    {
      window.open('__exportList.jsp?'+'<%=exp%>');
    }
	<%}%>
...
var objDescription='Escolha para que tipo de formato que esportar a listagem';
</script>
...
<form name="export" id="exportList" method=post action="__choseExportList.jsp">
...
<table border="0" cellspacing="0" width="100%" disabled style="border-collapse:collapse;display:none">
      ...
      <tr>
				<td width="100%" class="bar" align="left" colspan="5"><font size="3">Escolha o Número Máximo de Valores por Atributo</font></td>
			</tr>
      <tr>
...
%>            
            <tr>
							<td align="left"><input type="text" name="ATT_<%=att.p_name%>" value="1" size="4" class="text" onChange="validate(this);"></td>
						</tr>
<%
      }
      //**************
...
%>            
						<tr>
							<td align="center"><input type="checkbox" name="CHK_<%=att.p_name%>" value="ON" size="4" class="checkbox" onclick="checkAtt(this,<%=i%>);" ></td>
