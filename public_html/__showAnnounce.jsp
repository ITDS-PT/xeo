...
try {
    ...
    if(bosession== null) {
        response.sendRedirect("login.jsp?returnToPage=__showAnnounce.jsp");
        return;
    }
    if(boctx==null) {
    ...
    if ( operation != null )
    {
        ...
        {
            Ebo_AnnounceImpl.removeAnnounces( perf  , netgest.bo.impl.Ebo_AnnounceImpl.MY_MESSAGES );
        }
        else if ( operation.equalsIgnoreCase("mygroup") )
        {
            Ebo_AnnounceImpl.removeAnnounces( perf  , netgest.bo.impl.Ebo_AnnounceImpl.MY_GROUP_MESSAGES );
        }
        else if ( operation.equalsIgnoreCase("myrole") )
        {
            Ebo_AnnounceImpl.removeAnnounces( perf  , netgest.bo.impl.Ebo_AnnounceImpl.MY_ROLE_MESSAGES );
        }
        else if ( operation.equalsIgnoreCase("myworkqueue") )
        {
            Ebo_AnnounceImpl.removeAnnounces( perf  , netgest.bo.impl.Ebo_AnnounceImpl.MY_WORKQUEUE_MESSAGES );
        }
        else if ( operation.equalsIgnoreCase("all") )
        {
           Ebo_AnnounceImpl.removeAnnounces( perf  ,
               (byte) (Ebo_AnnounceImpl.MY_WORKQUEUE_MESSAGES+
                Ebo_AnnounceImpl.MY_GROUP_MESSAGES+
                Ebo_AnnounceImpl.MY_ROLE_MESSAGES+
                Ebo_AnnounceImpl.MY_MESSAGES)
           );
        }
...
<script>
var objLabel='<img align="absmiddle" hspace=3 src="resources/Ebo_AnnounceDetails/ico16.gif">Mensagens SGIS ';

function setAnnounce()
{

parent.announceArea.innerText='<%=netgest.bo.impl.Ebo_AnnounceImpl.getAnnounces( perf )%>';
}
    ...
    <tr height='100%'>
        <td  >
             ...
             <div style='border:1px solid #5991E4'width="100%" height="100%">
				<IFRAME id='myannounces' src='ebo_announcedetails_generalmyannounces.jsp?docid=<%=IDX%>&form=myAnnounces' frameBorder=0 width='100%' scrolling=no height='100%'></IFRAME>
             </div>
