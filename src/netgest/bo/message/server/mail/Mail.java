...
/**
 * This class allows to send a receive emails, using several IMAP,POP3 and SMTP Servers the structure of the email message must be 
 * compliant to the NETGEST table ND_MOV, this table stores all the email messages, the end user dont interact with an email server
 *  directly, instead he interacts with the database to see and send emails. This class his responsible to put emails in the database,
 *   and to send emails to the outside world using and SMTP.
 * 
 * @version 1.0
 * @author Enlace3, Lda
 * 
...
public class Mail {
	...
	// logger
	private static Logger logger = Logger.getLogger("netgest.bo.message.server.mail.Mail");

	/**
	 * Class that represents and email message internally in this Class, i.e a mail retrieved from a POP Server with JavaMail is parsed to fit this structure and then used internally in this class.
	 */
	/**
	...
	/**
	 * This method reads all emails for an user from a POP3/IMAP Server, transforms the emails in XML and saves it in the DataBase
	 * 
	 * @return <b>True</b> - if successfull <b>False</b> - if not Successfull
	 */
	public MailMessage[] readMail() throws RemoteException, MessagingException, IOException {
		return this.readMail(-1, null, false);
	}

	public MailMessage[] readMail(int numOfMessagesToRead, String foldername) throws RemoteException, MessagingException, IOException {
		return readMail(numOfMessagesToRead, foldername, false);
	}

	public MailMessage[] readMail(int numOfMessagesToRead, String foldername, boolean faxAccount) throws RemoteException, MessagingException, IOException {
		Folder folder = null;
	...
	public MailMessage setMessage(Session session, MimeMessage message, boolean faxAccount) throws Exception {
		MailMessage mailmsg = null;

		message = (MimeMessage) MailUtil.getEmailMessage(message);
		...
		// get message subject
		mailmsg.setSubject((message.getSubject() == null) ? null : MailUtil.convertString(message.getSubject()));
		...
		// get received date
		mailmsg.setReceivedDate((message.getReceivedDate() != null) ? message.getReceivedDate() : new Date());

		// get sent date
		mailmsg.setSentDate((message.getSentDate() != null) ? message.getSentDate() : new Date());
		...
		if (content instanceof Multipart) {
			if (message.isMimeType("application/pkcs7-mime")) {
				...
				try {
					...
					if (e != null && e.getMessage() != null && e.getMessage().toUpperCase().indexOf("MISSING START BOUNDARY") >= 0) {
		...
		// binary documents
		try {
			mailmsg.setBinaryMail(MailUtil.getBinaryMail("email.eml", "0", message));
		} catch (Exception e) {
		}
	...
	public void send(boolean saveEML) throws AddressException, MessagingException, IOException {
		...
		for (int k = 0; k < this.mailmessages.size(); k++) {
			...
			InternetAddress receipts = new InternetAddress(mailmsg.getReadReceiptEmail() == null ? mailmsg.getFrom().getFullAddress() : mailmsg.getReadReceiptEmail());
			...
			if (mailmsg.askForReadReceipt()) {
				...
				MailUtil.setAskReceipt(false, props, msg, receipts);
			}
