 ...
 * @since
 */
public class PostGreReaderAdapter implements ReaderAdapter
{
    /**
     *
     * @since
    ...
    public PostGreReaderAdapter(String dmlds)
    {
        dmlDs = dmlds;
    }
    ...
    private void _executeQuery(EboContext ctx, String query, List arguments,int page,int pagesize)
    {
        try
        {
            ...
            query=PostGreUtils.prepareSQLForPostGres(query);
        ...
        ResultSetMetaData rsltMetaData)
    {
        try
        {
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                columnName[i] = rsltMetaData.getColumnName(i + 1).toUpperCase();
