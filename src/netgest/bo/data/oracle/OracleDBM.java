...
public class OracleDBM
{
    ...
    public static final void hardClose()
    {
    ...
    public void createEbo_TemplateIndex(EboContext p_ctx, String schemaName )
    {
        ...
        try
        {
            if (!existsIndex(p_ctx, schemaName, "SYS_IM_EBO_TEMPLATE"))
            {
                ...
                dml.append("create index ").append(schemaName)
    ...
    private void createNgtdic(String newSchemaName) throws SQLException
    {
        try
        {
            ...
            dml.append("create table ").append(newSchemaName).append(".NGTDIC")
               .append("(").append("TABLENAME      VARCHAR2(50) not null,")
               .append("OBJECTNAME     VARCHAR2(50) not null,")
               .append("OBJECTTYPE     VARCHAR2(10) not null,")
               .append("REQUIRED       CHAR(1),")
               .append("FIELDSIZE      VARCHAR2(10),")
               .append("FIELDTYPE      VARCHAR2(50),")
               .append("FRIENDLYNAME   VARCHAR2(500),")
               .append("EXPRESSION     clob,")
               .append("RULES          VARCHAR2(255),")
               .append("PICTURE        VARCHAR2(200),")
               .append("TABLEREFERENCE VARCHAR2(100),")
               .append("FIELDREFERENCE VARCHAR2(255),")
               .append("MACROFIELD     VARCHAR2(50),")
               .append("SYS_ICN        NUMBER(7),")
               .append("SYS_USER       VARCHAR2(25),")
               .append("SYS_DTCREATE   DATE,").append("SYS_DTSAVE     DATE,")
               .append("DEFAULTVALUE   VARCHAR2(500),")
               .append("SEARCHDOMAIN   VARCHAR2(30),")
               .append("SCHEMA         VARCHAR2(30) not null,")
               .append("NGTTABLESPACE  VARCHAR2(30),")
               .append("NGTINITIAL     VARCHAR2(15),")
               .append("NGTNEXT        VARCHAR2(15),")
               .append("NGTPCTINCREASE VARCHAR2(15),")
               .append("NGTMINEXTENTS  VARCHAR2(15),")
               .append("NGTMAXEXTENTS  VARCHAR2(15),")
               .append("NGTPCTFREE     VARCHAR2(15),")
               .append("NGTPCTUSED     VARCHAR2(15),")
               .append("NGTINITRANS    VARCHAR2(15),")
               .append("NGTMAXTRANS    VARCHAR2(15),")
               .append("OWNER          VARCHAR2(30),")
               .append("DELETECASCADE  CHAR(1),")
               .append("CACHETTL       NUMBER(10) default -1").append(")");
            executeDDL(dml.toString(), name);
    ...
    private void createTextIndexJobAndProc()
    {
        StringBuffer dml = new StringBuffer();
        String name = p_ctx.getBoSession().getRepository().getName();
        ...
        try
        {
            pstm = p_ctx.getConnectionData().prepareStatement( "select * from user_procedures where object_name = 'P_REBUILDTEXTINDEX'" );
            rslt = pstm.executeQuery();
            if( !rslt.next() )
            {
                dml.append(
                    "create or replace procedure P_REBUILDTEXTINDEX is \n" +
                    "begin \n" +
                    "  ctx_ddl.sync_index('SYS_IM_EBO_TEXTINDEX'); \n" +
                    "end P_REBUILDTEXTINDEX; \n"
                );
                executeDDL(dml.toString(), name);
            }
            ...
            pstm = p_ctx.getConnectionData().prepareStatement( "select * from user_jobs where what like '%P_REBUILDTEXTINDEX;%'" );
            rslt = pstm.executeQuery();
            if( !rslt.next() )
            {
        ...
        throws SQLException
    {
        StringBuffer dml = new StringBuffer();
        String name = p_ctx.getBoSession().getRepository().getName();
        dml.append("create table ").append(newSchemaName).append(".ebo_textindex (text clob,")
        .append("SYS_ICN        NUMBER(7),")
        .append("SYS_USER       VARCHAR2(25),")
        .append("SYS_DTCREATE   DATE,")
        .append("SYS_DTSAVE     DATE,")
        .append("SYS_ORIGIN     VARCHAR2(30)")
        .append(")");
        executeDDL(dml.toString(), name);
    }
        ...
        String indexName) throws SQLException
    {
        ...
        try
        {
            ...
            pstm = cn.prepareStatement(
                    "select count(*) from USER_INDEXES where INDEX_NAME=? and TABLE_OWNER = ?");
...
//            pstm.close();
        ...
        String tableName) throws SQLException
    {
        ...
        try
        {
            cn = ctx.getDedicatedConnectionData();
            pstm = cn.prepareStatement(
                    "select count(*) from all_tables where TABLE_NAME=? and OWNER = ?");
        ...
        throws SQLException
    {
        ...
        try
        {
            ...
            if (objecttype.equalsIgnoreCase("T"))
            {
                ...
                String[] sysfdef =
                {
                    "VARCHAR2(25)", "NUMBER(7)",
                    "DATE DEFAULT sysdate", "DATE",
                    "VARCHAR2(30)"
                };
            ...
            if (objecttype.equalsIgnoreCase("F"))
            {
                ...
                {
                    dml = "ALTER TABLE " + node.getString("TABLENAME") +
                        " modify (\"" + node.getString("OBJECTNAME") + "\" ";
                }
                else if (modo.equals("3") && fldexi)
                {
                    ...
                    PreparedStatement pstmrelc = cn.prepareStatement(
                            "SELECT CONSTRAINT_NAME,CONSTRAINT_TYPE FROM USER_CONSTRAINTS WHERE CONSTRAINT_NAME IN (SELECT CONSTRAINT_NAME from user_cons_columns where table_name=? AND COLUMN_NAME=?)");
                ...
                if ((dml != null) && (dml.length() > 0) && !modo.equals("3"))
                {
                    ...
                    if ((mfield != null) &&
                            ...
                            !mfield.equals("CLOB") && !mfield.equals("BLOB") &&
                            !mfield.equals("MDATA")))
                    {
                        String ngtft = "";
            ...
            {
                PreparedStatement pstm = cn.prepareStatement(
                        "select COLUMN_NAME from User_Cons_Columns where table_name=? and constraint_name =? ORDER BY POSITION");
                pstm.setString(1, node.getString("TABLENAME"));
                pstm.setString(2, node.getString("OBJECTNAME"));
                ...
                boolean diff = !expression.toString().equals(node.getString(
                            "EXPRESSION"));
                rslt.close();
                pstm.close();
                ...
                if ((modo.equals("3") || diff) && exists) /* || (exists && !modo.equals("3")) */
                {
                    PreparedStatement pstmrefs = cn.prepareStatement(
                            "select constraint_name,table_name from USER_CONSTRAINTS WHERE R_constraint_name=?");
                    ...
                    pstmrefs = cn.prepareStatement(
                            "select constraint_name from User_Cons_Columns WHERE table_name=? and column_name in (" +
                            insql + ")");
                ...
                if (!modo.equals("3") && (!exists || diff))
                {
                    ...
                    else
                    {
                        dml = "alter table " + node.getString("TABLENAME") +
                            " add constraint " + node.getString("OBJECTNAME") +
                            " primary key (" + node.getString("EXPRESSION") +
                            ")";
                    }
            ...
            {
                PreparedStatement pstm = cn.prepareStatement(
                        "select column_name from user_cons_columns where constraint_name=? and table_name=? order by position");
                pstm.setString(1, node.getString("OBJECTNAME"));
            ...
            if (objecttype.startsWith("IDX"))
            {
                ...
                PreparedStatement pstm = cn.prepareStatement(
                        "select uniqueness from USER_INDEXES where index_name=? and table_name=?");
                pstm.setString(1, node.getString("OBJECTNAME"));
                pstm.setString(2, node.getString("TABLENAME"));
...
//    }
    ...
    public static String getNgtFieldTypeFromDDL(String oft) throws SQLException {
        if(oft.equals("VARCHAR2") || oft.equals("CHAR"))
              return "C";
        else if (oft.equals("DATE")) 
              return "D";
        else if (oft.equals("NUMBER")) 
              return "N";
        else if (oft.equals("CLOB")) 
              return "CL";
        else if (oft.equals("BLOB")) 
              return "BL";
        else if (oft.equals("BFILE")) 
              return "BF";
        else if (oft.equals("RAW")) 
              return "RAW";
    ...
    public static String getDDLFieldFromNGT(String fieldtype,String fieldlen) {
        ...
        if (fieldtype.equals("C"))  ngtdatatype="VARCHAR2";
        else if (fieldtype.equals("N")) ngtdatatype="NUMBER";
        else if (fieldtype.equals("D")) ngtdatatype="DATE";
        else if (fieldtype.equals("CL")) ngtdatatype="CLOB";
        else if (fieldtype.equals("BL")) ngtdatatype="BLOB";
        else if (fieldtype.equals("BF")) ngtdatatype="BFILE";
        else if (fieldtype.equals("RAW")) ngtdatatype="RAW";
