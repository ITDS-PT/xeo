 ...
 * 
 */
public class SqlServerReaderAdapter implements ReaderAdapter {
	...
	public SqlServerReaderAdapter(String dmlds) {
		dmlDs = dmlds;
	}
	...
	private void _executeQuery(EboContext ctx, String query, List arguments,int page,int pagesize) {
		try {
            ...
            else activePstm.setMaxRows(page*pagesize+toadd);
	...
	public boolean fetchRow(DataRow row, int rows) {
		...
		try {
			...
			for (int i = 0; ((rows == -1) || (i < rows)) && activeRslt.next(); i++) {
				ret = true;

				for (short z = 1; z <= meta.getColumnCount(); z++) {
					if (columnIdx[z] == 0) {
						columnIdx[z] = row.getDataSet().findColumn(
								meta.getColumnLabel(z));
					}
					if (columnIdx[z] > 0) {
						...
						case Types.CLOB:
							...
							if (srcclob != null) {
								...
								DataClob desclob = new DataClob(reader, 8192);
								srcclob = null;
	...
	public static final DataSetMetaData createDataSetMetaData(
		...
		try {
			...
			for (int i = 0; i < count; i++) {
				...
				columnName[i] = rsltMetaData.getColumnLabel(i + 1)
						.toUpperCase();
				...
				if (className.equals("java.lang.String")) {
					...
					columnDisplaySize[i] = rsltMetaData
							.getColumnDisplaySize(i + 1);
					columnType[i] = DataTypes.VARCHAR;
				}
				else if (className.equals("java.lang.Short")) {
					columnClassName[i] = "java.math.Short";
