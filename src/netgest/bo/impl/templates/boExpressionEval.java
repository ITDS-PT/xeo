...
{
    //logger
    private static Logger logger = Logger.getLogger("netgest.bo.impl.templates.boExpressionEval");
    ...
    //        
    //    }
    public static void setExpressionAttribute(Hashtable relobj, org.w3c.dom.Node xmlexpression, AttributeHandler att, byte typeInput)
    {
        try
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    if (att.getDefAttribute().getAtributeType() == boDefAttribute.TYPE_ATTRIBUTE)
                    {
                        ...
                        {
                            att.setValueDate(new Date(Long.parseLong(sval)),typeInput);
                        }
                        else
                        {
                            att.setValueString(sval, typeInput );
                        }
                    }
                    else
                    {
    ...
    public static Object evalExpression(Hashtable relobj, org.w3c.dom.Node xmlexpression)
        throws boExpressionException
    {
    ...
    public static String processExpression(Hashtable relobj, org.w3c.dom.Node xmlexpression)
        throws boExpressionException
    {
        LinkedList tkns = netgest.bo.impl.templates.XMLExpressionTokenizer.getTokens(xmlexpression);
        ...
        for (ptr = 0; ptr < tkns.size(); ptr++)
        {
            ...
            switch (tk.getType())
            {
            case netgest.bo.impl.templates.XMLExpressionTokenizer.VALUE_NUMBER:
                ptr = processNUMBER(ptr, tkns, executeStack);

                break;

            case netgest.bo.impl.templates.XMLExpressionTokenizer.VALUE_ATTRIBUTE:
                ptr = processATTRIBUTE(ptr, tkns, executeStack, relobj);

                break;

            case netgest.bo.impl.templates.XMLExpressionTokenizer.VALUE_OPERATOR:
            case netgest.bo.impl.templates.XMLExpressionTokenizer.VALUE_PARENTESES:
                ptr = processOPERATOR(ptr, tkns, executeStack);

                break;

            case netgest.bo.impl.templates.XMLExpressionTokenizer.VALUE_DATELITERAL:
                ptr = processDATELITERAL(ptr, tkns, executeStack);

                break;

            case netgest.bo.impl.templates.XMLExpressionTokenizer.VALUE_TEXT:
    ...
    private static int processDATELITERAL(int ptr, LinkedList tkn, ArrayList executeStack)
    {
        ...
        if ((wtkns.size() == 2) && (((CodeTokenizer.Token) wtkns.get(0)).getType() == CodeTokenizer.NUMBER) &&
    ...
    private static int processATTRIBUTE(int ptr, LinkedList tkn, ArrayList executeStack, Hashtable ctxobjs)
    {
        ...
        if (!"extendAttribute".equals(xatt[0]))
        {
            ...
            if ((ctxobj != null) && ctxobj.getBoDefinition().hasAttribute(att))
            {
                ...
                if ( value == null &&
                        (
                        objatt.getDefAttribute().getValueType() == boDefAttribute.VALUE_DATE
                        || objatt.getDefAttribute().getValueType() == boDefAttribute.VALUE_DATETIME
                        )
                        )
                {
                    ptr=-1;
                ...
                {
                executeStack.add(processValue(ptr,value,objatt,fromparent));
                }
                */
                String toadd = "";
        ...
        else
        {
            try
            {
                bridgeHandler bHandler = ctxobj.getBridge("extendAttribute");
                boObject extAttrObj = null;
                ...
                if (bHandler != null)
                {
                    ...
                    while (bHandler.next() && !found)
                    {
                        extAttrObj = bHandler.getObject();
                        shortAlias = extAttrObj.getAttribute("shortAlias").getValueString();

                        if (shortAlias.equals(xatt[1]))
                        {
                            found = true;
                        }
                    }
                    ...
                    if (extAttrObj != null)
                    {
                        long cadinalidade = extAttrObj.getAttribute("attributeCardinal").getValueLong();
                        String valueName = DocWfHTML.getExtendAttributeName(extAttrObj);
                        if (cadinalidade == 1)
                        {
                            long attributeType = extAttrObj.getAttribute("attributeType").getValueLong();
                            if(attributeType == 0)
                            {
                                String valueObjectBoui = extAttrObj.getAttribute(valueName).getValueString();
    
                                if (!"".equals(valueObjectBoui))
                                {
                                    valueObject = boObject.getBoManager().loadObject(ctxobj.getEboContext(), Long.parseLong(valueObjectBoui));
    
                                    if (valueObject != null)
                                    {
                                        AttributeHandler objatt = null;
                                        Object value = null;
    
                                        if (!xatt[1].equals(xatt[2]))
                                        {
                                            objatt = valueObject.getAttribute(xatt[2]);
                                            value = objatt.getValueObject();
                                        }
                                        else
                                        {
                                            objatt = valueObject.getAttribute("BOUI");
                                            value = valueObjectBoui;
                                        }
                            ...
                            {
                                String value = extAttrObj.getAttribute(valueName).getValueString();
                                if (!"".equals(value))
                                {
                                    executeStack.add(processValue(ptr, value, extAttrObj.getAttribute(valueName), fromparent));
                                }
                            }
                        }
                        else
                        {
                            String list = "";
                            bridgeHandler valueListHandler = extAttrObj.getBridge("valueList");
                            valueListHandler.beforeFirst();

                            while (valueListHandler.next())
                            {
                                valueObject = valueListHandler.getObject();
                                list += valueObject.getBoui();

                                if (!valueListHandler.getRslt().isLast())
                                {
                                    list += ";";
                                }
                            }

                            if (!valueListHandler.isEmpty())
                            {
                                executeStack.add(processValue(ptr, list, valueObject.getAttribute("BOUI"), fromparent));
                            }
                        }
                    }
                }
    ...
    private static String processValue(int ptr, Object value, AttributeHandler objatt, boolean fromparent)
    {
        String toadd = "";
