...
/**o parser que chamou este producer*/
     ...
     * Este tipo de árvores tem na cabeça uma String com o nome do objecto seleccionado e os seus filhos são do tipo boDef_Attribute
     * */
 ...
 *  */
   ...
   * */ 
    ...
    protected IQLObject getObjectDef() {
        return selObj;  
    }
    
    protected IQLObject objectSEL()
    {
        int i = 0;
        if(selobjPos<0)
        {
          selObj = (IQLObject)selectRules.get(0); //guarda referencia para o objecto
          i = 1;
        }
        else
          selObj = (IQLObject)selectRules.get(selobjPos); //guarda referencia para o objecto    
    ...
    {   
        ...
        objTable  = selObj.getNativeName( this.ext_obj );

        selectC = "SELECT "+hint+" ";          //constroi a respectiva clausla SELECT do SQL
        for(; i < selectRules.size(); i++)
        {
          if(i == selobjPos) {
        	  IQLObjectAttribute[] atts = selObj.getDefaultAttributes();
        	  for( int k=0; k < atts.length; k++ ) {
        		  
        		  if( k > 0 )
        			  selectC += ",";
        		  
        		  if( atts[k].getValueType() != IQLObjectAttribute.VALUE_UNKNOWN )
        			  selectC += objTable+"."+atts[k].getNativeName();
        		  else
        			  selectC += atts[k].getNativeName();
        ...
        //TODO: Security
        boDefHandler defSelObj = boDefHandler.getBoDefinition( selObj.getName() );
        if( defSelObj != null )
        {
            checkSecClassName( defSelObj );
    ...
    protected void addSecurityE(IQLObject obj)
    {
        //TODO: Tentar perceber por que é que é verificado se o objecto é abstrato, pois paeceme que nao pode ter
        // sergurança a instancia de se for abstracto.
        //&& obj.getClassType()!=boDefHandler.TYPE_ABSTRACT_CLASS implementado no provider.
        if(security && obj.implementsSecurityRowObjects() )
        {
            long[] ks;
            try{
            ...
            IQLObjectAttribute boAk = obj.getAttributeResolver().resolveAttribute(getExt(), "KEYS");
            IQLObjectAttribute boAkp = obj.getAttributeResolver().resolveAttribute(getExt(), "KEYS_PERMISSIONS");
            
            addChild( boAk.getRelation() , new Boolean(false), new Boolean(false), 0);
            addChild( boAkp.getRelation() , new Boolean(false), new Boolean(false), 0);
            for(int i=0; i < ks.length; i++)
            {
                keyRules.add( boAk.getRelation().getRelatedObject().getNativeName( this.ext_obj )+".CHILD$="+ks[i] );
                keyRules.add( boAkp.getRelation().getRelatedObject().getNativeName( this.ext_obj )+".CHILD$="+ks[i] );
            }
            keyRules.add(boAk.getRelation().getRelatedObject().getNativeName( this.ext_obj )+".CHILD$ is null");
        }
    }
    /**Adiciona os mecanismos de segurança para o referido objecto. Na prática adiciona as tabelas de chaves e impõe as condições no vector <tt>keysRules</tt>
     * @param obj    referencia para o objecto a ser analizado*/
    protected void addSecurity(IQLObject obj, boolean sel_obj)
    {
        ...
        if(security && obj.implementsSecurityRowObjects() )
        {
            ...
            String kTab = boAk.getRelation().getRelatedObject().getNativeName( this.ext_obj );
            String rule = "EXISTS (SELECT " + kTab+".CHILD$ FROM ";
            rule+= kTab + " WHERE " + kTab+".PARENT$ = ";
            
            rule += obj.getNativeName( this.ext_obj );            
            existsInFromTree( obj.getNativeName( this.ext_obj ) );
...
//            keyRules.add(boAkp.getBridge().getBoMasterTable()+".CHILD$ is null");
    ...
    * */
    ...
    protected abstract void prodFrom();
...
/*    protected void prodFrom()
    {
        ...
        if(root!=null && root.getName().equalsIgnoreCase(objTable) || !existsInFromTree(objTable)) //se o obejcto seleccionado não existir na árvore incluir na clausula from
        {
            fromC = "FROM "+objTable+" ";
            base_tab = objTable;
        }
...
*/    
    ...
    */
     ...
     * */
    protected void addOrderRule(String ref)
    {
        needOrder = true;
        orderRules.add(ref);
        if(groupRules.size() > 0)
    ...
    * */
    protected int setRootFrom(IQLObject qlobj)
    {
        ...
        {
            rootTab = (IQLObject)getRoot(i);
            if(rootTab.getName().equalsIgnoreCase( qlobj.getName() ))
                return i;
        }
        fromTree.add( new DefaultMutableTreeNode( qlobj ) );
        return i;
    }
    ...
    * */
    protected boolean addChild(IQLObjectRelation relation, Boolean need, Boolean inner, int pos)
    {
        IQLObject parent=relation.getLocalObject();
        IQLObjectAttribute newChild=relation.getParent();
        
        if(parent==null || newChild==null)  //ter s certeza que os dados de entrada estão correctos
            return false;
            
        //nomde das tabelas do filho e do pai            
        IQLObject refChild = newChild.getRelation().getRelatedObject();
        //boDefHandler boChild = boDefHandler.getBoDefinition(refChild);
        String schild = refChild.getNativeName( this.ext_obj );

//        boDefHandler boParent = boDefHandler.getBoDefinition(parent);
//        String realParent = parent.getNativeName();
        
        if(pos >= fromTree.size())  //caso ainda não tenha sido criada a árvore
        {
            fromTree.add( new DefaultMutableTreeNode( parent ) );
        }
        ...
        IQLObject s = ((IQLObject)node.getUserObject());  //a raiz da árvore é uma string
        boolean stop = false;
        while(!s.getName().equalsIgnoreCase( parent.getName() ) && !stop)  //achar na árvore o pai
        {
...
//                 }
            ...
            {
                s = ((IQLObjectRelation)((Triple)child.getUserObject()).getFirst()).getRelatedObject();
            }
        }
        node = child;
        int i;
...
*/      for(i=0; i < node.getChildCount(); i++) //ver se o pai já não possui um filho igual ao que queremos introduzir
        {
            ...
            IQLObjectRelation xrel = (IQLObjectRelation)((Triple)child.getUserObject()).getFirst();
            s = xrel.getRelatedObject();
            //s = boDefHandler.getBoDefinition(s).getBoMasterTable();
            boolean show = ((Boolean)((Triple)child.getUserObject()).getSecond()).booleanValue();
            if( s.getNativeName( this.ext_obj ).equalsIgnoreCase(schild) )
              if(xrel.getParent().getName().equalsIgnoreCase(newChild.getName()))
                if( newChild.isRelation() )
                {
                    if(xrel.getRelatedObject().getNativeName( this.ext_obj ).equalsIgnoreCase(newChild.getRelation().getRelatedObject().getNativeName( this.ext_obj )))
                    {
        ...
        node.add( new DefaultMutableTreeNode( new Triple( relation , need, inner ) ) );
    ...
    protected boolean existsInFromTree(String n)
    {
        ...
        IQLObject s = (IQLObject)node.getUserObject();
        if(s==null)    
            return false;
        else
        ...
        while(node!=null)       //procurar até percorrermos toda a árvore
        {
            ...
            att = (IQLObjectRelation)((Triple)node.getUserObject()).getFirst();

            String attTab  = null;
            attTab  = att.getRelatedObject().getNativeName( this.ext_obj );
     ...
     */
    protected void addFirstSelectRule(IQLObject rule, boolean withboui)
    {
      if(withboui)
     ...
     */ 
    protected IQLObject getRoot(int pos)
    {
        ...
        if(fromTree.size() > pos)
        {
            ...
            if(root != null)
                return (IQLObject)root.getUserObject();
            else
                return null;
        }
        else
