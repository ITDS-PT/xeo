...
{
    //logger
    private static Logger logger = Logger.getLogger("netgest.bo.system.boPoolManager");
    ...
    public ArrayList SYNCHRONIZED_POOLCLEAN = new ArrayList(1);
    
    // Init
    {
    	SYNCHRONIZED_POOLCLEAN.add( null );
    }
    
//    private static int THREAD_POOL_TARGET_SIZE = 3000;
    
//    private static final int THREAD_POOL_SHRINK_TARGET = 1000;
    ...
    // cada Owner tem uma entrada nesta Hastable , com outra hashtable com o BOUI ( no caso dos objects )

    public WeakHashMap   TimeoutContext = new WeakHashMap();
    // que por sua vez tem um ArrayList com as referencias que apontam pra  o mesmo objecto
    
    public  Hashtable    OwnedObjects   = new Hashtable();
    // objectos com a key Owner , ie todos os objectos que aquele owner tem (Arraylist de uniqueIDS )
    ...
    public  final void putObject(boPoolable object,Object[] keys, boolean release) 
    {
    ...
    private  final void putInMemoryArchive( String owner, boPoolable object )
    {
        ...
        ObjectPoolUSERNAMES.put( object.poolUniqueId() , ctx.getBoSession().getUser().getName() );
...
//            shrinkThreadObjectPool(list);
    ...
    private final void shrinkGlobalObjectPool() {
        ...
        if( ObjectPool.size() > GLOBAL_POOL_TARGET_SIZE ) {
        	
    		if ( System.currentTimeMillis() - GLOBAL_POOL_LASTCLEAN > 5000 ) {
    			clean = true;
    		}
        }
        else {
    		if ( System.currentTimeMillis() - GLOBAL_POOL_LASTCLEAN > 300000 ) {
    			clean = true;
    		}
        }
        if( clean && SYNCHRONIZED_POOLCLEAN.get(0) == null ) {
    		Object key;
    		synchronized ( SYNCHRONIZED_POOLCLEAN ) {
    			try 
    			{
	    			SYNCHRONIZED_POOLCLEAN.set( 0, boPoolManager.class );
    			
    			GLOBAL_POOL_LASTCLEAN = System.currentTimeMillis();
    			
		        	ArrayList removeList;
        	
		    		synchronized ( ObjectPool ) {
			        	removeList = new ArrayList();
				    	Enumeration enumKeys = ObjectPool.keys();
				    	while( enumKeys.hasMoreElements() ) {
				    		key = enumKeys.nextElement();
	    		boPoolable p = (boPoolable)ObjectPool.get( key );
	    		if( p != null && !p.poolIsStateFull() ) {
	    			removeList.add( key );
		    		if( removeList.size() > GLOBAL_POOL_SHRINK_TARGET ) {
		    			break;
		    		}
	    		}
	    	}
	    	...
	    	boPoolable	objectToWeak;
	    	while( removeIt.hasNext() ) {
	    		key = removeIt.next();
	    		objectToWeak = (boPoolable)ObjectPool.remove( key );
	    		if( objectToWeak != null ) {
	    			objectToWeak.isWeak = true;
	    			WeakObjectPool.put( key, new PoolWeakReference( objectToWeak.keys, objectToWeak, WeakObjectQueue ) );
	    		}
	    	}

	    	PoolWeakReference purgeRef;
	    	while( (purgeRef = (PoolWeakReference)WeakObjectQueue.poll()) != null ) {
    			WeakObjectPool.remove( purgeRef.uniqueId );
    			ObjectPoolUSERNAMES.remove( purgeRef.uniqueId );
    			if( purgeRef.owners != null && purgeRef.owners.size() > 0 ) {
    				Iterator ownerIt = purgeRef.owners.iterator();
    				while( ownerIt.hasNext() ) {
	    				removeObjectKeysFromOwner(  purgeRef.uniqueId, purgeRef.keys, ownerIt.next() );
    				}
    			}
	    	}
	    	
		    	if( ObjectPool.size() > (GLOBAL_POOL_TARGET_SIZE-GLOBAL_POOL_SHRINK_TARGET) ) {
		    		GLOBAL_POOL_TARGET_SIZE = ObjectPool.size() + GLOBAL_POOL_SHRINK_TARGET;
		    	}
    		}
    			finally {
    				SYNCHRONIZED_POOLCLEAN.set( 0 , null );
    			}
    		}
        }
    }
    ...
    private  final boPoolable _getObject( EboContext boctx, String owner, Object key )
    {
    ...
    private  final boPoolable __getObject( EboContext boctx, String owner, Object key ) 
    {
        boPoolable ret = null;
        ...
        {
        	synchronized ( keysht ) {
            ArrayList list = (ArrayList)keysht.get( key ); // saca as referencias 
            if ( list!= null  )
            {
                int size = list.size();
                for (int i = 0; i <  size ; i++) // verifica se alguma referencia está livre
                {
                    ret = (boPoolable)getFromMemoryArchive( boctx, owner, (String)list.get( i ) );
                    if( ret != null )
                    {
                        return ret;
                    }
    ...
    private  final boPoolable getFromMemoryArchive(EboContext ctx, String owner , String uniqueId )
    {
        ...
        if( ret != null && ret.keys != null ) {
        	if( ret.owners != null && ret.owners.size() > 0 ) {
        		...
        		while( it.hasNext() ) {
        			...
        			removeObjectKeysFromOwner( ret.poolUniqueId(), ret.keys, ownerKey );
        		}
        	}
    ...
    private final void removeObjectKeysFromOwner( Object uniqueId, Object[] keys, Object ownerKey ) {
                ...
                if( object != null )
                {
                    ...
                    { 
                        object.owners.remove( owner );
	                        removeObjectKeysFromOwner(object.poolUniqueId(), object.keys, owner);
                        object.owners.remove( ctx.poolUniqueId() );
	                    	removeObjectKeysFromOwner(object.poolUniqueId(), object.keys, ctx.poolUniqueId() );
	                    	
                        if( !ctx.poolUniqueId().equals( ctx.getPreferredPoolObjectOwner() ) )
                        {
                            object.owners.remove( ctx.getPreferredPoolObjectOwner() );
	                        	removeObjectKeysFromOwner(object.poolUniqueId(), object.keys, ctx.getPreferredPoolObjectOwner());
                        }
...
//                    }
        ...
        if( objectToRelease != null )
        {
        	...
        	while( it.hasNext() ) {
                ...
                if( objtorem != null )
                {
                    ...
                    {
                        ObjectPool.remove( objtorem.poolUniqueId() );
    ...
    private static class PoolWeakReference extends WeakReference {
    	...
    	List   	owners;
    	String	uniqueId;
    	public PoolWeakReference( Object[] keys, boPoolable referent, ReferenceQueue queue ) {
    ...
    public  final void  dumpPool( Writer toRet ) throws boRuntimeException, IOException
    {
        ...
        while ( xobjpool.hasMoreElements() )
        {
            ...
            toRet.write("<br>"+key+" -> "+bo.getBoui()+" <span style='color:blue'>"+bo.toString() +"</span> "+( bo.IsStateFull?"<b>statefull</b>":"")+( bo.isChanged()?"<b> changed</b>":"") +( o.ownerContext!=null ? " context <b> "+o.ownerContext+"</b> ) ":"")+" owner "+o.owners.toString()+"<span style='font:8px arial;color:green' >"+username+"</span>");
            if( o.getEboContext() != null )
            {
            	toRet.write("<span style='color:red'>"+o.getEboContext().poolUniqueId()+"</span>");
            }
            
            }
            else
            {
        	toRet.write("<br>"+key+" -> "+ ( o.ownerContext!=null ? " context <b> "+o.ownerContext+"</b> ) ":"")+" owner "+o.owners.toString()+"<span style='font:8px arial;color:green' >"+username+"</span>");
             if( o.getEboContext() != null )
            {
              toRet.write("<span style='color:red'>"+o.getEboContext().poolUniqueId()+"</span>");
            }
        ...
        while ( xx.hasMoreElements() )
        {
            ...
            if( xo!= null)
            {
            	toRet.write("<span style='color:green'>OWN :"+xo.toString()+"</span>");
            }
            toRet.write("<br>");
        }
    }
