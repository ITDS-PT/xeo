...
public class boAgentsControllerQuartz implements IboAgentsController
{
  //logger
    ...
    private Hashtable    activeQuartzTimers     = null; // Array with EJBTimers;


    private boApplication p_boapp;

    public boAgentsControllerQuartz( boApplication boapp )
    {
      this.p_boapp = boapp;
      this.activeQuartzTimers = new Hashtable();        
    ...
    {
        if( activeQuartzTimers.get( name ) != null )
        {
        	Object[] ret = new Object[1];
        	ret[0] = activeQuartzTimers.get( name );
    ...
    public void chekAndStartThread( int id_idx )
    {
        ...
        if (name!=null && !name.equals("") && ejbName!=null && !ejbName.equals(""))
        {
            try
            {
             ...
             //   this.activeQuartzTimers.put(name,timerejb);
            }
            finally
            {
    ...
    public boolean isAlive() {
        if (this.activeQuartzTimers!=null && !this.activeQuartzTimers.isEmpty())
            return true;
        else 
            return false;
    }
    ...
    {
        Enumeration oEnum = activeQuartzTimers.keys();
        while ( oEnum.hasMoreElements() )
        {            
            String currName = (String)oEnum.nextElement();            
            xeoEJBTimer currTimer=(xeoEJBTimer)this.activeQuartzTimers.get(currName);
            suspendAgent(currName);
            activeQuartzTimers.remove(currName);
    ...
    public void suspendAgent(String name)
    {
        try
        {
            xeoEJBTimer activeEJBTimer=(xeoEJBTimer)this.activeQuartzTimers.get(name);

            logger.finest(LoggerMessageLocalizer.getMessage("STOPPING_XEO_EJBTIMER")+" ["+ name +"]");
            activeEJBTimer.suspend(name);    
            activeQuartzTimers.remove(name);
