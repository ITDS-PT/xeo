...
public  class messageSgis extends message implements  Serializable {  
    ...
    public messageSgis() {
        ...
        bo_definition   = boDefHandler.getBoDefinition("messageSgis");
    ...
    public void init() throws boRuntimeException {
        ...
        if(p_mode == MODE_NEW && !p_clone)
        {
            ...
            if("messageSgis".equals(this.getName()))
            {
...
public  boolean disableWhen() throws boRuntimeException {
try {
if (netgest.bo.utils.Calculate.compare(this.getParent().getAttribute("preferedMedia").getValueString(), "Fax", 2)) 
{
return false;
} else {
return true;
...
return new String[] {"to", "toRef", "preferedMedia", "letterFormat", "faxFormat", "from", "statusMessage"} ;
}
...
this.faxFormat.setValueFormula(table, getDependences("faxFormat"));
}
} else {
...
this.faxFormat.setValueFormula(new Hashtable() , getDependences("faxFormat"));
}
}
}
