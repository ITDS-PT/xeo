...
public  class Ebo_Versioning extends boObject implements  Serializable {  

   
   
   
    public netgest.bo.runtime.attributes.boAttributeNumber changedObject;    
    ...
    public Ebo_Versioning() {
        super();
        bo_version      = "1.0";
        bo_name         = "Ebo_Versioning";
        bo_classregboui = "#BO.CLSREGBOUI#";
        bo_definition   = boDefHandler.getBoDefinition("Ebo_Versioning");
        ...
        atts.add(new boBridgeMasterAttribute(this,this.getBoDefinition().getAttributeRef("log"))); 

       
        
       
        atts.add(changedObject = new netgest.bo.runtime.attributes.boAttributeNumber(this,"changedObject"));
    ...
    public void init() throws boRuntimeException {
        ...
        if(p_mode == MODE_NEW && !p_clone)
        {
                ...
                if( changedObject.haveDefaultValue()) 
                {
                	if(changedObject.defaultValue() != null)
                	{
                    	changedObject._setValue(boConvertUtils.convertToBigDecimal(changedObject.defaultValue(), changedObject));
                    }
                    else
                    {
                    	changedObject._setValue(null);
                    }
                    changedObject.setInputType( AttributeHandler.INPUT_FROM_DEFAULT );
                }
            ...
            if("Ebo_Versioning".equals(this.getName()))
            {
    ...
    public bridgeHandler log()
    {
        return getBridge("log");        
    }
    ...
    public bridgeHandler getBridge( String bridgeName ) 
    {
        ...
        if( ret == null )
        {
        	...
        	if( bridgeName.equals("log") && (ret=brig.get("log"))==null )
        	{
            	brig.add( ret=new BridgeHandlerlog(new DataResultSet( getDataRow().getRecordChild( getEboContext() ,"log") ) , this ) );        
        	}
        	if( ret == null )
        	{
    ...
    public final class BridgeHandlerlog extends bridgeHandler {
        ...
        public BridgeHandlerlog(DataResultSet data,boObject parent) {
            super("log",
                  data,
                  parent.getBoDefinition().getAttributeRef("log").getBridge().getChildFieldName(),
                  parent
                 );

            p_fatherfield = parent.getBoDefinition().getAttributeRef("log").getBridge().getFatherFieldName();
            p_childfield  = parent.getBoDefinition().getAttributeRef("log").getBridge().getChildFieldName();
...
//			  if( newval != null ) allow=allow && att.onBeforeAdd(new boEvent(att, att.EVENT_BEFORE_ADD,newval));
			  ...
			  if( att.fireBeforeChangeEvent( att, null, newval) ) 
			  {
                  ...
                  if(getParent().onChangeSubmit("log"))
                  {
                      getParent().calculateFormula("log");
                  }
...
//					att.onAfterChange(new boEvent(att,att.EVENT_AFTER_CHANGE,chgval,null));
		...
		{
			return new BridgeHanlderlogRow( this, row );
		}

		public final class BridgeHanlderlogRow extends boBridgeRow
		{
			public BridgeHanlderlogRow( bridgeHandler bridge, netgest.bo.data.DataRow row )
			{
				super( bridge , row );
				addLine();
			}
			...
			public void addLine() 
			{
				...
				if(this.getAttribute("log")==null)
					xatt.add( new Handlerlog( getParent(), this ) );
...
return null;
        ...
        public final class Handlerlog extends BridgeObjAttributeHandler {
            public Handlerlog( boObject parent , boBridgeRow bridgerow ) {
                super( parent , bridgerow , bridgerow.getBridge().getDefAttribute().getName().equals("log")?parent.getBoDefinition().getAttributeRef("log"):bridgerow.getBridge().getDefAttribute().getBridge().getAttributeRef("log") );
            }
            public void setValueString(String value) {
                try {
                    ...
                    if(getParent().onChangeSubmit("log"))
                    {
                        ...
                        getParent().setCalculated(table, "log");
                        getParent().calculateFormula(table, "log");
                    }
                } catch (Exception e) {
            ...
            public String getValueString() throws boRuntimeException {
                try {
                    ...
                    String[] args = {getParent().getName(),"log"};
            ...
            public void setValueFormula(Hashtable table, String[]dependence) throws boRuntimeException
            {
                ...
                {                       
                    if(getParent().isWaiting(table, "log"))
                    {
                       getParent().setCalculated(table, "log");
                       this.setValue(boConvertUtils.convertToBigDecimal(formula(), this), false );
                    }
                    else if(!getParent().isCalculated(table, "log"))
                    {
                        getParent().setCalculated(table, "log");
                ...
                {   
                    ...
                    if(getParent().onChangeSubmit("log"))
                    {                 
                        getParent().setWaiting(table, "log");
                        getParent().calculateFormula(new Hashtable(table), "log");
                        //getParent().setCalculated(table, "log");
                    }                    
                }
            }
            ...
            public void setValue(BigDecimal newval, boolean recalc) throws boRuntimeException 
            {
                ...
                if( recalc && getParent().onChangeSubmit("log"))
                {
                    Hashtable table = new Hashtable();
                    getParent().setCalculated(table, "log");
                    getParent().calculateFormula(table, "log");
				...
				{
					getParent().getParentBridgeRow().getBridge().lineChanged("log");
				}
            }
...
	public boolean rollback() throws boRuntimeException {
