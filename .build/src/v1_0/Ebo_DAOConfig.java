...
public  class Ebo_DAOConfig extends boObject implements  Serializable {  

   
   
   
    public netgest.bo.runtime.attributes.boAttributeString logicOperator;    
    public netgest.bo.runtime.attributes.boAttributeString conditionFormula;    
    ...
    public Ebo_DAOConfig() {
        super();
        bo_version      = "1.0";
        bo_name         = "Ebo_DAOConfig";
        bo_classregboui = "#BO.CLSREGBOUI#";
        bo_definition   = boDefHandler.getBoDefinition("Ebo_DAOConfig");
        ...
        atts.add(new boBridgeMasterAttribute(this,this.getBoDefinition().getAttributeRef("extAtt_parameter")));  
        atts.add(new boBridgeMasterAttribute(this,this.getBoDefinition().getAttributeRef("linedepends"))); 

       
        
       
        atts.add(logicOperator = new netgest.bo.runtime.attributes.boAttributeString(this,"logicOperator"));
        
        atts.add(conditionFormula = new netgest.bo.runtime.attributes.boAttributeString(this,"conditionFormula"));
    ...
    public void init() throws boRuntimeException {
        ...
        if(p_mode == MODE_NEW && !p_clone)
        {
                ...
                if( logicOperator.haveDefaultValue()) 
                {
                	if(logicOperator.defaultValue() != null)
                	{
                    	logicOperator._setValue(boConvertUtils.convertToString(logicOperator.defaultValue(), logicOperator));
                    }
                    else
                    {
                    	logicOperator._setValue(null);
                    }
                    logicOperator.setInputType( AttributeHandler.INPUT_FROM_DEFAULT );
                }
            
                if( conditionFormula.haveDefaultValue()) 
                {
                	if(conditionFormula.defaultValue() != null)
                	{
                    	conditionFormula._setValue(boConvertUtils.convertToString(conditionFormula.defaultValue(), conditionFormula));
                    }
                    else
                    {
                    	conditionFormula._setValue(null);
                    }
                    conditionFormula.setInputType( AttributeHandler.INPUT_FROM_DEFAULT );
                }
            ...
            if("Ebo_DAOConfig".equals(this.getName()))
            {
    ...
    public bridgeHandler extAtt_parameter()
    {
        return getBridge("extAtt_parameter");        
    }
    
    public bridgeHandler linedepends()
    {
        return getBridge("linedepends");        
    }
    ...
    public bridgeHandler getBridge( String bridgeName ) 
    {
        ...
        if( ret == null )
        {
        	...
        	if( bridgeName.equals("extAtt_parameter") && (ret=brig.get("extAtt_parameter"))==null )
        	{
            	brig.add( ret=new BridgeHandlerextAtt_parameter(new DataResultSet( getDataRow().getRecordChild( getEboContext() ,"extAtt_parameter") ) , this ) );        
        	}
        	if( bridgeName.equals("linedepends") && (ret=brig.get("linedepends"))==null )
        	{
            	brig.add( ret=new BridgeHandlerlinedepends(new DataResultSet( getDataRow().getRecordChild( getEboContext() ,"linedepends") ) , this ) );        
        	}
        	if( ret == null )
        	{
    ...
    public final class BridgeHandlerextAtt_parameter extends bridgeHandler {
        ...
        public BridgeHandlerextAtt_parameter(DataResultSet data,boObject parent) {
            super("extAtt_parameter",
                  data,
                  parent.getBoDefinition().getAttributeRef("extAtt_parameter").getBridge().getChildFieldName(),
                  parent
                 );

            p_fatherfield = parent.getBoDefinition().getAttributeRef("extAtt_parameter").getBridge().getFatherFieldName();
            p_childfield  = parent.getBoDefinition().getAttributeRef("extAtt_parameter").getBridge().getChildFieldName();
...
//			  if( newval != null ) allow=allow && att.onBeforeAdd(new boEvent(att, att.EVENT_BEFORE_ADD,newval));
			  ...
			  if( att.fireBeforeChangeEvent( att, null, newval) ) 
			  {
                  ...
                  if(getParent().onChangeSubmit("extAtt_parameter"))
                  {
                      getParent().calculateFormula("extAtt_parameter");
                  }
...
//					att.onAfterChange(new boEvent(att,att.EVENT_AFTER_CHANGE,chgval,null));
		...
		{
			return new BridgeHanlderextAtt_parameterRow( this, row );
		}

		public final class BridgeHanlderextAtt_parameterRow extends boBridgeRow
		{
			public BridgeHanlderextAtt_parameterRow( bridgeHandler bridge, netgest.bo.data.DataRow row )
			{
				super( bridge , row );
				addLine();
			}
			...
			public void addLine() 
			{
				...
				if(this.getAttribute("extAtt_parameter")==null)
					xatt.add( new HandlerextAtt_parameter( getParent(), this ) );
...
return null;
        ...
        public final class HandlerextAtt_parameter extends BridgeObjAttributeHandler {
            public HandlerextAtt_parameter( boObject parent , boBridgeRow bridgerow ) {
                super( parent , bridgerow , bridgerow.getBridge().getDefAttribute().getName().equals("extAtt_parameter")?parent.getBoDefinition().getAttributeRef("extAtt_parameter"):bridgerow.getBridge().getDefAttribute().getBridge().getAttributeRef("extAtt_parameter") );
            }
            public void setValueString(String value) {
                try {
                    ...
                    if(getParent().onChangeSubmit("extAtt_parameter"))
                    {
                        ...
                        getParent().setCalculated(table, "extAtt_parameter");
                        getParent().calculateFormula(table, "extAtt_parameter");
                    }
                } catch (Exception e) {
            ...
            public String getValueString() throws boRuntimeException {
                try {
                    ...
                    String[] args = {getParent().getName(),"extAtt_parameter"};
            ...
            public void setValueFormula(Hashtable table, String[]dependence) throws boRuntimeException
            {
                ...
                {                       
                    if(getParent().isWaiting(table, "extAtt_parameter"))
                    {
                       getParent().setCalculated(table, "extAtt_parameter");
                       this.setValue(boConvertUtils.convertToBigDecimal(formula(), this), false );
                    }
                    else if(!getParent().isCalculated(table, "extAtt_parameter"))
                    {
                        getParent().setCalculated(table, "extAtt_parameter");
                ...
                else if(getParent().dependsFromWaiting(table, dependence))
                {
                    ...
                    {
                        getParent().clear(table, "extAtt_parameter"); 
                        this.setValue(boConvertUtils.convertToBigDecimal(formula(), this), false );
                    }
                }
                else
                {   
                    ...
                    if(getParent().onChangeSubmit("extAtt_parameter"))
                    {                 
                        getParent().setWaiting(table, "extAtt_parameter");
                        getParent().calculateFormula(new Hashtable(table), "extAtt_parameter");
                        //getParent().setCalculated(table, "extAtt_parameter");
                    }                    
                }
            }
            ...
            public void setValue(BigDecimal newval, boolean recalc) throws boRuntimeException 
            {
                ...
                if( recalc && getParent().onChangeSubmit("extAtt_parameter"))
                {
                    Hashtable table = new Hashtable();
                    getParent().setCalculated(table, "extAtt_parameter");
                    getParent().calculateFormula(table, "extAtt_parameter");
				...
				{
					getParent().getParentBridgeRow().getBridge().lineChanged("extAtt_parameter");
				}
            }
...
return null;
    ...
    public final class BridgeHandlerlinedepends extends bridgeHandler {
        private DataResultSet p_node;
        private String        p_fatherfield;
        private String        p_childfield;
        private boObject      p_parent;
        ...
        public BridgeHandlerlinedepends(DataResultSet data,boObject parent) {
            super("linedepends",
                  data,
                  parent.getBoDefinition().getAttributeRef("linedepends").getBridge().getChildFieldName(),
                  parent
                 );

            p_fatherfield = parent.getBoDefinition().getAttributeRef("linedepends").getBridge().getFatherFieldName();
            p_childfield  = parent.getBoDefinition().getAttributeRef("linedepends").getBridge().getChildFieldName();
...
//			  if( newval != null ) allow=allow && att.onBeforeAdd(new boEvent(att, att.EVENT_BEFORE_ADD,newval));
			  ...
			  if( att.fireBeforeChangeEvent( att, null, newval) ) 
			  {
                  ...
                  if(getParent().onChangeSubmit("linedepends"))
                  {
                      getParent().calculateFormula("linedepends");
                  }
                  att.fireAfterChangeEvent( att, null, newval );
//				  if( newval != null ) att.onAfterAdd(new boEvent( att ,att.EVENT_AFTER_ADD,newval));
//				  att.onAfterChange(new boEvent( att ,att.EVENT_AFTER_CHANGE, null ,newval));
...
//					att.onAfterChange(new boEvent(att,att.EVENT_AFTER_CHANGE,chgval,null));
		...
		public boBridgeRow createRow( netgest.bo.data.DataRow row )
		{
			return new BridgeHanlderlinedependsRow( this, row );
		}

		public final class BridgeHanlderlinedependsRow extends boBridgeRow
		{
			public BridgeHanlderlinedependsRow( bridgeHandler bridge, netgest.bo.data.DataRow row )
			{
				super( bridge , row );
				addLine();
			}
			...
			public void addLine() 
			{
				...
				if(this.getAttribute("linedepends")==null)
					xatt.add( new Handlerlinedepends( getParent(), this ) );
...
return null;
        ...
        public final class Handlerlinedepends extends BridgeObjAttributeHandler {
            public Handlerlinedepends( boObject parent , boBridgeRow bridgerow ) {
                super( parent , bridgerow , bridgerow.getBridge().getDefAttribute().getName().equals("linedepends")?parent.getBoDefinition().getAttributeRef("linedepends"):bridgerow.getBridge().getDefAttribute().getBridge().getAttributeRef("linedepends") );
            }
            public void setValueString(String value) {
                try {
                    this.setValue(boConvertUtils.convertToBigDecimal(value,this));
                    ...
                    {
                        Hashtable table = new Hashtable();                    
                        getParent().setCalculated(table, "linedepends");
                        getParent().calculateFormula(table, "linedepends");
                    }
                } catch (Exception e) {
                    super.setInvalid(e.getMessage(),value);
                }
            ...
            public String getValueString() throws boRuntimeException {
                try {
                    ...
                    String[] args = {getParent().getName(),"linedepends"};
                    throw new boRuntimeException(this.getClass().getName()+".load(EboContext,long)","BO-3003",e,args);
                }
            }
            ...
            public void setValueFormula(Hashtable table, String[]dependence) throws boRuntimeException
            {
                if(getParent().alreadyCalculated(table, dependence)) 
                {                       
                    if(getParent().isWaiting(table, "linedepends"))
                    {
                       getParent().setCalculated(table, "linedepends");
                       this.setValue(boConvertUtils.convertToBigDecimal(formula(), this), false );
                    }
                    else if(!getParent().isCalculated(table, "linedepends"))
                    {
                        getParent().setCalculated(table, "linedepends");
                        this.setValue(boConvertUtils.convertToBigDecimal(formula(), this), table );
                    }
                ...
                else if(getParent().dependsFromWaiting(table, dependence))
                {
                    ...
                    {
                        getParent().clear(table, "linedepends"); 
                        this.setValue(boConvertUtils.convertToBigDecimal(formula(), this), false );
                    }
                }
                else
                {   
                    ...
                    if(getParent().onChangeSubmit("linedepends"))
                    {                 
                        getParent().setWaiting(table, "linedepends");
                        getParent().calculateFormula(new Hashtable(table), "linedepends");
                        //getParent().setCalculated(table, "linedepends");
                    }                    
                }
            }
            ...
            public void setValue(BigDecimal newval, boolean recalc) throws boRuntimeException 
            {
                ...
                if( recalc && getParent().onChangeSubmit("linedepends"))
                {
                    Hashtable table = new Hashtable();
                    getParent().setCalculated(table, "linedepends");
                    getParent().calculateFormula(table, "linedepends");
				...
				if ( getParent().p_parentBridgeRow != null )
				{
					getParent().getParentBridgeRow().getBridge().lineChanged("linedepends");
				}
            }
