...
public  class Ebo_LOVDetails extends boObject implements  Serializable {  
    ...
    public Ebo_LOVDetails() {
        super();
        bo_version      = "1.0";
        bo_name         = "Ebo_LOVDetails";
        bo_classregboui = "#BO.CLSREGBOUI#";
        bo_definition   = boDefHandler.getBoDefinition("Ebo_LOVDetails");
        ...
        atts.add(new boBridgeMasterAttribute(this,this.getBoDefinition().getAttributeRef("flags"))); 
    ...
    public void init() throws boRuntimeException {
        ...
        if(p_mode == MODE_NEW && !p_clone)
        {
            ...
            if("Ebo_LOVDetails".equals(this.getName()))
            {
    ...
    public bridgeHandler flags()
    {
        return getBridge("flags");        
    }
    ...
    public bridgeHandler getBridge( String bridgeName ) 
    {
        ...
        if( ret == null )
        {
        	...
        	if( bridgeName.equals("flags") && (ret=brig.get("flags"))==null )
        	{
            	brig.add( ret=new BridgeHandlerflags(new DataResultSet( getDataRow().getRecordChild( getEboContext() ,"flags") ) , this ) );        
        	}
        	if( ret == null )
        	{
    ...
    public final class BridgeHandlerflags extends bridgeHandler {
        ...
        public BridgeHandlerflags(DataResultSet data,boObject parent) {
            super("flags",
                  data,
                  parent.getBoDefinition().getAttributeRef("flags").getBridge().getChildFieldName(),
                  parent
                 );

            p_fatherfield = parent.getBoDefinition().getAttributeRef("flags").getBridge().getFatherFieldName();
            p_childfield  = parent.getBoDefinition().getAttributeRef("flags").getBridge().getChildFieldName();
...
//			  if( newval != null ) allow=allow && att.onBeforeAdd(new boEvent(att, att.EVENT_BEFORE_ADD,newval));
			  ...
			  if( att.fireBeforeChangeEvent( att, null, newval) ) 
			  {
                  ...
                  if(getParent().onChangeSubmit("flags"))
                  {
                      getParent().calculateFormula("flags");
                  }
...
//					att.onAfterChange(new boEvent(att,att.EVENT_AFTER_CHANGE,chgval,null));
		...
		{
			return new BridgeHanlderflagsRow( this, row );
		}

		public final class BridgeHanlderflagsRow extends boBridgeRow
		{
			public BridgeHanlderflagsRow( bridgeHandler bridge, netgest.bo.data.DataRow row )
			{
				super( bridge , row );
				addLine();
			}
			...
			public void addLine() 
			{
				...
				if(this.getAttribute("flags")==null)
					xatt.add( new Handlerflags( getParent(), this ) );
...
return null;
        ...
        public final class Handlerflags extends BridgeObjAttributeHandler {
            public Handlerflags( boObject parent , boBridgeRow bridgerow ) {
                super( parent , bridgerow , bridgerow.getBridge().getDefAttribute().getName().equals("flags")?parent.getBoDefinition().getAttributeRef("flags"):bridgerow.getBridge().getDefAttribute().getBridge().getAttributeRef("flags") );
            }
            public void setValueString(String value) {
                try {
                    ...
                    if(getParent().onChangeSubmit("flags"))
                    {
                        ...
                        getParent().setCalculated(table, "flags");
                        getParent().calculateFormula(table, "flags");
                    }
                } catch (Exception e) {
            ...
            public String getValueString() throws boRuntimeException {
                try {
                    ...
                    String[] args = {getParent().getName(),"flags"};
            ...
            public void setValueFormula(Hashtable table, String[]dependence) throws boRuntimeException
            {
                ...
                {                       
                    if(getParent().isWaiting(table, "flags"))
                    {
                       getParent().setCalculated(table, "flags");
                       this.setValue(boConvertUtils.convertToBigDecimal(formula(), this), false );
                    }
                    else if(!getParent().isCalculated(table, "flags"))
                    {
                        getParent().setCalculated(table, "flags");
                ...
                {   
                    ...
                    if(getParent().onChangeSubmit("flags"))
                    {                 
                        getParent().setWaiting(table, "flags");
                        getParent().calculateFormula(new Hashtable(table), "flags");
                        //getParent().setCalculated(table, "flags");
                    }                    
                }
            }
            ...
            public void setValue(BigDecimal newval, boolean recalc) throws boRuntimeException 
            {
                ...
                if( recalc && getParent().onChangeSubmit("flags"))
                {
                    Hashtable table = new Hashtable();
                    getParent().setCalculated(table, "flags");
                    getParent().calculateFormula(table, "flags");
				...
				{
					getParent().getParentBridgeRow().getBridge().lineChanged("flags");
				}
            }
