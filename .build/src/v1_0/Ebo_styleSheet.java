...
public  class Ebo_styleSheet extends boObject implements  Serializable {  

   
   
   
    public netgest.bo.runtime.attributes.boAttributeString name;    
    public netgest.bo.runtime.attributes.boAttributeObject document;    
    ...
    public Ebo_styleSheet() {
        super();
        bo_version      = "1.0";
        bo_name         = "Ebo_styleSheet";
        bo_classregboui = "#BO.CLSREGBOUI#";
        bo_definition   = boDefHandler.getBoDefinition("Ebo_styleSheet");
        ...
        atts.add(document = new netgest.bo.runtime.attributes.boAttributeObject(this,"document"));
    ...
    public void init() throws boRuntimeException {
        ...
        if(p_mode == MODE_NEW && !p_clone)
        {
                ...
                if( document.haveDefaultValue()) 
                {
                	if(document.defaultValue() != null)
                	{
                    	document._setValue(boConvertUtils.convertToBigDecimal(document.defaultValue(), document));
                    }
                    else
                    {
                    	document._setValue(null);
                    }
                    document.setInputType( AttributeHandler.INPUT_FROM_DEFAULT );
                }
            ...
            if("Ebo_styleSheet".equals(this.getName()))
            {
