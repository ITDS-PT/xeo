...
public  class garbageController extends boObject implements  Serializable {  

   
   
   
    public netgest.bo.runtime.attributes.boAttributeString email;    
    public netgest.bo.runtime.attributes.boAttributeString server;    
    ...
    public garbageController() {
        super();
        bo_version      = "1.0";
        bo_name         = "garbageController";
        bo_classregboui = "#BO.CLSREGBOUI#";
        bo_definition   = boDefHandler.getBoDefinition("garbageController");
        ...
        atts.add(server = new netgest.bo.runtime.attributes.boAttributeString(this,"server"));
    ...
    public void init() throws boRuntimeException {
        ...
        if(p_mode == MODE_NEW && !p_clone)
        {
                ...
                if( server.haveDefaultValue()) 
                {
                	if(server.defaultValue() != null)
                	{
                    	server._setValue(boConvertUtils.convertToString(server.defaultValue(), server));
                    }
                    else
                    {
                    	server._setValue(null);
                    }
                    server.setInputType( AttributeHandler.INPUT_FROM_DEFAULT );
                }
            ...
            if("garbageController".equals(this.getName()))
            {
