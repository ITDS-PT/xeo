...
public  class Ebo_TextIndex extends netgest.bo.impl.Ebo_TextIndexImpl implements  Serializable {  

   
   
   
    public netgest.bo.runtime.attributes.boAttributeObject ui;    
    public netgest.bo.runtime.attributes.boAttributeString uiClass;    
    public netgest.bo.runtime.attributes.boAttributeString text;    
    ...
    public Ebo_TextIndex() {
        super();
        bo_version      = "1.0";
        bo_name         = "Ebo_TextIndex";
        bo_classregboui = "#BO.CLSREGBOUI#";
        bo_definition   = boDefHandler.getBoDefinition("Ebo_TextIndex");
        ...
        atts.add(uiClass = new netgest.bo.runtime.attributes.boAttributeString(this,"uiClass"));
        
        atts.add(text = new netgest.bo.runtime.attributes.boAttributeString(this,"text"));
    ...
    public void init() throws boRuntimeException {
        ...
        if(p_mode == MODE_NEW && !p_clone)
        {
                ...
                if( uiClass.haveDefaultValue()) 
                {
                	if(uiClass.defaultValue() != null)
                	{
                    	uiClass._setValue(boConvertUtils.convertToString(uiClass.defaultValue(), uiClass));
                    }
                    else
                    {
                    	uiClass._setValue(null);
                    }
                    uiClass.setInputType( AttributeHandler.INPUT_FROM_DEFAULT );
                }
            
                if( text.haveDefaultValue()) 
                {
                	if(text.defaultValue() != null)
                	{
                    	text._setValue(boConvertUtils.convertToString(text.defaultValue(), text));
                    }
                    else
                    {
                    	text._setValue(null);
                    }
                    text.setInputType( AttributeHandler.INPUT_FROM_DEFAULT );
                }
            ...
            if("Ebo_TextIndex".equals(this.getName()))
            {
