...
public  class Ebo_DAOdepends extends boObject implements  Serializable {  

   
   
   
    public netgest.bo.runtime.attributes.boAttributeString DAO_code;    
    public netgest.bo.runtime.attributes.boAttributeNumber lag;    
    ...
    public Ebo_DAOdepends() {
        super();
        bo_version      = "1.0";
        bo_name         = "Ebo_DAOdepends";
        bo_classregboui = "#BO.CLSREGBOUI#";
        bo_definition   = boDefHandler.getBoDefinition("Ebo_DAOdepends");
        ...
        atts.add(DAO_code = new netgest.bo.runtime.attributes.boAttributeString(this,"DAO_code"));
        
        atts.add(lag = new netgest.bo.runtime.attributes.boAttributeNumber(this,"lag"));
    ...
    public void init() throws boRuntimeException {
        ...
        if(p_mode == MODE_NEW && !p_clone)
        {
                ...
                if( DAO_code.haveDefaultValue()) 
                {
                	if(DAO_code.defaultValue() != null)
                	{
                    	DAO_code._setValue(boConvertUtils.convertToString(DAO_code.defaultValue(), DAO_code));
                    }
                    else
                    {
                    	DAO_code._setValue(null);
                    }
                    DAO_code.setInputType( AttributeHandler.INPUT_FROM_DEFAULT );
                }
            
                if( lag.haveDefaultValue()) 
                {
                	if(lag.defaultValue() != null)
                	{
                    	lag._setValue(boConvertUtils.convertToBigDecimal(lag.defaultValue(), lag));
                    }
                    else
                    {
                    	lag._setValue(null);
                    }
                    lag.setInputType( AttributeHandler.INPUT_FROM_DEFAULT );
                }
            ...
            if("Ebo_DAOdepends".equals(this.getName()))
            {
...
	public StringBuffer getCardIDByDAOCode() throws boRuntimeException {
