...
public  class mailAccount_folder extends boObject implements  Serializable {  

   
   
   
    public netgest.bo.runtime.attributes.boAttributeString lastmessageid;    
    public netgest.bo.runtime.attributes.boAttributeString folder;    
    ...
    public mailAccount_folder() {
        super();
        bo_version      = "1.0";
        bo_name         = "mailAccount_folder";
        bo_classregboui = "#BO.CLSREGBOUI#";
        bo_definition   = boDefHandler.getBoDefinition("mailAccount_folder");
        ...
        atts.add(new boBridgeMasterAttribute(this,this.getBoDefinition().getAttributeRef("errors"))); 

       
        
       
        atts.add(lastmessageid = new netgest.bo.runtime.attributes.boAttributeString(this,"lastmessageid"));
        
        atts.add(folder = new netgest.bo.runtime.attributes.boAttributeString(this,"folder"));
    ...
    public void init() throws boRuntimeException {
        ...
        if(p_mode == MODE_NEW && !p_clone)
        {
                ...
                if( lastmessageid.haveDefaultValue()) 
                {
                	if(lastmessageid.defaultValue() != null)
                	{
                    	lastmessageid._setValue(boConvertUtils.convertToString(lastmessageid.defaultValue(), lastmessageid));
                    }
                    else
                    {
                    	lastmessageid._setValue(null);
                    }
                    lastmessageid.setInputType( AttributeHandler.INPUT_FROM_DEFAULT );
                }
            
                if( folder.haveDefaultValue()) 
                {
                	if(folder.defaultValue() != null)
                	{
                    	folder._setValue(boConvertUtils.convertToString(folder.defaultValue(), folder));
                    }
                    else
                    {
                    	folder._setValue(null);
                    }
                    folder.setInputType( AttributeHandler.INPUT_FROM_DEFAULT );
                }
            ...
            if("mailAccount_folder".equals(this.getName()))
            {
    ...
    public bridgeHandler errors()
    {
        return getBridge("errors");        
    }
    ...
    public bridgeHandler getBridge( String bridgeName ) 
    {
        ...
        if( ret == null )
        {
        	...
        	if( bridgeName.equals("errors") && (ret=brig.get("errors"))==null )
        	{
            	brig.add( ret=new BridgeHandlererrors(new DataResultSet( getDataRow().getRecordChild( getEboContext() ,"errors") ) , this ) );        
        	}
        	if( ret == null )
        	{
    ...
    public final class BridgeHandlererrors extends bridgeHandler {
        ...
        public BridgeHandlererrors(DataResultSet data,boObject parent) {
            super("errors",
                  data,
                  parent.getBoDefinition().getAttributeRef("errors").getBridge().getChildFieldName(),
                  parent
                 );

            p_fatherfield = parent.getBoDefinition().getAttributeRef("errors").getBridge().getFatherFieldName();
            p_childfield  = parent.getBoDefinition().getAttributeRef("errors").getBridge().getChildFieldName();
...
//			  if( newval != null ) allow=allow && att.onBeforeAdd(new boEvent(att, att.EVENT_BEFORE_ADD,newval));
			  ...
			  if( att.fireBeforeChangeEvent( att, null, newval) ) 
			  {
                  ...
                  if(getParent().onChangeSubmit("errors"))
                  {
                      getParent().calculateFormula("errors");
                  }
...
//					att.onAfterChange(new boEvent(att,att.EVENT_AFTER_CHANGE,chgval,null));
		...
		{
			return new BridgeHanldererrorsRow( this, row );
		}

		public final class BridgeHanldererrorsRow extends boBridgeRow
		{
			public BridgeHanldererrorsRow( bridgeHandler bridge, netgest.bo.data.DataRow row )
			{
				super( bridge , row );
				addLine();
			}
			...
			public void addLine() 
			{
				...
				if(this.getAttribute("errors")==null)
					xatt.add( new Handlererrors( getParent(), this ) );
...
return null;
        ...
        public final class Handlererrors extends BridgeObjAttributeHandler {
            public Handlererrors( boObject parent , boBridgeRow bridgerow ) {
                super( parent , bridgerow , bridgerow.getBridge().getDefAttribute().getName().equals("errors")?parent.getBoDefinition().getAttributeRef("errors"):bridgerow.getBridge().getDefAttribute().getBridge().getAttributeRef("errors") );
            }
            public void setValueString(String value) {
                try {
                    ...
                    if(getParent().onChangeSubmit("errors"))
                    {
                        ...
                        getParent().setCalculated(table, "errors");
                        getParent().calculateFormula(table, "errors");
                    }
                } catch (Exception e) {
            ...
            public String getValueString() throws boRuntimeException {
                try {
                    ...
                    String[] args = {getParent().getName(),"errors"};
            ...
            public void setValueFormula(Hashtable table, String[]dependence) throws boRuntimeException
            {
                ...
                {                       
                    if(getParent().isWaiting(table, "errors"))
                    {
                       getParent().setCalculated(table, "errors");
                       this.setValue(boConvertUtils.convertToBigDecimal(formula(), this), false );
                    }
                    else if(!getParent().isCalculated(table, "errors"))
                    {
                        getParent().setCalculated(table, "errors");
                ...
                {   
                    ...
                    if(getParent().onChangeSubmit("errors"))
                    {                 
                        getParent().setWaiting(table, "errors");
                        getParent().calculateFormula(new Hashtable(table), "errors");
                        //getParent().setCalculated(table, "errors");
                    }                    
                }
            }
            ...
            public void setValue(BigDecimal newval, boolean recalc) throws boRuntimeException 
            {
                ...
                if( recalc && getParent().onChangeSubmit("errors"))
                {
                    Hashtable table = new Hashtable();
                    getParent().setCalculated(table, "errors");
                    getParent().calculateFormula(table, "errors");
				...
				{
					getParent().getParentBridgeRow().getBridge().lineChanged("errors");
				}
            }
...
	return netgest.bo.system.boMail.checkInBox(this);
