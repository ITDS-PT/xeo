...
public  class yearHolidays extends boObject implements  Serializable {  

   
   
   
    public netgest.bo.runtime.attributes.boAttributeString yearSelect;    
    public netgest.bo.runtime.attributes.boAttributeDate year;    
    ...
    public yearHolidays() {
        super();
        bo_version      = "1.0";
        bo_name         = "yearHolidays";
        bo_classregboui = "#BO.CLSREGBOUI#";
        bo_definition   = boDefHandler.getBoDefinition("yearHolidays");
        ...
        atts.add(new boBridgeMasterAttribute(this,this.getBoDefinition().getAttributeRef("holidays"))); 

       
        
       
        atts.add(yearSelect = new netgest.bo.runtime.attributes.boAttributeString(this,"yearSelect"));
        
        atts.add(year = new netgest.bo.runtime.attributes.boAttributeDate(this,"year"));
    ...
    public void init() throws boRuntimeException {
        ...
        if(p_mode == MODE_NEW && !p_clone)
        {
                ...
                if( yearSelect.haveDefaultValue()) 
                {
                	if(yearSelect.defaultValue() != null)
                	{
                    	yearSelect._setValue(boConvertUtils.convertToString(yearSelect.defaultValue(), yearSelect));
                    }
                    else
                    {
                    	yearSelect._setValue(null);
                    }
                    yearSelect.setInputType( AttributeHandler.INPUT_FROM_DEFAULT );
                }
            
                if( year.haveDefaultValue()) 
                {
                	if(year.defaultValue() != null)
                	{
                    	year._setValue(boConvertUtils.convertToTimestamp(year.defaultValue(), year));
                    }
                    else
                    {
                    	year._setValue(null);
                    }
                    year.setInputType( AttributeHandler.INPUT_FROM_DEFAULT );
                }
            ...
            if("yearHolidays".equals(this.getName()))
            {
    ...
    public bridgeHandler holidays()
    {
        return getBridge("holidays");        
    }
    ...
    public bridgeHandler getBridge( String bridgeName ) 
    {
        ...
        if( ret == null )
        {
        	...
        	if( bridgeName.equals("holidays") && (ret=brig.get("holidays"))==null )
        	{
            	brig.add( ret=new BridgeHandlerholidays(new DataResultSet( getDataRow().getRecordChild( getEboContext() ,"holidays") ) , this ) );        
        	}
        	if( ret == null )
        	{
    ...
    public final class BridgeHandlerholidays extends bridgeHandler {
        ...
        public BridgeHandlerholidays(DataResultSet data,boObject parent) {
            super("holidays",
                  data,
                  parent.getBoDefinition().getAttributeRef("holidays").getBridge().getChildFieldName(),
                  parent
                 );

            p_fatherfield = parent.getBoDefinition().getAttributeRef("holidays").getBridge().getFatherFieldName();
            p_childfield  = parent.getBoDefinition().getAttributeRef("holidays").getBridge().getChildFieldName();
...
//			  if( newval != null ) allow=allow && att.onBeforeAdd(new boEvent(att, att.EVENT_BEFORE_ADD,newval));
			  ...
			  if( att.fireBeforeChangeEvent( att, null, newval) ) 
			  {
                  ...
                  if(getParent().onChangeSubmit("holidays"))
                  {
                      getParent().calculateFormula("holidays");
                  }
...
//					att.onAfterChange(new boEvent(att,att.EVENT_AFTER_CHANGE,chgval,null));
		...
		{
			return new BridgeHanlderholidaysRow( this, row );
		}

		public final class BridgeHanlderholidaysRow extends boBridgeRow
		{
			public BridgeHanlderholidaysRow( bridgeHandler bridge, netgest.bo.data.DataRow row )
			{
				super( bridge , row );
				addLine();
			}
			...
			public void addLine() 
			{
				...
				if(this.getAttribute("holidays")==null)
					xatt.add( new Handlerholidays( getParent(), this ) );
...
return null;
        ...
        public final class Handlerholidays extends BridgeObjAttributeHandler {
            public Handlerholidays( boObject parent , boBridgeRow bridgerow ) {
                super( parent , bridgerow , bridgerow.getBridge().getDefAttribute().getName().equals("holidays")?parent.getBoDefinition().getAttributeRef("holidays"):bridgerow.getBridge().getDefAttribute().getBridge().getAttributeRef("holidays") );
            }
            public void setValueString(String value) {
                try {
                    ...
                    if(getParent().onChangeSubmit("holidays"))
                    {
                        ...
                        getParent().setCalculated(table, "holidays");
                        getParent().calculateFormula(table, "holidays");
                    }
                } catch (Exception e) {
            ...
            public String getValueString() throws boRuntimeException {
                try {
                    ...
                    String[] args = {getParent().getName(),"holidays"};
            ...
            public void setValueFormula(Hashtable table, String[]dependence) throws boRuntimeException
            {
                ...
                {                       
                    if(getParent().isWaiting(table, "holidays"))
                    {
                       getParent().setCalculated(table, "holidays");
                       this.setValue(boConvertUtils.convertToBigDecimal(formula(), this), false );
                    }
                    else if(!getParent().isCalculated(table, "holidays"))
                    {
                        getParent().setCalculated(table, "holidays");
                ...
                else if(getParent().dependsFromWaiting(table, dependence))
                {
                    ...
                    {
                        getParent().clear(table, "holidays"); 
                        this.setValue(boConvertUtils.convertToBigDecimal(formula(), this), false );
                    }
                }
                else
                {   
                    ...
                    if(getParent().onChangeSubmit("holidays"))
                    {                 
                        getParent().setWaiting(table, "holidays");
                        getParent().calculateFormula(new Hashtable(table), "holidays");
                        //getParent().setCalculated(table, "holidays");
                    }                    
                }
            }
            ...
            public void setValue(BigDecimal newval, boolean recalc) throws boRuntimeException 
            {
                ...
                if( recalc && getParent().onChangeSubmit("holidays"))
                {
                    Hashtable table = new Hashtable();
                    getParent().setCalculated(table, "holidays");
                    getParent().calculateFormula(table, "holidays");
				...
				{
					getParent().getParentBridgeRow().getBridge().lineChanged("holidays");
				}
            }
