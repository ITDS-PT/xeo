...
public  class horario_dia extends boObject implements  Serializable {  
    ...
    public horario_dia() {
        ...
        bo_definition   = boDefHandler.getBoDefinition("horario_dia");
        ...
        atts.add(new boBridgeMasterAttribute(this,this.getBoDefinition().getAttributeRef("periodos"))); 
    ...
    public void init() throws boRuntimeException {
        ...
        if(p_mode == MODE_NEW && !p_clone)
        {
            ...
            if("horario_dia".equals(this.getName()))
            {
    ...
    public bridgeHandler periodos()
    {
        return getBridge("periodos");        
    }
    ...
    public bridgeHandler getBridge( String bridgeName ) 
    {
        ...
        if( ret == null )
        {
        	...
        	if( bridgeName.equals("periodos") && (ret=brig.get("periodos"))==null )
        	{
            	brig.add( ret=new BridgeHandlerperiodos(new DataResultSet( getDataRow().getRecordChild( getEboContext() ,"periodos") ) , this ) );        
        	}
        	if( ret == null )
        	{
    ...
    public final class BridgeHandlerperiodos extends bridgeHandler {
        ...
        public BridgeHandlerperiodos(DataResultSet data,boObject parent) {
            super("periodos",
                  data,
                  parent.getBoDefinition().getAttributeRef("periodos").getBridge().getChildFieldName(),
                  parent
                 );

            p_fatherfield = parent.getBoDefinition().getAttributeRef("periodos").getBridge().getFatherFieldName();
            p_childfield  = parent.getBoDefinition().getAttributeRef("periodos").getBridge().getChildFieldName();
...
//			  if( newval != null ) allow=allow && att.onBeforeAdd(new boEvent(att, att.EVENT_BEFORE_ADD,newval));
			  ...
			  if( att.fireBeforeChangeEvent( att, null, newval) ) 
			  {
                  ...
                  if(getParent().onChangeSubmit("periodos"))
                  {
                      getParent().calculateFormula("periodos");
                  }
...
//					att.onAfterChange(new boEvent(att,att.EVENT_AFTER_CHANGE,chgval,null));
		...
		{
			return new BridgeHanlderperiodosRow( this, row );
		}

		public final class BridgeHanlderperiodosRow extends boBridgeRow
		{
			public BridgeHanlderperiodosRow( bridgeHandler bridge, netgest.bo.data.DataRow row )
			{
				super( bridge , row );
				addLine();
			}
			...
			public void addLine() 
			{
				...
				if(this.getAttribute("periodos")==null)
					xatt.add( new Handlerperiodos( getParent(), this ) );
...
return null;
        ...
        public final class Handlerperiodos extends BridgeObjAttributeHandler {
            public Handlerperiodos( boObject parent , boBridgeRow bridgerow ) {
                super( parent , bridgerow , bridgerow.getBridge().getDefAttribute().getName().equals("periodos")?parent.getBoDefinition().getAttributeRef("periodos"):bridgerow.getBridge().getDefAttribute().getBridge().getAttributeRef("periodos") );
            }
            public void setValueString(String value) {
                try {
                    ...
                    if(getParent().onChangeSubmit("periodos"))
                    {
                        ...
                        getParent().setCalculated(table, "periodos");
                        getParent().calculateFormula(table, "periodos");
                    }
                } catch (Exception e) {
            ...
            public String getValueString() throws boRuntimeException {
                try {
                    ...
                    String[] args = {getParent().getName(),"periodos"};
            ...
            public void setValueFormula(Hashtable table, String[]dependence) throws boRuntimeException
            {
                ...
                {                       
                    if(getParent().isWaiting(table, "periodos"))
                    {
                       getParent().setCalculated(table, "periodos");
                       this.setValue(boConvertUtils.convertToBigDecimal(formula(), this), false );
                    }
                    else if(!getParent().isCalculated(table, "periodos"))
                    {
                        getParent().setCalculated(table, "periodos");
                ...
                else if(getParent().dependsFromWaiting(table, dependence))
                {
                    ...
                    {
                        getParent().clear(table, "periodos"); 
                        this.setValue(boConvertUtils.convertToBigDecimal(formula(), this), false );
                    }
                }
                else
                {   
                    ...
                    if(getParent().onChangeSubmit("periodos"))
                    {                 
                        getParent().setWaiting(table, "periodos");
                        getParent().calculateFormula(new Hashtable(table), "periodos");
                        //getParent().setCalculated(table, "periodos");
                    }                    
                }
            }
            ...
            public void setValue(BigDecimal newval, boolean recalc) throws boRuntimeException 
            {
                ...
                if( recalc && getParent().onChangeSubmit("periodos"))
                {
                    Hashtable table = new Hashtable();
                    getParent().setCalculated(table, "periodos");
                    getParent().calculateFormula(table, "periodos");
				...
				{
					getParent().getParentBridgeRow().getBridge().lineChanged("periodos");
				}
            }
