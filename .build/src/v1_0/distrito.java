...
public  class distrito extends boObject implements  Serializable {  
    ...
    public distrito() {
        ...
        bo_definition   = boDefHandler.getBoDefinition("distrito");
        ...
        atts.add(new boBridgeMasterAttribute(this,this.getBoDefinition().getAttributeRef("concelhos"))); 
    ...
    public void init() throws boRuntimeException {
        ...
        if(p_mode == MODE_NEW && !p_clone)
        {
            ...
            if("distrito".equals(this.getName()))
            {
    ...
    public bridgeHandler concelhos()
    {
        return getBridge("concelhos");        
    }
    ...
    public bridgeHandler getBridge( String bridgeName ) 
    {
        ...
        if( ret == null )
        {
        	...
        	if( bridgeName.equals("concelhos") && (ret=brig.get("concelhos"))==null )
        	{
            	brig.add( ret=new BridgeHandlerconcelhos(new DataResultSet( getDataRow().getRecordChild( getEboContext() ,"concelhos") ) , this ) );        
        	}
        	if( ret == null )
        	{
...
return null;
    ...
    public final class BridgeHandlerconcelhos extends bridgeHandler {
        ...
        public BridgeHandlerconcelhos(DataResultSet data,boObject parent) {
            super("concelhos",
                  data,
                  parent.getBoDefinition().getAttributeRef("concelhos").getBridge().getChildFieldName(),
                  parent
                 );

            p_fatherfield = parent.getBoDefinition().getAttributeRef("concelhos").getBridge().getFatherFieldName();
            p_childfield  = parent.getBoDefinition().getAttributeRef("concelhos").getBridge().getChildFieldName();
...
//			  if( newval != null ) allow=allow && att.onBeforeAdd(new boEvent(att, att.EVENT_BEFORE_ADD,newval));
			  ...
			  if( att.fireBeforeChangeEvent( att, null, newval) ) 
			  {
                  ...
                  if(getParent().onChangeSubmit("concelhos"))
                  {
                      getParent().calculateFormula("concelhos");
                  }
...
//					att.onAfterChange(new boEvent(att,att.EVENT_AFTER_CHANGE,chgval,null));
		...
		{
			return new BridgeHanlderconcelhosRow( this, row );
		}

		public final class BridgeHanlderconcelhosRow extends boBridgeRow
		{
			public BridgeHanlderconcelhosRow( bridgeHandler bridge, netgest.bo.data.DataRow row )
			{
				super( bridge , row );
				addLine();
			}
			...
			public void addLine() 
			{
				...
				if(this.getAttribute("concelhos")==null)
					xatt.add( new Handlerconcelhos( getParent(), this ) );
...
return null;
        ...
        public final class Handlerconcelhos extends BridgeObjAttributeHandler {
            public Handlerconcelhos( boObject parent , boBridgeRow bridgerow ) {
                super( parent , bridgerow , bridgerow.getBridge().getDefAttribute().getName().equals("concelhos")?parent.getBoDefinition().getAttributeRef("concelhos"):bridgerow.getBridge().getDefAttribute().getBridge().getAttributeRef("concelhos") );
            }
            public void setValueString(String value) {
                try {
                    ...
                    if(getParent().onChangeSubmit("concelhos"))
                    {
                        ...
                        getParent().setCalculated(table, "concelhos");
                        getParent().calculateFormula(table, "concelhos");
                    }
                } catch (Exception e) {
                    super.setInvalid(e.getMessage(),value);
                }
            ...
            public String getValueString() throws boRuntimeException {
                try {
                    ...
                    String[] args = {getParent().getName(),"concelhos"};
                    throw new boRuntimeException(this.getClass().getName()+".load(EboContext,long)","BO-3003",e,args);
                }
            }
            ...
            public void setValueFormula(Hashtable table, String[]dependence) throws boRuntimeException
            {
                ...
                {                       
                    if(getParent().isWaiting(table, "concelhos"))
                    {
                       getParent().setCalculated(table, "concelhos");
                       this.setValue(boConvertUtils.convertToBigDecimal(formula(), this), false );
                    }
                    else if(!getParent().isCalculated(table, "concelhos"))
                    {
                        getParent().setCalculated(table, "concelhos");
                ...
                else if(getParent().dependsFromWaiting(table, dependence))
                {
                    ...
                    {
                        getParent().clear(table, "concelhos"); 
                        this.setValue(boConvertUtils.convertToBigDecimal(formula(), this), false );
                    }
                }
                else
                {   
                    ...
                    if(getParent().onChangeSubmit("concelhos"))
                    {                 
                        getParent().setWaiting(table, "concelhos");
                        getParent().calculateFormula(new Hashtable(table), "concelhos");
                        //getParent().setCalculated(table, "concelhos");
                    }                    
                }
            }
            ...
            public void setValue(BigDecimal newval, boolean recalc) throws boRuntimeException 
            {
                ...
                if( recalc && getParent().onChangeSubmit("concelhos"))
                {
                    Hashtable table = new Hashtable();
                    getParent().setCalculated(table, "concelhos");
                    getParent().calculateFormula(table, "concelhos");
				...
				{
					getParent().getParentBridgeRow().getBridge().lineChanged("concelhos");
				}
            }
