...
public  class uiListBar extends uiObject implements  Serializable {  

   
   
   
    public netgest.bo.runtime.attributes.boAttributeString activeItem;    
    ...
    public uiListBar() {
        ...
        bo_definition   = boDefHandler.getBoDefinition("uiListBar");
        ...
        atts.add(new boBridgeMasterAttribute(this,this.getBoDefinition().getAttributeRef("content"))); 

       
        
       
        atts.add(activeItem = new netgest.bo.runtime.attributes.boAttributeString(this,"activeItem"));
    ...
    public void init() throws boRuntimeException {
        ...
        if(p_mode == MODE_NEW && !p_clone)
        {
                ...
                if( activeItem.haveDefaultValue()) 
                {
                	if(activeItem.defaultValue() != null)
                	{
                    	activeItem._setValue(boConvertUtils.convertToString(activeItem.defaultValue(), activeItem));
                    }
                    else
                    {
                    	activeItem._setValue(null);
                    }
                    activeItem.setInputType( AttributeHandler.INPUT_FROM_DEFAULT );
                }
            
    
            if("uiListBar".equals(this.getName()))
            {
    ...
    public bridgeHandler content()
    {
        return getBridge("content");        
    }
    ...
    public bridgeHandler getBridge( String bridgeName ) 
    {
        ...
        if( ret == null )
        {
        	...
        	if( bridgeName.equals("content") && (ret=brig.get("content"))==null )
        	{
            	brig.add( ret=new BridgeHandlercontent(new DataResultSet( getDataRow().getRecordChild( getEboContext() ,"content") ) , this ) );        
        	}
        	if( ret == null )
        	{
    ...
    public final class BridgeHandlercontent extends bridgeHandler {
        ...
        public BridgeHandlercontent(DataResultSet data,boObject parent) {
            super("content",
                  data,
                  parent.getBoDefinition().getAttributeRef("content").getBridge().getChildFieldName(),
                  parent
                 );

            p_fatherfield = parent.getBoDefinition().getAttributeRef("content").getBridge().getFatherFieldName();
            p_childfield  = parent.getBoDefinition().getAttributeRef("content").getBridge().getChildFieldName();
...
//			  if( newval != null ) allow=allow && att.onBeforeAdd(new boEvent(att, att.EVENT_BEFORE_ADD,newval));
			  ...
			  if( att.fireBeforeChangeEvent( att, null, newval) ) 
			  {
                  ...
                  if(getParent().onChangeSubmit("content"))
                  {
                      getParent().calculateFormula("content");
                  }
...
//					att.onAfterChange(new boEvent(att,att.EVENT_AFTER_CHANGE,chgval,null));
		...
		{
			return new BridgeHanldercontentRow( this, row );
		}

		public final class BridgeHanldercontentRow extends boBridgeRow
		{
			public BridgeHanldercontentRow( bridgeHandler bridge, netgest.bo.data.DataRow row )
			{
				super( bridge , row );
				addLine();
			}
			...
			public void addLine() 
			{
				...
				if(this.getAttribute("content")==null)
					xatt.add( new Handlercontent( getParent(), this ) );
...
return null;
        ...
        public final class Handlercontent extends BridgeObjAttributeHandler {
            public Handlercontent( boObject parent , boBridgeRow bridgerow ) {
                super( parent , bridgerow , bridgerow.getBridge().getDefAttribute().getName().equals("content")?parent.getBoDefinition().getAttributeRef("content"):bridgerow.getBridge().getDefAttribute().getBridge().getAttributeRef("content") );
            }
            public void setValueString(String value) {
                try {
                    ...
                    if(getParent().onChangeSubmit("content"))
                    {
                        ...
                        getParent().setCalculated(table, "content");
                        getParent().calculateFormula(table, "content");
                    }
                } catch (Exception e) {
            ...
            public String getValueString() throws boRuntimeException {
                try {
                    ...
                    String[] args = {getParent().getName(),"content"};
            ...
            public void setValueFormula(Hashtable table, String[]dependence) throws boRuntimeException
            {
                ...
                {                       
                    if(getParent().isWaiting(table, "content"))
                    {
                       getParent().setCalculated(table, "content");
                       this.setValue(boConvertUtils.convertToBigDecimal(formula(), this), false );
                    }
                    else if(!getParent().isCalculated(table, "content"))
                    {
                        getParent().setCalculated(table, "content");
                ...
                {   
                    ...
                    if(getParent().onChangeSubmit("content"))
                    {                 
                        getParent().setWaiting(table, "content");
                        getParent().calculateFormula(new Hashtable(table), "content");
                        //getParent().setCalculated(table, "content");
                    }                    
                }
            }
            ...
            public void setValue(BigDecimal newval, boolean recalc) throws boRuntimeException 
            {
                ...
                if( recalc && getParent().onChangeSubmit("content"))
                {
                    Hashtable table = new Hashtable();
                    getParent().setCalculated(table, "content");
                    getParent().calculateFormula(table, "content");
				...
				{
					getParent().getParentBridgeRow().getBridge().lineChanged("content");
				}
            }
