...
public  class xwfSerialObject extends boObject implements  Serializable {  

   
   
   
    public netgest.bo.runtime.attributes.boAttributeObject objectCls;    
    public netgest.bo.runtime.attributes.boAttributeString xmlSerial;    
    public netgest.bo.runtime.attributes.boAttributeNumber program;    
    public netgest.bo.runtime.attributes.boAttributeNumber uid;    
    ...
    public xwfSerialObject() {
        super();
        bo_version      = "1.0";
        bo_name         = "xwfSerialObject";
        bo_classregboui = "#BO.CLSREGBOUI#";
        bo_definition   = boDefHandler.getBoDefinition("xwfSerialObject");
        ...
        atts.add(objectCls = new netgest.bo.runtime.attributes.boAttributeObject(this,"objectCls"));
        
        atts.add(xmlSerial = new netgest.bo.runtime.attributes.boAttributeString(this,"xmlSerial"));
        
        atts.add(program = new netgest.bo.runtime.attributes.boAttributeNumber(this,"program"));
        
        atts.add(uid = new netgest.bo.runtime.attributes.boAttributeNumber(this,"uid"));
    ...
    public void init() throws boRuntimeException {
        ...
        if(p_mode == MODE_NEW && !p_clone)
        {
                ...
                if( objectCls.haveDefaultValue()) 
                {
                	if(objectCls.defaultValue() != null)
                	{
                    	objectCls._setValue(boConvertUtils.convertToBigDecimal(objectCls.defaultValue(), objectCls));
                    }
                    else
                    {
                    	objectCls._setValue(null);
                    }
                    objectCls.setInputType( AttributeHandler.INPUT_FROM_DEFAULT );
                }
            
                if( xmlSerial.haveDefaultValue()) 
                {
                	if(xmlSerial.defaultValue() != null)
                	{
                    	xmlSerial._setValue(boConvertUtils.convertToString(xmlSerial.defaultValue(), xmlSerial));
                    }
                    else
                    {
                    	xmlSerial._setValue(null);
                    }
                    xmlSerial.setInputType( AttributeHandler.INPUT_FROM_DEFAULT );
                }
                ...
                if( uid.haveDefaultValue()) 
                {
                	if(uid.defaultValue() != null)
                	{
                    	uid._setValue(boConvertUtils.convertToBigDecimal(uid.defaultValue(), uid));
                    }
                    else
                    {
                    	uid._setValue(null);
                    }
                    uid.setInputType( AttributeHandler.INPUT_FROM_DEFAULT );
                }
            ...
            if("xwfSerialObject".equals(this.getName()))
            {
