...
public  class messageSystem extends message implements  Serializable {  

   
   
   
    public netgest.bo.runtime.attributes.boAttributeDate delivery;    
    public netgest.bo.runtime.attributes.boAttributeDate read;    
   
     
    public messageSystem() {
        super();
        bo_version      = "1.0";
        bo_name         = "messageSystem";
        bo_classregboui = "#BO.CLSREGBOUI#";
        bo_definition   = boDefHandler.getBoDefinition("messageSystem");
        ...
        atts.add(delivery = new netgest.bo.runtime.attributes.boAttributeDate(this,"delivery"));
        
        atts.add(read = new netgest.bo.runtime.attributes.boAttributeDate(this,"read"));
    ...
    public void init() throws boRuntimeException {
        ...
        if(p_mode == MODE_NEW && !p_clone)
        {
                ...
                if( delivery.haveDefaultValue()) 
                {
                	if(delivery.defaultValue() != null)
                	{
                    	delivery._setValue(boConvertUtils.convertToTimestamp(delivery.defaultValue(), delivery));
                    }
                    else
                    {
                    	delivery._setValue(null);
                    }
                    delivery.setInputType( AttributeHandler.INPUT_FROM_DEFAULT );
                }
            
                if( read.haveDefaultValue()) 
                {
                	if(read.defaultValue() != null)
                	{
                    	read._setValue(boConvertUtils.convertToTimestamp(read.defaultValue(), read));
                    }
                    else
                    {
                    	read._setValue(null);
                    }
                    read.setInputType( AttributeHandler.INPUT_FROM_DEFAULT );
                }
            
    
            if("messageSystem".equals(this.getName()))
            {
