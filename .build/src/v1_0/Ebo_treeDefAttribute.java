...
public  class Ebo_treeDefAttribute extends boObject implements  Serializable {  

   
   
   
    public netgest.bo.runtime.attributes.boAttributeString name;    
    public netgest.bo.runtime.attributes.boAttributeNumber width;    
    public netgest.bo.runtime.attributes.boAttributeString order;    
    public netgest.bo.runtime.attributes.boAttributeNumber orderPos;    
    ...
    public Ebo_treeDefAttribute() {
        super();
        bo_version      = "1.0";
        bo_name         = "Ebo_treeDefAttribute";
        bo_classregboui = "#BO.CLSREGBOUI#";
        bo_definition   = boDefHandler.getBoDefinition("Ebo_treeDefAttribute");
        ...
        atts.add(width = new netgest.bo.runtime.attributes.boAttributeNumber(this,"width"));
        
        atts.add(order = new netgest.bo.runtime.attributes.boAttributeString(this,"order"));
        
        atts.add(orderPos = new netgest.bo.runtime.attributes.boAttributeNumber(this,"orderPos"));
    ...
    public void init() throws boRuntimeException {
        ...
        if(p_mode == MODE_NEW && !p_clone)
        {
                ...
                if( width.haveDefaultValue()) 
                {
                	if(width.defaultValue() != null)
                	{
                    	width._setValue(boConvertUtils.convertToBigDecimal(width.defaultValue(), width));
                    }
                    else
                    {
                    	width._setValue(null);
                    }
                    width.setInputType( AttributeHandler.INPUT_FROM_DEFAULT );
                }
            
                if( order.haveDefaultValue()) 
                {
                	if(order.defaultValue() != null)
                	{
                    	order._setValue(boConvertUtils.convertToString(order.defaultValue(), order));
                    }
                    else
                    {
                    	order._setValue(null);
                    }
                    order.setInputType( AttributeHandler.INPUT_FROM_DEFAULT );
                }
            
                if( orderPos.haveDefaultValue()) 
                {
                	if(orderPos.defaultValue() != null)
                	{
                    	orderPos._setValue(boConvertUtils.convertToBigDecimal(orderPos.defaultValue(), orderPos));
                    }
                    else
                    {
                    	orderPos._setValue(null);
                    }
                    orderPos.setInputType( AttributeHandler.INPUT_FROM_DEFAULT );
                }
            ...
            if("Ebo_treeDefAttribute".equals(this.getName()))
            {
