...
public  class uiObjectHTML extends uiObject implements  Serializable {  

   
   
   
    public netgest.bo.runtime.attributes.boAttributeString htmlcode;    
   
     
    public uiObjectHTML() {
        super();
        bo_version      = "1.0";
        bo_name         = "uiObjectHTML";
        bo_classregboui = "#BO.CLSREGBOUI#";
        bo_definition   = boDefHandler.getBoDefinition("uiObjectHTML");
        ...
        atts.add(htmlcode = new netgest.bo.runtime.attributes.boAttributeString(this,"htmlcode"));
    ...
    public void init() throws boRuntimeException {
        ...
        if(p_mode == MODE_NEW && !p_clone)
        {
                ...
                if( htmlcode.haveDefaultValue()) 
                {
                	if(htmlcode.defaultValue() != null)
                	{
                    	htmlcode._setValue(boConvertUtils.convertToString(htmlcode.defaultValue(), htmlcode));
                    }
                    else
                    {
                    	htmlcode._setValue(null);
                    }
                    htmlcode.setInputType( AttributeHandler.INPUT_FROM_DEFAULT );
                }
            
    
            if("uiObjectHTML".equals(this.getName()))
            {
