...
public  class Ebo_Repository extends boObject implements  Serializable {  

   
   
   
    public netgest.bo.runtime.attributes.boAttributeString name;    
    public netgest.bo.runtime.attributes.boAttributeString schemaName;    
    ...
    public Ebo_Repository() {
        super();
        bo_version      = "1.0";
        bo_name         = "Ebo_Repository";
        bo_classregboui = "#BO.CLSREGBOUI#";
        bo_definition   = boDefHandler.getBoDefinition("Ebo_Repository");
        ...
        atts.add(schemaName = new netgest.bo.runtime.attributes.boAttributeString(this,"schemaName"));
    ...
    public void init() throws boRuntimeException {
        ...
        if(p_mode == MODE_NEW && !p_clone)
        {
                ...
                if( schemaName.haveDefaultValue()) 
                {
                	if(schemaName.defaultValue() != null)
                	{
                    	schemaName._setValue(boConvertUtils.convertToString(schemaName.defaultValue(), schemaName));
                    }
                    else
                    {
                    	schemaName._setValue(null);
                    }
                    schemaName.setInputType( AttributeHandler.INPUT_FROM_DEFAULT );
                }
            ...
            if("Ebo_Repository".equals(this.getName()))
            {
