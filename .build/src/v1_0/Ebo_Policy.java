...
public  class Ebo_Policy extends boObject implements  Serializable {  
    ...
    public Ebo_Policy() {
        ...
        bo_definition   = boDefHandler.getBoDefinition("Ebo_Policy");
        ...
        atts.add(new boBridgeMasterAttribute(this,this.getBoDefinition().getAttributeRef("rule"))); 
    ...
    public void init() throws boRuntimeException {
        ...
        if(p_mode == MODE_NEW && !p_clone)
        {
            ...
            if("Ebo_Policy".equals(this.getName()))
            {
    ...
    public bridgeHandler rule()
    {
        return getBridge("rule");        
    }
    ...
    public bridgeHandler getBridge( String bridgeName ) 
    {
        ...
        if( ret == null )
        {
        	...
        	if( bridgeName.equals("rule") && (ret=brig.get("rule"))==null )
        	{
            	brig.add( ret=new BridgeHandlerrule(new DataResultSet( getDataRow().getRecordChild( getEboContext() ,"rule") ) , this ) );        
        	}
        	if( ret == null )
        	{
    ...
    public final class BridgeHandlerrule extends bridgeHandler {
        ...
        public BridgeHandlerrule(DataResultSet data,boObject parent) {
            super("rule",
                  data,
                  parent.getBoDefinition().getAttributeRef("rule").getBridge().getChildFieldName(),
                  parent
                 );

            p_fatherfield = parent.getBoDefinition().getAttributeRef("rule").getBridge().getFatherFieldName();
            p_childfield  = parent.getBoDefinition().getAttributeRef("rule").getBridge().getChildFieldName();
...
//			  if( newval != null ) allow=allow && att.onBeforeAdd(new boEvent(att, att.EVENT_BEFORE_ADD,newval));
			  ...
			  if( att.fireBeforeChangeEvent( att, null, newval) ) 
			  {
                  ...
                  if(getParent().onChangeSubmit("rule"))
                  {
                      getParent().calculateFormula("rule");
                  }
...
//					att.onAfterChange(new boEvent(att,att.EVENT_AFTER_CHANGE,chgval,null));
		...
		{
			return new BridgeHanlderruleRow( this, row );
		}

		public final class BridgeHanlderruleRow extends boBridgeRow
		{
			public BridgeHanlderruleRow( bridgeHandler bridge, netgest.bo.data.DataRow row )
			{
				super( bridge , row );
				addLine();
			}
			...
			public void addLine() 
			{
				...
				if(this.getAttribute("rule")==null)
					xatt.add( new Handlerrule( getParent(), this ) );
...
return null;
        ...
        public final class Handlerrule extends BridgeObjAttributeHandler {
            public Handlerrule( boObject parent , boBridgeRow bridgerow ) {
                super( parent , bridgerow , bridgerow.getBridge().getDefAttribute().getName().equals("rule")?parent.getBoDefinition().getAttributeRef("rule"):bridgerow.getBridge().getDefAttribute().getBridge().getAttributeRef("rule") );
            }
            public void setValueString(String value) {
                try {
                    ...
                    if(getParent().onChangeSubmit("rule"))
                    {
                        ...
                        getParent().setCalculated(table, "rule");
                        getParent().calculateFormula(table, "rule");
                    }
                } catch (Exception e) {
            ...
            public String getValueString() throws boRuntimeException {
                try {
                    ...
                    String[] args = {getParent().getName(),"rule"};
            ...
            public void setValueFormula(Hashtable table, String[]dependence) throws boRuntimeException
            {
                ...
                {                       
                    if(getParent().isWaiting(table, "rule"))
                    {
                       getParent().setCalculated(table, "rule");
                       this.setValue(boConvertUtils.convertToBigDecimal(formula(), this), false );
                    }
                    else if(!getParent().isCalculated(table, "rule"))
                    {
                        getParent().setCalculated(table, "rule");
                ...
                {   
                    ...
                    if(getParent().onChangeSubmit("rule"))
                    {                 
                        getParent().setWaiting(table, "rule");
                        getParent().calculateFormula(new Hashtable(table), "rule");
                        //getParent().setCalculated(table, "rule");
                    }                    
                }
            }
            ...
            public void setValue(BigDecimal newval, boolean recalc) throws boRuntimeException 
            {
                ...
                if( recalc && getParent().onChangeSubmit("rule"))
                {
                    Hashtable table = new Hashtable();
                    getParent().setCalculated(table, "rule");
                    getParent().calculateFormula(table, "rule");
				...
				{
					getParent().getParentBridgeRow().getBridge().lineChanged("rule");
				}
            }
...
	netgest.bo.security.securityRights.cleanPolicies();
