...
public  class daywork extends boObject implements  Serializable {  
    ...
    public daywork() {
        ...
        bo_definition   = boDefHandler.getBoDefinition("daywork");
        ...
        atts.add(new boBridgeMasterAttribute(this,this.getBoDefinition().getAttributeRef("periods"))); 
    ...
    public void init() throws boRuntimeException {
        ...
        if(p_mode == MODE_NEW && !p_clone)
        {
            ...
            if("daywork".equals(this.getName()))
            {
    ...
    public bridgeHandler periods()
    {
        return getBridge("periods");        
    }
    ...
    public bridgeHandler getBridge( String bridgeName ) 
    {
        ...
        if( ret == null )
        {
        	...
        	if( bridgeName.equals("periods") && (ret=brig.get("periods"))==null )
        	{
            	brig.add( ret=new BridgeHandlerperiods(new DataResultSet( getDataRow().getRecordChild( getEboContext() ,"periods") ) , this ) );        
        	}
        	if( ret == null )
        	{
    ...
    public final class BridgeHandlerperiods extends bridgeHandler {
        ...
        public BridgeHandlerperiods(DataResultSet data,boObject parent) {
            super("periods",
                  data,
                  parent.getBoDefinition().getAttributeRef("periods").getBridge().getChildFieldName(),
                  parent
                 );

            p_fatherfield = parent.getBoDefinition().getAttributeRef("periods").getBridge().getFatherFieldName();
            p_childfield  = parent.getBoDefinition().getAttributeRef("periods").getBridge().getChildFieldName();
...
//			  if( newval != null ) allow=allow && att.onBeforeAdd(new boEvent(att, att.EVENT_BEFORE_ADD,newval));
			  ...
			  if( att.fireBeforeChangeEvent( att, null, newval) ) 
			  {
                  ...
                  if(getParent().onChangeSubmit("periods"))
                  {
                      getParent().calculateFormula("periods");
                  }
...
//					att.onAfterChange(new boEvent(att,att.EVENT_AFTER_CHANGE,chgval,null));
		...
		{
			return new BridgeHanlderperiodsRow( this, row );
		}

		public final class BridgeHanlderperiodsRow extends boBridgeRow
		{
			public BridgeHanlderperiodsRow( bridgeHandler bridge, netgest.bo.data.DataRow row )
			{
				super( bridge , row );
				addLine();
			}
			...
			public void addLine() 
			{
				...
				if(this.getAttribute("periods")==null)
					xatt.add( new Handlerperiods( getParent(), this ) );
...
return null;
        ...
        public final class Handlerperiods extends BridgeObjAttributeHandler {
            public Handlerperiods( boObject parent , boBridgeRow bridgerow ) {
                super( parent , bridgerow , bridgerow.getBridge().getDefAttribute().getName().equals("periods")?parent.getBoDefinition().getAttributeRef("periods"):bridgerow.getBridge().getDefAttribute().getBridge().getAttributeRef("periods") );
            }
            public void setValueString(String value) {
                try {
                    ...
                    if(getParent().onChangeSubmit("periods"))
                    {
                        ...
                        getParent().setCalculated(table, "periods");
                        getParent().calculateFormula(table, "periods");
                    }
                } catch (Exception e) {
            ...
            public String getValueString() throws boRuntimeException {
                try {
                    ...
                    String[] args = {getParent().getName(),"periods"};
            ...
            public void setValueFormula(Hashtable table, String[]dependence) throws boRuntimeException
            {
                ...
                {                       
                    if(getParent().isWaiting(table, "periods"))
                    {
                       getParent().setCalculated(table, "periods");
                       this.setValue(boConvertUtils.convertToBigDecimal(formula(), this), false );
                    }
                    else if(!getParent().isCalculated(table, "periods"))
                    {
                        getParent().setCalculated(table, "periods");
                ...
                {   
                    ...
                    if(getParent().onChangeSubmit("periods"))
                    {                 
                        getParent().setWaiting(table, "periods");
                        getParent().calculateFormula(new Hashtable(table), "periods");
                        //getParent().setCalculated(table, "periods");
                    }                    
                }
            }
            ...
            public void setValue(BigDecimal newval, boolean recalc) throws boRuntimeException 
            {
                ...
                if( recalc && getParent().onChangeSubmit("periods"))
                {
                    Hashtable table = new Hashtable();
                    getParent().setCalculated(table, "periods");
                    getParent().calculateFormula(table, "periods");
				...
				{
					getParent().getParentBridgeRow().getBridge().lineChanged("periods");
				}
            }
