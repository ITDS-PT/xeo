...
public  class imobilizado extends boObject implements  Serializable {  
    ...
    public imobilizado() {
        ...
        bo_definition   = boDefHandler.getBoDefinition("imobilizado");
        ...
        atts.add(new boBridgeMasterAttribute(this,this.getBoDefinition().getAttributeRef("SecurityGroup")));  
        atts.add(new boBridgeMasterAttribute(this,this.getBoDefinition().getAttributeRef("SecurityRole")));  
        atts.add(new boBridgeMasterAttribute(this,this.getBoDefinition().getAttributeRef("SecurityCompartment"))); 
    ...
    public void init() throws boRuntimeException {
        ...
        if(p_mode == MODE_NEW && !p_clone)
        {
            ...
            if("imobilizado".equals(this.getName()))
            {
    ...
    public bridgeHandler SecurityGroup()
    {
        return getBridge("SecurityGroup");        
    }
    
    public bridgeHandler SecurityRole()
    {
        return getBridge("SecurityRole");        
    }
    
    public bridgeHandler SecurityCompartment()
    {
        return getBridge("SecurityCompartment");        
    }
    ...
    public bridgeHandler getBridge( String bridgeName ) 
    {
        ...
        if( ret == null )
        {
        	...
        	if( bridgeName.equals("SecurityGroup") && (ret=brig.get("SecurityGroup"))==null )
        	{
            	brig.add( ret=new BridgeHandlerSecurityGroup(new DataResultSet( getDataRow().getRecordChild( getEboContext() ,"SecurityGroup") ) , this ) );        
        	}
        	if( bridgeName.equals("SecurityRole") && (ret=brig.get("SecurityRole"))==null )
        	{
            	brig.add( ret=new BridgeHandlerSecurityRole(new DataResultSet( getDataRow().getRecordChild( getEboContext() ,"SecurityRole") ) , this ) );        
        	}
        	if( bridgeName.equals("SecurityCompartment") && (ret=brig.get("SecurityCompartment"))==null )
        	{
            	brig.add( ret=new BridgeHandlerSecurityCompartment(new DataResultSet( getDataRow().getRecordChild( getEboContext() ,"SecurityCompartment") ) , this ) );        
        	}
        	if( ret == null )
        	{
    ...
    public final class BridgeHandlerSecurityGroup extends bridgeHandler {
        ...
        public BridgeHandlerSecurityGroup(DataResultSet data,boObject parent) {
            super("SecurityGroup",
                  data,
                  parent.getBoDefinition().getAttributeRef("SecurityGroup").getBridge().getChildFieldName(),
                  parent
                 );

            p_fatherfield = parent.getBoDefinition().getAttributeRef("SecurityGroup").getBridge().getFatherFieldName();
            p_childfield  = parent.getBoDefinition().getAttributeRef("SecurityGroup").getBridge().getChildFieldName();
...
//			  if( newval != null ) allow=allow && att.onBeforeAdd(new boEvent(att, att.EVENT_BEFORE_ADD,newval));
			  ...
			  if( att.fireBeforeChangeEvent( att, null, newval) ) 
			  {
                  ...
                  if(getParent().onChangeSubmit("SecurityGroup"))
                  {
                      getParent().calculateFormula("SecurityGroup");
                  }
...
//					att.onAfterChange(new boEvent(att,att.EVENT_AFTER_CHANGE,chgval,null));
		...
		{
			return new BridgeHanlderSecurityGroupRow( this, row );
		}

		public final class BridgeHanlderSecurityGroupRow extends boBridgeRow
		{
			public BridgeHanlderSecurityGroupRow( bridgeHandler bridge, netgest.bo.data.DataRow row )
			{
				super( bridge , row );
				addLine();
			}
			...
			public void addLine() 
			{
				...
				if(this.getAttribute("SecurityGroup")==null)
					xatt.add( new HandlerSecurityGroup( getParent(), this ) );
...
return null;
        ...
        public final class HandlerSecurityGroup extends BridgeObjAttributeHandler {
            public HandlerSecurityGroup( boObject parent , boBridgeRow bridgerow ) {
                super( parent , bridgerow , bridgerow.getBridge().getDefAttribute().getName().equals("SecurityGroup")?parent.getBoDefinition().getAttributeRef("SecurityGroup"):bridgerow.getBridge().getDefAttribute().getBridge().getAttributeRef("SecurityGroup") );
            }
            public void setValueString(String value) {
                try {
                    ...
                    if(getParent().onChangeSubmit("SecurityGroup"))
                    {
                        ...
                        getParent().setCalculated(table, "SecurityGroup");
                        getParent().calculateFormula(table, "SecurityGroup");
                    }
                } catch (Exception e) {
            ...
            public String getValueString() throws boRuntimeException {
                try {
                    ...
                    String[] args = {getParent().getName(),"SecurityGroup"};
            ...
            public void setValueFormula(Hashtable table, String[]dependence) throws boRuntimeException
            {
                ...
                {                       
                    if(getParent().isWaiting(table, "SecurityGroup"))
                    {
                       getParent().setCalculated(table, "SecurityGroup");
                       this.setValue(boConvertUtils.convertToBigDecimal(formula(), this), false );
                    }
                    else if(!getParent().isCalculated(table, "SecurityGroup"))
                    {
                        getParent().setCalculated(table, "SecurityGroup");
                ...
                else if(getParent().dependsFromWaiting(table, dependence))
                {
                    ...
                    {
                        getParent().clear(table, "SecurityGroup"); 
                        this.setValue(boConvertUtils.convertToBigDecimal(formula(), this), false );
                    }
                }
                else
                {   
                    ...
                    if(getParent().onChangeSubmit("SecurityGroup"))
                    {                 
                        getParent().setWaiting(table, "SecurityGroup");
                        getParent().calculateFormula(new Hashtable(table), "SecurityGroup");
                        //getParent().setCalculated(table, "SecurityGroup");
                    }                    
                }
            }
            ...
            public void setValue(BigDecimal newval, boolean recalc) throws boRuntimeException 
            {
                ...
                if( recalc && getParent().onChangeSubmit("SecurityGroup"))
                {
                    Hashtable table = new Hashtable();
                    getParent().setCalculated(table, "SecurityGroup");
                    getParent().calculateFormula(table, "SecurityGroup");
				...
				{
					getParent().getParentBridgeRow().getBridge().lineChanged("SecurityGroup");
				}
            }
...
return null;
    ...
    public final class BridgeHandlerSecurityRole extends bridgeHandler {
        private DataResultSet p_node;
        private String        p_fatherfield;
        private String        p_childfield;
        private boObject      p_parent;
        ...
        public BridgeHandlerSecurityRole(DataResultSet data,boObject parent) {
            super("SecurityRole",
                  data,
                  parent.getBoDefinition().getAttributeRef("SecurityRole").getBridge().getChildFieldName(),
                  parent
                 );

            p_fatherfield = parent.getBoDefinition().getAttributeRef("SecurityRole").getBridge().getFatherFieldName();
            p_childfield  = parent.getBoDefinition().getAttributeRef("SecurityRole").getBridge().getChildFieldName();
...
//			  if( newval != null ) allow=allow && att.onBeforeAdd(new boEvent(att, att.EVENT_BEFORE_ADD,newval));
			  ...
			  if( att.fireBeforeChangeEvent( att, null, newval) ) 
			  {
                  ...
                  if(getParent().onChangeSubmit("SecurityRole"))
                  {
                      getParent().calculateFormula("SecurityRole");
                  }
                  att.fireAfterChangeEvent( att, null, newval );
//				  if( newval != null ) att.onAfterAdd(new boEvent( att ,att.EVENT_AFTER_ADD,newval));
//				  att.onAfterChange(new boEvent( att ,att.EVENT_AFTER_CHANGE, null ,newval));
...
//					att.onAfterChange(new boEvent(att,att.EVENT_AFTER_CHANGE,chgval,null));
		...
		public boBridgeRow createRow( netgest.bo.data.DataRow row )
		{
			return new BridgeHanlderSecurityRoleRow( this, row );
		}

		public final class BridgeHanlderSecurityRoleRow extends boBridgeRow
		{
			public BridgeHanlderSecurityRoleRow( bridgeHandler bridge, netgest.bo.data.DataRow row )
			{
				super( bridge , row );
				addLine();
			}
			...
			public void addLine() 
			{
				...
				if(this.getAttribute("SecurityRole")==null)
					xatt.add( new HandlerSecurityRole( getParent(), this ) );
...
return null;
        ...
        public final class HandlerSecurityRole extends BridgeObjAttributeHandler {
            public HandlerSecurityRole( boObject parent , boBridgeRow bridgerow ) {
                super( parent , bridgerow , bridgerow.getBridge().getDefAttribute().getName().equals("SecurityRole")?parent.getBoDefinition().getAttributeRef("SecurityRole"):bridgerow.getBridge().getDefAttribute().getBridge().getAttributeRef("SecurityRole") );
            }
            public void setValueString(String value) {
                try {
                    this.setValue(boConvertUtils.convertToBigDecimal(value,this));
                    ...
                    {
                        Hashtable table = new Hashtable();                    
                        getParent().setCalculated(table, "SecurityRole");
                        getParent().calculateFormula(table, "SecurityRole");
                    }
                } catch (Exception e) {
                    super.setInvalid(e.getMessage(),value);
                }
            ...
            public String getValueString() throws boRuntimeException {
                try {
                    ...
                    String[] args = {getParent().getName(),"SecurityRole"};
                    throw new boRuntimeException(this.getClass().getName()+".load(EboContext,long)","BO-3003",e,args);
                }
            }
            ...
            public void setValueFormula(Hashtable table, String[]dependence) throws boRuntimeException
            {
                if(getParent().alreadyCalculated(table, dependence)) 
                {                       
                    if(getParent().isWaiting(table, "SecurityRole"))
                    {
                       getParent().setCalculated(table, "SecurityRole");
                       this.setValue(boConvertUtils.convertToBigDecimal(formula(), this), false );
                    }
                    else if(!getParent().isCalculated(table, "SecurityRole"))
                    {
                        getParent().setCalculated(table, "SecurityRole");
                        this.setValue(boConvertUtils.convertToBigDecimal(formula(), this), table );
                    }
                ...
                else if(getParent().dependsFromWaiting(table, dependence))
                {
                    ...
                    {
                        getParent().clear(table, "SecurityRole"); 
                        this.setValue(boConvertUtils.convertToBigDecimal(formula(), this), false );
                    }
                }
                else
                {   
                    ...
                    if(getParent().onChangeSubmit("SecurityRole"))
                    {                 
                        getParent().setWaiting(table, "SecurityRole");
                        getParent().calculateFormula(new Hashtable(table), "SecurityRole");
                        //getParent().setCalculated(table, "SecurityRole");
                    }                    
                }
            }
            ...
            public void setValue(BigDecimal newval, boolean recalc) throws boRuntimeException 
            {
                ...
                if( recalc && getParent().onChangeSubmit("SecurityRole"))
                {
                    Hashtable table = new Hashtable();
                    getParent().setCalculated(table, "SecurityRole");
                    getParent().calculateFormula(table, "SecurityRole");
				...
				if ( getParent().p_parentBridgeRow != null )
				{
					getParent().getParentBridgeRow().getBridge().lineChanged("SecurityRole");
				}
            }
...
return null;
    ...
    public final class BridgeHandlerSecurityCompartment extends bridgeHandler {
        private DataResultSet p_node;
        private String        p_fatherfield;
        private String        p_childfield;
        private boObject      p_parent;
        ...
        public BridgeHandlerSecurityCompartment(DataResultSet data,boObject parent) {
            super("SecurityCompartment",
                  data,
                  parent.getBoDefinition().getAttributeRef("SecurityCompartment").getBridge().getChildFieldName(),
                  parent
                 );

            p_fatherfield = parent.getBoDefinition().getAttributeRef("SecurityCompartment").getBridge().getFatherFieldName();
            p_childfield  = parent.getBoDefinition().getAttributeRef("SecurityCompartment").getBridge().getChildFieldName();
...
//			  if( newval != null ) allow=allow && att.onBeforeAdd(new boEvent(att, att.EVENT_BEFORE_ADD,newval));
			  ...
			  if( att.fireBeforeChangeEvent( att, null, newval) ) 
			  {
                  ...
                  if(getParent().onChangeSubmit("SecurityCompartment"))
                  {
                      getParent().calculateFormula("SecurityCompartment");
                  }
                  att.fireAfterChangeEvent( att, null, newval );
//				  if( newval != null ) att.onAfterAdd(new boEvent( att ,att.EVENT_AFTER_ADD,newval));
//				  att.onAfterChange(new boEvent( att ,att.EVENT_AFTER_CHANGE, null ,newval));
...
//					att.onAfterChange(new boEvent(att,att.EVENT_AFTER_CHANGE,chgval,null));
		...
		public boBridgeRow createRow( netgest.bo.data.DataRow row )
		{
			return new BridgeHanlderSecurityCompartmentRow( this, row );
		}

		public final class BridgeHanlderSecurityCompartmentRow extends boBridgeRow
		{
			public BridgeHanlderSecurityCompartmentRow( bridgeHandler bridge, netgest.bo.data.DataRow row )
			{
				super( bridge , row );
				addLine();
			}
			...
			public void addLine() 
			{
				...
				if(this.getAttribute("SecurityCompartment")==null)
					xatt.add( new HandlerSecurityCompartment( getParent(), this ) );
...
return null;
        ...
        public final class HandlerSecurityCompartment extends BridgeObjAttributeHandler {
            public HandlerSecurityCompartment( boObject parent , boBridgeRow bridgerow ) {
                super( parent , bridgerow , bridgerow.getBridge().getDefAttribute().getName().equals("SecurityCompartment")?parent.getBoDefinition().getAttributeRef("SecurityCompartment"):bridgerow.getBridge().getDefAttribute().getBridge().getAttributeRef("SecurityCompartment") );
            }
            public void setValueString(String value) {
                try {
                    this.setValue(boConvertUtils.convertToBigDecimal(value,this));
                    ...
                    {
                        Hashtable table = new Hashtable();                    
                        getParent().setCalculated(table, "SecurityCompartment");
                        getParent().calculateFormula(table, "SecurityCompartment");
                    }
                } catch (Exception e) {
                    super.setInvalid(e.getMessage(),value);
                }
            ...
            public String getValueString() throws boRuntimeException {
                try {
                    ...
                    String[] args = {getParent().getName(),"SecurityCompartment"};
                    throw new boRuntimeException(this.getClass().getName()+".load(EboContext,long)","BO-3003",e,args);
                }
            }
            ...
            public void setValueFormula(Hashtable table, String[]dependence) throws boRuntimeException
            {
                if(getParent().alreadyCalculated(table, dependence)) 
                {                       
                    if(getParent().isWaiting(table, "SecurityCompartment"))
                    {
                       getParent().setCalculated(table, "SecurityCompartment");
                       this.setValue(boConvertUtils.convertToBigDecimal(formula(), this), false );
                    }
                    else if(!getParent().isCalculated(table, "SecurityCompartment"))
                    {
                        getParent().setCalculated(table, "SecurityCompartment");
                        this.setValue(boConvertUtils.convertToBigDecimal(formula(), this), table );
                    }
                ...
                else if(getParent().dependsFromWaiting(table, dependence))
                {
                    ...
                    {
                        getParent().clear(table, "SecurityCompartment"); 
                        this.setValue(boConvertUtils.convertToBigDecimal(formula(), this), false );
                    }
                }
                else
                {   
                    ...
                    if(getParent().onChangeSubmit("SecurityCompartment"))
                    {                 
                        getParent().setWaiting(table, "SecurityCompartment");
                        getParent().calculateFormula(new Hashtable(table), "SecurityCompartment");
                        //getParent().setCalculated(table, "SecurityCompartment");
                    }                    
                }
            }
            ...
            public void setValue(BigDecimal newval, boolean recalc) throws boRuntimeException 
            {
                ...
                if( recalc && getParent().onChangeSubmit("SecurityCompartment"))
                {
                    Hashtable table = new Hashtable();
                    getParent().setCalculated(table, "SecurityCompartment");
                    getParent().calculateFormula(table, "SecurityCompartment");
				...
				if ( getParent().p_parentBridgeRow != null )
				{
					getParent().getParentBridgeRow().getBridge().lineChanged("SecurityCompartment");
				}
            }
