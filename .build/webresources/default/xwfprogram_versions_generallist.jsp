...
if(bosession== null) {
    response.sendRedirect("login.jsp?returnPage=xwfprogram_versions_generallist.jsp");
    return;
}
if(boctx==null) {
...
<script>var objLabel='Lista de  Programa WorkFlow';</script>
...
<form class="objectForm" name="boForm" id="<%=IDX%>">
   <%
BOI=currObjectList.getObject();grid = DOC.getGRID("g11499509");
if(grid==null) { 
  bodef=boDefHandler.getBoDefinition("xwfProgramDef");  boDefBridge bridgeDef=boDefHandler.getBoDefinition("xwfProgram").getAttributeRef("versions").getBridge();  grid = DOC.createGRID("g11499509","std",true);
  grid.setTitle();  
...
xattributes.put("width","200");xattributes.put("referenceLabel","xwfProgramDef.version");grid.addColAtr(bodef.getAttributeRef("version"),200,xattributes);
}if ( ( BOI != null && BOI.getAttribute("status").hasRights() ) || securityRights.hasRights( DOC.getEboContext() , currObjectList.getBoDef().getName() ,"status", securityRights.READ )){
xattributes=new Hashtable();
xattributes.put("width","100");xattributes.put("referenceLabel","xwfProgramDef.status");grid.addColAtr(bodef.getAttributeRef("status"),100,xattributes);
}xattributes=new Hashtable();
xattributes.put("width","100");grid.addColBridgeAtr(bridgeDef.getAttributeRef("active"),100,xattributes);
} //endCreategrid
grid.render(pageContext,currObjectList,DOC, DOCLIST);%>

</form>
<%}%>
<FORM name='boFormSubmit' method='post' action='xwfprogram_versions_generallist.jsp'>
