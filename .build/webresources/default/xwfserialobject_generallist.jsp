...
if(bosession== null) {
    response.sendRedirect("login.jsp?returnPage=xwfserialobject_generallist.jsp");
    return;
}
if(boctx==null) {
...
<script>var objLabel='Lista de  Objecto serializado';</script>
...
<form class="objectForm" name="boForm" id="<%=IDX%>">
   ...
   <%
BOI=currObjectList.getObject();grid = DOC.getGRID("g16854396");
if(grid==null) { 
  bodef=boDefHandler.getBoDefinition("xwfSerialObject");  grid = DOC.createGRID("g16854396","std");
  grid.setTitle();  
if ( ( BOI != null && BOI.getAttribute("uid").hasRights() ) || securityRights.hasRights( DOC.getEboContext() , currObjectList.getBoDef().getName() ,"uid", securityRights.READ )){
xattributes=new Hashtable();
xattributes.put("width","100");grid.addColAtr(bodef.getAttributeRef("uid"),100,xattributes);
}if ( ( BOI != null && BOI.getAttribute("objectCls").hasRights() ) || securityRights.hasRights( DOC.getEboContext() , currObjectList.getBoDef().getName() ,"objectCls", securityRights.READ )){
xattributes=new Hashtable();
xattributes.put("width","100");grid.addColAtr(bodef.getAttributeRef("objectCls"),100,"","",xattributes);
}} //endCreategrid
...
<%}%>
<FORM name='boFormSubmit' method='post' action='xwfserialobject_generallist.jsp'>
