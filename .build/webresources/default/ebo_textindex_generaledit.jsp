...
if(bosession== null) {
    response.sendRedirect("login.jsp?returnPage=ebo_textindex_generaledit.jsp");
    return;
}
if(boctx==null) {
...
if ((BOI==null && securityRights.hasRights(boctx,"Ebo_TextIndex")) || 
(BOI!=null && BOI.exists() && securityRights.hasRights(boctx,"Ebo_TextIndex") && securityOPL.canRead(BOI)) || 
(BOI!=null && !BOI.exists() && securityRights.hasRights(boctx,"Ebo_TextIndex",securityRights.ADD)) 
) {
if (BOI!=null && BOI.exists() && !BOI.isEnabledforRequest &&(!securityRights.hasRights(boctx,"Ebo_TextIndex",securityRights.WRITE) ||  !securityOPL.canWrite(BOI) && !BOI.isEnabledforRequest)) { 
  BOI.setDisabled(); }
                        ...
                        <TR>
                           <TD><%if (DOC.hasCategoryRights("identification")){ %>
                              <DIV id="<%=DOCLIST.cvui%>_tabbody_0" class="tab">
                                 <%{ int cvui32582021=DOCLIST.cvui;%>
                                 <table>
                                    <tr>
                                       <td>
                                          <%
BOI=currObjectList.getObject();sec = DOC.getSection("s31262160");
if(sec != null)
  sec.setTitle(DOC.getCategoryLabel("general","identification"));
if(sec==null) { 
  sec = DOC.createSection("s31262160","");
  sec.p_name = "";
  sec.p_id = "";
sec.p_showLabel=false;
  bodef=boDefHandler.getBoDefinition("Ebo_TextIndex");  sec.setTitle(DOC.getCategoryLabel("general","identification"));
  docHTML_sectionRow row; 
boolean canAccessui=BOI.getAttribute("ui").canAccess();

if (canAccessui ) { 
row=sec.addRow();
if( canAccessui) {
xattributes=new Hashtable();
xattributes.put("showlabel","yes");row.addCell(bodef.getAttributeRef("ui"),xattributes,"","");
} else row.addCell();
row.addCell();
} 
boolean canAccessuiClass=BOI.getAttribute("uiClass").canAccess();

if (canAccessuiClass ) { 
row=sec.addRow();
if( canAccessuiClass) {
xattributes=new Hashtable();
xattributes.put("showlabel","yes");row.addCell(bodef.getAttributeRef("uiClass"),xattributes);
} else row.addCell();
row.addCell();
} 
boolean canAccesstext=BOI.getAttribute("text").canAccess();

if (canAccesstext ) { 
row=sec.addRow();
if( canAccesstext) {
xattributes=new Hashtable();
xattributes.put("showlabel","yes");row.addCell(bodef.getAttributeRef("text"),xattributes);
} else row.addCell();
row.addCell();
} 
} //endCreateSection
sec.render(pageContext,currObjectList,DOCLIST,DOC);%></td>
                                    ...
                                    <tr>
                                       <td>
                                          <%
BOI=currObjectList.getObject();sec = DOC.getSection("s27843390");
if(sec != null)
  sec.setTitle(DOC.getCategoryLabel("general","identification"));
if(sec==null) { 
  sec = DOC.createSection("s27843390","");
  sec.p_name = "";
  sec.p_id = "";
sec.p_showLabel=false;
  bodef=boDefHandler.getBoDefinition("Ebo_TextIndex");  sec.setTitle(DOC.getCategoryLabel("general","identification"));
...
if( canAccesstext) {
xattributes=new Hashtable();
xattributes.put("showlabel","no");row.addCell(bodef.getAttributeRef("text"),xattributes);
} else row.addCell();
} 
} //endCreateSection
sec.render(pageContext,currObjectList,DOCLIST,DOC);%></td>
...
<%}%>
<FORM name='boFormSubmit' method='post' action='ebo_textindex_generaledit.jsp'>
