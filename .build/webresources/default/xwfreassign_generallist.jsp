...
if(bosession== null) {
    response.sendRedirect("login.jsp?returnPage=xwfreassign_generallist.jsp");
    return;
}
if(boctx==null) {
...
<script>var objLabel='Lista de  Reassignar Actividade';</script>
...
<form class="objectForm" name="boForm" id="<%=IDX%>">
   ...
   <%
BOI=currObjectList.getObject();grid = DOC.getGRID("g14056815");
if(grid==null) { 
  bodef=boDefHandler.getBoDefinition("xwfReassign");  grid = DOC.createGRID("g14056815","std");
  grid.setTitle();  
...
}if ( ( BOI != null && BOI.getAttribute("from").hasRights() ) || securityRights.hasRights( DOC.getEboContext() , currObjectList.getBoDef().getName() ,"from", securityRights.READ )){
xattributes=new Hashtable();
xattributes.put("width","150");grid.addColAtr(bodef.getAttributeRef("from"),150,"","",xattributes);
...
<%}%>
<FORM name='boFormSubmit' method='post' action='xwfreassign_generallist.jsp'>
