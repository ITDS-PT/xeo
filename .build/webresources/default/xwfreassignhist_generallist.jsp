...
if(bosession== null) {
    response.sendRedirect("login.jsp?returnPage=xwfreassignhist_generallist.jsp");
    return;
}
if(boctx==null) {
...
<script>var objLabel='Lista de  Histórico Atribuições';</script>
...
<form class="objectForm" name="boForm" id="<%=IDX%>">
   ...
   <%
BOI=currObjectList.getObject();grid = DOC.getGRID("g16252379");
if(grid==null) { 
  bodef=boDefHandler.getBoDefinition("xwfReassignHist");  grid = DOC.createGRID("g16252379","std");
  grid.setTitle();  
if ( ( BOI != null && BOI.getAttribute("CREATOR").hasRights() ) || securityRights.hasRights( DOC.getEboContext() , currObjectList.getBoDef().getName() ,"CREATOR", securityRights.READ )){
xattributes=new Hashtable();
xattributes.put("width","150");grid.addColAtr(bodef.getAttributeRef("CREATOR"),150,"","",xattributes);
}if ( ( BOI != null && BOI.getAttribute("de").hasRights() ) || securityRights.hasRights( DOC.getEboContext() , currObjectList.getBoDef().getName() ,"de", securityRights.READ )){
xattributes=new Hashtable();
xattributes.put("width","150");grid.addColAtr(bodef.getAttributeRef("de"),150,"","",xattributes);
...
xattributes=new Hashtable();
xattributes.put("width","150");grid.addColAtr(bodef.getAttributeRef("to"),150,"","",xattributes);
...
xattributes=new Hashtable();
xattributes.put("width","120");grid.addColAtr(bodef.getAttributeRef("SYS_DTCREATE"),120,xattributes);
}if ( ( BOI != null && BOI.getAttribute("actvLabel").hasRights() ) || securityRights.hasRights( DOC.getEboContext() , currObjectList.getBoDef().getName() ,"actvLabel", securityRights.READ )){
xattributes=new Hashtable();
xattributes.put("width","120");grid.addColAtr(bodef.getAttributeRef("actvLabel"),120,xattributes);
}if ( ( BOI != null && BOI.getAttribute("progLabel").hasRights() ) || securityRights.hasRights( DOC.getEboContext() , currObjectList.getBoDef().getName() ,"progLabel", securityRights.READ )){
xattributes=new Hashtable();
xattributes.put("width","120");grid.addColAtr(bodef.getAttributeRef("progLabel"),120,xattributes);
}if ( ( BOI != null && BOI.getAttribute("description").hasRights() ) || securityRights.hasRights( DOC.getEboContext() , currObjectList.getBoDef().getName() ,"description", securityRights.READ )){
xattributes=new Hashtable();
xattributes.put("width","200");grid.addColAtr(bodef.getAttributeRef("description"),200,xattributes);
}} //endCreategrid
...
<%}%>
<FORM name='boFormSubmit' method='post' action='xwfreassignhist_generallist.jsp'>
