...
if(bosession== null) {
    response.sendRedirect("login.jsp?returnPage=xwfdecision_generaledit.jsp");
    return;
}
if(boctx==null) {
...
if ((BOI==null && securityRights.hasRights(boctx,"xwfDecision")) || 
(BOI!=null && BOI.exists() && securityRights.hasRights(boctx,"xwfDecision") && securityOPL.canRead(BOI)) || 
(BOI!=null && !BOI.exists() && securityRights.hasRights(boctx,"xwfDecision",securityRights.ADD)) 
) {
if (BOI!=null && BOI.exists() && !BOI.isEnabledforRequest &&(!securityRights.hasRights(boctx,"xwfDecision",securityRights.WRITE) ||  !securityOPL.canWrite(BOI) && !BOI.isEnabledforRequest)) { 
  BOI.setDisabled(); }
                        ...
                        <TR>
                           <TD><%if (DOC.hasCategoryRights("processo")){ %>
                              <DIV id="<%=DOCLIST.cvui%>_tabbody_0" class="tab">
                                 <%{ int cvui7966840=DOCLIST.cvui;%>
                                 <%
BOI=currObjectList.getObject();sec = DOC.getSection("s16502302");
if(sec != null)
  sec.setTitle("Geral");
if(sec==null) { 
  sec = DOC.createSection("s16502302","");
  sec.p_name = "section_id";
  sec.p_id = "section_id";
  bodef=boDefHandler.getBoDefinition("xwfDecision");  sec.setTitle("Geral");
  sec.p_height="100%";
...
boolean canAccessanswer=BOI.getAttribute("answer").canAccess();

if (canAccessperformer || canAccessanswer ) { 
row=sec.addRow();
...
if( canAccessanswer) {
xattributes=new Hashtable();
xattributes.put("showlabel","yes");row.addCell(bodef.getAttributeRef("answer"),xattributes);
} else row.addCell();
} 
...
if( canAccessjustification) {
xattributes=new Hashtable();
xattributes.put("height","100%");xattributes.put("showlabel","no");row.addCell(bodef.getAttributeRef("justification"),xattributes);
} else row.addCell();
} 
} //endCreateSection
sec.render(pageContext,currObjectList,DOCLIST,DOC);%>

                                 <%DOCLIST.cvui=cvui7966840;}%>
                              </DIV>
                              <%}%>
                           </TD>
                        </TR>
...
<%}%>
<FORM name='boFormSubmit' method='post' action='xwfdecision_generaledit.jsp'>
