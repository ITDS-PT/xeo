...
if(bosession== null) {
    response.sendRedirect("login.jsp?returnPage=messagedelivered_generallist.jsp");
    return;
}
if(boctx==null) {
...
<form class="objectForm" name="boForm" id="<%=IDX%>">
   ...
   <%
BOI=currObjectList.getObject();grid = DOC.getGRID("g18591357");
if(grid==null) { 
  bodef=boDefHandler.getBoDefinition("messageDelivered");  grid = DOC.createGRID("g18591357","std");
  grid.setTitle();  
...
xattributes.put("width","120");grid.addColAtr(bodef.getAttributeRef("from"),120,"","",xattributes);
...
}if ( ( BOI != null && BOI.getAttribute("delivery").hasRights() ) || securityRights.hasRights( DOC.getEboContext() , currObjectList.getBoDef().getName() ,"delivery", securityRights.READ )){
xattributes=new Hashtable();
xattributes.put("width","110");grid.addColAtr(bodef.getAttributeRef("delivery"),110,xattributes);
}if ( ( BOI != null && BOI.getAttribute("read").hasRights() ) || securityRights.hasRights( DOC.getEboContext() , currObjectList.getBoDef().getName() ,"read", securityRights.READ )){
xattributes=new Hashtable();
xattributes.put("width","110");grid.addColAtr(bodef.getAttributeRef("read"),110,xattributes);
}} //endCreategrid
...
<%}%>
<FORM name='boFormSubmit' method='post' action='messagedelivered_generallist.jsp'>
