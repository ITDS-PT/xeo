...
if(bosession== null) {
    response.sendRedirect("login.jsp?returnPage=ebo_announcedetails_generalmyannounces.jsp");
    return;
}
if(boctx==null) {
...
<form class="objectForm" name="boForm" id="<%=IDX%>">
   <%
boDefHandler xbodef = boDefHandler.getBoDefinition( "Ebo_AnnounceDetails");
docHTML_treeRuntime xtree=docHTML_treeServer.getTree(xbodef , xbodef.getPath("Viewers.general.forms.myAnnounces.treeView"), DOC );
docHTML_treeView.render(xtree, pageContext ,DOC,DOCLIST );
%>
<script> window.gridKey='5826885'</script>  

</form>
<%}%>
<FORM name='boFormSubmit' method='post' action='ebo_announcedetails_generalmyannounces.jsp'>
