...
if(bosession== null) {
    response.sendRedirect("login.jsp?returnPage=ebo_repository_generaledit.jsp");
    return;
}
if(boctx==null) {
...
if ((BOI==null && securityRights.hasRights(boctx,"Ebo_Repository")) || 
(BOI!=null && BOI.exists() && securityRights.hasRights(boctx,"Ebo_Repository") && securityOPL.canRead(BOI)) || 
(BOI!=null && !BOI.exists() && securityRights.hasRights(boctx,"Ebo_Repository",securityRights.ADD)) 
) {
if (BOI!=null && BOI.exists() && !BOI.isEnabledforRequest &&(!securityRights.hasRights(boctx,"Ebo_Repository",securityRights.WRITE) ||  !securityOPL.canWrite(BOI) && !BOI.isEnabledforRequest)) { 
  BOI.setDisabled(); }
...
<form class="objectForm" name="boForm" id="<%=IDX%>">
   ...
   <%if(DOC.hasCategoryRights("repository")){%>
   <TABLE id="<%=DOCLIST.cvui%>" class="layout" voui="panel" cellspacing="0" cellpadding="0">
      <TBODY>
         ...
         <TR>
            <TD><%if (DOC.hasCategoryRights("repository")){ %>
               <DIV id="<%=DOCLIST.cvui%>_tabbody_0" class="tab">
                  <%{ int cvui17215513=DOCLIST.cvui;%>
                  <%
BOI=currObjectList.getObject();sec = DOC.getSection("s1264191");
if(sec != null)
  sec.setTitle(DOC.getCategoryLabel("general","repository"));
if(sec==null) { 
  sec = DOC.createSection("s1264191","");
  sec.p_name = "";
  sec.p_id = "";
sec.p_showLabel=false;
  bodef=boDefHandler.getBoDefinition("Ebo_Repository");  sec.setTitle(DOC.getCategoryLabel("general","repository"));
  sec.p_height="100%";
...
boolean canAccessschemaName=BOI.getAttribute("schemaName").canAccess();

if (canAccessschemaName ) { 
row=sec.addRow();
if( canAccessschemaName) {
xattributes=new Hashtable();
xattributes.put("showlabel","yes");row.addCell(bodef.getAttributeRef("schemaName"),xattributes);
} else row.addCell();
} 
...
<%}%>
<FORM name='boFormSubmit' method='post' action='ebo_repository_generaledit.jsp'>
