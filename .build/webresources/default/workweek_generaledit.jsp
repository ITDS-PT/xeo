...
if(bosession== null) {
    response.sendRedirect("login.jsp?returnPage=workweek_generaledit.jsp");
    return;
}
if(boctx==null) {
...
if ((BOI==null && securityRights.hasRights(boctx,"workweek")) || 
(BOI!=null && BOI.exists() && securityRights.hasRights(boctx,"workweek") && securityOPL.canRead(BOI)) || 
(BOI!=null && !BOI.exists() && securityRights.hasRights(boctx,"workweek",securityRights.ADD)) 
) {
if (BOI!=null && BOI.exists() && !BOI.isEnabledforRequest &&(!securityRights.hasRights(boctx,"workweek",securityRights.WRITE) ||  !securityOPL.canWrite(BOI) && !BOI.isEnabledforRequest)) { 
  BOI.setDisabled(); }
         ...
         <TR>
            <TD><%if (DOC.hasCategoryRights("identification")){ %>
               <DIV id="<%=DOCLIST.cvui%>_tabbody_0" class="tab">
                  <%{ int cvui26264924=DOCLIST.cvui;%>
                  <%
BOI=currObjectList.getObject();sec = DOC.getSection("s26026489");
if(sec != null)
  sec.setTitle(DOC.getCategoryLabel("general","identification"));
if(sec==null) { 
  sec = DOC.createSection("s26026489","");
  sec.p_name = "";
  sec.p_id = "";
sec.p_showLabel=false;
  bodef=boDefHandler.getBoDefinition("workweek");  sec.setTitle(DOC.getCategoryLabel("general","identification"));
...
if( canAccessdescription) {
xattributes=new Hashtable();
xattributes.put("showlabel","no");row.addCell(bodef.getAttributeRef("description"),xattributes);
} else row.addCell();
} 
boolean canAccessday1=BOI.getAttribute("day1").canAccess();

if (canAccessday1 ) { 
row=sec.addRow();
if( canAccessday1) {
xattributes=new Hashtable();
xattributes.put("showlabel","yes");row.addCell(bodef.getAttributeRef("day1"),xattributes,"","");
} else row.addCell();
} 
boolean canAccessday2=BOI.getAttribute("day2").canAccess();

if (canAccessday2 ) { 
row=sec.addRow();
if( canAccessday2) {
xattributes=new Hashtable();
xattributes.put("showlabel","yes");row.addCell(bodef.getAttributeRef("day2"),xattributes,"","");
} else row.addCell();
} 
boolean canAccessday3=BOI.getAttribute("day3").canAccess();

if (canAccessday3 ) { 
row=sec.addRow();
if( canAccessday3) {
xattributes=new Hashtable();
xattributes.put("showlabel","yes");row.addCell(bodef.getAttributeRef("day3"),xattributes,"","");
} else row.addCell();
} 
boolean canAccessday4=BOI.getAttribute("day4").canAccess();

if (canAccessday4 ) { 
row=sec.addRow();
if( canAccessday4) {
xattributes=new Hashtable();
xattributes.put("showlabel","yes");row.addCell(bodef.getAttributeRef("day4"),xattributes,"","");
} else row.addCell();
} 
boolean canAccessday5=BOI.getAttribute("day5").canAccess();

if (canAccessday5 ) { 
row=sec.addRow();
if( canAccessday5) {
xattributes=new Hashtable();
xattributes.put("showlabel","yes");row.addCell(bodef.getAttributeRef("day5"),xattributes,"","");
} else row.addCell();
} 
boolean canAccessday6=BOI.getAttribute("day6").canAccess();

if (canAccessday6 ) { 
row=sec.addRow();
if( canAccessday6) {
xattributes=new Hashtable();
xattributes.put("showlabel","yes");row.addCell(bodef.getAttributeRef("day6"),xattributes,"","");
} else row.addCell();
} 
boolean canAccessday7=BOI.getAttribute("day7").canAccess();

if (canAccessday7 ) { 
row=sec.addRow();
if( canAccessday7) {
xattributes=new Hashtable();
xattributes.put("showlabel","yes");row.addCell(bodef.getAttributeRef("day7"),xattributes,"","");
} else row.addCell();
} 
} //endCreateSection
sec.render(pageContext,currObjectList,DOCLIST,DOC);%>

                  <%DOCLIST.cvui=cvui26264924;}%>
               </DIV>
               <%}%>
            </TD>
         </TR>
...
<%}%>
<FORM name='boFormSubmit' method='post' action='workweek_generaledit.jsp'>
