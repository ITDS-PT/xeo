...
if(bosession== null) {
    response.sendRedirect("login.jsp?returnPage=ebo_package_generaledit.jsp");
    return;
}
if(boctx==null) {
...
if ((BOI==null && securityRights.hasRights(boctx,"Ebo_Package")) || 
(BOI!=null && BOI.exists() && securityRights.hasRights(boctx,"Ebo_Package") && securityOPL.canRead(BOI)) || 
(BOI!=null && !BOI.exists() && securityRights.hasRights(boctx,"Ebo_Package",securityRights.ADD)) 
) {
if (BOI!=null && BOI.exists() && !BOI.isEnabledforRequest &&(!securityRights.hasRights(boctx,"Ebo_Package",securityRights.WRITE) ||  !securityOPL.canWrite(BOI) && !BOI.isEnabledforRequest)) { 
  BOI.setDisabled(); }
                        ...
                        <TR>
                           <TD><%if (DOC.hasCategoryRights("pack")){ %>
                              <DIV id="<%=DOCLIST.cvui%>_tabbody_0" class="tab">
                                 <%{ int cvui15685301=DOCLIST.cvui;%>
                                 <%
BOI=currObjectList.getObject();sec = DOC.getSection("s19242802");
if(sec != null)
  sec.setTitle(DOC.getCategoryLabel("general","pack"));
if(sec==null) { 
  sec = DOC.createSection("s19242802","");
  sec.p_name = "";
  sec.p_id = "";
sec.p_showLabel=false;
  bodef=boDefHandler.getBoDefinition("Ebo_Package");  sec.setTitle(DOC.getCategoryLabel("general","pack"));
...
<%}%>
<FORM name='boFormSubmit' method='post' action='ebo_package_generaledit.jsp'>
