...
if(bosession== null) {
    response.sendRedirect("login.jsp?returnPage=ebo_schedule_generaledit.jsp");
    return;
}
if(boctx==null) {
...
if ((BOI==null && securityRights.hasRights(boctx,"Ebo_Schedule")) || 
(BOI!=null && BOI.exists() && securityRights.hasRights(boctx,"Ebo_Schedule") && securityOPL.canRead(BOI)) || 
(BOI!=null && !BOI.exists() && securityRights.hasRights(boctx,"Ebo_Schedule",securityRights.ADD)) 
) {
if (BOI!=null && BOI.exists() && !BOI.isEnabledforRequest &&(!securityRights.hasRights(boctx,"Ebo_Schedule",securityRights.WRITE) ||  !securityOPL.canWrite(BOI) && !BOI.isEnabledforRequest)) { 
  BOI.setDisabled(); }
...
<form class="objectForm" name="boForm" id="<%=IDX%>">
   ...
   <%if(DOC.hasCategoryRights("general")){%>
               ...
               <DIV id="barea_<%=DOCLIST.cvui%>_0" class="areaSEL">
                  ...
                  <TABLE id="<%=DOCLIST.cvui%>" class="layout" voui="panel" cellspacing="0" cellpadding="0">
                     <TBODY>
                                 ...
                                 <TBODY>
                                    <TR>
                                       <TD style="padding:0px" id="<%=DOCLIST.cvui%>_tabs" valign="bottom" noWrap="yes">
                                          <%if (DOC.hasCategoryRights("general")){ %><SPAN class="tab tabOn" id="<%=DOCLIST.cvui%>_tabheader_0" tabNumber="<%=DOCLIST.cvui%>" tabIndex="<%=DOC.getTabindex(DOC.PANEL, BOI==null?"null" : BOI.getName(), String.valueOf(BOI==null?0:BOI.bo_boui), String.valueOf(DOCLIST.cvui) + "_tabheader_0", DOCLIST)%>"><%=DOC.getCategoryLabel_for_TAB_Header("general")%></SPAN><%}%><%if (DOC.hasCategoryRights("scheduling")){ %><SPAN class="tab" id="<%=DOCLIST.cvui%>_tabheader_1" tabNumber="<%=DOCLIST.cvui%>" tabIndex="<%=DOC.getTabindex(DOC.PANEL, BOI==null?"null" : BOI.getName(), String.valueOf(BOI==null?0:BOI.bo_boui), String.valueOf(DOCLIST.cvui) + "_tabheader_1", DOCLIST)%>"><%=DOC.getCategoryLabel_for_TAB_Header("scheduling")%></SPAN><%}%>
                                       </TD>
                                    </TR>
                                 </TBODY>
                              </TABLE>
                        ...
                        <TR>
                           <TD><%if (DOC.hasCategoryRights("general")){ %>
                              <DIV id="<%=DOCLIST.cvui%>_tabbody_0" class="tab">
                                 <%{ int cvui23006419=DOCLIST.cvui;%>
                                 <%
BOI=currObjectList.getObject();sec = DOC.getSection("s21165427");
if(sec != null)
  sec.setTitle("");
if(sec==null) { 
  sec = DOC.createSection("s21165427","");
  sec.p_name = "section_general";
  sec.p_id = "section_general";
sec.p_showLabel=false;
  bodef=boDefHandler.getBoDefinition("Ebo_Schedule");  sec.setTitle("");
  sec.p_height="100%";
...
boolean canAccesslastresultcode=BOI.getAttribute("lastresultcode").canAccess();
boolean canAccessexecutiontime=BOI.getAttribute("executiontime").canAccess();

if (canAccesslastresultcode || canAccessexecutiontime ) { 
row=sec.addRow();
if( canAccesslastresultcode) {
xattributes=new Hashtable();
xattributes.put("showlabel","yes");row.addCell(bodef.getAttributeRef("lastresultcode"),xattributes);
} else row.addCell();
if( canAccessexecutiontime) {
xattributes=new Hashtable();
xattributes.put("showlabel","yes");row.addCell(bodef.getAttributeRef("executiontime"),xattributes);
} else row.addCell();
} 
boolean canAccesslastruntime=BOI.getAttribute("lastruntime").canAccess();
boolean canAccessnextruntime=BOI.getAttribute("nextruntime").canAccess();

if (canAccesslastruntime || canAccessnextruntime ) { 
row=sec.addRow();
if( canAccesslastruntime) {
xattributes=new Hashtable();
xattributes.put("showlabel","yes");row.addCell(bodef.getAttributeRef("lastruntime"),xattributes);
} else row.addCell();
if( canAccessnextruntime) {
xattributes=new Hashtable();
xattributes.put("showlabel","yes");row.addCell(bodef.getAttributeRef("nextruntime"),xattributes);
} else row.addCell();
} 
boolean canAccessstate=BOI.getAttribute("state").canAccess();
boolean canAccessactiveStatus=BOI.getAttribute("activeStatus").canAccess();

if (canAccessstate || canAccessactiveStatus ) { 
row=sec.addRow();
if( canAccessstate) {
xattributes=new Hashtable();
xattributes.put("showlabel","yes");row.addCell(bodef.getAttributeRef("state"),xattributes);
} else row.addCell();
if( canAccessactiveStatus) {
xattributes=new Hashtable();
xattributes.put("showlabel","yes");row.addCell(bodef.getAttributeRef("activeStatus"),xattributes);
} else row.addCell();
} 
boolean canAccessstartdate=BOI.getAttribute("startdate").canAccess();
boolean canAccessenddate=BOI.getAttribute("enddate").canAccess();

if (canAccessstartdate || canAccessenddate ) { 
row=sec.addRow();
if( canAccessstartdate) {
xattributes=new Hashtable();
xattributes.put("showlabel","yes");row.addCell(bodef.getAttributeRef("startdate"),xattributes);
} else row.addCell();
if( canAccessenddate) {
xattributes=new Hashtable();
...
boolean canAccesserrormessage=BOI.getAttribute("errormessage").canAccess();

if (canAccesserrormessage ) { 
row=sec.addRow();
if( canAccesserrormessage) {
xattributes=new Hashtable();
xattributes.put("showlabel","yes");xattributes.put("height","100%");row.addCell(bodef.getAttributeRef("errormessage"),xattributes);
} else row.addCell();
} 
} //endCreateSection
sec.render(pageContext,currObjectList,DOCLIST,DOC);%>

                                 <%DOCLIST.cvui=cvui23006419;}%>
                              </DIV>
                              <%}%>
                              <%if (DOC.hasCategoryRights("scheduling")){ %>
                              <DIV id="<%=DOCLIST.cvui%>_tabbody_1" class="tab">
                                 <%{ int cvui16806263=DOCLIST.cvui;%>
                                 <%
BOI=currObjectList.getObject();sec = DOC.getSection("s1662725");
if(sec != null)
  sec.setTitle("");
if(sec==null) { 
  sec = DOC.createSection("s1662725","");
  sec.p_name = "section_schedule";
  sec.p_id = "section_schedule";
sec.p_showLabel=false;
  bodef=boDefHandler.getBoDefinition("Ebo_Schedule");  sec.setTitle("");
  docHTML_sectionRow row; 
boolean canAccessjavaclass=BOI.getAttribute("javaclass").canAccess();

if (canAccessjavaclass ) { 
row=sec.addRow();
if( canAccessjavaclass) {
xattributes=new Hashtable();
xattributes.put("showlabel","yes");row.addCell(bodef.getAttributeRef("javaclass"),xattributes);
} else row.addCell();
} 
boolean canAccessparameters=BOI.getAttribute("parameters").canAccess();

if (canAccessparameters ) { 
row=sec.addRow();
if( canAccessparameters) {
xattributes=new Hashtable();
xattributes.put("showlabel","yes");row.addCell(bodef.getAttributeRef("parameters"),xattributes);
} else row.addCell();
} 
boolean canAccesstimeout=BOI.getAttribute("timeout").canAccess();

if (canAccesstimeout ) { 
row=sec.addRow();
if( canAccesstimeout) {
xattributes=new Hashtable();
xattributes.put("showlabel","yes");row.addCell(bodef.getAttributeRef("timeout"),xattributes);
} else row.addCell();
} 
boolean canAccessonerrorpar=BOI.getAttribute("onerrorpar").canAccess();

if (canAccessonerrorpar ) { 
row=sec.addRow();
if( canAccessonerrorpar) {
xattributes=new Hashtable();
xattributes.put("showlabel","yes");row.addCell(bodef.getAttributeRef("onerrorpar"),xattributes);
} else row.addCell();
} 
boolean canAccesstype=BOI.getAttribute("type").canAccess();

if (canAccesstype ) { 
row=sec.addRow();
if( canAccesstype) {
xattributes=new Hashtable();
xattributes.put("showlabel","yes");row.addCell(bodef.getAttributeRef("type"),xattributes);
} else row.addCell();
} 
boolean canAccessinterval=BOI.getAttribute("interval").canAccess();

if (canAccessinterval ) { 
row=sec.addRow();
if( canAccessinterval) {
xattributes=new Hashtable();
xattributes.put("showlabel","yes");row.addCell(bodef.getAttributeRef("interval"),xattributes);
} else row.addCell();
row.addCell();
} 
boolean canAccessevery=BOI.getAttribute("every").canAccess();

if (canAccessevery ) { 
row=sec.addRow();
if( canAccessevery) {
xattributes=new Hashtable();
xattributes.put("showlabel","yes");row.addCell(bodef.getAttributeRef("every"),xattributes);
} else row.addCell();
row.addCell();
} 
boolean canAccesseveryweek=BOI.getAttribute("everyweek").canAccess();

if (canAccesseveryweek ) { 
row=sec.addRow();
if( canAccesseveryweek) {
xattributes=new Hashtable();
xattributes.put("showlabel","yes");row.addCell(bodef.getAttributeRef("everyweek"),xattributes);
} else row.addCell();
row.addCell();
} 
boolean canAccesseverymonth=BOI.getAttribute("everymonth").canAccess();

if (canAccesseverymonth ) { 
row=sec.addRow();
if( canAccesseverymonth) {
xattributes=new Hashtable();
xattributes.put("showlabel","yes");row.addCell(bodef.getAttributeRef("everymonth"),xattributes);
} else row.addCell();
row.addCell();
} 
boolean canAccesswhen=BOI.getAttribute("when").canAccess();

if (canAccesswhen ) { 
row=sec.addRow();
if( canAccesswhen) {
xattributes=new Hashtable();
xattributes.put("showlabel","yes");row.addCell(bodef.getAttributeRef("when"),xattributes);
} else row.addCell();
row.addCell();
} 
} //endCreateSection
...
<%}%>
<FORM name='boFormSubmit' method='post' action='ebo_schedule_generaledit.jsp'>
