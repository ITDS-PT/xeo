...
if(bosession== null) {
    response.sendRedirect("login.jsp?returnPage=ebo_policyrule_packages_generallist.jsp");
    return;
}
if(boctx==null) {
...
<script>var objLabel='Lista de  Regra de Segurança';</script>
...
<form class="objectForm" name="boForm" id="<%=IDX%>">
   <%
BOI=currObjectList.getObject();grid = DOC.getGRID("g10080677");
if(grid==null) { 
  bodef=boDefHandler.getBoDefinition("boObject");  boDefBridge bridgeDef=boDefHandler.getBoDefinition("Ebo_PolicyRule").getAttributeRef("packages").getBridge();  grid = DOC.createGRID("g10080677","std",true);
  grid.setTitle();  
xattributes=new Hashtable();
xattributes.put("width","200");xattributes.put("label","Aplicação/Pacote");grid.addColSpecial("childObject.cardId",200,xattributes);
} //endCreategrid
grid.render(pageContext,currObjectList,DOC, DOCLIST);%>

</form>
<%}%>
<FORM name='boFormSubmit' method='post' action='ebo_policyrule_packages_generallist.jsp'>
