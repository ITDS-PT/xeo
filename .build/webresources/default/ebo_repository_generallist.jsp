...
if(bosession== null) {
    response.sendRedirect("login.jsp?returnPage=ebo_repository_generallist.jsp");
    return;
}
if(boctx==null) {
...
<script>var objLabel='Lista de  Repositório';</script>
...
<form class="objectForm" name="boForm" id="<%=IDX%>">
   ...
   <%
BOI=currObjectList.getObject();grid = DOC.getGRID("g25278150");
if(grid==null) { 
  bodef=boDefHandler.getBoDefinition("Ebo_Repository");  grid = DOC.createGRID("g25278150","std");
  grid.setTitle();  
...
}if ( ( BOI != null && BOI.getAttribute("schemaName").hasRights() ) || securityRights.hasRights( DOC.getEboContext() , currObjectList.getBoDef().getName() ,"schemaName", securityRights.READ )){
xattributes=new Hashtable();
xattributes.put("width","150");grid.addColAtr(bodef.getAttributeRef("schemaName"),150,xattributes);
...
xattributes=new Hashtable();
xattributes.put("width","250");grid.addColAtr(bodef.getAttributeRef("description"),250,xattributes);
}} //endCreategrid
...
<%}%>
<FORM name='boFormSubmit' method='post' action='ebo_repository_generallist.jsp'>
