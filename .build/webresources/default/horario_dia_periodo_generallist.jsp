...
if(bosession== null) {
    response.sendRedirect("login.jsp?returnPage=horario_dia_periodo_generallist.jsp");
    return;
}
if(boctx==null) {
...
<script>var objLabel='Lista de  Periodo do dia ';</script>
...
<form class="objectForm" name="boForm" id="<%=IDX%>">
   ...
   <%
BOI=currObjectList.getObject();grid = DOC.getGRID("g33029369");
if(grid==null) { 
  bodef=boDefHandler.getBoDefinition("horario_dia_periodo");  grid = DOC.createGRID("g33029369","std");
  grid.setTitle();  
...
xattributes.put("width","150");grid.addColAtr(bodef.getAttributeRef("description"),150,xattributes);
}if ( ( BOI != null && BOI.getAttribute("beginTimePeriod").hasRights() ) || securityRights.hasRights( DOC.getEboContext() , currObjectList.getBoDef().getName() ,"beginTimePeriod", securityRights.READ )){
xattributes=new Hashtable();
xattributes.put("width","150");grid.addColAtr(bodef.getAttributeRef("beginTimePeriod"),150,xattributes);
}if ( ( BOI != null && BOI.getAttribute("endTimePeriod").hasRights() ) || securityRights.hasRights( DOC.getEboContext() , currObjectList.getBoDef().getName() ,"endTimePeriod", securityRights.READ )){
xattributes=new Hashtable();
xattributes.put("width","150");grid.addColAtr(bodef.getAttributeRef("endTimePeriod"),150,xattributes);
}} //endCreategrid
...
<%}%>
<FORM name='boFormSubmit' method='post' action='horario_dia_periodo_generallist.jsp'>
