...
if(bosession== null) {
    response.sendRedirect("login.jsp?returnPage=xwfwait_generaledit.jsp");
    return;
}
if(boctx==null) {
...
if ((BOI==null && securityRights.hasRights(boctx,"xwfWait")) || 
(BOI!=null && BOI.exists() && securityRights.hasRights(boctx,"xwfWait") && securityOPL.canRead(BOI)) || 
(BOI!=null && !BOI.exists() && securityRights.hasRights(boctx,"xwfWait",securityRights.ADD)) 
) {
if (BOI!=null && BOI.exists() && !BOI.isEnabledforRequest &&(!securityRights.hasRights(boctx,"xwfWait",securityRights.WRITE) ||  !securityOPL.canWrite(BOI) && !BOI.isEnabledforRequest)) { 
  BOI.setDisabled(); }
                        ...
                        <TR>
                           <TD><%if (DOC.hasCategoryRights("global")){ %>
                              <DIV id="<%=DOCLIST.cvui%>_tabbody_0" class="tab">
                                 <%{ int cvui19245494=DOCLIST.cvui;%>
                                 <%
BOI=currObjectList.getObject();sec = DOC.getSection("s27848285");
if(sec != null)
  sec.setTitle(DOC.getCategoryLabel("general","global"));
if(sec==null) { 
  sec = DOC.createSection("s27848285","");
  sec.p_name = "";
  sec.p_id = "";
sec.p_showLabel=false;
  bodef=boDefHandler.getBoDefinition("xwfWait");  sec.setTitle(DOC.getCategoryLabel("general","global"));
...
boolean canAccessbeginTime=BOI.getAttribute("beginTime").canAccess();
boolean canAccessendTime=BOI.getAttribute("endTime").canAccess();

if (canAccessbeginTime || canAccessendTime ) { 
row=sec.addRow();
if( canAccessbeginTime) {
xattributes=new Hashtable();
xattributes.put("showlabel","yes");row.addCell(bodef.getAttributeRef("beginTime"),xattributes);
} else row.addCell();
if( canAccessendTime) {
xattributes=new Hashtable();
xattributes.put("showlabel","yes");row.addCell(bodef.getAttributeRef("endTime"),xattributes);
} else row.addCell();
} 
...
<%}%>
<FORM name='boFormSubmit' method='post' action='xwfwait_generaledit.jsp'>
