...
if(bosession== null) {
    response.sendRedirect("login.jsp?returnPage=xwfwait_generallist.jsp");
    return;
}
if(boctx==null) {
...
<script>var objLabel='Lista de  Temporizador';</script>
...
<form class="objectForm" name="boForm" id="<%=IDX%>">
   ...
   <%
BOI=currObjectList.getObject();grid = DOC.getGRID("g28922595");
if(grid==null) { 
  bodef=boDefHandler.getBoDefinition("xwfWait");  grid = DOC.createGRID("g28922595","std");
  grid.setTitle();  
...
}if ( ( BOI != null && BOI.getAttribute("endTime").hasRights() ) || securityRights.hasRights( DOC.getEboContext() , currObjectList.getBoDef().getName() ,"endTime", securityRights.READ )){
xattributes=new Hashtable();
xattributes.put("width","100");grid.addColAtr(bodef.getAttributeRef("endTime"),100,xattributes);
}} //endCreategrid
...
<%}%>
<FORM name='boFormSubmit' method='post' action='xwfwait_generallist.jsp'>
