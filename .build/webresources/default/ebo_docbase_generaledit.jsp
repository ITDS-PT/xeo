...
if(bosession== null) {
    response.sendRedirect("login.jsp?returnPage=ebo_docbase_generaledit.jsp");
    return;
}
if(boctx==null) {
...
if ((BOI==null && securityRights.hasRights(boctx,"Ebo_DocBase")) || 
(BOI!=null && BOI.exists() && securityRights.hasRights(boctx,"Ebo_DocBase") && securityOPL.canRead(BOI)) || 
(BOI!=null && !BOI.exists() && securityRights.hasRights(boctx,"Ebo_DocBase",securityRights.ADD)) 
) {
if (BOI!=null && BOI.exists() && !BOI.isEnabledforRequest &&(!securityRights.hasRights(boctx,"Ebo_DocBase",securityRights.WRITE) ||  !securityOPL.canWrite(BOI) && !BOI.isEnabledforRequest)) { 
  BOI.setDisabled(); }
...
<form class="objectForm" name="boForm" id="<%=IDX%>">
   ...
   <TABLE id="<%=DOCLIST.cvui%>" class="layout" voui="panel" bo_node="global" order="10" cellspacing="0" cellpadding="0">
      <TBODY>
         <TR height="25">
            <TD>
         ...
         <TR>
            <TD><%if (DOC.hasCategoryRights("global")){ %>
               <DIV id="<%=DOCLIST.cvui%>_tabbody_0" class="tab" style="overflow-y:auto">
                  <%{ int cvui30140412=DOCLIST.cvui;%>
                  <%
BOI=currObjectList.getObject();sec = DOC.getSection("s32200035");
if(sec != null)
  sec.setTitle(DOC.getCategoryLabel("general","global"));
if(sec==null) { 
  sec = DOC.createSection("s32200035","");
  sec.p_name = "seccao_master";
  sec.p_id = "seccao_master";
sec.p_showLabel=false;
  bodef=boDefHandler.getBoDefinition("Ebo_DocBase");  sec.setTitle(DOC.getCategoryLabel("general","global"));
...
sec.render(pageContext,currObjectList,DOCLIST,DOC);%>
                  ...
                  <%
BOI=currObjectList.getObject();sec = DOC.getSection("s20121018");
if(sec != null)
  sec.setTitle("Caracteristicas");
if(sec==null) { 
  sec = DOC.createSection("s20121018","");
  sec.p_name = "phisicalProperties";
  sec.p_id = "phisicalProperties";
  bodef=boDefHandler.getBoDefinition("Ebo_DocBase");  sec.setTitle("Caracteristicas");
  docHTML_sectionRow row; 
...
<%}%>
<FORM name='boFormSubmit' method='post' action='ebo_docbase_generaledit.jsp'>
