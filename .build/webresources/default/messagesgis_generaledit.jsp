...
if(bosession== null) {
    response.sendRedirect("login.jsp?returnPage=messagesgis_generaledit.jsp");
    return;
}
if(boctx==null) {
...
if ((BOI==null && securityRights.hasRights(boctx,"messageSgis")) || 
(BOI!=null && BOI.exists() && securityRights.hasRights(boctx,"messageSgis") && securityOPL.canRead(BOI)) || 
(BOI!=null && !BOI.exists() && securityRights.hasRights(boctx,"messageSgis",securityRights.ADD)) 
) {
if (BOI!=null && BOI.exists() && !BOI.isEnabledforRequest &&(!securityRights.hasRights(boctx,"messageSgis",securityRights.WRITE) ||  !securityOPL.canWrite(BOI) && !BOI.isEnabledforRequest)) { 
  BOI.setDisabled(); }
         ...
         <TR>
            <TD><%if (DOC.hasCategoryRights("settings")){ %>
               <DIV id="<%=DOCLIST.cvui%>_tabbody_0" class="tab" style="overflow-y:auto">
                  <%{ int cvui6618997=DOCLIST.cvui;%>
                  <%
BOI=currObjectList.getObject();sec = DOC.getSection("s12542126");
if(sec != null)
  sec.setTitle("");
if(sec==null) { 
  sec = DOC.createSection("s12542126","");
  sec.p_name = "section_settingsRES";
  sec.p_id = "section_settingsRES";
sec.p_showLabel=false;
  bodef=boDefHandler.getBoDefinition("messageSgis");  sec.setTitle("");
  sec.p_height="520px";
...
sec.render(pageContext,currObjectList,DOCLIST,DOC);%>
                  ...
                  <%DOCLIST.cvui=cvui6618997;}%>
               </DIV>
               <%}%>
               <%if (DOC.hasCategoryRights("settingsRES")){ %>
               <DIV id="<%=DOCLIST.cvui%>_tabbody_1" class="tab" style="overflow-y:auto">
                  <%{ int cvui6577571=DOCLIST.cvui;%>
                  <%
BOI=currObjectList.getObject();sec = DOC.getSection("s20950758");
if(sec != null)
  sec.setTitle("");
if(sec==null) { 
  sec = DOC.createSection("s20950758","");
  sec.p_name = "section_settingsRES";
  sec.p_id = "section_settingsRES";
sec.p_showLabel=false;
  bodef=boDefHandler.getBoDefinition("messageSgis");  sec.setTitle("");
...
sec.render(pageContext,currObjectList,DOCLIST,DOC);%>
                  ...
                  <%DOCLIST.cvui=cvui6577571;}%>
               </DIV>
               <%}%>
               <%if (DOC.hasCategoryRights("options")){ %>
               <DIV id="<%=DOCLIST.cvui%>_tabbody_2" class="tab" style="overflow-y:auto">
                  <%{ int cvui32747761=DOCLIST.cvui;%>
                  <table>
                     <tr height="60px">
                        <td>
                           <%
BOI=currObjectList.getObject();sec = DOC.getSection("s31839374");
if(sec != null)
  sec.setTitle("");
if(sec==null) { 
  sec = DOC.createSection("s31839374","");
...
<%}%>
<FORM name='boFormSubmit' method='post' action='messagesgis_generaledit.jsp'>
