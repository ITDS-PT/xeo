...
if(bosession== null) {
    response.sendRedirect("login.jsp?returnPage=xwfprogramdef_generaledit.jsp");
    return;
}
if(boctx==null) {
...
if ((BOI==null && securityRights.hasRights(boctx,"xwfProgramDef")) || 
(BOI!=null && BOI.exists() && securityRights.hasRights(boctx,"xwfProgramDef") && securityOPL.canRead(BOI)) || 
(BOI!=null && !BOI.exists() && securityRights.hasRights(boctx,"xwfProgramDef",securityRights.ADD)) 
) {
if (BOI!=null && BOI.exists() && !BOI.isEnabledforRequest &&(!securityRights.hasRights(boctx,"xwfProgramDef",securityRights.WRITE) ||  !securityOPL.canWrite(BOI) && !BOI.isEnabledforRequest)) { 
  BOI.setDisabled(); }
...
<%}%>
<TR><TD valign=top style="padding:10px;background-color:#F2F7FA" align=left>
<%}}%>
...
<form class="objectForm" name="boForm" id="<%=IDX%>">
   ...
   <%if(DOC.hasCategoryRights("flow")|| DOC.hasCategoryRights("identification")|| DOC.hasCategoryRights("input")){%>
   <TABLE id="<%=DOCLIST.cvui%>" class="layout" style="background-color:#F2F7FA" cellspacing="0" cellpadding="0">
      <TBODY>
         <TR height="25">
            <TD>
                  ...
                  <TBODY>
                     <TR>
                        <TD style="padding:0px" id="<%=DOCLIST.cvui%>_tabs" valign="bottom" noWrap="yes">
                           <%if (DOC.hasCategoryRights("flow")){ %><SPAN class="tab tabOn" id="<%=DOCLIST.cvui%>_tabheader_0" tabNumber="<%=DOCLIST.cvui%>" tabIndex="<%=DOC.getTabindex(DOC.PANEL, BOI==null?"null" : BOI.getName(), String.valueOf(BOI==null?0:BOI.bo_boui), String.valueOf(DOCLIST.cvui) + "_tabheader_0", DOCLIST)%>"><%=DOC.getCategoryLabel_for_TAB_Header("flow")%></SPAN><%}%><%if (DOC.hasCategoryRights("identification")){ %><SPAN class="tab" id="<%=DOCLIST.cvui%>_tabheader_1" name="tab_share" tabNumber="<%=DOCLIST.cvui%>" tabIndex="<%=DOC.getTabindex(DOC.PANEL, BOI==null?"tab_share" : BOI.getName(), String.valueOf(BOI==null?0:BOI.bo_boui), String.valueOf(DOCLIST.cvui) + "_tabheader_1", DOCLIST)%>"><%=DOC.getCategoryLabel_for_TAB_Header("identification")%></SPAN><%}%><%if (DOC.hasCategoryRights("input")){ %><SPAN class="tab" id="<%=DOCLIST.cvui%>_tabheader_2" name="tab_startes" tabNumber="<%=DOCLIST.cvui%>" tabIndex="<%=DOC.getTabindex(DOC.PANEL, BOI==null?"tab_startes" : BOI.getName(), String.valueOf(BOI==null?0:BOI.bo_boui), String.valueOf(DOCLIST.cvui) + "_tabheader_2", DOCLIST)%>">Objectos de Input</SPAN><%}%>
                        </TD>
                     </TR>
                  </TBODY>
               </TABLE>
         ...
         <TR>
            <TD><%if (DOC.hasCategoryRights("flow")){ %>
               <DIV id="<%=DOCLIST.cvui%>_tabbody_0" class="tab">
                  <%{ int cvui7798186=DOCLIST.cvui;%>
                  <table height="100%" width="100%">
                     <tr>
                        <td>
                           <%
String incFlow1246043466240 = "programBoui=" + (String.valueOf(BOI.getBoui()))+"&designerMode=" + ("edit")+"&docid=" + (IDX)+"&docid=" + ( DOC.getDocIdx() ) ;
%>
<IFRAME  src="__xwfDesigner.jsp?<%=incFlow1246043466240%>" xsrc="__xwfDesigner.jsp?<%=incFlow1246043466240%>" id='inc_Flow' order='90' name='incFlow' frameBorder='0' width='100%' height='100%' scrolling='no' tabindex='<%=DOC.getTabindex(DOC.IFRAME, BOI==null? "incFlow" : BOI.getName(), String.valueOf(parent_boui==null?( BOI==null?0 : BOI.getBoui()  ): new Long(parent_boui).longValue()), "inc_Flow", DOCLIST)%>'></IFRAME></td>
                     </tr>
                  </table>
                  <%DOCLIST.cvui=cvui7798186;}%>
               </DIV>
               <%}%>
               <%if (DOC.hasCategoryRights("identification")){ %>
               <DIV id="<%=DOCLIST.cvui%>_tabbody_1" class="tab">
                  <%{ int cvui507336=DOCLIST.cvui;%>
                  <%
BOI=currObjectList.getObject();sec = DOC.getSection("s3974892");
if(sec != null)
  sec.setTitle(DOC.getCategoryLabel("general","identification"));
if(sec==null) { 
  sec = DOC.createSection("s3974892","");
  sec.p_name = "";
  sec.p_id = "";
sec.p_showLabel=false;
  bodef=boDefHandler.getBoDefinition("xwfProgramDef");  sec.setTitle(DOC.getCategoryLabel("general","identification"));
  sec.p_height="100%";
...
boolean canAccessversion=BOI.getAttribute("version").canAccess();

if (canAccessadministrator || canAccessversion ) { 
row=sec.addRow();
...
if( canAccessversion) {
xattributes=new Hashtable();
xattributes.put("showlabel","yes");row.addCell(bodef.getAttributeRef("version"),xattributes);
} else row.addCell();
} 
...
sec.render(pageContext,currObjectList,DOCLIST,DOC);%>
                  ...
                  <%DOCLIST.cvui=cvui507336;}%>
               </DIV>
               <%}%>
               <%if (DOC.hasCategoryRights("input")){ %>
               <DIV id="<%=DOCLIST.cvui%>_tabbody_2" class="tab">
                  <%{ int cvui32695709=DOCLIST.cvui;%>
                  <div width="100%" height="100%"><% if(BOI.getAttribute("input").hasRights()){%> <IFRAME id='inc_xwfProgramDef__<%=BOI.bo_boui%>__input' xsrc='<%="ebo_clsreg_generallist.jsp?docid="+IDX+"&method=list&parent_attribute=input&parent_boui="+BOI.bo_boui %>' frameBorder=0 width='100%' scrolling=no height='100%' tabindex='<%=DOC.getTabindex(DOC.IFRAME, BOI.getName(), String.valueOf(BOI.bo_boui), "inc_xwfProgramDef__" + BOI.bo_boui + "__input", DOCLIST)%>'></IFRAME><% } %> </div>
                  <%DOCLIST.cvui=cvui32695709;}%>
               </DIV>
               <%}%>
            </TD>
         </TR>
...
<%}%>
<FORM name='boFormSubmit' method='post' action='xwfprogramdef_generaledit.jsp'>
