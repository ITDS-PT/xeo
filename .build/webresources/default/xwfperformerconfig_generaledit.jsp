...
if(bosession== null) {
    response.sendRedirect("login.jsp?returnPage=xwfperformerconfig_generaledit.jsp");
    return;
}
if(boctx==null) {
...
if ((BOI==null && securityRights.hasRights(boctx,"xwfPerformerConfig")) || 
(BOI!=null && BOI.exists() && securityRights.hasRights(boctx,"xwfPerformerConfig") && securityOPL.canRead(BOI)) || 
(BOI!=null && !BOI.exists() && securityRights.hasRights(boctx,"xwfPerformerConfig",securityRights.ADD)) 
) {
if (BOI!=null && BOI.exists() && !BOI.isEnabledforRequest &&(!securityRights.hasRights(boctx,"xwfPerformerConfig",securityRights.WRITE) ||  !securityOPL.canWrite(BOI) && !BOI.isEnabledforRequest)) { 
  BOI.setDisabled(); }
                  ...
                  <TBODY>
                     <TR>
                        <TD style="padding:0px" id="<%=DOCLIST.cvui%>_tabs" valign="bottom" noWrap="yes">
                           <%if (DOC.hasCategoryRights("identification")){ %><SPAN class="tab tabOn" id="<%=DOCLIST.cvui%>_tabheader_0" tabNumber="<%=DOCLIST.cvui%>" tabIndex="<%=DOC.getTabindex(DOC.PANEL, BOI==null?"null" : BOI.getName(), String.valueOf(BOI==null?0:BOI.bo_boui), String.valueOf(DOCLIST.cvui) + "_tabheader_0", DOCLIST)%>">Definições</SPAN><%}%>
                        </TD>
                     </TR>
                  </TBODY>
               </TABLE>
         ...
         <TR>
            <TD><%if (DOC.hasCategoryRights("identification")){ %>
               <DIV id="<%=DOCLIST.cvui%>_tabbody_0" class="tab">
                  <%{ int cvui4511193=DOCLIST.cvui;%>
                  <table>
                     <tr height="1%">
                        <td>
                           <%
BOI=currObjectList.getObject();sec = DOC.getSection("s20162837");
if(sec != null)
  sec.setTitle("");
if(sec==null) { 
  sec = DOC.createSection("s20162837","");
  sec.p_name = "";
  sec.p_id = "";
sec.p_showLabel=false;
  bodef=boDefHandler.getBoDefinition("xwfPerformerConfig");  sec.setTitle("");
...
sec.render(pageContext,currObjectList,DOCLIST,DOC);%></td>
                     ...
                     <tr height="99%">
                        <td>
                           <div height="100%" width="100%"><% if(BOI.getAttribute("wfQueues").hasRights()){%> <IFRAME id='inc_xwfPerformerConfig__<%=BOI.bo_boui%>__wfQueues' xsrc='<%="icontact_generallist.jsp?docid="+IDX+"&method=list&parent_attribute=wfQueues&parent_boui="+BOI.bo_boui %>' frameBorder=0 width='100%' scrolling=no height='100%' tabindex='<%=DOC.getTabindex(DOC.IFRAME, BOI.getName(), String.valueOf(BOI.bo_boui), "inc_xwfPerformerConfig__" + BOI.bo_boui + "__wfQueues", DOCLIST)%>'></IFRAME><% } %> </div></td>
                     </tr>
                  </table>
                  <%DOCLIST.cvui=cvui4511193;}%>
               </DIV>
               <%}%>
            </TD>
         </TR>
...
<%}%>
<FORM name='boFormSubmit' method='post' action='xwfperformerconfig_generaledit.jsp'>
