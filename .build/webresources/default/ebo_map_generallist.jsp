...
if(bosession== null) {
    response.sendRedirect("login.jsp?returnPage=ebo_map_generallist.jsp");
    return;
}
if(boctx==null) {
...
<script>var objLabel='Lista de  Map';</script>
...
<form class="objectForm" name="boForm" id="<%=IDX%>">
   ...
   <%
BOI=currObjectList.getObject();grid = DOC.getGRID("g24750052");
if(grid==null) { 
  bodef=boDefHandler.getBoDefinition("Ebo_Map");  grid = DOC.createGRID("g24750052","std");
  grid.setTitle();  
if ( ( BOI != null && BOI.getAttribute("objectAttributeName").hasRights() ) || securityRights.hasRights( DOC.getEboContext() , currObjectList.getBoDef().getName() ,"objectAttributeName", securityRights.READ )){
xattributes=new Hashtable();
xattributes.put("width","100");grid.addColAtr(bodef.getAttributeRef("objectAttributeName"),100,xattributes);
...
}if ( ( BOI != null && BOI.getAttribute("transformation").hasRights() ) || securityRights.hasRights( DOC.getEboContext() , currObjectList.getBoDef().getName() ,"transformation", securityRights.READ )){
xattributes=new Hashtable();
xattributes.put("width","150");grid.addColAtr(bodef.getAttributeRef("transformation"),150,xattributes);
}} //endCreategrid
...
<%}%>
<FORM name='boFormSubmit' method='post' action='ebo_map_generallist.jsp'>
