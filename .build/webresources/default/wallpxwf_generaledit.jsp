...
if(bosession== null) {
    response.sendRedirect("login.jsp?returnPage=wallpxwf_generaledit.jsp");
    return;
}
if(boctx==null) {
...
if ((BOI==null && securityRights.hasRights(boctx,"wallpxwf")) || 
(BOI!=null && BOI.exists() && securityRights.hasRights(boctx,"wallpxwf") && securityOPL.canRead(BOI)) || 
(BOI!=null && !BOI.exists() && securityRights.hasRights(boctx,"wallpxwf",securityRights.ADD)) 
) {
if (BOI!=null && BOI.exists() && !BOI.isEnabledforRequest &&(!securityRights.hasRights(boctx,"wallpxwf",securityRights.WRITE) ||  !securityOPL.canWrite(BOI) && !BOI.isEnabledforRequest)) { 
  BOI.setDisabled(); }
      ...
      <TBODY>
         <TR>
            <TD class="leftbar"><SPAN class="leftbar" id="<%=DOCLIST.cvui%>" defaultArea="area_<%=DOCLIST.cvui%>_0">
                  ...
                  <%}%>
                  <%if (DOC.hasCategoryRights("identification")){ %>
                  <DIV class="lbItem" id="area_<%=DOCLIST.cvui%>_1" tabIndex="<%=DOC.getTabindex(DOC.AREA, BOI==null?"area_" + String.valueOf(DOCLIST.cvui) + "_1" : BOI.getName(), String.valueOf(BOI==null?0:BOI.bo_boui), "area_" + String.valueOf(DOCLIST.cvui) + "_1", DOCLIST)%>" relatedAreaId="barea_<%=DOCLIST.cvui%>_1" areaNumber="<%=DOCLIST.cvui%>" onclick="so('<%=DOCLIST.cvui%>');loadArea('barea_<%=DOCLIST.cvui%>_1');"><IMG src="resources/area.gif"/>Todas</DIV>
                  <%}%>
               </SPAN></TD>
            <TD id="Areas_<%=DOCLIST.cvui%>">
               <%if (DOC.hasCategoryRights("identification")){ %>
               <DIV id="barea_<%=DOCLIST.cvui%>_0" class="areaSEL">
                  <%{ int cvui16783568=DOCLIST.cvui;%>
                  <table height="100%" width="100%">
                     <tr>
                        <td>
                           <IFRAME id='inc_wallpxwf_xwfActivity' src='xwfactivity_generalmywork.jsp?docid=<%=IDX%>&form=myWork' frameBorder=0 width='100%' scrolling=no height='100%' tabindex='<%=DOC.getTabindex(DOC.IFRAME, "inc_wallpxwf_xwfActivity", DOCLIST)%>'></IFRAME></td>
                     </tr>
                     <tr height="1%">
                                 ...
                                 <TR>
                                    <TD colspan="3" valign="top" style="height:145px" class="box_body">
                                       <div width="100%" height="100%">
                                          <IFRAME id='inc_wallpxwf_Ebo_ShortCut' src='ebo_shortcut_generallist.jsp?docid=<%=IDX%>&form=list&menu=no&canSelectRows=no&showPreview=no&boql=select+Ebo_ShortCut+where+performer%3DCTX_PERFORMER_BOUI' frameBorder=0 width='100%' scrolling=no height='145px' tabindex='<%=DOC.getTabindex(DOC.IFRAME, "inc_wallpxwf_Ebo_ShortCut", DOCLIST)%>'></IFRAME>
                                       </div>
                                    </TD>
                                 </TR>
                              </TBODY>
                  ...
                  <%DOCLIST.cvui=cvui16783568;}%>
               </DIV>
               <%}%>
               <%if (DOC.hasCategoryRights("identification")){ %>
               <DIV id="barea_<%=DOCLIST.cvui%>_1" class="area">
                  <%{ int cvui27649258=DOCLIST.cvui;%>
...
<%}%>
<FORM name='boFormSubmit' method='post' action='wallpxwf_generaledit.jsp'>
