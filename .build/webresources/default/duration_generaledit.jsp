...
if(bosession== null) {
    response.sendRedirect("login.jsp?returnPage=duration_generaledit.jsp");
    return;
}
if(boctx==null) {
...
if ((BOI==null && securityRights.hasRights(boctx,"duration")) || 
(BOI!=null && BOI.exists() && securityRights.hasRights(boctx,"duration") && securityOPL.canRead(BOI)) || 
(BOI!=null && !BOI.exists() && securityRights.hasRights(boctx,"duration",securityRights.ADD)) 
) {
if (BOI!=null && BOI.exists() && !BOI.isEnabledforRequest &&(!securityRights.hasRights(boctx,"duration",securityRights.WRITE) ||  !securityOPL.canWrite(BOI) && !BOI.isEnabledforRequest)) { 
  BOI.setDisabled(); }
...
<form class="objectForm" name="boForm" id="<%=IDX%>">
   <%
BOI=currObjectList.getObject();sec = DOC.getSection("s16888682");
if(sec != null)
  sec.setTitle(DOC.getCategoryLabel("general",""));
if(sec==null) { 
  sec = DOC.createSection("s16888682","");
  sec.p_name = "";
  sec.p_id = "";
sec.p_showLabel=false;
  bodef=boDefHandler.getBoDefinition("duration");  sec.setTitle(DOC.getCategoryLabel("general",""));
  docHTML_sectionRow row; 
boolean canAccessworkDate=BOI.getAttribute("workDate").canAccess();

if (canAccessworkDate ) { 
row=sec.addRow();
if( canAccessworkDate) {
xattributes=new Hashtable();
xattributes.put("showlabel","yes");row.addCell(bodef.getAttributeRef("workDate"),xattributes);
} else row.addCell();
} 
...
<%}%>
<FORM name='boFormSubmit' method='post' action='duration_generaledit.jsp'>
