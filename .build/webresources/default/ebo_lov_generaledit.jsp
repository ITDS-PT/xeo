...
if(bosession== null) {
    response.sendRedirect("login.jsp?returnPage=ebo_lov_generaledit.jsp");
    return;
}
if(boctx==null) {
...
if ((BOI==null && securityRights.hasRights(boctx,"Ebo_LOV")) || 
(BOI!=null && BOI.exists() && securityRights.hasRights(boctx,"Ebo_LOV") && securityOPL.canRead(BOI)) || 
(BOI!=null && !BOI.exists() && securityRights.hasRights(boctx,"Ebo_LOV",securityRights.ADD)) 
) {
if (BOI!=null && BOI.exists() && !BOI.isEnabledforRequest &&(!securityRights.hasRights(boctx,"Ebo_LOV",securityRights.WRITE) ||  !securityOPL.canWrite(BOI) && !BOI.isEnabledforRequest)) { 
  BOI.setDisabled(); }
...
<form class="objectForm" name="boForm" id="<%=IDX%>">
   ...
   <%if(DOC.hasCategoryRights("global")|| DOC.hasCategoryRights("relations")){%>
      ...
      <TBODY>
         <TR>
            <TD class="leftbar"><SPAN class="leftbar" id="<%=DOCLIST.cvui%>" defaultArea="area_<%=DOCLIST.cvui%>_0">
                  ...
                  <%}%>
                  <%if (DOC.hasCategoryRights("relations")){ %>
                  <DIV class="lbItem" id="area_<%=DOCLIST.cvui%>_1" tabIndex="<%=DOC.getTabindex(DOC.AREA, BOI==null?"area_" + String.valueOf(DOCLIST.cvui) + "_1" : BOI.getName(), String.valueOf(BOI==null?0:BOI.bo_boui), "area_" + String.valueOf(DOCLIST.cvui) + "_1", DOCLIST)%>" relatedAreaId="barea_<%=DOCLIST.cvui%>_1" areaNumber="<%=DOCLIST.cvui%>" onclick="so('<%=DOCLIST.cvui%>');loadArea('barea_<%=DOCLIST.cvui%>_1');"><IMG src="resources/area.gif"/>Detalhes</DIV>
                  <%}%>
               </SPAN></TD>
            <TD id="Areas_<%=DOCLIST.cvui%>">
               <%if (DOC.hasCategoryRights("global")){ %>
               <DIV id="barea_<%=DOCLIST.cvui%>_0" class="areaSEL">
                  <%{ int cvui20974951=DOCLIST.cvui;%>
                  <%DOCLIST.cvui=DOCLIST.getVui(IDX, BOI.getBoui(), "barea_" + DOCLIST.cvui + "_0");%>
                  <%if(DOC.hasCategoryRights("global")|| DOC.hasCategoryRights("sql")){%>
                                 ...
                                 <TBODY>
                                    <TR>
                                       <TD style="padding:0px" id="<%=DOCLIST.cvui%>_tabs" valign="bottom" noWrap="yes">
                                          <%if (DOC.hasCategoryRights("global")){ %><SPAN class="tab tabOn" id="<%=DOCLIST.cvui%>_tabheader_0" tabNumber="<%=DOCLIST.cvui%>" tabIndex="<%=DOC.getTabindex(DOC.PANEL, BOI==null?"null" : BOI.getName(), String.valueOf(BOI==null?0:BOI.bo_boui), String.valueOf(DOCLIST.cvui) + "_tabheader_0", DOCLIST)%>"><%=DOC.getCategoryLabel_for_TAB_Header("global")%></SPAN><%}%><%if (DOC.hasCategoryRights("sql")){ %><SPAN class="tab" id="<%=DOCLIST.cvui%>_tabheader_1" tabNumber="<%=DOCLIST.cvui%>" tabIndex="<%=DOC.getTabindex(DOC.PANEL, BOI==null?"null" : BOI.getName(), String.valueOf(BOI==null?0:BOI.bo_boui), String.valueOf(DOCLIST.cvui) + "_tabheader_1", DOCLIST)%>"><%=DOC.getCategoryLabel_for_TAB_Header("sql")%></SPAN><%}%>
                                       </TD>
                                    </TR>
                                 </TBODY>
                              </TABLE>
                        ...
                        <TR>
                           <TD><%if (DOC.hasCategoryRights("global")){ %>
                              <DIV id="<%=DOCLIST.cvui%>_tabbody_0" class="tab">
                                 <%{ int cvui3140342=DOCLIST.cvui;%>
                                 <%
BOI=currObjectList.getObject();sec = DOC.getSection("s32164328");
if(sec != null)
  sec.setTitle(DOC.getCategoryLabel("general","global"));
if(sec==null) { 
  sec = DOC.createSection("s32164328","");
  sec.p_name = "";
  sec.p_id = "";
sec.p_showLabel=false;
  bodef=boDefHandler.getBoDefinition("Ebo_LOV");  sec.setTitle(DOC.getCategoryLabel("general","global"));
...
boolean canAccesslang=BOI.getAttribute("lang").canAccess();

if (canAccesslang ) { 
row=sec.addRow();
if( canAccesslang) {
xattributes=new Hashtable();
xattributes.put("showlabel","yes");row.addCell(bodef.getAttributeRef("lang"),xattributes);
} else row.addCell();
} 
boolean canAccessfromTable=BOI.getAttribute("fromTable").canAccess();

if (canAccessfromTable ) { 
row=sec.addRow();
if( canAccessfromTable) {
xattributes=new Hashtable();
xattributes.put("showlabel","yes");row.addCell(bodef.getAttributeRef("fromTable"),xattributes);
} else row.addCell();
} 
boolean canAccesswhereClause=BOI.getAttribute("whereClause").canAccess();

if (canAccesswhereClause ) { 
row=sec.addRow();
if( canAccesswhereClause) {
xattributes=new Hashtable();
xattributes.put("showlabel","yes");row.addCell(bodef.getAttributeRef("whereClause"),xattributes);
} else row.addCell();
} 
boolean canAccessfieldForLabel=BOI.getAttribute("fieldForLabel").canAccess();

if (canAccessfieldForLabel ) { 
row=sec.addRow();
if( canAccessfieldForLabel) {
xattributes=new Hashtable();
xattributes.put("showlabel","yes");row.addCell(bodef.getAttributeRef("fieldForLabel"),xattributes);
} else row.addCell();
} 
boolean canAccessfieldForCode=BOI.getAttribute("fieldForCode").canAccess();

if (canAccessfieldForCode ) { 
row=sec.addRow();
if( canAccessfieldForCode) {
xattributes=new Hashtable();
xattributes.put("showlabel","yes");row.addCell(bodef.getAttributeRef("fieldForCode"),xattributes);
} else row.addCell();
} 
} //endCreateSection
sec.render(pageContext,currObjectList,DOCLIST,DOC);%>

                                 <%DOCLIST.cvui=cvui3140342;}%>
                              </DIV>
                              <%}%>
                              <%if (DOC.hasCategoryRights("sql")){ %>
                              <DIV id="<%=DOCLIST.cvui%>_tabbody_1" class="tab">
                                 <%{ int cvui23969568=DOCLIST.cvui;%>
                                 <%
BOI=currObjectList.getObject();sec = DOC.getSection("s2029213");
if(sec != null)
  sec.setTitle(DOC.getCategoryLabel("general","sql"));
if(sec==null) { 
  sec = DOC.createSection("s2029213","");
  sec.p_name = "";
  sec.p_id = "";
  bodef=boDefHandler.getBoDefinition("Ebo_LOV");  sec.setTitle(DOC.getCategoryLabel("general","sql"));
  docHTML_sectionRow row; 
boolean canAccesssqlQuery=BOI.getAttribute("sqlQuery").canAccess();

if (canAccesssqlQuery ) { 
row=sec.addRow();
if( canAccesssqlQuery) {
xattributes=new Hashtable();
xattributes.put("showlabel","no");row.addCell(bodef.getAttributeRef("sqlQuery"),xattributes);
} else row.addCell();
} 
} //endCreateSection
sec.render(pageContext,currObjectList,DOCLIST,DOC);%>

                                 <%DOCLIST.cvui=cvui23969568;}%>
                              </DIV>
                              <%}%>
                           </TD>
                        </TR>
               ...
               <DIV id="barea_<%=DOCLIST.cvui%>_1" class="area">
                  ...
                  <TABLE id="<%=DOCLIST.cvui%>" class="layout" cellspacing="0" cellpadding="0">
                     <TBODY>
                           ...
                           <TD>
                              <TABLE cellpadding="0" cellspacing="0" class="tabBar" id="<%=DOCLIST.cvui%>_body" onkeyup="so('<%=DOCLIST.cvui%>');onKeyUpTab_std(event)" onmouseover="so('<%=DOCLIST.cvui%>');onOverTab_std(event)" onmouseout="so('<%=DOCLIST.cvui%>');onOutTab_std(event)" ondragenter="so('<%=DOCLIST.cvui%>');ondragenterTab_std(event)" ondragover="so('<%=DOCLIST.cvui%>');ondragoverTab_std(event)" onclick="so('<%=DOCLIST.cvui%>');onClickTab_std(event)">
                                 <TBODY>
                                    <TR>
                                       <TD style="padding:0px" id="<%=DOCLIST.cvui%>_tabs" valign="bottom" noWrap="yes">
                                          <%if (DOC.hasCategoryRights("detalhes")){ %><SPAN class="tab tabOn" id="<%=DOCLIST.cvui%>_tabheader_0" tabNumber="<%=DOCLIST.cvui%>" tabIndex="<%=DOC.getTabindex(DOC.PANEL, BOI==null?"null" : BOI.getName(), String.valueOf(BOI==null?0:BOI.bo_boui), String.valueOf(DOCLIST.cvui) + "_tabheader_0", DOCLIST)%>"><%=DOC.getCategoryLabel_for_TAB_Header("detalhes")%></SPAN><%}%>
                                       </TD>
                                    </TR>
                                 </TBODY>
                              </TABLE>
                        ...
                        <TR>
                           <TD><%if (DOC.hasCategoryRights("detalhes")){ %>
                              <DIV id="<%=DOCLIST.cvui%>_tabbody_0" class="tab">
                                 <%{ int cvui5557153=DOCLIST.cvui;%>
                                 <div width="100%" height="100%"><% if(BOI.getAttribute("details").hasRights()){%> <IFRAME id='inc_Ebo_LOV__<%=BOI.bo_boui%>__details' xsrc='<%="ebo_lovdetails_generallist.jsp?docid="+IDX+"&method=list&parent_attribute=details&parent_boui="+BOI.bo_boui %>' frameBorder=0 width='100%' scrolling=no height='100%' tabindex='<%=DOC.getTabindex(DOC.IFRAME, BOI.getName(), String.valueOf(BOI.bo_boui), "inc_Ebo_LOV__" + BOI.bo_boui + "__details", DOCLIST)%>'></IFRAME><% } %> </div>
                                 <%DOCLIST.cvui=cvui5557153;}%>
                              </DIV>
                              <%}%>
                           </TD>
                        </TR>
...
<%}%>
<FORM name='boFormSubmit' method='post' action='ebo_lov_generaledit.jsp'>
