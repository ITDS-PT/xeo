...
if(bosession== null) {
    response.sendRedirect("login.jsp?returnPage=ebo_log_generallist.jsp");
    return;
}
if(boctx==null) {
...
<script>var objLabel='Lista de  Log de Alterações';</script>
...
<form class="objectForm" name="boForm" id="<%=IDX%>">
   ...
   <%
BOI=currObjectList.getObject();grid = DOC.getGRID("g28337075");
if(grid==null) { 
  bodef=boDefHandler.getBoDefinition("Ebo_Log");  grid = DOC.createGRID("g28337075","std");
  grid.setTitle();  
...
xattributes.put("width","200");grid.addColAtr(bodef.getAttributeRef("name"),200,xattributes);
...
xattributes=new Hashtable();
xattributes.put("width","200");grid.addColAtr(bodef.getAttributeRef("value"),200,xattributes);
}if ( ( BOI != null && BOI.getAttribute("action").hasRights() ) || securityRights.hasRights( DOC.getEboContext() , currObjectList.getBoDef().getName() ,"action", securityRights.READ )){
xattributes=new Hashtable();
xattributes.put("width","100");grid.addColAtr(bodef.getAttributeRef("action"),100,xattributes);
}if ( ( BOI != null && BOI.getAttribute("line").hasRights() ) || securityRights.hasRights( DOC.getEboContext() , currObjectList.getBoDef().getName() ,"line", securityRights.READ )){
xattributes=new Hashtable();
xattributes.put("width","100");grid.addColAtr(bodef.getAttributeRef("line"),100,xattributes);
}if ( ( BOI != null && BOI.getAttribute("version").hasRights() ) || securityRights.hasRights( DOC.getEboContext() , currObjectList.getBoDef().getName() ,"version", securityRights.READ )){
xattributes=new Hashtable();
xattributes.put("width","100");grid.addColAtr(bodef.getAttributeRef("version"),100,xattributes);
}} //endCreategrid
...
<%}%>
<FORM name='boFormSubmit' method='post' action='ebo_log_generallist.jsp'>
