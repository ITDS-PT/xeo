...
if(bosession== null) {
    response.sendRedirect("login.jsp?returnPage=xwfactivityreceive_generalinbox.jsp");
    return;
}
if(boctx==null) {
...
<form class="objectForm" name="boForm" id="<%=IDX%>">
   <div width="100%" height="100%"><IFRAME    src="__explorer.jsp?docid=<%=DOC.getDocIdx()%>&pageName=<%=pageContext.getPage().getClass().getName()%>&objectName=xwfActivityReceive&form=inbox" id='explorer' frameBorder='0' width='100%' height='100%' scrolling='no' tabindex='0'></IFRAME></div><script> window.gridKey='12827592'</script>  

</form>
<%}%>
<FORM name='boFormSubmit' method='post' action='xwfactivityreceive_generalinbox.jsp'>
