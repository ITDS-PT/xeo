...
if(bosession== null) {
    response.sendRedirect("login.jsp?returnPage=holiday_generaledit.jsp");
    return;
}
if(boctx==null) {
...
if ((BOI==null && securityRights.hasRights(boctx,"holiday")) || 
(BOI!=null && BOI.exists() && securityRights.hasRights(boctx,"holiday") && securityOPL.canRead(BOI)) || 
(BOI!=null && !BOI.exists() && securityRights.hasRights(boctx,"holiday",securityRights.ADD)) 
) {
if (BOI!=null && BOI.exists() && !BOI.isEnabledforRequest &&(!securityRights.hasRights(boctx,"holiday",securityRights.WRITE) ||  !securityOPL.canWrite(BOI) && !BOI.isEnabledforRequest)) { 
  BOI.setDisabled(); }
...
<form class="objectForm" name="boForm" id="<%=IDX%>">
   ...
   <%if(DOC.hasCategoryRights("holiday")){%>
               ...
               <DIV id="barea_<%=DOCLIST.cvui%>_0" class="areaSEL">
                  ...
                  <TABLE id="<%=DOCLIST.cvui%>" class="layout" voui="panel" cellspacing="0" cellpadding="0">
                     <TBODY>
                                 ...
                                 <TBODY>
                                    <TR>
                                       <TD style="padding:0px" id="<%=DOCLIST.cvui%>_tabs" valign="bottom" noWrap="yes">
                                          <%if (DOC.hasCategoryRights("holiday")){ %><SPAN class="tab tabOn" id="<%=DOCLIST.cvui%>_tabheader_0" tabNumber="<%=DOCLIST.cvui%>" tabIndex="<%=DOC.getTabindex(DOC.PANEL, BOI==null?"null" : BOI.getName(), String.valueOf(BOI==null?0:BOI.bo_boui), String.valueOf(DOCLIST.cvui) + "_tabheader_0", DOCLIST)%>"><%=DOC.getCategoryLabel_for_TAB_Header("holiday")%></SPAN><%}%>
                                       </TD>
                                    </TR>
                                 </TBODY>
                              </TABLE>
                        ...
                        <TR>
                           <TD><%if (DOC.hasCategoryRights("holiday")){ %>
                              <DIV id="<%=DOCLIST.cvui%>_tabbody_0" class="tab">
                                 <%{ int cvui10791662=DOCLIST.cvui;%>
                                 <%
BOI=currObjectList.getObject();sec = DOC.getSection("s13768600");
if(sec != null)
  sec.setTitle("");
if(sec==null) { 
  sec = DOC.createSection("s13768600","");
  sec.p_name = "section_holiday";
  sec.p_id = "section_holiday";
sec.p_showLabel=false;
  bodef=boDefHandler.getBoDefinition("holiday");  sec.setTitle("");
  docHTML_sectionRow row; 
boolean canAccessday=BOI.getAttribute("day").canAccess();

if (canAccessday ) { 
row=sec.addRow();
if( canAccessday) {
xattributes=new Hashtable();
xattributes.put("showlabel","yes");row.addCell(bodef.getAttributeRef("day"),xattributes);
} else row.addCell();
row.addCell();
} 
boolean canAccessdefined=BOI.getAttribute("defined").canAccess();

if (canAccessdefined ) { 
row=sec.addRow();
if( canAccessdefined) {
xattributes=new Hashtable();
xattributes.put("showlabel","yes");row.addCell(bodef.getAttributeRef("defined"),xattributes);
} else row.addCell();
} 
boolean canAccessrepeat=BOI.getAttribute("repeat").canAccess();

if (canAccessrepeat ) { 
row=sec.addRow();
if( canAccessrepeat) {
xattributes=new Hashtable();
xattributes.put("showlabel","yes");row.addCell(bodef.getAttributeRef("repeat"),xattributes);
} else row.addCell();
} 
...
<%}%>
<FORM name='boFormSubmit' method='post' action='holiday_generaledit.jsp'>
