...
if(bosession== null) {
    response.sendRedirect("login.jsp?returnPage=polloption_generaledit.jsp");
    return;
}
if(boctx==null) {
...
if ((BOI==null && securityRights.hasRights(boctx,"PollOption")) || 
(BOI!=null && BOI.exists() && securityRights.hasRights(boctx,"PollOption") && securityOPL.canRead(BOI)) || 
(BOI!=null && !BOI.exists() && securityRights.hasRights(boctx,"PollOption",securityRights.ADD)) 
) {
if (BOI!=null && BOI.exists() && !BOI.isEnabledforRequest &&(!securityRights.hasRights(boctx,"PollOption",securityRights.WRITE) ||  !securityOPL.canWrite(BOI) && !BOI.isEnabledforRequest)) { 
  BOI.setDisabled(); }
                        ...
                        <TR>
                           <TD><%if (DOC.hasCategoryRights("processo")){ %>
                              <DIV id="<%=DOCLIST.cvui%>_tabbody_0" class="tab">
                                 <%{ int cvui27473873=DOCLIST.cvui;%>
                                 <%
BOI=currObjectList.getObject();sec = DOC.getSection("s10851539");
if(sec != null)
  sec.setTitle("Geral");
if(sec==null) { 
  sec = DOC.createSection("s10851539","");
  sec.p_name = "section_id";
  sec.p_id = "section_id";
  bodef=boDefHandler.getBoDefinition("PollOption");  sec.setTitle("Geral");
  docHTML_sectionRow row; 
boolean canAccesscodeOption=BOI.getAttribute("codeOption").canAccess();

if (canAccesscodeOption ) { 
row=sec.addRow();
if( canAccesscodeOption) {
xattributes=new Hashtable();
xattributes.put("showlabel","yes");row.addCell(bodef.getAttributeRef("codeOption"),xattributes);
} else row.addCell();
} 
boolean canAccesslabelOption=BOI.getAttribute("labelOption").canAccess();

if (canAccesslabelOption ) { 
row=sec.addRow();
if( canAccesslabelOption) {
xattributes=new Hashtable();
xattributes.put("showlabel","yes");row.addCell(bodef.getAttributeRef("labelOption"),xattributes);
} else row.addCell();
} 
} //endCreateSection
sec.render(pageContext,currObjectList,DOCLIST,DOC);%>

                                 <%DOCLIST.cvui=cvui27473873;}%>
                              </DIV>
                              <%}%>
                           </TD>
                        </TR>
...
<%}%>
<FORM name='boFormSubmit' method='post' action='polloption_generaledit.jsp'>
