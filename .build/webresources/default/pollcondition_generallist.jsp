...
if(bosession== null) {
    response.sendRedirect("login.jsp?returnPage=pollcondition_generallist.jsp");
    return;
}
if(boctx==null) {
...
<script>var objLabel='Lista de  Condição de Votação';</script>
...
<form class="objectForm" name="boForm" id="<%=IDX%>">
   ...
   <%
BOI=currObjectList.getObject();grid = DOC.getGRID("g4159914");
if(grid==null) { 
  bodef=boDefHandler.getBoDefinition("PollCondition");  grid = DOC.createGRID("g4159914","std");
  grid.setTitle();  
if ( ( BOI != null && BOI.getAttribute("code").hasRights() ) || securityRights.hasRights( DOC.getEboContext() , currObjectList.getBoDef().getName() ,"code", securityRights.READ )){
xattributes=new Hashtable();
xattributes.put("width","100");grid.addColAtr(bodef.getAttributeRef("code"),100,xattributes);
}if ( ( BOI != null && BOI.getAttribute("option").hasRights() ) || securityRights.hasRights( DOC.getEboContext() , currObjectList.getBoDef().getName() ,"option", securityRights.READ )){
xattributes=new Hashtable();
xattributes.put("width","150");grid.addColAtr(bodef.getAttributeRef("option"),150,"","",xattributes);
}if ( ( BOI != null && BOI.getAttribute("voter").hasRights() ) || securityRights.hasRights( DOC.getEboContext() , currObjectList.getBoDef().getName() ,"voter", securityRights.READ )){
xattributes=new Hashtable();
xattributes.put("width","150");grid.addColAtr(bodef.getAttributeRef("voter"),150,"","",xattributes);
}if ( ( BOI != null && BOI.getAttribute("criteria").hasRights() ) || securityRights.hasRights( DOC.getEboContext() , currObjectList.getBoDef().getName() ,"criteria", securityRights.READ )){
xattributes=new Hashtable();
xattributes.put("width","150");grid.addColAtr(bodef.getAttributeRef("criteria"),150,xattributes);
}if ( ( BOI != null && BOI.getAttribute("expression").hasRights() ) || securityRights.hasRights( DOC.getEboContext() , currObjectList.getBoDef().getName() ,"expression", securityRights.READ )){
xattributes=new Hashtable();
xattributes.put("width","150");grid.addColAtr(bodef.getAttributeRef("expression"),150,xattributes);
}} //endCreategrid
...
<%}%>
<FORM name='boFormSubmit' method='post' action='pollcondition_generallist.jsp'>
