...
if(bosession== null) {
    response.sendRedirect("login.jsp?returnPage=ebo_login_generallist.jsp");
    return;
}
if(boctx==null) {
...
<script>var objLabel='Lista de  Login';</script>
...
<form class="objectForm" name="boForm" id="<%=IDX%>">
   <%
BOI=currObjectList.getObject();grid = DOC.getGRID("g27399492");
if(grid==null) { 
  bodef=boDefHandler.getBoDefinition("Ebo_Login");  grid = DOC.createGRID("g27399492","std");
  grid.setTitle();  
if ( ( BOI != null && BOI.getAttribute("user").hasRights() ) || securityRights.hasRights( DOC.getEboContext() , currObjectList.getBoDef().getName() ,"user", securityRights.READ )){
xattributes=new Hashtable();
xattributes.put("width","150");grid.addColAtr(bodef.getAttributeRef("user"),150,"","",xattributes);
}if ( ( BOI != null && BOI.getAttribute("data").hasRights() ) || securityRights.hasRights( DOC.getEboContext() , currObjectList.getBoDef().getName() ,"data", securityRights.READ )){
xattributes=new Hashtable();
xattributes.put("width","90");grid.addColAtr(bodef.getAttributeRef("data"),90,xattributes);
}if ( ( BOI != null && BOI.getAttribute("movementType").hasRights() ) || securityRights.hasRights( DOC.getEboContext() , currObjectList.getBoDef().getName() ,"movementType", securityRights.READ )){
xattributes=new Hashtable();
xattributes.put("width","90");grid.addColAtr(bodef.getAttributeRef("movementType"),90,xattributes);
}if ( ( BOI != null && BOI.getAttribute("repository").hasRights() ) || securityRights.hasRights( DOC.getEboContext() , currObjectList.getBoDef().getName() ,"repository", securityRights.READ )){
xattributes=new Hashtable();
xattributes.put("width","90");grid.addColAtr(bodef.getAttributeRef("repository"),90,xattributes);
}if ( ( BOI != null && BOI.getAttribute("iProfile").hasRights() ) || securityRights.hasRights( DOC.getEboContext() , currObjectList.getBoDef().getName() ,"iProfile", securityRights.READ )){
xattributes=new Hashtable();
xattributes.put("width","120");grid.addColAtr(bodef.getAttributeRef("iProfile"),120,"","",xattributes);
}if ( ( BOI != null && BOI.getAttribute("remoteAddr").hasRights() ) || securityRights.hasRights( DOC.getEboContext() , currObjectList.getBoDef().getName() ,"remoteAddr", securityRights.READ )){
xattributes=new Hashtable();
xattributes.put("width","90");grid.addColAtr(bodef.getAttributeRef("remoteAddr"),90,xattributes);
}if ( ( BOI != null && BOI.getAttribute("remoteHost").hasRights() ) || securityRights.hasRights( DOC.getEboContext() , currObjectList.getBoDef().getName() ,"remoteHost", securityRights.READ )){
xattributes=new Hashtable();
xattributes.put("width","90");grid.addColAtr(bodef.getAttributeRef("remoteHost"),90,xattributes);
}if ( ( BOI != null && BOI.getAttribute("remoteUser").hasRights() ) || securityRights.hasRights( DOC.getEboContext() , currObjectList.getBoDef().getName() ,"remoteUser", securityRights.READ )){
xattributes=new Hashtable();
xattributes.put("width","90");grid.addColAtr(bodef.getAttributeRef("remoteUser"),90,xattributes);
}if ( ( BOI != null && BOI.getAttribute("remoteSessionId").hasRights() ) || securityRights.hasRights( DOC.getEboContext() , currObjectList.getBoDef().getName() ,"remoteSessionId", securityRights.READ )){
xattributes=new Hashtable();
xattributes.put("width","90");grid.addColAtr(bodef.getAttributeRef("remoteSessionId"),90,xattributes);
}} //endCreategrid
grid.render(pageContext,currObjectList,DOC, DOCLIST);%>

</form>
<%}%>
<FORM name='boFormSubmit' method='post' action='ebo_login_generallist.jsp'>
