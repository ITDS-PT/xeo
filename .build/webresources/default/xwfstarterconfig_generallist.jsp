...
if(bosession== null) {
    response.sendRedirect("login.jsp?returnPage=xwfstarterconfig_generallist.jsp");
    return;
}
if(boctx==null) {
...
<script>var objLabel='Lista de  Configuração Automática de Fluxos';</script>
...
<form class="objectForm" name="boForm" id="<%=IDX%>">
   ...
   <%
BOI=currObjectList.getObject();grid = DOC.getGRID("g22876490");
if(grid==null) { 
  bodef=boDefHandler.getBoDefinition("xwfStarterConfig");  grid = DOC.createGRID("g22876490","std");
  grid.setTitle();  
...
}if ( ( BOI != null && BOI.getAttribute("object").hasRights() ) || securityRights.hasRights( DOC.getEboContext() , currObjectList.getBoDef().getName() ,"object", securityRights.READ )){
xattributes=new Hashtable();
xattributes.put("width","250");grid.addColAtr(bodef.getAttributeRef("object"),250,"","",xattributes);
...
xattributes=new Hashtable();
xattributes.put("width","120");grid.addColAtr(bodef.getAttributeRef("beginDate"),120,xattributes);
...
xattributes=new Hashtable();
xattributes.put("width","120");grid.addColAtr(bodef.getAttributeRef("endDate"),120,xattributes);
}} //endCreategrid
...
<%}%>
<FORM name='boFormSubmit' method='post' action='xwfstarterconfig_generallist.jsp'>
