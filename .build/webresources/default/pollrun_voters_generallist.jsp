...
if(bosession== null) {
    response.sendRedirect("login.jsp?returnPage=pollrun_voters_generallist.jsp");
    return;
}
if(boctx==null) {
...
<form class="objectForm" name="boForm" id="<%=IDX%>">
   <%
BOI=currObjectList.getObject();grid = DOC.getGRID("g17562091");
if(grid==null) { 
  bodef=boDefHandler.getBoDefinition("iContact");  boDefBridge bridgeDef=boDefHandler.getBoDefinition("PollRun").getAttributeRef("voters").getBridge();  grid = DOC.createGRID("g17562091","std",true);
  grid.setTitle();  
...
xattributes.put("width","200");grid.addColBridgeAtr(bridgeDef.getAttributeRef("option"),200,"","",xattributes);
xattributes=new Hashtable();
xattributes.put("width","200");grid.addColBridgeAtr(bridgeDef.getAttributeRef("nameV"),200,xattributes);
} //endCreategrid
grid.render(pageContext,currObjectList,DOC, DOCLIST);%>

</form>
<%}%>
<FORM name='boFormSubmit' method='post' action='pollrun_voters_generallist.jsp'>
