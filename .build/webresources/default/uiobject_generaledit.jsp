...
if(bosession== null) {
    response.sendRedirect("login.jsp?returnPage=uiobject_generaledit.jsp");
    return;
}
if(boctx==null) {
...
if ((BOI==null && securityRights.hasRights(boctx,"uiObject")) || 
(BOI!=null && BOI.exists() && securityRights.hasRights(boctx,"uiObject") && securityOPL.canRead(BOI)) || 
(BOI!=null && !BOI.exists() && securityRights.hasRights(boctx,"uiObject",securityRights.ADD)) 
) {
if (BOI!=null && BOI.exists() && !BOI.isEnabledforRequest &&(!securityRights.hasRights(boctx,"uiObject",securityRights.WRITE) ||  !securityOPL.canWrite(BOI) && !BOI.isEnabledforRequest)) { 
  BOI.setDisabled(); }
         ...
         <TR>
            <TD><%if (DOC.hasCategoryRights("settings")){ %>
               <DIV id="<%=DOCLIST.cvui%>_tabbody_0" class="tab">
                  <%{ int cvui30440136=DOCLIST.cvui;%>
                  <%
BOI=currObjectList.getObject();sec = DOC.getSection("s2948639");
if(sec != null)
  sec.setTitle(DOC.getCategoryLabel("general","settings"));
if(sec==null) { 
  sec = DOC.createSection("s2948639","");
  sec.p_name = "";
  sec.p_id = "";
sec.p_showLabel=false;
  bodef=boDefHandler.getBoDefinition("uiObject");  sec.setTitle(DOC.getCategoryLabel("general","settings"));
...
<%}%>
<FORM name='boFormSubmit' method='post' action='uiobject_generaledit.jsp'>
