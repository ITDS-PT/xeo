...
if(bosession== null) {
    response.sendRedirect("login.jsp?returnPage=ebo_history_generallist.jsp");
    return;
}
if(boctx==null) {
...
<script>var objLabel='Lista de  Controlo de Versionamento';</script>
...
<form class="objectForm" name="boForm" id="<%=IDX%>">
   ...
   <%
BOI=currObjectList.getObject();grid = DOC.getGRID("g4711920");
if(grid==null) { 
  bodef=boDefHandler.getBoDefinition("Ebo_History");  grid = DOC.createGRID("g4711920","std");
  grid.setTitle();  
...
xattributes=new Hashtable();
xattributes.put("width","40");grid.addColAtr(bodef.getAttributeRef("user"),40,"","",xattributes);
}if ( ( BOI != null && BOI.getAttribute("versionDate").hasRights() ) || securityRights.hasRights( DOC.getEboContext() , currObjectList.getBoDef().getName() ,"versionDate", securityRights.READ )){
xattributes=new Hashtable();
xattributes.put("width","40");grid.addColAtr(bodef.getAttributeRef("versionDate"),40,xattributes);
}if ( ( BOI != null && BOI.getAttribute("comment").hasRights() ) || securityRights.hasRights( DOC.getEboContext() , currObjectList.getBoDef().getName() ,"comment", securityRights.READ )){
xattributes=new Hashtable();
xattributes.put("width","40");grid.addColAtr(bodef.getAttributeRef("comment"),40,xattributes);
}} //endCreategrid
...
<%}%>
<FORM name='boFormSubmit' method='post' action='ebo_history_generallist.jsp'>
