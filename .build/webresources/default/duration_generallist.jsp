...
if(bosession== null) {
    response.sendRedirect("login.jsp?returnPage=duration_generallist.jsp");
    return;
}
if(boctx==null) {
...
<script>var objLabel='Lista de  Historico de trabalho';</script>
...
<form class="objectForm" name="boForm" id="<%=IDX%>">
   ...
   <%
BOI=currObjectList.getObject();grid = DOC.getGRID("g11154499");
if(grid==null) { 
  bodef=boDefHandler.getBoDefinition("duration");  grid = DOC.createGRID("g11154499","std");
  grid.setTitle();  
if ( ( BOI != null && BOI.getAttribute("journal").hasRights() ) || securityRights.hasRights( DOC.getEboContext() , currObjectList.getBoDef().getName() ,"journal", securityRights.READ )){
xattributes=new Hashtable();
xattributes.put("width","100");grid.addColAtr(bodef.getAttributeRef("journal"),100,xattributes);
...
xattributes=new Hashtable();
xattributes.put("width","100");grid.addColAtr(bodef.getAttributeRef("performer"),100,"","",xattributes);
}if ( ( BOI != null && BOI.getAttribute("workDate").hasRights() ) || securityRights.hasRights( DOC.getEboContext() , currObjectList.getBoDef().getName() ,"workDate", securityRights.READ )){
xattributes=new Hashtable();
xattributes.put("width","100");grid.addColAtr(bodef.getAttributeRef("workDate"),100,xattributes);
}if ( ( BOI != null && BOI.getAttribute("duration").hasRights() ) || securityRights.hasRights( DOC.getEboContext() , currObjectList.getBoDef().getName() ,"duration", securityRights.READ )){
xattributes=new Hashtable();
xattributes.put("width","100");grid.addColAtr(bodef.getAttributeRef("duration"),100,xattributes);
}} //endCreategrid
...
<%}%>
<FORM name='boFormSubmit' method='post' action='duration_generallist.jsp'>
