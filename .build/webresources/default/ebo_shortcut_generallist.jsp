...
if(bosession== null) {
    response.sendRedirect("login.jsp?returnPage=ebo_shortcut_generallist.jsp");
    return;
}
if(boctx==null) {
...
<script>var objLabel='Lista de  Atalho';</script>
...
<form class="objectForm" name="boForm" id="<%=IDX%>">
   ...
   <%
BOI=currObjectList.getObject();grid = DOC.getGRID("g29034380");
if(grid==null) { 
  bodef=boDefHandler.getBoDefinition("Ebo_ShortCut");  grid = DOC.createGRID("g29034380","std");
  grid.setTitle();  
...
}if ( ( BOI != null && BOI.getAttribute("cls").hasRights() ) || securityRights.hasRights( DOC.getEboContext() , currObjectList.getBoDef().getName() ,"cls", securityRights.READ )){
xattributes=new Hashtable();
xattributes.put("width","150");grid.addColAtr(bodef.getAttributeRef("cls"),150,"","",xattributes);
...
xattributes=new Hashtable();
xattributes.put("width","3");grid.addColAtr(bodef.getAttributeRef("action"),3,xattributes);
}} //endCreategrid
...
<%}%>
<FORM name='boFormSubmit' method='post' action='ebo_shortcut_generallist.jsp'>
