...
if(bosession== null) {
    response.sendRedirect("login.jsp?returnPage=ebo_versioning_generaledit.jsp");
    return;
}
if(boctx==null) {
...
if ((BOI==null && securityRights.hasRights(boctx,"Ebo_Versioning")) || 
(BOI!=null && BOI.exists() && securityRights.hasRights(boctx,"Ebo_Versioning") && securityOPL.canRead(BOI)) || 
(BOI!=null && !BOI.exists() && securityRights.hasRights(boctx,"Ebo_Versioning",securityRights.ADD)) 
) {
if (BOI!=null && BOI.exists() && !BOI.isEnabledforRequest &&(!securityRights.hasRights(boctx,"Ebo_Versioning",securityRights.WRITE) ||  !securityOPL.canWrite(BOI) && !BOI.isEnabledforRequest)) { 
  BOI.setDisabled(); }
...
<form class="objectForm" name="boForm" id="<%=IDX%>">
   ...
   <%if(DOC.hasCategoryRights("geral")|| DOC.hasCategoryRights("log")){%>
                  ...
                  <TBODY>
                     <TR>
                        <TD style="padding:0px" id="<%=DOCLIST.cvui%>_tabs" valign="bottom" noWrap="yes">
                           <%if (DOC.hasCategoryRights("geral")){ %><SPAN class="tab tabOn" id="<%=DOCLIST.cvui%>_tabheader_0" tabNumber="<%=DOCLIST.cvui%>" tabIndex="<%=DOC.getTabindex(DOC.PANEL, BOI==null?"null" : BOI.getName(), String.valueOf(BOI==null?0:BOI.bo_boui), String.valueOf(DOCLIST.cvui) + "_tabheader_0", DOCLIST)%>">Geral</SPAN><%}%><%if (DOC.hasCategoryRights("log")){ %><SPAN class="tab" id="<%=DOCLIST.cvui%>_tabheader_1" tabNumber="<%=DOCLIST.cvui%>" tabIndex="<%=DOC.getTabindex(DOC.PANEL, BOI==null?"null" : BOI.getName(), String.valueOf(BOI==null?0:BOI.bo_boui), String.valueOf(DOCLIST.cvui) + "_tabheader_1", DOCLIST)%>"><%=DOC.getCategoryLabel_for_TAB_Header("log")%></SPAN><%}%>
                        </TD>
                     </TR>
                  </TBODY>
               </TABLE>
         ...
         <TR>
            <TD><%if (DOC.hasCategoryRights("geral")){ %>
               <DIV id="<%=DOCLIST.cvui%>_tabbody_0" class="tab">
                  <%{ int cvui18496860=DOCLIST.cvui;%>
                  <%
BOI=currObjectList.getObject();sec = DOC.getSection("s28484135");
if(sec != null)
  sec.setTitle("Dados");
if(sec==null) { 
  sec = DOC.createSection("s28484135","");
  sec.p_name = "";
  sec.p_id = "";
sec.p_showLabel=false;
  bodef=boDefHandler.getBoDefinition("Ebo_Versioning");  sec.setTitle("Dados");
  docHTML_sectionRow row; 
boolean canAccesschangedObject=BOI.getAttribute("changedObject").canAccess();
boolean canAccessversion=BOI.getAttribute("version").canAccess();

if (canAccesschangedObject || canAccessversion ) { 
row=sec.addRow();
if( canAccesschangedObject) {
xattributes=new Hashtable();
xattributes.put("showlabel","yes");row.addCell(bodef.getAttributeRef("changedObject"),xattributes);
} else row.addCell();
if( canAccessversion) {
xattributes=new Hashtable();
...
boolean canAccessCREATOR=BOI.getAttribute("CREATOR").canAccess();
boolean canAccessSYS_DTCREATE=BOI.getAttribute("SYS_DTCREATE").canAccess();

if (canAccessCREATOR || canAccessSYS_DTCREATE ) { 
row=sec.addRow();
if( canAccessCREATOR) {
xattributes=new Hashtable();
xattributes.put("showlabel","yes");row.addCell(bodef.getAttributeRef("CREATOR"),xattributes,"","");
} else row.addCell();
if( canAccessSYS_DTCREATE) {
xattributes=new Hashtable();
xattributes.put("showlabel","yes");row.addCell(bodef.getAttributeRef("SYS_DTCREATE"),xattributes);
} else row.addCell();
} 
} //endCreateSection
sec.render(pageContext,currObjectList,DOCLIST,DOC);%>

                  <%DOCLIST.cvui=cvui18496860;}%>
               </DIV>
               <%}%>
               <%if (DOC.hasCategoryRights("log")){ %>
               <DIV id="<%=DOCLIST.cvui%>_tabbody_1" class="tab">
                  <%{ int cvui11791317=DOCLIST.cvui;%>
                  <div width="100%" height="100%"><% if(BOI.getAttribute("log").hasRights()){%> <IFRAME id='inc_Ebo_Versioning__<%=BOI.bo_boui%>__log' xsrc='<%="ebo_log_generallist.jsp?docid="+IDX+"&method=list&parent_attribute=log&parent_boui="+BOI.bo_boui %>' frameBorder=0 width='100%' scrolling=no height='100%' tabindex='<%=DOC.getTabindex(DOC.IFRAME, BOI.getName(), String.valueOf(BOI.bo_boui), "inc_Ebo_Versioning__" + BOI.bo_boui + "__log", DOCLIST)%>'></IFRAME><% } %> </div>
                  <%DOCLIST.cvui=cvui11791317;}%>
               </DIV>
               <%}%>
            </TD>
         </TR>
...
<%}%>
<FORM name='boFormSubmit' method='post' action='ebo_versioning_generaledit.jsp'>
