...
if(bosession== null) {
    response.sendRedirect("login.jsp?returnPage=xwfprogramruntime_generallist.jsp");
    return;
}
if(boctx==null) {
...
<script>var objLabel='Lista de  Programa WorkFlow em Execução';</script>
...
<form class="objectForm" name="boForm" id="<%=IDX%>">
   ...
   <%
BOI=currObjectList.getObject();grid = DOC.getGRID("g19064248");
if(grid==null) { 
  bodef=boDefHandler.getBoDefinition("xwfProgramRuntime");  grid = DOC.createGRID("g19064248","std");
  grid.setTitle();  
if ( ( BOI != null && BOI.getAttribute("runningState").hasRights() ) || securityRights.hasRights( DOC.getEboContext() , currObjectList.getBoDef().getName() ,"runningState", securityRights.READ )){
xattributes=new Hashtable();
xattributes.put("width","50");}if ( ( BOI != null && BOI.getAttribute("label").hasRights() ) || securityRights.hasRights( DOC.getEboContext() , currObjectList.getBoDef().getName() ,"label", securityRights.READ )){
xattributes=new Hashtable();
xattributes.put("width","50");grid.addColAtr(bodef.getAttributeRef("label"),50,xattributes);
...
<%}%>
<FORM name='boFormSubmit' method='post' action='xwfprogramruntime_generallist.jsp'>
