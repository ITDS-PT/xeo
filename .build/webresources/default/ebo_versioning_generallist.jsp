...
if(bosession== null) {
    response.sendRedirect("login.jsp?returnPage=ebo_versioning_generallist.jsp");
    return;
}
if(boctx==null) {
...
<script>var objLabel='Lista de  Objecto de Versão';</script>
...
<form class="objectForm" name="boForm" id="<%=IDX%>">
   ...
   <%
BOI=currObjectList.getObject();grid = DOC.getGRID("g4204225");
if(grid==null) { 
  bodef=boDefHandler.getBoDefinition("Ebo_Versioning");  grid = DOC.createGRID("g4204225","std");
  grid.setTitle();  
if ( ( BOI != null && BOI.getAttribute("changedObject").hasRights() ) || securityRights.hasRights( DOC.getEboContext() , currObjectList.getBoDef().getName() ,"changedObject", securityRights.READ )){
xattributes=new Hashtable();
xattributes.put("width","200");grid.addColAtr(bodef.getAttributeRef("changedObject"),200,xattributes);
...
xattributes=new Hashtable();
xattributes.put("width","200");grid.addColAtr(bodef.getAttributeRef("version"),200,xattributes);
...
xattributes=new Hashtable();
xattributes.put("width","200");grid.addColAtr(bodef.getAttributeRef("SYS_DTCREATE"),200,xattributes);
...
xattributes=new Hashtable();
xattributes.put("width","200");grid.addColAtr(bodef.getAttributeRef("CREATOR"),200,"","",xattributes);
}} //endCreategrid
...
<%}%>
<FORM name='boFormSubmit' method='post' action='ebo_versioning_generallist.jsp'>
