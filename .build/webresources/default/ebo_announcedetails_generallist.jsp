...
if(bosession== null) {
    response.sendRedirect("login.jsp?returnPage=ebo_announcedetails_generallist.jsp");
    return;
}
if(boctx==null) {
...
<script>var objLabel='Lista de  Anuncio detalhe';</script>
...
<form class="objectForm" name="boForm" id="<%=IDX%>">
   ...
   <%
BOI=currObjectList.getObject();grid = DOC.getGRID("g4739932");
if(grid==null) { 
  bodef=boDefHandler.getBoDefinition("Ebo_AnnounceDetails");  grid = DOC.createGRID("g4739932","std");
  grid.setTitle();  
if ( ( BOI != null && BOI.getAttribute("referenceObject").hasRights() ) || securityRights.hasRights( DOC.getEboContext() , currObjectList.getBoDef().getName() ,"referenceObject", securityRights.READ )){
xattributes=new Hashtable();
xattributes.put("width","150");grid.addColAtr(bodef.getAttributeRef("referenceObject"),150,"","",xattributes);
}if ( ( BOI != null && BOI.getAttribute("message").hasRights() ) || securityRights.hasRights( DOC.getEboContext() , currObjectList.getBoDef().getName() ,"message", securityRights.READ )){
xattributes=new Hashtable();
xattributes.put("width","100");grid.addColAtr(bodef.getAttributeRef("message"),100,xattributes);
}} //endCreategrid
...
<%}%>
<FORM name='boFormSubmit' method='post' action='ebo_announcedetails_generallist.jsp'>
