...
if(bosession== null) {
    response.sendRedirect("login.jsp?returnPage=mailaccount_generaledit.jsp");
    return;
}
if(boctx==null) {
...
if ((BOI==null && securityRights.hasRights(boctx,"mailAccount")) || 
(BOI!=null && BOI.exists() && securityRights.hasRights(boctx,"mailAccount") && securityOPL.canRead(BOI)) || 
(BOI!=null && !BOI.exists() && securityRights.hasRights(boctx,"mailAccount",securityRights.ADD)) 
) {
if (BOI!=null && BOI.exists() && !BOI.isEnabledforRequest &&(!securityRights.hasRights(boctx,"mailAccount",securityRights.WRITE) ||  !securityOPL.canWrite(BOI) && !BOI.isEnabledforRequest)) { 
  BOI.setDisabled(); }
...
<form class="objectForm" name="boForm" id="<%=IDX%>">
   ...
   <%if(DOC.hasCategoryRights("settings")|| DOC.hasCategoryRights("folder")|| DOC.hasCategoryRights("folder")){%>
      ...
      <TBODY>
         <TR>
            <TD class="leftbar"><SPAN class="leftbar" id="<%=DOCLIST.cvui%>" defaultArea="area_<%=DOCLIST.cvui%>_0">
                  ...
                  <%}%>
                  <%if (DOC.hasCategoryRights("folder")){ %>
                  <DIV class="lbItem" id="area_<%=DOCLIST.cvui%>_1" tabIndex="<%=DOC.getTabindex(DOC.AREA, BOI==null?"area_" + String.valueOf(DOCLIST.cvui) + "_1" : BOI.getName(), String.valueOf(BOI==null?0:BOI.bo_boui), "area_" + String.valueOf(DOCLIST.cvui) + "_1", DOCLIST)%>" relatedAreaId="barea_<%=DOCLIST.cvui%>_1" areaNumber="<%=DOCLIST.cvui%>" onclick="so('<%=DOCLIST.cvui%>');loadArea('barea_<%=DOCLIST.cvui%>_1');"><IMG src="resources/area.gif"/><%=DOC.getCategoryLabel("general","folder")+"<span class='area_numbers' id='folder_"+BOI.getBoui()+"'>("+BOI.getBridge("folder").getRowCount()+")</span>"%></DIV>
                  <%}%>
                  <%if (DOC.hasCategoryRights("folder")){ %>
                  <DIV class="lbItem" id="area_<%=DOCLIST.cvui%>_2" tabIndex="<%=DOC.getTabindex(DOC.AREA, BOI==null?"area_" + String.valueOf(DOCLIST.cvui) + "_2" : BOI.getName(), String.valueOf(BOI==null?0:BOI.bo_boui), "area_" + String.valueOf(DOCLIST.cvui) + "_2", DOCLIST)%>" relatedAreaId="barea_<%=DOCLIST.cvui%>_2" areaNumber="<%=DOCLIST.cvui%>" onclick="so('<%=DOCLIST.cvui%>');loadArea('barea_<%=DOCLIST.cvui%>_2');"><IMG src="resources/area.gif"/>Spam</DIV>
                  <%}%>
               </SPAN></TD>
            <TD id="Areas_<%=DOCLIST.cvui%>">
               <%if (DOC.hasCategoryRights("settings")){ %>
               <DIV id="barea_<%=DOCLIST.cvui%>_0" class="areaSEL">
                  <%{ int cvui17567137=DOCLIST.cvui;%>
                        ...
                        <TR>
                           <TD><%if (DOC.hasCategoryRights("settings")){ %>
                              <DIV id="<%=DOCLIST.cvui%>_tabbody_0" class="tab">
                                 <%{ int cvui5431618=DOCLIST.cvui;%>
                                 <%
BOI=currObjectList.getObject();sec = DOC.getSection("s21164485");
if(sec != null)
  sec.setTitle("");
if(sec==null) { 
  sec = DOC.createSection("s21164485","");
  sec.p_name = "section_mails";
  sec.p_id = "section_mails";
sec.p_showLabel=false;
  bodef=boDefHandler.getBoDefinition("mailAccount");  sec.setTitle("");
  sec.p_height="220px";
...
boolean canAccessfaxAddress=BOI.getAttribute("faxAddress").canAccess();

if (canAccessfaxAddress ) { 
row=sec.addRow();
if( canAccessfaxAddress) {
xattributes=new Hashtable();
xattributes.put("showlabel","yes");row.addCell(bodef.getAttributeRef("faxAddress"),xattributes);
} else row.addCell();
} 
...
if (canAccessusername || canAccesspassword ) { 
row=sec.addRow();
...
xattributes=new Hashtable();
xattributes.put("showlabel","yes");xattributes.put("reserveLabelSpace","yes");xattributes.put("relatedAttribute","password");xattributes.put("class","netgest.bo.dochtml.viewerImpl.AttributesFieldRenderPassword");ICustomField classInst = (ICustomField)Class.forName("netgest.bo.dochtml.viewerImpl.AttributesFieldRenderPassword" ).newInstance();row.addCell(classInst,bodef.getAttributeRef("password"),xattributes);
} else row.addCell();
} 
boolean canAccesssmtphost=BOI.getAttribute("smtphost").canAccess();

if (canAccesssmtphost ) { 
row=sec.addRow();
if( canAccesssmtphost) {
xattributes=new Hashtable();
xattributes.put("showlabel","yes");row.addCell(bodef.getAttributeRef("smtphost"),xattributes);
} else row.addCell();
} 
boolean canAccessreceivehost=BOI.getAttribute("receivehost").canAccess();

if (canAccessreceivehost ) { 
row=sec.addRow();
if( canAccessreceivehost) {
xattributes=new Hashtable();
xattributes.put("showlabel","yes");row.addCell(bodef.getAttributeRef("receivehost"),xattributes);
} else row.addCell();
} 
boolean canAccessmailprotocol=BOI.getAttribute("mailprotocol").canAccess();

if (canAccessmailprotocol ) { 
row=sec.addRow();
if( canAccessmailprotocol) {
xattributes=new Hashtable();
xattributes.put("showlabel","yes");row.addCell(bodef.getAttributeRef("mailprotocol"),xattributes);
} else row.addCell();
} 
boolean canAccessincludeinbox=BOI.getAttribute("includeinbox").canAccess();

if (canAccessincludeinbox ) { 
row=sec.addRow();
if( canAccessincludeinbox) {
xattributes=new Hashtable();
xattributes.put("showlabel","yes");row.addCell(bodef.getAttributeRef("includeinbox"),xattributes);
} else row.addCell();
} 
boolean canAccessdeletemessages=BOI.getAttribute("deletemessages").canAccess();

if (canAccessdeletemessages ) { 
row=sec.addRow();
if( canAccessdeletemessages) {
xattributes=new Hashtable();
xattributes.put("showlabel","yes");row.addCell(bodef.getAttributeRef("deletemessages"),xattributes);
} else row.addCell();
} 
boolean canAccessprocessonlyXEOMessages=BOI.getAttribute("processonlyXEOMessages").canAccess();

if (canAccessprocessonlyXEOMessages ) { 
row=sec.addRow();
if( canAccessprocessonlyXEOMessages) {
xattributes=new Hashtable();
xattributes.put("showlabel","yes");row.addCell(bodef.getAttributeRef("processonlyXEOMessages"),xattributes);
} else row.addCell();
} 
boolean canAccessactive=BOI.getAttribute("active").canAccess();

if (canAccessactive ) { 
row=sec.addRow();
if( canAccessactive) {
xattributes=new Hashtable();
xattributes.put("showlabel","yes");row.addCell(bodef.getAttributeRef("active"),xattributes);
} else row.addCell();
} 
...
sec.render(pageContext,currObjectList,DOCLIST,DOC);%>
                                 ...
                                 <%
BOI=currObjectList.getObject();sec = DOC.getSection("s966979");
if(sec != null)
  sec.setTitle("Notificar E-Mail's");
if(sec==null) { 
  sec = DOC.createSection("s966979","");
  sec.p_name = "section_notifica";
  sec.p_id = "section_notifica";
  bodef=boDefHandler.getBoDefinition("mailAccount");  sec.setTitle("Notificar E-Mail's");
  sec.p_height="120px";
  docHTML_sectionRow row; 
...
if( canAccessnotifica) {
xattributes=new Hashtable();
xattributes.put("showlabel","no");xattributes.put("height","100%");row.addCell(bodef.getAttributeRef("notifica"),xattributes);
} else row.addCell();
} 
} //endCreateSection
sec.render(pageContext,currObjectList,DOCLIST,DOC);%>

                                 <%DOCLIST.cvui=cvui5431618;}%>
                              </DIV>
                              <%}%>
                           </TD>
                        </TR>
                  ...
                  <%}%>
                  <%DOCLIST.cvui=cvui17567137;}%>
               </DIV>
               <%}%>
               <%if (DOC.hasCategoryRights("folder")){ %>
               <DIV id="barea_<%=DOCLIST.cvui%>_1" class="area">
                  <%{ int cvui11672285=DOCLIST.cvui;%>
                     ...
                     <TBODY>
                           ...
                           <TD>
                              <TABLE cellpadding="0" cellspacing="0" class="tabBar" id="<%=DOCLIST.cvui%>_body" onkeyup="so('<%=DOCLIST.cvui%>');onKeyUpTab_std(event)" onmouseover="so('<%=DOCLIST.cvui%>');onOverTab_std(event)" onmouseout="so('<%=DOCLIST.cvui%>');onOutTab_std(event)" ondragenter="so('<%=DOCLIST.cvui%>');ondragenterTab_std(event)" ondragover="so('<%=DOCLIST.cvui%>');ondragoverTab_std(event)" onclick="so('<%=DOCLIST.cvui%>');onClickTab_std(event)">
                                 <TBODY>
                                    <TR>
                                       <TD style="padding:0px" id="<%=DOCLIST.cvui%>_tabs" valign="bottom" noWrap="yes">
                                          <%if (DOC.hasCategoryRights("")){ %><SPAN class="tab tabOn" id="<%=DOCLIST.cvui%>_tabheader_0" tabNumber="<%=DOCLIST.cvui%>" tabIndex="<%=DOC.getTabindex(DOC.PANEL, BOI==null?"null" : BOI.getName(), String.valueOf(BOI==null?0:BOI.bo_boui), String.valueOf(DOCLIST.cvui) + "_tabheader_0", DOCLIST)%>">Pastas</SPAN><%}%>
                                       </TD>
                                    </TR>
                                 </TBODY>
                              </TABLE>
                        ...
                        <TR>
                           <TD><%if (DOC.hasCategoryRights("")){ %>
                              <DIV id="<%=DOCLIST.cvui%>_tabbody_0" class="tab">
                                 <%{ int cvui16894856=DOCLIST.cvui;%>
                                 <div width="100%" height="100%"><% if(BOI.getAttribute("folder").hasRights()){%> <IFRAME id='inc_mailAccount__<%=BOI.bo_boui%>__folder' xsrc='<%="mailaccount_folder_generallist.jsp?docid="+IDX+"&method=list&parent_attribute=folder&parent_boui="+BOI.bo_boui %>' frameBorder=0 width='100%' scrolling=no height='100%' tabindex='<%=DOC.getTabindex(DOC.IFRAME, BOI.getName(), String.valueOf(BOI.bo_boui), "inc_mailAccount__" + BOI.bo_boui + "__folder", DOCLIST)%>'></IFRAME><% } %> </div>
                                 <%DOCLIST.cvui=cvui16894856;}%>
                              </DIV>
                              <%}%>
                           </TD>
                        </TR>
                  ...
                  <%}%>
                  <%DOCLIST.cvui=cvui11672285;}%>
               </DIV>
               <%}%>
               <%if (DOC.hasCategoryRights("folder")){ %>
               <DIV id="barea_<%=DOCLIST.cvui%>_2" class="area">
                  <%{ int cvui14243178=DOCLIST.cvui;%>
                  ...
                  <TABLE id="<%=DOCLIST.cvui%>" class="layout" cellspacing="0" cellpadding="0">
                     <TBODY>
                           ...
                           <TD>
                              <TABLE cellpadding="0" cellspacing="0" class="tabBar" id="<%=DOCLIST.cvui%>_body" onkeyup="so('<%=DOCLIST.cvui%>');onKeyUpTab_std(event)" onmouseover="so('<%=DOCLIST.cvui%>');onOverTab_std(event)" onmouseout="so('<%=DOCLIST.cvui%>');onOutTab_std(event)" ondragenter="so('<%=DOCLIST.cvui%>');ondragenterTab_std(event)" ondragover="so('<%=DOCLIST.cvui%>');ondragoverTab_std(event)" onclick="so('<%=DOCLIST.cvui%>');onClickTab_std(event)">
                                 <TBODY>
                                    <TR>
                                       <TD style="padding:0px" id="<%=DOCLIST.cvui%>_tabs" valign="bottom" noWrap="yes">
                                          <%if (DOC.hasCategoryRights("")){ %><SPAN class="tab tabOn" id="<%=DOCLIST.cvui%>_tabheader_0" tabNumber="<%=DOCLIST.cvui%>" tabIndex="<%=DOC.getTabindex(DOC.PANEL, BOI==null?"null" : BOI.getName(), String.valueOf(BOI==null?0:BOI.bo_boui), String.valueOf(DOCLIST.cvui) + "_tabheader_0", DOCLIST)%>">Spam Email's</SPAN><%}%>
                                       </TD>
                                    </TR>
                                 </TBODY>
                              </TABLE>
                        ...
                        <TR>
                           <TD><%if (DOC.hasCategoryRights("")){ %>
                              <DIV id="<%=DOCLIST.cvui%>_tabbody_0" class="tab">
                                 <%{ int cvui7783010=DOCLIST.cvui;%>
                                 <div width="100%" height="100%"><% if(BOI.getAttribute("spam").hasRights()){%> <IFRAME id='inc_mailAccount__<%=BOI.bo_boui%>__spam' xsrc='<%="mailaccount_spam_generallist.jsp?docid="+IDX+"&method=list&parent_attribute=spam&parent_boui="+BOI.bo_boui %>' frameBorder=0 width='100%' scrolling=no height='100%' tabindex='<%=DOC.getTabindex(DOC.IFRAME, BOI.getName(), String.valueOf(BOI.bo_boui), "inc_mailAccount__" + BOI.bo_boui + "__spam", DOCLIST)%>'></IFRAME><% } %> </div>
                                 <%DOCLIST.cvui=cvui7783010;}%>
                              </DIV>
                              <%}%>
                           </TD>
                        </TR>
...
<%}%>
<FORM name='boFormSubmit' method='post' action='mailaccount_generaledit.jsp'>
