...
if(bosession== null) {
    response.sendRedirect("login.jsp?returnPage=uiobjecthtml_generaledit.jsp");
    return;
}
if(boctx==null) {
...
if ((BOI==null && securityRights.hasRights(boctx,"uiObjectHTML")) || 
(BOI!=null && BOI.exists() && securityRights.hasRights(boctx,"uiObjectHTML") && securityOPL.canRead(BOI)) || 
(BOI!=null && !BOI.exists() && securityRights.hasRights(boctx,"uiObjectHTML",securityRights.ADD)) 
) {
if (BOI!=null && BOI.exists() && !BOI.isEnabledforRequest &&(!securityRights.hasRights(boctx,"uiObjectHTML",securityRights.WRITE) ||  !securityOPL.canWrite(BOI) && !BOI.isEnabledforRequest)) { 
  BOI.setDisabled(); }
...
<form class="objectForm" name="boForm" id="<%=IDX%>">
   ...
   <%if(DOC.hasCategoryRights("settings")|| DOC.hasCategoryRights("htmlcode")){%>
                  ...
                  <TBODY>
                     <TR>
                        <TD style="padding:0px" id="<%=DOCLIST.cvui%>_tabs" valign="bottom" noWrap="yes">
                           <%if (DOC.hasCategoryRights("settings")){ %><SPAN class="tab tabOn" id="<%=DOCLIST.cvui%>_tabheader_0" name="settings" tabNumber="<%=DOCLIST.cvui%>" tabIndex="<%=DOC.getTabindex(DOC.PANEL, BOI==null?"settings" : BOI.getName(), String.valueOf(BOI==null?0:BOI.bo_boui), String.valueOf(DOCLIST.cvui) + "_tabheader_0", DOCLIST)%>"><%=DOC.getCategoryLabel_for_TAB_Header("settings")%></SPAN><%}%><%if (DOC.hasCategoryRights("htmlcode")){ %><SPAN class="tab" id="<%=DOCLIST.cvui%>_tabheader_1" name="tab_HTML" tabNumber="<%=DOCLIST.cvui%>" tabIndex="<%=DOC.getTabindex(DOC.PANEL, BOI==null?"tab_HTML" : BOI.getName(), String.valueOf(BOI==null?0:BOI.bo_boui), String.valueOf(DOCLIST.cvui) + "_tabheader_1", DOCLIST)%>"><%=DOC.getCategoryLabel_for_TAB_Header("htmlcode")%></SPAN><%}%>
                        </TD>
                     </TR>
                  </TBODY>
               </TABLE>
         ...
         <TR>
            <TD><%if (DOC.hasCategoryRights("settings")){ %>
               <DIV id="<%=DOCLIST.cvui%>_tabbody_0" class="tab">
                  <%{ int cvui10670379=DOCLIST.cvui;%>
                  <%
BOI=currObjectList.getObject();sec = DOC.getSection("s22175853");
if(sec != null)
  sec.setTitle(DOC.getCategoryLabel("general","settings"));
if(sec==null) { 
  sec = DOC.createSection("s22175853","");
  sec.p_name = "";
  sec.p_id = "";
sec.p_showLabel=false;
  bodef=boDefHandler.getBoDefinition("uiObjectHTML");  sec.setTitle(DOC.getCategoryLabel("general","settings"));
...
sec.render(pageContext,currObjectList,DOCLIST,DOC);%>
                  ...
                  <%DOCLIST.cvui=cvui10670379;}%>
               </DIV>
               <%}%>
               <%if (DOC.hasCategoryRights("htmlcode")){ %>
               <DIV id="<%=DOCLIST.cvui%>_tabbody_1" class="tab">
                  <%{ int cvui20497400=DOCLIST.cvui;%>
                  <%
BOI=currObjectList.getObject();sec = DOC.getSection("s30012764");
if(sec != null)
  sec.setTitle(DOC.getCategoryLabel("general","htmlcode"));
if(sec==null) { 
  sec = DOC.createSection("s30012764","");
  sec.p_name = "";
  sec.p_id = "";
sec.p_showLabel=false;
  bodef=boDefHandler.getBoDefinition("uiObjectHTML");  sec.setTitle(DOC.getCategoryLabel("general","htmlcode"));
  sec.p_height="100%";
  docHTML_sectionRow row; 
boolean canAccesshtmlcode=BOI.getAttribute("htmlcode").canAccess();

if (canAccesshtmlcode ) { 
row=sec.addRow();
if( canAccesshtmlcode) {
xattributes=new Hashtable();
xattributes.put("height","100%");xattributes.put("showlabel","no");row.addCell(bodef.getAttributeRef("htmlcode"),xattributes);
} else row.addCell();
} 
} //endCreateSection
sec.render(pageContext,currObjectList,DOCLIST,DOC);%>

                  <%DOCLIST.cvui=cvui20497400;}%>
               </DIV>
               <%}%>
            </TD>
         </TR>
...
<%}%>
<FORM name='boFormSubmit' method='post' action='uiobjecthtml_generaledit.jsp'>
