...
if(bosession== null) {
    response.sendRedirect("login.jsp?returnPage=xeo_mailmerge_generallist.jsp");
    return;
}
if(boctx==null) {
...
<script>var objLabel='Lista de  Mail Merge';</script>
...
<form class="objectForm" name="boForm" id="<%=IDX%>">
   ...
   <%
BOI=currObjectList.getObject();grid = DOC.getGRID("g32243415");
if(grid==null) { 
  bodef=boDefHandler.getBoDefinition("XEO_MailMerge");  grid = DOC.createGRID("g32243415","std");
  grid.setTitle();  
...
}if ( ( BOI != null && BOI.getAttribute("object_type").hasRights() ) || securityRights.hasRights( DOC.getEboContext() , currObjectList.getBoDef().getName() ,"object_type", securityRights.READ )){
xattributes=new Hashtable();
xattributes.put("width","100");grid.addColAtr(bodef.getAttributeRef("object_type"),100,"","",xattributes);
}} //endCreategrid
...
<%}%>
<FORM name='boFormSubmit' method='post' action='xeo_mailmerge_generallist.jsp'>
