...
if(bosession== null) {
    response.sendRedirect("login.jsp?returnPage=horario_dia_periodo_generaledit.jsp");
    return;
}
if(boctx==null) {
...
if ((BOI==null && securityRights.hasRights(boctx,"horario_dia_periodo")) || 
(BOI!=null && BOI.exists() && securityRights.hasRights(boctx,"horario_dia_periodo") && securityOPL.canRead(BOI)) || 
(BOI!=null && !BOI.exists() && securityRights.hasRights(boctx,"horario_dia_periodo",securityRights.ADD)) 
) {
if (BOI!=null && BOI.exists() && !BOI.isEnabledforRequest &&(!securityRights.hasRights(boctx,"horario_dia_periodo",securityRights.WRITE) ||  !securityOPL.canWrite(BOI) && !BOI.isEnabledforRequest)) { 
  BOI.setDisabled(); }
         ...
         <TR>
            <TD><%if (DOC.hasCategoryRights("identification")){ %>
               <DIV id="<%=DOCLIST.cvui%>_tabbody_0" class="tab">
                  <%{ int cvui5069215=DOCLIST.cvui;%>
                  <%
BOI=currObjectList.getObject();sec = DOC.getSection("s3601574");
if(sec != null)
  sec.setTitle(DOC.getCategoryLabel("general","identification"));
if(sec==null) { 
  sec = DOC.createSection("s3601574","");
  sec.p_name = "";
  sec.p_id = "";
sec.p_showLabel=false;
  bodef=boDefHandler.getBoDefinition("horario_dia_periodo");  sec.setTitle(DOC.getCategoryLabel("general","identification"));
...
boolean canAccessbeginTimePeriod=BOI.getAttribute("beginTimePeriod").canAccess();

if (canAccessbeginTimePeriod ) { 
row=sec.addRow();
if( canAccessbeginTimePeriod) {
xattributes=new Hashtable();
xattributes.put("showlabel","yes");row.addCell(bodef.getAttributeRef("beginTimePeriod"),xattributes);
} else row.addCell();
} 
boolean canAccessendTimePeriod=BOI.getAttribute("endTimePeriod").canAccess();

if (canAccessendTimePeriod ) { 
row=sec.addRow();
if( canAccessendTimePeriod) {
xattributes=new Hashtable();
xattributes.put("showlabel","yes");row.addCell(bodef.getAttributeRef("endTimePeriod"),xattributes);
} else row.addCell();
} 
boolean canAccessworkTime=BOI.getAttribute("workTime").canAccess();

if (canAccessworkTime ) { 
row=sec.addRow();
if( canAccessworkTime) {
xattributes=new Hashtable();
xattributes.put("showlabel","yes");row.addCell(bodef.getAttributeRef("workTime"),xattributes);
} else row.addCell();
} 
boolean canAccesslaunchTime=BOI.getAttribute("launchTime").canAccess();

if (canAccesslaunchTime ) { 
row=sec.addRow();
if( canAccesslaunchTime) {
xattributes=new Hashtable();
xattributes.put("showlabel","yes");row.addCell(bodef.getAttributeRef("launchTime"),xattributes);
} else row.addCell();
} 
} //endCreateSection
sec.render(pageContext,currObjectList,DOCLIST,DOC);%>

                  <%DOCLIST.cvui=cvui5069215;}%>
               </DIV>
               <%}%>
            </TD>
         </TR>
...
<%}%>
<FORM name='boFormSubmit' method='post' action='horario_dia_periodo_generaledit.jsp'>
