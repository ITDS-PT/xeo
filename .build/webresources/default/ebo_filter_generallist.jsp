...
if(bosession== null) {
    response.sendRedirect("login.jsp?returnPage=ebo_filter_generallist.jsp");
    return;
}
if(boctx==null) {
...
<script>var objLabel='Lista de  Filtro';</script>
...
<form class="objectForm" name="boForm" id="<%=IDX%>">
   ...
   <%
BOI=currObjectList.getObject();grid = DOC.getGRID("g79251");
if(grid==null) { 
  bodef=boDefHandler.getBoDefinition("Ebo_Filter");  grid = DOC.createGRID("g79251","std");
  grid.setTitle();  
...
}if ( ( BOI != null && BOI.getAttribute("masterObjectClass").hasRights() ) || securityRights.hasRights( DOC.getEboContext() , currObjectList.getBoDef().getName() ,"masterObjectClass", securityRights.READ )){
xattributes=new Hashtable();
xattributes.put("width","100");grid.addColAtr(bodef.getAttributeRef("masterObjectClass"),100,"","",xattributes);
}} //endCreategrid
...
<%}%>
<FORM name='boFormSubmit' method='post' action='ebo_filter_generallist.jsp'>
