...
if(bosession== null) {
    response.sendRedirect("login.jsp?returnPage=xeo_mailmerge_res_generallist.jsp");
    return;
}
if(boctx==null) {
...
<script>var objLabel='Lista de  Mail Merge Resultados';</script>
...
<form class="objectForm" name="boForm" id="<%=IDX%>">
   ...
   <%
BOI=currObjectList.getObject();grid = DOC.getGRID("g1354396");
if(grid==null) { 
  bodef=boDefHandler.getBoDefinition("XEO_MailMerge_res");  grid = DOC.createGRID("g1354396","std");
  grid.setTitle();  
if ( ( BOI != null && BOI.getAttribute("number").hasRights() ) || securityRights.hasRights( DOC.getEboContext() , currObjectList.getBoDef().getName() ,"number", securityRights.READ )){
xattributes=new Hashtable();
xattributes.put("width","100");grid.addColAtr(bodef.getAttributeRef("number"),100,xattributes);
...
}if ( ( BOI != null && BOI.getAttribute("result").hasRights() ) || securityRights.hasRights( DOC.getEboContext() , currObjectList.getBoDef().getName() ,"result", securityRights.READ )){
xattributes=new Hashtable();
xattributes.put("width","100");grid.addColAtr(bodef.getAttributeRef("result"),100,"","",xattributes);
}} //endCreategrid
...
<%}%>
<FORM name='boFormSubmit' method='post' action='xeo_mailmerge_res_generallist.jsp'>
