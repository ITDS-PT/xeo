...
if(bosession== null) {
    response.sendRedirect("login.jsp?returnPage=pollcondition_generaledit.jsp");
    return;
}
if(boctx==null) {
...
if ((BOI==null && securityRights.hasRights(boctx,"PollCondition")) || 
(BOI!=null && BOI.exists() && securityRights.hasRights(boctx,"PollCondition") && securityOPL.canRead(BOI)) || 
(BOI!=null && !BOI.exists() && securityRights.hasRights(boctx,"PollCondition",securityRights.ADD)) 
) {
if (BOI!=null && BOI.exists() && !BOI.isEnabledforRequest &&(!securityRights.hasRights(boctx,"PollCondition",securityRights.WRITE) ||  !securityOPL.canWrite(BOI) && !BOI.isEnabledforRequest)) { 
  BOI.setDisabled(); }
                        ...
                        <TR>
                           <TD><%if (DOC.hasCategoryRights("processo")){ %>
                              <DIV id="<%=DOCLIST.cvui%>_tabbody_0" class="tab">
                                 <%{ int cvui22408915=DOCLIST.cvui;%>
                                 <%
BOI=currObjectList.getObject();sec = DOC.getSection("s11693488");
if(sec != null)
  sec.setTitle("Geral");
if(sec==null) { 
  sec = DOC.createSection("s11693488","");
  sec.p_name = "section_id";
  sec.p_id = "section_id";
  bodef=boDefHandler.getBoDefinition("PollCondition");  sec.setTitle("Geral");
  docHTML_sectionRow row; 
boolean canAccesscode=BOI.getAttribute("code").canAccess();
boolean canAccessinstantEval=BOI.getAttribute("instantEval").canAccess();

if (canAccesscode || canAccessinstantEval ) { 
row=sec.addRow();
if( canAccesscode) {
xattributes=new Hashtable();
xattributes.put("showlabel","yes");row.addCell(bodef.getAttributeRef("code"),xattributes);
} else row.addCell();
if( canAccessinstantEval) {
xattributes=new Hashtable();
xattributes.put("showlabel","yes");row.addCell(bodef.getAttributeRef("instantEval"),xattributes);
} else row.addCell();
} 
boolean canAccessfield=BOI.getAttribute("field").canAccess();
boolean canAccessoption=BOI.getAttribute("option").canAccess();

if (canAccessfield || canAccessoption ) { 
row=sec.addRow();
if( canAccessfield) {
xattributes=new Hashtable();
xattributes.put("showlabel","yes");row.addCell(bodef.getAttributeRef("field"),xattributes);
} else row.addCell();
if( canAccessoption) {
xattributes=new Hashtable();
xattributes.put("showlabel","yes");row.addCell(bodef.getAttributeRef("option"),xattributes,"","");
} else row.addCell();
} 
boolean canAccessvoter=BOI.getAttribute("voter").canAccess();
...
if( canAccessvoter) {
xattributes=new Hashtable();
xattributes.put("showlabel","yes");row.addCell(bodef.getAttributeRef("voter"),xattributes,"","");
} else row.addCell();
} 
boolean canAccesscriteria=BOI.getAttribute("criteria").canAccess();
boolean canAccessexpression=BOI.getAttribute("expression").canAccess();

if (canAccesscriteria || canAccessexpression ) { 
row=sec.addRow();
if( canAccesscriteria) {
xattributes=new Hashtable();
xattributes.put("showlabel","yes");row.addCell(bodef.getAttributeRef("criteria"),xattributes);
} else row.addCell();
if( canAccessexpression) {
xattributes=new Hashtable();
xattributes.put("showlabel","yes");row.addCell(bodef.getAttributeRef("expression"),xattributes);
} else row.addCell();
} 
} //endCreateSection
sec.render(pageContext,currObjectList,DOCLIST,DOC);%>

                                 <%DOCLIST.cvui=cvui22408915;}%>
                              </DIV>
                              <%}%>
                           </TD>
                        </TR>
...
<%}%>
<FORM name='boFormSubmit' method='post' action='pollcondition_generaledit.jsp'>
