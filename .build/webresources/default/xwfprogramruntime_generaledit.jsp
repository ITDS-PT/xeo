...
if(bosession== null) {
    response.sendRedirect("login.jsp?returnPage=xwfprogramruntime_generaledit.jsp");
    return;
}
if(boctx==null) {
...
if ((BOI==null && securityRights.hasRights(boctx,"xwfProgramRuntime")) || 
(BOI!=null && BOI.exists() && securityRights.hasRights(boctx,"xwfProgramRuntime") && securityOPL.canRead(BOI)) || 
(BOI!=null && !BOI.exists() && securityRights.hasRights(boctx,"xwfProgramRuntime",securityRights.ADD)) 
) {
if (BOI!=null && BOI.exists() && !BOI.isEnabledforRequest &&(!securityRights.hasRights(boctx,"xwfProgramRuntime",securityRights.WRITE) ||  !securityOPL.canWrite(BOI) && !BOI.isEnabledforRequest)) { 
  BOI.setDisabled(); }
...
if(true){
StringBuffer url = new StringBuffer("?method=edit&masterdoc=true&docid="+IDX);
  ...
  response.sendRedirect( "__xwfWorkPlace.jsp" + url.toString() + "&runtimeProgramBoui="+BOI.getBoui());
  return;         
}
...
<form class="objectForm" name="boForm" id="<%=IDX%>">
   ...
   <%if(DOC.hasCategoryRights("identification")|| DOC.hasCategoryRights("identification")|| DOC.hasCategoryRights("identification")){%>
      ...
      <TBODY>
         <TR>
            <TD class="leftbar"><SPAN class="leftbar" id="<%=DOCLIST.cvui%>" defaultArea="area_<%=DOCLIST.cvui%>_0">
                  ...
                  <%}%>
                  <%if (DOC.hasCategoryRights("identification")){ %>
                  <DIV class="lbItem" id="area_<%=DOCLIST.cvui%>_1" tabIndex="<%=DOC.getTabindex(DOC.AREA, BOI==null?"area_" + String.valueOf(DOCLIST.cvui) + "_1" : BOI.getName(), String.valueOf(BOI==null?0:BOI.bo_boui), "area_" + String.valueOf(DOCLIST.cvui) + "_1", DOCLIST)%>" relatedAreaId="barea_<%=DOCLIST.cvui%>_1" areaNumber="<%=DOCLIST.cvui%>" onclick="so('<%=DOCLIST.cvui%>');loadArea('barea_<%=DOCLIST.cvui%>_1');"><IMG src="resources/area.gif"/>Variáveis</DIV>
                  <%}%>
                  <%if (DOC.hasCategoryRights("identification")){ %>
                  <DIV class="lbItem" id="area_<%=DOCLIST.cvui%>_2" tabIndex="<%=DOC.getTabindex(DOC.AREA, BOI==null?"area_" + String.valueOf(DOCLIST.cvui) + "_2" : BOI.getName(), String.valueOf(BOI==null?0:BOI.bo_boui), "area_" + String.valueOf(DOCLIST.cvui) + "_2", DOCLIST)%>" relatedAreaId="barea_<%=DOCLIST.cvui%>_2" areaNumber="<%=DOCLIST.cvui%>" onclick="so('<%=DOCLIST.cvui%>');loadArea('barea_<%=DOCLIST.cvui%>_2');"><IMG src="resources/area.gif"/>Participantes</DIV>
                  <%}%>
               </SPAN></TD>
            <TD id="Areas_<%=DOCLIST.cvui%>">
               <%if (DOC.hasCategoryRights("identification")){ %>
               <DIV id="barea_<%=DOCLIST.cvui%>_0" class="areaSEL">
                  <%{ int cvui16904864=DOCLIST.cvui;%>
                        ...
                        <TR>
                           <TD><%if (DOC.hasCategoryRights("identification")){ %>
                              <DIV id="<%=DOCLIST.cvui%>_tabbody_0" class="tab">
                                 <%{ int cvui14689655=DOCLIST.cvui;%>
                                 <%
BOI=currObjectList.getObject();sec = DOC.getSection("s29083354");
if(sec != null)
  sec.setTitle(DOC.getCategoryLabel("general","identification"));
if(sec==null) { 
  sec = DOC.createSection("s29083354","");
  sec.p_name = "";
  sec.p_id = "";
sec.p_showLabel=false;
  bodef=boDefHandler.getBoDefinition("xwfProgramRuntime");  sec.setTitle(DOC.getCategoryLabel("general","identification"));
  sec.p_height="100%";
  docHTML_sectionRow row; 
boolean canAccessversion=BOI.getAttribute("version").canAccess();
boolean canAccessstatus=BOI.getAttribute("status").canAccess();

if (canAccessversion || canAccessstatus ) { 
row=sec.addRow();
if( canAccessversion) {
...
if( canAccessstatus) {
xattributes=new Hashtable();
xattributes.put("showlabel","yes");row.addCell(bodef.getAttributeRef("status"),xattributes);
} else row.addCell();
} 
boolean canAccessflow=BOI.getAttribute("flow").canAccess();

if (canAccessflow ) { 
row=sec.addRow();
if( canAccessflow) {
xattributes=new Hashtable();
xattributes.put("showlabel","no");xattributes.put("height","100%");row.addCell(bodef.getAttributeRef("flow"),xattributes);
} else row.addCell();
} 
} //endCreateSection
sec.render(pageContext,currObjectList,DOCLIST,DOC);%>

                                 <%DOCLIST.cvui=cvui14689655;}%>
                              </DIV>
                              <%}%>
                           </TD>
                        </TR>
                  ...
                  <%}%>
                  <%DOCLIST.cvui=cvui16904864;}%>
               </DIV>
               <%}%>
               <%if (DOC.hasCategoryRights("identification")){ %>
               <DIV id="barea_<%=DOCLIST.cvui%>_1" class="area">
                  <%{ int cvui16523900=DOCLIST.cvui;%>
                  ...
                  <TABLE id="<%=DOCLIST.cvui%>" class="layout" voui="panel" cellspacing="0" cellpadding="0">
                     <TBODY>
                        ...
                        <TR>
                           <TD><%if (DOC.hasCategoryRights("identification")){ %>
                              <DIV id="<%=DOCLIST.cvui%>_tabbody_0" class="tab">
                                 <%{ int cvui20720802=DOCLIST.cvui;%>
                                 <div width="100%" height="100%"><% if(BOI.getAttribute("variables").hasRights()){%> <IFRAME id='inc_xwfProgramRuntime__<%=BOI.bo_boui%>__variables' xsrc='<%="xwfvariable_generallist.jsp?docid="+IDX+"&method=list&parent_attribute=variables&parent_boui="+BOI.bo_boui %>' frameBorder=0 width='100%' scrolling=no height='100%' tabindex='<%=DOC.getTabindex(DOC.IFRAME, BOI.getName(), String.valueOf(BOI.bo_boui), "inc_xwfProgramRuntime__" + BOI.bo_boui + "__variables", DOCLIST)%>'></IFRAME><% } %> </div>
                                 <%DOCLIST.cvui=cvui20720802;}%>
                              </DIV>
                              <%}%>
                           </TD>
                        </TR>
                  ...
                  <%}%>
                  <%DOCLIST.cvui=cvui16523900;}%>
               </DIV>
               <%}%>
               <%if (DOC.hasCategoryRights("identification")){ %>
               <DIV id="barea_<%=DOCLIST.cvui%>_2" class="area">
                  <%{ int cvui26633534=DOCLIST.cvui;%>
                  ...
                  <TABLE id="<%=DOCLIST.cvui%>" class="layout" voui="panel" cellspacing="0" cellpadding="0">
                     <TBODY>
                        ...
                        <TR>
                           <TD><%if (DOC.hasCategoryRights("identification")){ %>
                              <DIV id="<%=DOCLIST.cvui%>_tabbody_0" class="tab">
                                 <%{ int cvui13684842=DOCLIST.cvui;%>
                                 <div width="100%" height="100%"><% if(BOI.getAttribute("participants").hasRights()){%> <IFRAME id='inc_xwfProgramRuntime__<%=BOI.bo_boui%>__participants' xsrc='<%="xwfparticipant_generallist.jsp?docid="+IDX+"&method=list&parent_attribute=participants&parent_boui="+BOI.bo_boui %>' frameBorder=0 width='100%' scrolling=no height='100%' tabindex='<%=DOC.getTabindex(DOC.IFRAME, BOI.getName(), String.valueOf(BOI.bo_boui), "inc_xwfProgramRuntime__" + BOI.bo_boui + "__participants", DOCLIST)%>'></IFRAME><% } %> </div>
                                 <%DOCLIST.cvui=cvui13684842;}%>
                              </DIV>
                              <%}%>
                           </TD>
                        </TR>
...
<%}%>
<FORM name='boFormSubmit' method='post' action='xwfprogramruntime_generaledit.jsp'>
