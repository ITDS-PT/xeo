...
if(bosession== null) {
    response.sendRedirect("login.jsp?returnPage=ebo_schedule_generallist.jsp");
    return;
}
if(boctx==null) {
...
<script>var objLabel='Lista de  Agendamento do Sistema';</script>
...
<form class="objectForm" name="boForm" id="<%=IDX%>">
   ...
   <%
BOI=currObjectList.getObject();grid = DOC.getGRID("g28135865");
if(grid==null) { 
  bodef=boDefHandler.getBoDefinition("Ebo_Schedule");  grid = DOC.createGRID("g28135865","std");
  grid.setTitle();  
...
}if ( ( BOI != null && BOI.getAttribute("id").hasRights() ) || securityRights.hasRights( DOC.getEboContext() , currObjectList.getBoDef().getName() ,"id", securityRights.READ )){
xattributes=new Hashtable();
xattributes.put("width","150");grid.addColAtr(bodef.getAttributeRef("id"),150,xattributes);
}} //endCreategrid
...
<%}%>
<FORM name='boFormSubmit' method='post' action='ebo_schedule_generallist.jsp'>
