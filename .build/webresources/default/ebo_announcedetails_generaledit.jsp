...
if(bosession== null) {
    response.sendRedirect("login.jsp?returnPage=ebo_announcedetails_generaledit.jsp");
    return;
}
if(boctx==null) {
...
if ((BOI==null && securityRights.hasRights(boctx,"Ebo_AnnounceDetails")) || 
(BOI!=null && BOI.exists() && securityRights.hasRights(boctx,"Ebo_AnnounceDetails") && securityOPL.canRead(BOI)) || 
(BOI!=null && !BOI.exists() && securityRights.hasRights(boctx,"Ebo_AnnounceDetails",securityRights.ADD)) 
) {
if (BOI!=null && BOI.exists() && !BOI.isEnabledforRequest &&(!securityRights.hasRights(boctx,"Ebo_AnnounceDetails",securityRights.WRITE) ||  !securityOPL.canWrite(BOI) && !BOI.isEnabledforRequest)) { 
  BOI.setDisabled(); }
         ...
         <TR>
            <TD><%if (DOC.hasCategoryRights("identification")){ %>
               <DIV id="<%=DOCLIST.cvui%>_tabbody_0" class="tab">
                  <%{ int cvui21126531=DOCLIST.cvui;%>
                  <%
BOI=currObjectList.getObject();sec = DOC.getSection("s611197");
if(sec != null)
  sec.setTitle(DOC.getCategoryLabel("general","identification"));
if(sec==null) { 
  sec = DOC.createSection("s611197","");
  sec.p_name = "";
  sec.p_id = "";
sec.p_showLabel=false;
  bodef=boDefHandler.getBoDefinition("Ebo_AnnounceDetails");  sec.setTitle(DOC.getCategoryLabel("general","identification"));
  sec.p_height="100%";
...
<%}%>
<FORM name='boFormSubmit' method='post' action='ebo_announcedetails_generaledit.jsp'>
