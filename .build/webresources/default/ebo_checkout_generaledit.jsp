...
if(bosession== null) {
    response.sendRedirect("login.jsp?returnPage=ebo_checkout_generaledit.jsp");
    return;
}
if(boctx==null) {
...
if ((BOI==null && securityRights.hasRights(boctx,"Ebo_CheckOut")) || 
(BOI!=null && BOI.exists() && securityRights.hasRights(boctx,"Ebo_CheckOut") && securityOPL.canRead(BOI)) || 
(BOI!=null && !BOI.exists() && securityRights.hasRights(boctx,"Ebo_CheckOut",securityRights.ADD)) 
) {
if (BOI!=null && BOI.exists() && !BOI.isEnabledforRequest &&(!securityRights.hasRights(boctx,"Ebo_CheckOut",securityRights.WRITE) ||  !securityOPL.canWrite(BOI) && !BOI.isEnabledforRequest)) { 
  BOI.setDisabled(); }
...
<form class="objectForm" name="boForm" id="<%=IDX%>">
   <%
BOI=currObjectList.getObject();sec = DOC.getSection("s23012162");
if(sec != null)
  sec.setTitle("");
if(sec==null) { 
  sec = DOC.createSection("s23012162","");
  sec.p_name = "section_general";
  sec.p_id = "section_general";
sec.p_showLabel=false;
  bodef=boDefHandler.getBoDefinition("Ebo_CheckOut");  sec.setTitle("");
  sec.p_height="400px";
...
boolean canAccessserialized=BOI.getAttribute("serialized").canAccess();

if (canAccessserialized ) { 
row=sec.addRow();
if( canAccessserialized) {
xattributes=new Hashtable();
xattributes.put("showlabel","no");xattributes.put("height","100%");row.addCell(bodef.getAttributeRef("serialized"),xattributes);
} else row.addCell();
} 
} //endCreateSection
...
<%}%>
<FORM name='boFormSubmit' method='post' action='ebo_checkout_generaledit.jsp'>
