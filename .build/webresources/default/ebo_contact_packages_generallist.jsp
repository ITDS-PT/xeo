...
if(bosession== null) {
    response.sendRedirect("login.jsp?returnPage=ebo_contact_packages_generallist.jsp");
    return;
}
if(boctx==null) {
...
<form class="objectForm" name="boForm" id="<%=IDX%>">
   <%
BOI=currObjectList.getObject();grid = DOC.getGRID("g24357506");
if(grid==null) { 
  bodef=boDefHandler.getBoDefinition("boObject");  boDefBridge bridgeDef=boDefHandler.getBoDefinition("Ebo_Contact").getAttributeRef("packages").getBridge();  grid = DOC.createGRID("g24357506","std",true);
  grid.setTitle();  
...
<%}%>
<FORM name='boFormSubmit' method='post' action='ebo_contact_packages_generallist.jsp'>
