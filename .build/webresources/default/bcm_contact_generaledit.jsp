...
if(bosession== null) {
    response.sendRedirect("login.jsp?returnPage=bcm_contact_generaledit.jsp");
    return;
}
if(boctx==null) {
...
if ((BOI==null && securityRights.hasRights(boctx,"BCM_Contact")) || 
(BOI!=null && BOI.exists() && securityRights.hasRights(boctx,"BCM_Contact") && securityOPL.canRead(BOI)) || 
(BOI!=null && !BOI.exists() && securityRights.hasRights(boctx,"BCM_Contact",securityRights.ADD)) 
) {
if (BOI!=null && BOI.exists() && !BOI.isEnabledforRequest &&(!securityRights.hasRights(boctx,"BCM_Contact",securityRights.WRITE) ||  !securityOPL.canWrite(BOI) && !BOI.isEnabledforRequest)) { 
  BOI.setDisabled(); }
...
<form class="objectForm" name="boForm" id="<%=IDX%>">
   ...
   <%if(DOC.hasCategoryRights("bcmContact")){%>
   <TABLE id="<%=DOCLIST.cvui%>" class="layout" voui="panel" cellspacing="0" cellpadding="0">
      <TBODY>
                  ...
                  <TBODY>
                     <TR>
                        <TD style="padding:0px" id="<%=DOCLIST.cvui%>_tabs" valign="bottom" noWrap="yes">
                           <%if (DOC.hasCategoryRights("bcmContact")){ %><SPAN class="tab tabOn" id="<%=DOCLIST.cvui%>_tabheader_0" name="tab_bcmContact" tabNumber="<%=DOCLIST.cvui%>" tabIndex="<%=DOC.getTabindex(DOC.PANEL, BOI==null?"tab_bcmContact" : BOI.getName(), String.valueOf(BOI==null?0:BOI.bo_boui), String.valueOf(DOCLIST.cvui) + "_tabheader_0", DOCLIST)%>"><%=DOC.getCategoryLabel_for_TAB_Header("bcmContact")%></SPAN><%}%>
                        </TD>
                     </TR>
                  </TBODY>
               </TABLE>
         ...
         <TR>
            <TD><%if (DOC.hasCategoryRights("bcmContact")){ %>
               <DIV id="<%=DOCLIST.cvui%>_tabbody_0" class="tab">
                  <%{ int cvui15615811=DOCLIST.cvui;%>
                  <label><%=DOC.getCategoryLabel("general","bcmContact")%></label>
                  <%
BOI=currObjectList.getObject();sec = DOC.getSection("s25735269");
if(sec != null)
  sec.setTitle(DOC.getCategoryLabel("general","bcmContact"));
if(sec==null) { 
  sec = DOC.createSection("s25735269","");
  sec.p_name = "";
  sec.p_id = "";
sec.p_showLabel=false;
  bodef=boDefHandler.getBoDefinition("BCM_Contact");  sec.setTitle(DOC.getCategoryLabel("general","bcmContact"));
  docHTML_sectionRow row; 
boolean canAccessentity=BOI.getAttribute("entity").canAccess();

if (canAccessentity ) { 
row=sec.addRow();
if( canAccessentity) {
xattributes=new Hashtable();
xattributes.put("showlabel","yes");row.addCell(bodef.getAttributeRef("entity"),xattributes,"","");
} else row.addCell();
} 
...
boolean canAccesscontactName=BOI.getAttribute("contactName").canAccess();

if (canAccesscontactName ) { 
row=sec.addRow();
if( canAccesscontactName) {
xattributes=new Hashtable();
xattributes.put("showlabel","yes");row.addCell(bodef.getAttributeRef("contactName"),xattributes);
} else row.addCell();
} 
boolean canAccesscargo=BOI.getAttribute("cargo").canAccess();

if (canAccesscargo ) { 
row=sec.addRow();
if( canAccesscargo) {
xattributes=new Hashtable();
xattributes.put("showlabel","yes");row.addCell(bodef.getAttributeRef("cargo"),xattributes);
} else row.addCell();
row.addCell();
} 
boolean canAccesstype=BOI.getAttribute("type").canAccess();
boolean canAccessnum_con=BOI.getAttribute("num_con").canAccess();

if (canAccesstype || canAccessnum_con ) { 
row=sec.addRow();
if( canAccesstype) {
xattributes=new Hashtable();
...
if( canAccessnum_con) {
xattributes=new Hashtable();
xattributes.put("showlabel","yes");row.addCell(bodef.getAttributeRef("num_con"),xattributes);
} else row.addCell();
} 
boolean canAccessstreet=BOI.getAttribute("street").canAccess();

if (canAccessstreet ) { 
row=sec.addRow();
if( canAccessstreet) {
xattributes=new Hashtable();
xattributes.put("showlabel","yes");row.addCell(bodef.getAttributeRef("street"),xattributes);
} else row.addCell();
} 
boolean canAccessaddressNumber=BOI.getAttribute("addressNumber").canAccess();
boolean canAccesslocal=BOI.getAttribute("local").canAccess();

if (canAccessaddressNumber || canAccesslocal ) { 
row=sec.addRow();
if( canAccessaddressNumber) {
xattributes=new Hashtable();
xattributes.put("showlabel","yes");row.addCell(bodef.getAttributeRef("addressNumber"),xattributes);
} else row.addCell();
if( canAccesslocal) {
xattributes=new Hashtable();
xattributes.put("showlabel","yes");row.addCell(bodef.getAttributeRef("local"),xattributes);
} else row.addCell();
} 
boolean canAccesspostalCode=BOI.getAttribute("postalCode").canAccess();
boolean canAccesspostalLocal=BOI.getAttribute("postalLocal").canAccess();

if (canAccesspostalCode || canAccesspostalLocal ) { 
row=sec.addRow();
if( canAccesspostalCode) {
xattributes=new Hashtable();
xattributes.put("showlabel","yes");row.addCell(bodef.getAttributeRef("postalCode"),xattributes);
} else row.addCell();
if( canAccesspostalLocal) {
xattributes=new Hashtable();
xattributes.put("showlabel","yes");row.addCell(bodef.getAttributeRef("postalLocal"),xattributes);
} else row.addCell();
} 
} //endCreateSection
sec.render(pageContext,currObjectList,DOCLIST,DOC);%>

                  <%DOCLIST.cvui=cvui15615811;}%>
               </DIV>
               <%}%>
            </TD>
         </TR>
...
<%}%>
<FORM name='boFormSubmit' method='post' action='bcm_contact_generaledit.jsp'>
