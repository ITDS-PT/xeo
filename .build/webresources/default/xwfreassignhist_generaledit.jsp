...
if(bosession== null) {
    response.sendRedirect("login.jsp?returnPage=xwfreassignhist_generaledit.jsp");
    return;
}
if(boctx==null) {
...
if ((BOI==null && securityRights.hasRights(boctx,"xwfReassignHist")) || 
(BOI!=null && BOI.exists() && securityRights.hasRights(boctx,"xwfReassignHist") && securityOPL.canRead(BOI)) || 
(BOI!=null && !BOI.exists() && securityRights.hasRights(boctx,"xwfReassignHist",securityRights.ADD)) 
) {
if (BOI!=null && BOI.exists() && !BOI.isEnabledforRequest &&(!securityRights.hasRights(boctx,"xwfReassignHist",securityRights.WRITE) ||  !securityOPL.canWrite(BOI) && !BOI.isEnabledforRequest)) { 
  BOI.setDisabled(); }
...
<form class="objectForm" name="boForm" id="<%=IDX%>">
   <%
BOI=currObjectList.getObject();sec = DOC.getSection("s4780482");
if(sec != null)
  sec.setTitle("");
if(sec==null) { 
  sec = DOC.createSection("s4780482","");
  sec.p_name = "xwfreassign";
  sec.p_id = "xwfreassign";
sec.p_showLabel=false;
  bodef=boDefHandler.getBoDefinition("xwfReassignHist");  sec.setTitle("");
  sec.p_height="100px";
  docHTML_sectionRow row; 
boolean canAccessassignType=BOI.getAttribute("assignType").canAccess();

if (canAccessassignType ) { 
row=sec.addRow();
if( canAccessassignType) {
xattributes=new Hashtable();
xattributes.put("showlabel","yes");row.addCell(bodef.getAttributeRef("assignType"),xattributes);
} else row.addCell();
row.addCell();
} 
...
boolean canAccessde=BOI.getAttribute("de").canAccess();
boolean canAccessto=BOI.getAttribute("to").canAccess();

if (canAccessde || canAccessto ) { 
row=sec.addRow();
if( canAccessde) {
xattributes=new Hashtable();
xattributes.put("showlabel","yes");row.addCell(bodef.getAttributeRef("de"),xattributes,"","");
} else row.addCell();
if( canAccessto) {
xattributes=new Hashtable();
...
boolean canAccessactvLabel=BOI.getAttribute("actvLabel").canAccess();

if (canAccessactvLabel ) { 
row=sec.addRow();
if( canAccessactvLabel) {
xattributes=new Hashtable();
xattributes.put("showlabel","yes");row.addCell(bodef.getAttributeRef("actvLabel"),xattributes);
} else row.addCell();
} 
boolean canAccessprogLabel=BOI.getAttribute("progLabel").canAccess();

if (canAccessprogLabel ) { 
row=sec.addRow();
if( canAccessprogLabel) {
xattributes=new Hashtable();
xattributes.put("showlabel","yes");row.addCell(bodef.getAttributeRef("progLabel"),xattributes);
} else row.addCell();
} 
...
if( canAccessdescription) {
xattributes=new Hashtable();
xattributes.put("height","250px");xattributes.put("showlabel","no");row.addCell(bodef.getAttributeRef("description"),xattributes);
} else row.addCell();
} 
} //endCreateSection
...
<%}%>
<FORM name='boFormSubmit' method='post' action='xwfreassignhist_generaledit.jsp'>
