...
if(bosession== null) {
    response.sendRedirect("login.jsp?returnPage=mailaccount_spam_generallist.jsp");
    return;
}
if(boctx==null) {
...
<script>var objLabel='Lista de  Conta de Correio';</script>
...
<form class="objectForm" name="boForm" id="<%=IDX%>">
   ...
   <%
BOI=currObjectList.getObject();grid = DOC.getGRID("g7232356");
if(grid==null) { 
  bodef=boDefHandler.getBoDefinition("garbageController");  boDefBridge bridgeDef=boDefHandler.getBoDefinition("mailAccount").getAttributeRef("spam").getBridge();  grid = DOC.createGRID("g7232356","std",true);
  grid.setTitle();  
...
xattributes.put("width","200");grid.addColAtr(bodef.getAttributeRef("email"),200,xattributes);
}if ( ( BOI != null && BOI.getAttribute("server").hasRights() ) || securityRights.hasRights( DOC.getEboContext() , currObjectList.getBoDef().getName() ,"server", securityRights.READ )){
xattributes=new Hashtable();
xattributes.put("width","100");grid.addColAtr(bodef.getAttributeRef("server"),100,xattributes);
}} //endCreategrid
...
<%}%>
<FORM name='boFormSubmit' method='post' action='mailaccount_spam_generallist.jsp'>
