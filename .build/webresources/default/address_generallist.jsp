...
if(bosession== null) {
    response.sendRedirect("login.jsp?returnPage=address_generallist.jsp");
    return;
}
if(boctx==null) {
...
<script>var objLabel='Lista de  Morada';</script>
...
<form class="objectForm" name="boForm" id="<%=IDX%>">
   ...
   <%
BOI=currObjectList.getObject();grid = DOC.getGRID("g2936445");
if(grid==null) { 
  bodef=boDefHandler.getBoDefinition("address");  grid = DOC.createGRID("g2936445","std");
  grid.setTitle();  
...
}if ( ( BOI != null && BOI.getAttribute("freguesia").hasRights() ) || securityRights.hasRights( DOC.getEboContext() , currObjectList.getBoDef().getName() ,"freguesia", securityRights.READ )){
xattributes=new Hashtable();
xattributes.put("width","100");grid.addColAtr(bodef.getAttributeRef("freguesia"),100,xattributes);
}if ( ( BOI != null && BOI.getAttribute("concelho").hasRights() ) || securityRights.hasRights( DOC.getEboContext() , currObjectList.getBoDef().getName() ,"concelho", securityRights.READ )){
xattributes=new Hashtable();
xattributes.put("width","100");grid.addColAtr(bodef.getAttributeRef("concelho"),100,xattributes);
}} //endCreategrid
...
<%}%>
<FORM name='boFormSubmit' method='post' action='address_generallist.jsp'>
