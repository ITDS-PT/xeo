...
if(bosession== null) {
    response.sendRedirect("login.jsp?returnPage=ebo_extendattribute_generaledit.jsp");
    return;
}
if(boctx==null) {
...
if ((BOI==null && securityRights.hasRights(boctx,"Ebo_ExtendAttribute")) || 
(BOI!=null && BOI.exists() && securityRights.hasRights(boctx,"Ebo_ExtendAttribute") && securityOPL.canRead(BOI)) || 
(BOI!=null && !BOI.exists() && securityRights.hasRights(boctx,"Ebo_ExtendAttribute",securityRights.ADD)) 
) {
if (BOI!=null && BOI.exists() && !BOI.isEnabledforRequest &&(!securityRights.hasRights(boctx,"Ebo_ExtendAttribute",securityRights.WRITE) ||  !securityOPL.canWrite(BOI) && !BOI.isEnabledforRequest)) { 
  BOI.setDisabled(); }
                        ...
                        <TR>
                           <TD><%if (DOC.hasCategoryRights("global")){ %>
                              <DIV id="<%=DOCLIST.cvui%>_tabbody_0" class="tab">
                                 <%{ int cvui14038778=DOCLIST.cvui;%>
                                 <%
BOI=currObjectList.getObject();sec = DOC.getSection("s28541068");
if(sec != null)
  sec.setTitle(DOC.getCategoryLabel("general","global"));
if(sec==null) { 
  sec = DOC.createSection("s28541068","");
  sec.p_name = "";
  sec.p_id = "";
sec.p_showLabel=false;
  bodef=boDefHandler.getBoDefinition("Ebo_ExtendAttribute");  sec.setTitle(DOC.getCategoryLabel("general","global"));
  docHTML_sectionRow row; 
boolean canAccessalias=BOI.getAttribute("alias").canAccess();

if (canAccessalias ) { 
row=sec.addRow();
if( canAccessalias) {
xattributes=new Hashtable();
xattributes.put("showlabel","yes");row.addCell(bodef.getAttributeRef("alias"),xattributes);
} else row.addCell();
} 
boolean canAccessshortAlias=BOI.getAttribute("shortAlias").canAccess();

if (canAccessshortAlias ) { 
row=sec.addRow();
if( canAccessshortAlias) {
xattributes=new Hashtable();
xattributes.put("showlabel","yes");row.addCell(bodef.getAttributeRef("shortAlias"),xattributes);
} else row.addCell();
} 
boolean canAccessattributeRequire=BOI.getAttribute("attributeRequire").canAccess();

if (canAccessattributeRequire ) { 
row=sec.addRow();
if( canAccessattributeRequire) {
xattributes=new Hashtable();
xattributes.put("showlabel","yes");row.addCell(bodef.getAttributeRef("attributeRequire"),xattributes);
} else row.addCell();
} 
boolean canAccessattributeConstraints=BOI.getAttribute("attributeConstraints").canAccess();

if (canAccessattributeConstraints ) { 
row=sec.addRow();
if( canAccessattributeConstraints) {
xattributes=new Hashtable();
xattributes.put("showlabel","yes");row.addCell(bodef.getAttributeRef("attributeConstraints"),xattributes);
} else row.addCell();
} 
boolean canAccesschanged=BOI.getAttribute("changed").canAccess();

if (canAccesschanged ) { 
row=sec.addRow();
if( canAccesschanged) {
xattributes=new Hashtable();
xattributes.put("showlabel","yes");row.addCell(bodef.getAttributeRef("changed"),xattributes);
} else row.addCell();
} 
boolean canAccessquickView=BOI.getAttribute("quickView").canAccess();

if (canAccessquickView ) { 
row=sec.addRow();
if( canAccessquickView) {
xattributes=new Hashtable();
xattributes.put("showlabel","yes");row.addCell(bodef.getAttributeRef("quickView"),xattributes);
} else row.addCell();
} 
boolean canAccessattributeType=BOI.getAttribute("attributeType").canAccess();

if (canAccessattributeType ) { 
row=sec.addRow();
if( canAccessattributeType) {
xattributes=new Hashtable();
xattributes.put("showlabel","yes");row.addCell(bodef.getAttributeRef("attributeType"),xattributes);
} else row.addCell();
row.addCell();
} 
boolean canAccessattributeCardinal=BOI.getAttribute("attributeCardinal").canAccess();
boolean canAccessobject=BOI.getAttribute("object").canAccess();

if (canAccessattributeCardinal || canAccessobject ) { 
row=sec.addRow();
if( canAccessattributeCardinal) {
xattributes=new Hashtable();
xattributes.put("showlabel","yes");row.addCell(bodef.getAttributeRef("attributeCardinal"),xattributes);
} else row.addCell();
if( canAccessobject) {
xattributes=new Hashtable();
xattributes.put("showlabel","yes");row.addCell(bodef.getAttributeRef("object"),xattributes,"","");
} else row.addCell();
} 
boolean canAccesslov=BOI.getAttribute("lov").canAccess();

if (canAccesslov ) { 
row=sec.addRow();
if( canAccesslov) {
xattributes=new Hashtable();
xattributes.put("showlabel","yes");row.addCell(bodef.getAttributeRef("lov"),xattributes,"","");
} else row.addCell();
row.addCell();
} 
boolean canAccessminNumber=BOI.getAttribute("minNumber").canAccess();
boolean canAccessmaxNumber=BOI.getAttribute("maxNumber").canAccess();

if (canAccessminNumber || canAccessmaxNumber ) { 
row=sec.addRow();
if( canAccessminNumber) {
xattributes=new Hashtable();
xattributes.put("showlabel","yes");row.addCell(bodef.getAttributeRef("minNumber"),xattributes);
} else row.addCell();
if( canAccessmaxNumber) {
xattributes=new Hashtable();
xattributes.put("showlabel","yes");row.addCell(bodef.getAttributeRef("maxNumber"),xattributes);
} else row.addCell();
} 
boolean canAccessdecimals=BOI.getAttribute("decimals").canAccess();
boolean canAccessminDecimals=BOI.getAttribute("minDecimals").canAccess();

if (canAccessdecimals || canAccessminDecimals ) { 
row=sec.addRow();
if( canAccessdecimals) {
xattributes=new Hashtable();
xattributes.put("showlabel","yes");row.addCell(bodef.getAttributeRef("decimals"),xattributes);
} else row.addCell();
if( canAccessminDecimals) {
xattributes=new Hashtable();
xattributes.put("showlabel","yes");row.addCell(bodef.getAttributeRef("minDecimals"),xattributes);
} else row.addCell();
} 
boolean canAccessgrouping=BOI.getAttribute("grouping").canAccess();

if (canAccessgrouping ) { 
row=sec.addRow();
if( canAccessgrouping) {
xattributes=new Hashtable();
xattributes.put("showlabel","yes");row.addCell(bodef.getAttributeRef("grouping"),xattributes);
} else row.addCell();
row.addCell();
} 
} //endCreateSection
...
<%}%>
<FORM name='boFormSubmit' method='post' action='ebo_extendattribute_generaledit.jsp'>
