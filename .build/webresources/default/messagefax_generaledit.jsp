...
if(bosession== null) {
    response.sendRedirect("login.jsp?returnPage=messagefax_generaledit.jsp");
    return;
}
if(boctx==null) {
...
if ((BOI==null && securityRights.hasRights(boctx,"messageFax")) || 
(BOI!=null && BOI.exists() && securityRights.hasRights(boctx,"messageFax") && securityOPL.canRead(BOI)) || 
(BOI!=null && !BOI.exists() && securityRights.hasRights(boctx,"messageFax",securityRights.ADD)) 
) {
if (BOI!=null && BOI.exists() && !BOI.isEnabledforRequest &&(!securityRights.hasRights(boctx,"messageFax",securityRights.WRITE) ||  !securityOPL.canWrite(BOI) && !BOI.isEnabledforRequest)) { 
  BOI.setDisabled(); }
         ...
         <TR>
            <TD><%if (DOC.hasCategoryRights("settings")){ %>
               <DIV id="<%=DOCLIST.cvui%>_tabbody_0" class="tab" style="overflow-y:auto">
                  <%{ int cvui28482549=DOCLIST.cvui;%>
                  <%
BOI=currObjectList.getObject();sec = DOC.getSection("s18688009");
if(sec != null)
  sec.setTitle("");
if(sec==null) { 
  sec = DOC.createSection("s18688009","");
  sec.p_name = "section_settingsRES";
  sec.p_id = "section_settingsRES";
sec.p_showLabel=false;
  bodef=boDefHandler.getBoDefinition("messageFax");  sec.setTitle("");
  sec.p_height="520px";
...
sec.render(pageContext,currObjectList,DOCLIST,DOC);%>
                  ...
                  <%DOCLIST.cvui=cvui28482549;}%>
               </DIV>
               <%}%>
               <%if (DOC.hasCategoryRights("settingsRES")){ %>
               <DIV id="<%=DOCLIST.cvui%>_tabbody_1" class="tab" style="overflow-y:auto">
                  <%{ int cvui19893868=DOCLIST.cvui;%>
                  <%
BOI=currObjectList.getObject();sec = DOC.getSection("s19886011");
if(sec != null)
  sec.setTitle("");
if(sec==null) { 
  sec = DOC.createSection("s19886011","");
  sec.p_name = "section_settingsRES";
  sec.p_id = "section_settingsRES";
sec.p_showLabel=false;
  bodef=boDefHandler.getBoDefinition("messageFax");  sec.setTitle("");
...
sec.render(pageContext,currObjectList,DOCLIST,DOC);%>
                  ...
                  <%DOCLIST.cvui=cvui19893868;}%>
               </DIV>
               <%}%>
               <%if (DOC.hasCategoryRights("options")){ %>
               <DIV id="<%=DOCLIST.cvui%>_tabbody_2" class="tab" style="overflow-y:auto">
                  <%{ int cvui22053241=DOCLIST.cvui;%>
                  <table>
                     <tr height="60px">
                        <td>
                           <%
BOI=currObjectList.getObject();sec = DOC.getSection("s6571565");
if(sec != null)
  sec.setTitle("");
if(sec==null) { 
  sec = DOC.createSection("s6571565","");
...
<%}%>
<FORM name='boFormSubmit' method='post' action='messagefax_generaledit.jsp'>
