...
if(bosession== null) {
    response.sendRedirect("login.jsp?returnPage=ebo_flag_generaledit.jsp");
    return;
}
if(boctx==null) {
...
if ((BOI==null && securityRights.hasRights(boctx,"Ebo_Flag")) || 
(BOI!=null && BOI.exists() && securityRights.hasRights(boctx,"Ebo_Flag") && securityOPL.canRead(BOI)) || 
(BOI!=null && !BOI.exists() && securityRights.hasRights(boctx,"Ebo_Flag",securityRights.ADD)) 
) {
if (BOI!=null && BOI.exists() && !BOI.isEnabledforRequest &&(!securityRights.hasRights(boctx,"Ebo_Flag",securityRights.WRITE) ||  !securityOPL.canWrite(BOI) && !BOI.isEnabledforRequest)) { 
  BOI.setDisabled(); }
                        ...
                        <TR>
                           <TD><%if (DOC.hasCategoryRights("Geral")){ %>
                              <DIV id="<%=DOCLIST.cvui%>_tabbody_0" class="tab">
                                 <%{ int cvui21708995=DOCLIST.cvui;%>
                                 <%
BOI=currObjectList.getObject();sec = DOC.getSection("s25756018");
if(sec != null)
  sec.setTitle(DOC.getCategoryLabel("general","Geral"));
if(sec==null) { 
  sec = DOC.createSection("s25756018","");
  sec.p_name = "";
  sec.p_id = "";
sec.p_showLabel=false;
  bodef=boDefHandler.getBoDefinition("Ebo_Flag");  sec.setTitle(DOC.getCategoryLabel("general","Geral"));
...
<%}%>
<FORM name='boFormSubmit' method='post' action='ebo_flag_generaledit.jsp'>
