...
if(bosession== null) {
    response.sendRedirect("login.jsp?returnPage=ebo_wordtemplate_generallist.jsp");
    return;
}
if(boctx==null) {
...
<script>var objLabel='Lista de  Modelos Word';</script>
...
<form class="objectForm" name="boForm" id="<%=IDX%>">
   ...
   <%
BOI=currObjectList.getObject();grid = DOC.getGRID("g30472724");
if(grid==null) { 
  bodef=boDefHandler.getBoDefinition("Ebo_WordTemplate");  grid = DOC.createGRID("g30472724","std");
  grid.setTitle();  
...
grid.render(pageContext,currObjectList,DOC, DOCLIST);%>
   ...
   <%if(DOC.hasCategoryRights("global")){ %>
   <%
BOI=currObjectList.getObject();grid = DOC.getGRID("g13790283");
if(grid==null) { 
  bodef=boDefHandler.getBoDefinition("Ebo_WordTemplate");  grid = DOC.createGRID("g13790283","std");
  grid.setTitle();  
...
<%}%>
<FORM name='boFormSubmit' method='post' action='ebo_wordtemplate_generallist.jsp'>
