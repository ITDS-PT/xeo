...
if(bosession== null) {
    response.sendRedirect("login.jsp?returnPage=ebo_template_dao_generallist.jsp");
    return;
}
if(boctx==null) {
...
<form class="objectForm" name="boForm" id="<%=IDX%>">
   <%
BOI=currObjectList.getObject();grid = DOC.getGRID("g17275770");
if(grid==null) { 
  bodef=boDefHandler.getBoDefinition("boObject");  boDefBridge bridgeDef=boDefHandler.getBoDefinition("Ebo_Template").getAttributeRef("DAO").getBridge();  grid = DOC.createGRID("g17275770","std",true);
  grid.setTitle();  
xattributes=new Hashtable();
xattributes.put("width","80");grid.addColBridgeAtr(bridgeDef.getAttributeRef("code"),80,xattributes);
xattributes=new Hashtable();
xattributes.put("width","100");grid.addColBridgeAtr(bridgeDef.getAttributeRef("config"),100,"","",xattributes);
xattributes=new Hashtable();
xattributes.put("width","60");grid.addColBridgeAtr(bridgeDef.getAttributeRef("DAO_assignedQueue"),60,"","",xattributes);
} //endCreategrid
grid.render(pageContext,currObjectList,DOC, DOCLIST);%>

</form>
<%}%>
<FORM name='boFormSubmit' method='post' action='ebo_template_dao_generallist.jsp'>
