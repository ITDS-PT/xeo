...
if(bosession== null) {
    response.sendRedirect("login.jsp?returnPage=xwfvariable_generaledit.jsp");
    return;
}
if(boctx==null) {
...
if ((BOI==null && securityRights.hasRights(boctx,"xwfVariable")) || 
(BOI!=null && BOI.exists() && securityRights.hasRights(boctx,"xwfVariable") && securityOPL.canRead(BOI)) || 
(BOI!=null && !BOI.exists() && securityRights.hasRights(boctx,"xwfVariable",securityRights.ADD)) 
) {
if (BOI!=null && BOI.exists() && !BOI.isEnabledforRequest &&(!securityRights.hasRights(boctx,"xwfVariable",securityRights.WRITE) ||  !securityOPL.canWrite(BOI) && !BOI.isEnabledforRequest)) { 
  BOI.setDisabled(); }
                        ...
                        <TR>
                           <TD><%if (DOC.hasCategoryRights("global")){ %>
                              <DIV id="<%=DOCLIST.cvui%>_tabbody_0" class="tab">
                                 <%{ int cvui28479110=DOCLIST.cvui;%>
                                 <%
BOI=currObjectList.getObject();sec = DOC.getSection("s27990247");
if(sec != null)
  sec.setTitle(DOC.getCategoryLabel("general","global"));
if(sec==null) { 
  sec = DOC.createSection("s27990247","");
  sec.p_name = "";
  sec.p_id = "";
sec.p_showLabel=false;
  bodef=boDefHandler.getBoDefinition("xwfVariable");  sec.setTitle(DOC.getCategoryLabel("general","global"));
...
boolean canAccessrequired=BOI.getAttribute("required").canAccess();

if (canAccessrequired ) { 
row=sec.addRow();
if( canAccessrequired) {
xattributes=new Hashtable();
xattributes.put("showlabel","yes");row.addCell(bodef.getAttributeRef("required"),xattributes);
} else row.addCell();
} 
boolean canAccessmode=BOI.getAttribute("mode").canAccess();
boolean canAccessinput=BOI.getAttribute("input").canAccess();

if (canAccessmode || canAccessinput ) { 
row=sec.addRow();
if( canAccessmode) {
xattributes=new Hashtable();
...
if( canAccessinput) {
xattributes=new Hashtable();
xattributes.put("showlabel","yes");row.addCell(bodef.getAttributeRef("input"),xattributes);
} else row.addCell();
} 
row=sec.addRow();
row.addCell();
...
if (canAccesslov || canAccessvalue ) { 
row=sec.addRow();
if( canAccesslov) {
xattributes=new Hashtable();
...
<%}%>
<FORM name='boFormSubmit' method='post' action='xwfvariable_generaledit.jsp'>
