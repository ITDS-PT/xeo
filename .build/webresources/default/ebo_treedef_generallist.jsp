...
if(bosession== null) {
    response.sendRedirect("login.jsp?returnPage=ebo_treedef_generallist.jsp");
    return;
}
if(boctx==null) {
...
<script>var objLabel='Lista de  Definição de Listagem';</script>
...
<form class="objectForm" name="boForm" id="<%=IDX%>">
   ...
   <%
BOI=currObjectList.getObject();grid = DOC.getGRID("g30664582");
if(grid==null) { 
  bodef=boDefHandler.getBoDefinition("Ebo_treeDef");  grid = DOC.createGRID("g30664582","std");
  grid.setTitle();  
...
}if ( ( BOI != null && BOI.getAttribute("expName").hasRights() ) || securityRights.hasRights( DOC.getEboContext() , currObjectList.getBoDef().getName() ,"expName", securityRights.READ )){
xattributes=new Hashtable();
xattributes.put("width","200");grid.addColAtr(bodef.getAttributeRef("expName"),200,xattributes);
}if ( ( BOI != null && BOI.getAttribute("objectType").hasRights() ) || securityRights.hasRights( DOC.getEboContext() , currObjectList.getBoDef().getName() ,"objectType", securityRights.READ )){
xattributes=new Hashtable();
xattributes.put("width","150");grid.addColAtr(bodef.getAttributeRef("objectType"),150,xattributes);
}} //endCreategrid
...
<%}%>
<FORM name='boFormSubmit' method='post' action='ebo_treedef_generallist.jsp'>
