...
if(bosession== null) {
    response.sendRedirect("login.jsp?returnPage=messageletter_generalreceiveexplorer.jsp");
    return;
}
if(boctx==null) {
...
<form class="objectForm" name="boForm" id="<%=IDX%>">
   <div width="100%" height="100%"><IFRAME    src="__explorer.jsp?docid=<%=DOC.getDocIdx()%>&pageName=<%=pageContext.getPage().getClass().getName()%>&objectName=messageLetter&form=receiveExplorer" id='explorer' frameBorder='0' width='100%' height='100%' scrolling='no' tabindex='0'></IFRAME></div><script> window.gridKey='11378576'</script>  

</form>
<%}%>
<FORM name='boFormSubmit' method='post' action='messageletter_generalreceiveexplorer.jsp'>
