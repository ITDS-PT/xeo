...
if(bosession== null) {
    response.sendRedirect("login.jsp?returnPage=ebo_history_generaledit.jsp");
    return;
}
if(boctx==null) {
...
if ((BOI==null && securityRights.hasRights(boctx,"Ebo_History")) || 
(BOI!=null && BOI.exists() && securityRights.hasRights(boctx,"Ebo_History") && securityOPL.canRead(BOI)) || 
(BOI!=null && !BOI.exists() && securityRights.hasRights(boctx,"Ebo_History",securityRights.ADD)) 
) {
if (BOI!=null && BOI.exists() && !BOI.isEnabledforRequest &&(!securityRights.hasRights(boctx,"Ebo_History",securityRights.WRITE) ||  !securityOPL.canWrite(BOI) && !BOI.isEnabledforRequest)) { 
  BOI.setDisabled(); }
...
if(doNotRedirect == null || "".equals(doNotRedirect))
{
  ...
  boObject sourceObject = DOC.getObject(BOI.getAttribute("object").getValueLong());
  if(sourceObject != null && sourceObject.exists())
  {
      sourceObject.getObjectVersionControl().getObjectFromVersion(BOI.getAttribute("version").getValueLong());  
      sourceObject.setDisabled();  
      ...
      DOC.setMasterObjectOverriding(sourceObject);      
      String jspNamePrefix = sourceObject.getBoDefinition().getName().toLowerCase();
      response.sendRedirect( jspNamePrefix + "_generaledit.jsp" + url.toString() + "&masterdoc=true&boui=" + sourceObject.getBoui());
      return;         
  }
}
...
<form class="objectForm" name="boForm" id="<%=IDX%>">
   <%
BOI=currObjectList.getObject();sec = DOC.getSection("s16508973");
if(sec != null)
  sec.setTitle("");
if(sec==null) { 
  sec = DOC.createSection("s16508973","");
  sec.p_name = "section_general";
  sec.p_id = "section_general";
sec.p_showLabel=false;
  bodef=boDefHandler.getBoDefinition("Ebo_History");  sec.setTitle("");
  sec.p_height="20%";
  docHTML_sectionRow row; 
boolean canAccessversion=BOI.getAttribute("version").canAccess();
boolean canAccessversionDate=BOI.getAttribute("versionDate").canAccess();

if (canAccessversion || canAccessversionDate ) { 
row=sec.addRow();
if( canAccessversion) {
xattributes=new Hashtable();
xattributes.put("showlabel","yes");row.addCell(bodef.getAttributeRef("version"),xattributes);
} else row.addCell();
if( canAccessversionDate) {
xattributes=new Hashtable();
xattributes.put("showlabel","yes");row.addCell(bodef.getAttributeRef("versionDate"),xattributes);
} else row.addCell();
} 
...
boolean canAccesscomment=BOI.getAttribute("comment").canAccess();

if (canAccesscomment ) { 
row=sec.addRow();
if( canAccesscomment) {
xattributes=new Hashtable();
xattributes.put("showlabel","yes");row.addCell(bodef.getAttributeRef("comment"),xattributes);
} else row.addCell();
} 
} //endCreateSection
...
<%}%>
<FORM name='boFormSubmit' method='post' action='ebo_history_generaledit.jsp'>
