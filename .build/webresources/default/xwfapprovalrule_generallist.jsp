...
if(bosession== null) {
    response.sendRedirect("login.jsp?returnPage=xwfapprovalrule_generallist.jsp");
    return;
}
if(boctx==null) {
...
<script>var objLabel='Lista de  Hierarquia de aprovação';</script>
...
<form class="objectForm" name="boForm" id="<%=IDX%>">
   ...
   <%
BOI=currObjectList.getObject();grid = DOC.getGRID("g31498561");
if(grid==null) { 
  bodef=boDefHandler.getBoDefinition("xwfApprovalRule");  grid = DOC.createGRID("g31498561","std");
  grid.setTitle();  
...
}if ( ( BOI != null && BOI.getAttribute("mode").hasRights() ) || securityRights.hasRights( DOC.getEboContext() , currObjectList.getBoDef().getName() ,"mode", securityRights.READ )){
xattributes=new Hashtable();
xattributes.put("width","150");grid.addColAtr(bodef.getAttributeRef("mode"),150,xattributes);
}} //endCreategrid
...
<%}%>
<FORM name='boFormSubmit' method='post' action='xwfapprovalrule_generallist.jsp'>
