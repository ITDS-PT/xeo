...
if(bosession== null) {
    response.sendRedirect("login.jsp?returnPage=forwarding_generallist.jsp");
    return;
}
if(boctx==null) {
...
<script>var objLabel='Lista de  Despacho';</script>
...
<form class="objectForm" name="boForm" id="<%=IDX%>">
   ...
   <%
BOI=currObjectList.getObject();grid = DOC.getGRID("g32397856");
if(grid==null) { 
  bodef=boDefHandler.getBoDefinition("forwarding");  grid = DOC.createGRID("g32397856","std");
  grid.setTitle();  
if ( ( BOI != null && BOI.getAttribute("forwarding").hasRights() ) || securityRights.hasRights( DOC.getEboContext() , currObjectList.getBoDef().getName() ,"forwarding", securityRights.READ )){
xattributes=new Hashtable();
xattributes.put("width","50");grid.addColAtr(bodef.getAttributeRef("forwarding"),50,xattributes);
}if ( ( BOI != null && BOI.getAttribute("dtdoc").hasRights() ) || securityRights.hasRights( DOC.getEboContext() , currObjectList.getBoDef().getName() ,"dtdoc", securityRights.READ )){
xattributes=new Hashtable();
xattributes.put("width","50");grid.addColAtr(bodef.getAttributeRef("dtdoc"),50,xattributes);
...
<%}%>
<FORM name='boFormSubmit' method='post' action='forwarding_generallist.jsp'>
