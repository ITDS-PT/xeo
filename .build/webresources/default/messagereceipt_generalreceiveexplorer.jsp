...
if(bosession== null) {
    response.sendRedirect("login.jsp?returnPage=messagereceipt_generalreceiveexplorer.jsp");
    return;
}
if(boctx==null) {
...
<form class="objectForm" name="boForm" id="<%=IDX%>">
   <div width="100%" height="100%"><IFRAME    src="__explorer.jsp?docid=<%=DOC.getDocIdx()%>&pageName=<%=pageContext.getPage().getClass().getName()%>&objectName=messageReceipt&form=receiveExplorer" id='explorer' frameBorder='0' width='100%' height='100%' scrolling='no' tabindex='0'></IFRAME></div><script> window.gridKey='21509425'</script>  

</form>
<%}%>
<FORM name='boFormSubmit' method='post' action='messagereceipt_generalreceiveexplorer.jsp'>
