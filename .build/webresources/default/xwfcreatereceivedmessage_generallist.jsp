...
if(bosession== null) {
    response.sendRedirect("login.jsp?returnPage=xwfcreatereceivedmessage_generallist.jsp");
    return;
}
if(boctx==null) {
...
<script>var objLabel='Lista de  Receber';</script>
...
<form class="objectForm" name="boForm" id="<%=IDX%>">
   ...
   <%
BOI=currObjectList.getObject();grid = DOC.getGRID("g14050887");
if(grid==null) { 
  bodef=boDefHandler.getBoDefinition("xwfCreateReceivedMessage");  grid = DOC.createGRID("g14050887","std");
  grid.setTitle();  
...
xattributes.put("width","200");grid.addColAtr(bodef.getAttributeRef("label"),200,xattributes);
...
xattributes=new Hashtable();
xattributes.put("width","150");grid.addColAtr(bodef.getAttributeRef("assignedQueue"),150,"","",xattributes);
}if ( ( BOI != null && BOI.getAttribute("performer").hasRights() ) || securityRights.hasRights( DOC.getEboContext() , currObjectList.getBoDef().getName() ,"performer", securityRights.READ )){
xattributes=new Hashtable();
xattributes.put("width","150");grid.addColAtr(bodef.getAttributeRef("performer"),150,"","",xattributes);
...
}if ( ( BOI != null && BOI.getAttribute("endDate").hasRights() ) || securityRights.hasRights( DOC.getEboContext() , currObjectList.getBoDef().getName() ,"endDate", securityRights.READ )){
xattributes=new Hashtable();
xattributes.put("width","110");grid.addColAtr(bodef.getAttributeRef("endDate"),110,xattributes);
}if ( ( BOI != null && BOI.getAttribute("deadLineDate").hasRights() ) || securityRights.hasRights( DOC.getEboContext() , currObjectList.getBoDef().getName() ,"deadLineDate", securityRights.READ )){
xattributes=new Hashtable();
xattributes.put("width","110");grid.addColAtr(bodef.getAttributeRef("deadLineDate"),110,xattributes);
}} //endCreategrid
...
<%}%>
<FORM name='boFormSubmit' method='post' action='xwfcreatereceivedmessage_generallist.jsp'>
