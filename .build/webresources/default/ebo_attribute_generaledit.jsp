...
if(bosession== null) {
    response.sendRedirect("login.jsp?returnPage=ebo_attribute_generaledit.jsp");
    return;
}
if(boctx==null) {
...
if ((BOI==null && securityRights.hasRights(boctx,"Ebo_Attribute")) || 
(BOI!=null && BOI.exists() && securityRights.hasRights(boctx,"Ebo_Attribute") && securityOPL.canRead(BOI)) || 
(BOI!=null && !BOI.exists() && securityRights.hasRights(boctx,"Ebo_Attribute",securityRights.ADD)) 
) {
if (BOI!=null && BOI.exists() && !BOI.isEnabledforRequest &&(!securityRights.hasRights(boctx,"Ebo_Attribute",securityRights.WRITE) ||  !securityOPL.canWrite(BOI) && !BOI.isEnabledforRequest)) { 
  BOI.setDisabled(); }
...
<form class="objectForm" name="boForm" id="<%=IDX%>">
   ...
   <%if(DOC.hasCategoryRights("identification")|| DOC.hasCategoryRights("security")){%>
      ...
      <TBODY>
         <TR>
            <TD class="leftbar"><SPAN class="leftbar" id="<%=DOCLIST.cvui%>" defaultArea="area_<%=DOCLIST.cvui%>_0">
                  ...
                  <DIV class="lbItem" name="security" id="area_<%=DOCLIST.cvui%>_1" tabIndex="<%=DOC.getTabindex(DOC.AREA, BOI==null?"area_" + String.valueOf(DOCLIST.cvui) + "_1" : BOI.getName(), String.valueOf(BOI==null?0:BOI.bo_boui), "area_" + String.valueOf(DOCLIST.cvui) + "_1", DOCLIST)%>" relatedAreaId="barea_<%=DOCLIST.cvui%>_1" areaNumber="<%=DOCLIST.cvui%>" onclick="so('<%=DOCLIST.cvui%>');loadArea('barea_<%=DOCLIST.cvui%>_1');"><IMG src="resources/area.gif"/><%=DOC.getCategoryLabel("general","security")+"<span class='area_numbers' id='security_"+BOI.getBoui()+"'>("+BOI.getBridge("security").getRowCount()+")</span>"%></DIV>
                  <%}%>
               </SPAN></TD>
            <TD id="Areas_<%=DOCLIST.cvui%>">
               <%if (DOC.hasCategoryRights("identification")){ %>
               <DIV id="barea_<%=DOCLIST.cvui%>_0" class="areaSEL">
                  <%{ int cvui24713244=DOCLIST.cvui;%>
                        ...
                        <TR>
                           <TD><%if (DOC.hasCategoryRights("identification")){ %>
                              <DIV id="<%=DOCLIST.cvui%>_tabbody_0" class="tab">
                                 <%{ int cvui18732805=DOCLIST.cvui;%>
                                 <%
BOI=currObjectList.getObject();sec = DOC.getSection("s1025153");
if(sec != null)
  sec.setTitle(DOC.getCategoryLabel("general","identification"));
if(sec==null) { 
  sec = DOC.createSection("s1025153","");
  sec.p_name = "";
  sec.p_id = "";
sec.p_showLabel=false;
  bodef=boDefHandler.getBoDefinition("Ebo_Attribute");  sec.setTitle(DOC.getCategoryLabel("general","identification"));
  sec.p_height="100%";
...
boolean canAccessclsReg=BOI.getAttribute("clsReg").canAccess();

if (canAccessclsReg ) { 
row=sec.addRow();
if( canAccessclsReg) {
xattributes=new Hashtable();
xattributes.put("showlabel","yes");row.addCell(bodef.getAttributeRef("clsReg"),xattributes,"","");
} else row.addCell();
row.addCell();
} 
...
sec.render(pageContext,currObjectList,DOCLIST,DOC);%>
                  ...
                  <%}%>
                  <%DOCLIST.cvui=cvui24713244;}%>
               </DIV>
               <%}%>
               <%if (DOC.hasCategoryRights("security")){ %>
               <DIV id="barea_<%=DOCLIST.cvui%>_1" class="area">
                  <%{ int cvui16330478=DOCLIST.cvui;%>
...
<%}%>
<FORM name='boFormSubmit' method='post' action='ebo_attribute_generaledit.jsp'>
