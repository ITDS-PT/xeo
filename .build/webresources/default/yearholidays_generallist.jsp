...
if(bosession== null) {
    response.sendRedirect("login.jsp?returnPage=yearholidays_generallist.jsp");
    return;
}
if(boctx==null) {
...
<script>var objLabel='Lista de  Feriados/Ano';</script>
...
<form class="objectForm" name="boForm" id="<%=IDX%>">
   <%if(DOC.hasCategoryRights("list_year_holidays")){ %>
   <%
BOI=currObjectList.getObject();grid = DOC.getGRID("g9432427");
if(grid==null) { 
  bodef=boDefHandler.getBoDefinition("yearHolidays");  grid = DOC.createGRID("g9432427","std");
  grid.setTitle();  
if ( ( BOI != null && BOI.getAttribute("yearSelect").hasRights() ) || securityRights.hasRights( DOC.getEboContext() , currObjectList.getBoDef().getName() ,"yearSelect", securityRights.READ )){
xattributes=new Hashtable();
xattributes.put("width","50");grid.addColAtr(bodef.getAttributeRef("yearSelect"),50,xattributes);
...
xattributes=new Hashtable();
xattributes.put("width","350");grid.addColAtr(bodef.getAttributeRef("description"),350,xattributes);
}} //endCreategrid
...
<%}%>
<FORM name='boFormSubmit' method='post' action='yearholidays_generallist.jsp'>
