...
if(bosession== null) {
    response.sendRedirect("login.jsp?returnPage=ebo_perf_generaledit.jsp");
    return;
}
if(boctx==null) {
...
if ((BOI==null && securityRights.hasRights(boctx,"Ebo_Perf")) || 
(BOI!=null && BOI.exists() && securityRights.hasRights(boctx,"Ebo_Perf") && securityOPL.canRead(BOI)) || 
(BOI!=null && !BOI.exists() && securityRights.hasRights(boctx,"Ebo_Perf",securityRights.ADD)) 
) {
if (BOI!=null && BOI.exists() && !BOI.isEnabledforRequest &&(!securityRights.hasRights(boctx,"Ebo_Perf",securityRights.WRITE) ||  !securityOPL.canWrite(BOI) && !BOI.isEnabledforRequest)) { 
  BOI.setDisabled(); }
         ...
         <TR>
            ...
            <TD id="Areas_<%=DOCLIST.cvui%>">
               ...
               <DIV id="barea_<%=DOCLIST.cvui%>_0" class="areaSEL">
                  <%{ int cvui1247900=DOCLIST.cvui;%>
                  <%DOCLIST.cvui=DOCLIST.getVui(IDX, BOI.getBoui(), "barea_" + DOCLIST.cvui + "_0");%>
                  <%if(DOC.hasCategoryRights("identification")|| DOC.hasCategoryRights("office_address")|| DOC.hasCategoryRights("home_address")|| DOC.hasCategoryRights("correspondencia_address")|| DOC.hasCategoryRights("holiday_address")|| DOC.hasCategoryRights("addresses")){%>
                                 ...
                                 <TBODY>
                                    <TR>
                                       <TD style="padding:0px" id="<%=DOCLIST.cvui%>_tabs" valign="bottom" noWrap="yes">
                                          <%if (DOC.hasCategoryRights("identification")){ %><SPAN class="tab tabOn" id="<%=DOCLIST.cvui%>_tabheader_0" tabNumber="<%=DOCLIST.cvui%>" tabIndex="<%=DOC.getTabindex(DOC.PANEL, BOI==null?"null" : BOI.getName(), String.valueOf(BOI==null?0:BOI.bo_boui), String.valueOf(DOCLIST.cvui) + "_tabheader_0", DOCLIST)%>"><%=DOC.getCategoryLabel_for_TAB_Header("identification")%></SPAN><%}%><%if (DOC.hasCategoryRights("office_address")){ %><SPAN class="tab" id="<%=DOCLIST.cvui%>_tabheader_1" tabNumber="<%=DOCLIST.cvui%>" tabIndex="<%=DOC.getTabindex(DOC.PANEL, BOI==null?"null" : BOI.getName(), String.valueOf(BOI==null?0:BOI.bo_boui), String.valueOf(DOCLIST.cvui) + "_tabheader_1", DOCLIST)%>"><%=DOC.getCategoryLabel_for_TAB_Header("office_address")%></SPAN><%}%><%if (DOC.hasCategoryRights("home_address")){ %><SPAN class="tab" id="<%=DOCLIST.cvui%>_tabheader_2" tabNumber="<%=DOCLIST.cvui%>" tabIndex="<%=DOC.getTabindex(DOC.PANEL, BOI==null?"null" : BOI.getName(), String.valueOf(BOI==null?0:BOI.bo_boui), String.valueOf(DOCLIST.cvui) + "_tabheader_2", DOCLIST)%>"><%=DOC.getCategoryLabel_for_TAB_Header("home_address")%></SPAN><%}%><%if (DOC.hasCategoryRights("correspondencia_address")){ %><SPAN class="tab" id="<%=DOCLIST.cvui%>_tabheader_3" tabNumber="<%=DOCLIST.cvui%>" tabIndex="<%=DOC.getTabindex(DOC.PANEL, BOI==null?"null" : BOI.getName(), String.valueOf(BOI==null?0:BOI.bo_boui), String.valueOf(DOCLIST.cvui) + "_tabheader_3", DOCLIST)%>"><%=DOC.getCategoryLabel_for_TAB_Header("correspondencia_address")%></SPAN><%}%><%if (DOC.hasCategoryRights("holiday_address")){ %><SPAN class="tab" id="<%=DOCLIST.cvui%>_tabheader_4" tabNumber="<%=DOCLIST.cvui%>" tabIndex="<%=DOC.getTabindex(DOC.PANEL, BOI==null?"null" : BOI.getName(), String.valueOf(BOI==null?0:BOI.bo_boui), String.valueOf(DOCLIST.cvui) + "_tabheader_4", DOCLIST)%>"><%=DOC.getCategoryLabel_for_TAB_Header("holiday_address")%></SPAN><%}%><%if (DOC.hasCategoryRights("addresses")){ %><SPAN class="tab" id="<%=DOCLIST.cvui%>_tabheader_5" tabNumber="<%=DOCLIST.cvui%>" tabIndex="<%=DOC.getTabindex(DOC.PANEL, BOI==null?"null" : BOI.getName(), String.valueOf(BOI==null?0:BOI.bo_boui), String.valueOf(DOCLIST.cvui) + "_tabheader_5", DOCLIST)%>"><%=DOC.getCategoryLabel_for_TAB_Header("addresses")%></SPAN><%}%>
                                       </TD>
                                    </TR>
                                 </TBODY>
                              </TABLE>
                        ...
                        <TR>
                           <TD><%if (DOC.hasCategoryRights("identification")){ %>
                              <DIV id="<%=DOCLIST.cvui%>_tabbody_0" class="tab">
                                 <%{ int cvui1935572=DOCLIST.cvui;%>
                                 <%
BOI=currObjectList.getObject();sec = DOC.getSection("s16920313");
if(sec != null)
  sec.setTitle(DOC.getCategoryLabel("general","identification"));
if(sec==null) { 
  sec = DOC.createSection("s16920313","");
  sec.p_name = "";
  sec.p_id = "";
sec.p_showLabel=false;
  bodef=boDefHandler.getBoDefinition("Ebo_Perf");  sec.setTitle(DOC.getCategoryLabel("general","identification"));
...
boolean canAccessdataAdmissao=BOI.getAttribute("dataAdmissao").canAccess();
boolean canAccesssituacao=BOI.getAttribute("situacao").canAccess();

if (canAccessdataAdmissao || canAccesssituacao ) { 
row=sec.addRow();
if( canAccessdataAdmissao) {
xattributes=new Hashtable();
xattributes.put("showlabel","yes");row.addCell(bodef.getAttributeRef("dataAdmissao"),xattributes);
} else row.addCell();
if( canAccesssituacao) {
xattributes=new Hashtable();
xattributes.put("showlabel","yes");row.addCell(bodef.getAttributeRef("situacao"),xattributes);
} else row.addCell();
} 
...
sec.render(pageContext,currObjectList,DOCLIST,DOC);%>
                  ...
                  <%}%>
                  <%DOCLIST.cvui=cvui1247900;}%>
               </DIV>
               <%}%>
               <%if (DOC.hasCategoryRights("identification")){ %>
               <DIV id="barea_<%=DOCLIST.cvui%>_1" class="area">
                  <%{ int cvui20481059=DOCLIST.cvui;%>
                     ...
                     <TBODY>
                        ...
                        <TR>
                           <TD><%if (DOC.hasCategoryRights("queues")){ %>
                              <DIV id="<%=DOCLIST.cvui%>_tabbody_0" class="tab">
                                 <%{ int cvui23799519=DOCLIST.cvui;%>
                                 <div width="100%" height="100%"><% if(BOI.getAttribute("queues").hasRights()){%> <IFRAME id='inc_Ebo_Perf__<%=BOI.bo_boui%>__queues' xsrc='<%="workqueue_generallist.jsp?docid="+IDX+"&method=list&parent_attribute=queues&parent_boui="+BOI.bo_boui %>' frameBorder=0 width='100%' scrolling=no height='100%' tabindex='<%=DOC.getTabindex(DOC.IFRAME, BOI.getName(), String.valueOf(BOI.bo_boui), "inc_Ebo_Perf__" + BOI.bo_boui + "__queues", DOCLIST)%>'></IFRAME><% } %> </div>
                                 <%DOCLIST.cvui=cvui23799519;}%>
                              </DIV>
                              <%}%>
                           </TD>
                        </TR>
                  ...
                  <%}%>
                  <%DOCLIST.cvui=cvui20481059;}%>
               </DIV>
               <%}%>
               <%if (DOC.hasCategoryRights("groups")){ %>
               <DIV id="barea_<%=DOCLIST.cvui%>_2" class="area">
                  <%{ int cvui29687104=DOCLIST.cvui;%>
                     ...
                     <TBODY>
                        ...
                        <TR>
                           <TD><%if (DOC.hasCategoryRights("groups")){ %>
                              <DIV id="<%=DOCLIST.cvui%>_tabbody_0" class="tab">
                                 <%{ int cvui30315630=DOCLIST.cvui;%>
                                 <div width="100%" height="100%"><% if(BOI.getAttribute("groups").hasRights()){%> <IFRAME id='inc_Ebo_Perf__<%=BOI.bo_boui%>__groups' xsrc='<%="ebo_group_generallist.jsp?docid="+IDX+"&method=list&parent_attribute=groups&parent_boui="+BOI.bo_boui %>' frameBorder=0 width='100%' scrolling=no height='100%' tabindex='<%=DOC.getTabindex(DOC.IFRAME, BOI.getName(), String.valueOf(BOI.bo_boui), "inc_Ebo_Perf__" + BOI.bo_boui + "__groups", DOCLIST)%>'></IFRAME><% } %> </div>
                                 <%DOCLIST.cvui=cvui30315630;}%>
                              </DIV>
                              <%}%>
                           </TD>
                        </TR>
                  ...
                  <%}%>
                  <%DOCLIST.cvui=cvui29687104;}%>
               </DIV>
               <%}%>
               <%if (DOC.hasCategoryRights("roles")){ %>
               <DIV id="barea_<%=DOCLIST.cvui%>_3" class="area">
                  <%{ int cvui24308401=DOCLIST.cvui;%>
                  ...
                  <%}%>
                  <%DOCLIST.cvui=cvui24308401;}%>
               </DIV>
               <%}%>
               <%if (DOC.hasCategoryRights("packages")){ %>
               <DIV id="barea_<%=DOCLIST.cvui%>_4" class="area">
                  <%{ int cvui12922286=DOCLIST.cvui;%>
                     ...
                     <TBODY>
                        ...
                        <TR>
                           <TD><%if (DOC.hasCategoryRights("packages")){ %>
                              <DIV id="<%=DOCLIST.cvui%>_tabbody_0" class="tab">
                                 <%{ int cvui20578049=DOCLIST.cvui;%>
                                 <div width="100%" height="100%"><% if(BOI.getAttribute("packages").hasRights()){%> <IFRAME id='inc_Ebo_Perf__<%=BOI.bo_boui%>__packages' xsrc='<%="ebo_perf_packages_generallist.jsp?docid="+IDX+"&method=list&parent_attribute=packages&parent_boui="+BOI.bo_boui %>' frameBorder=0 width='100%' scrolling=no height='100%' tabindex='<%=DOC.getTabindex(DOC.IFRAME, BOI.getName(), String.valueOf(BOI.bo_boui), "inc_Ebo_Perf__" + BOI.bo_boui + "__packages", DOCLIST)%>'></IFRAME><% } %> </div>
                                 <%DOCLIST.cvui=cvui20578049;}%>
                              </DIV>
                              <%}%>
                           </TD>
                        </TR>
                  ...
                  <%}%>
                  <%DOCLIST.cvui=cvui12922286;}%>
               </DIV>
               <%}%>
               <%if (DOC.hasCategoryRights("iProfile")){ %>
               <DIV id="barea_<%=DOCLIST.cvui%>_5" class="area">
                  <%{ int cvui9748533=DOCLIST.cvui;%>
                     ...
                     <TBODY>
                        ...
                        <TR>
                           <TD><%if (DOC.hasCategoryRights("iProfile")){ %>
                              <DIV id="<%=DOCLIST.cvui%>_tabbody_0" class="tab">
                                 <%{ int cvui30873727=DOCLIST.cvui;%>
                                 <div width="100%" height="100%"><% if(BOI.getAttribute("iProfile").hasRights()){%> <IFRAME id='inc_Ebo_Perf__<%=BOI.bo_boui%>__iProfile' xsrc='<%="uiprofile_generallist.jsp?docid="+IDX+"&method=list&parent_attribute=iProfile&parent_boui="+BOI.bo_boui %>' frameBorder=0 width='100%' scrolling=no height='100%' tabindex='<%=DOC.getTabindex(DOC.IFRAME, BOI.getName(), String.valueOf(BOI.bo_boui), "inc_Ebo_Perf__" + BOI.bo_boui + "__iProfile", DOCLIST)%>'></IFRAME><% } %> </div>
                                 <%DOCLIST.cvui=cvui30873727;}%>
                              </DIV>
                              <%}%>
                           </TD>
                        </TR>
                  ...
                  <%}%>
                  <%DOCLIST.cvui=cvui9748533;}%>
               </DIV>
               <%}%>
               <%if (DOC.hasCategoryRights("")){ %>
               <DIV id="barea_<%=DOCLIST.cvui%>_6" class="area">
                  <%{ int cvui9004774=DOCLIST.cvui;%>
                     ...
                     <TBODY>
                        ...
                        <TR>
                           <TD><%if (DOC.hasCategoryRights("")){ %>
                              <DIV id="<%=DOCLIST.cvui%>_tabbody_0" class="tab">
                                 <%{ int cvui12762286=DOCLIST.cvui;%>
                                 <div width="100%" height="100%"><% if(BOI.getAttribute("emailAccounts").hasRights()){%> <IFRAME id='inc_Ebo_Perf__<%=BOI.bo_boui%>__emailAccounts' xsrc='<%="ebo_perf_emailaccounts_generallist.jsp?docid="+IDX+"&method=list&parent_attribute=emailAccounts&parent_boui="+BOI.bo_boui %>' frameBorder=0 width='100%' scrolling=no height='100%' tabindex='<%=DOC.getTabindex(DOC.IFRAME, BOI.getName(), String.valueOf(BOI.bo_boui), "inc_Ebo_Perf__" + BOI.bo_boui + "__emailAccounts", DOCLIST)%>'></IFRAME><% } %> </div>
                                 <%DOCLIST.cvui=cvui12762286;}%>
                              </DIV>
                              <%}%>
                           </TD>
                        </TR>
...
<%}%>
<FORM name='boFormSubmit' method='post' action='ebo_perf_generaledit.jsp'>
