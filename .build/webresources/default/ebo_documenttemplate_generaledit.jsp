...
if(bosession== null) {
    response.sendRedirect("login.jsp?returnPage=ebo_documenttemplate_generaledit.jsp");
    return;
}
if(boctx==null) {
...
if ((BOI==null && securityRights.hasRights(boctx,"Ebo_DocumentTemplate")) || 
(BOI!=null && BOI.exists() && securityRights.hasRights(boctx,"Ebo_DocumentTemplate") && securityOPL.canRead(BOI)) || 
(BOI!=null && !BOI.exists() && securityRights.hasRights(boctx,"Ebo_DocumentTemplate",securityRights.ADD)) 
) {
if (BOI!=null && BOI.exists() && !BOI.isEnabledforRequest &&(!securityRights.hasRights(boctx,"Ebo_DocumentTemplate",securityRights.WRITE) ||  !securityOPL.canWrite(BOI) && !BOI.isEnabledforRequest)) { 
  BOI.setDisabled(); }
...
<title>
nbo «Ebo_DocumentTemplate»</title>
...
<form class="objectForm" name="boForm" id="<%=IDX%>">
   ...
   <%if(DOC.hasCategoryRights("global")|| DOC.hasCategoryRights("")){%>
   <TABLE id="<%=DOCLIST.cvui%>" class="layout" voui="panel" bo_node="global" style="padding:10px" order="10" cellspacing="0" cellpadding="0">
      <TBODY>
         <TR height="25">
            <TD>
                  ...
                  <TBODY>
                     <TR>
                        <TD style="padding:0px" id="<%=DOCLIST.cvui%>_tabs" valign="bottom" noWrap="yes">
                           <%if (DOC.hasCategoryRights("global")){ %><SPAN class="tab tabOn" id="<%=DOCLIST.cvui%>_tabheader_0" name="tabprincipal" tabNumber="<%=DOCLIST.cvui%>" tabIndex="<%=DOC.getTabindex(DOC.PANEL, BOI==null?"tabprincipal" : BOI.getName(), String.valueOf(BOI==null?0:BOI.bo_boui), String.valueOf(DOCLIST.cvui) + "_tabheader_0", DOCLIST)%>"><%=DOC.getCategoryLabel_for_TAB_Header("global")%></SPAN><%}%><%if (DOC.hasCategoryRights("")){ %><SPAN class="tab" id="<%=DOCLIST.cvui%>_tabheader_1" name="reference" tabNumber="<%=DOCLIST.cvui%>" tabIndex="<%=DOC.getTabindex(DOC.PANEL, BOI==null?"reference" : BOI.getName(), String.valueOf(BOI==null?0:BOI.bo_boui), String.valueOf(DOCLIST.cvui) + "_tabheader_1", DOCLIST)%>">Referencias do documento</SPAN><%}%>
                        </TD>
                     </TR>
                  </TBODY>
               </TABLE>
         ...
         <TR>
            <TD><%if (DOC.hasCategoryRights("global")){ %>
               <DIV id="<%=DOCLIST.cvui%>_tabbody_0" class="tab" style="overflow-y:auto">
                  <%{ int cvui11421180=DOCLIST.cvui;%>
                  <%
BOI=currObjectList.getObject();sec = DOC.getSection("s24431813");
if(sec != null)
  sec.setTitle(DOC.getCategoryLabel("general","global"));
if(sec==null) { 
  sec = DOC.createSection("s24431813","");
  sec.p_name = "seccao_master_document";
  sec.p_id = "seccao_master_document";
sec.p_showLabel=false;
  bodef=boDefHandler.getBoDefinition("Ebo_DocumentTemplate");  sec.setTitle(DOC.getCategoryLabel("general","global"));
...
boolean canAccessfile=BOI.getAttribute("file").canAccess();

if (canAccessfile ) { 
row=sec.addRow();
if( canAccessfile) {
xattributes=new Hashtable();
xattributes.put("showlabel","yes");row.addCell(bodef.getAttributeRef("file"),xattributes);
} else row.addCell();
} 
boolean canAccessowner=BOI.getAttribute("owner").canAccess();

if (canAccessowner ) { 
row=sec.addRow();
if( canAccessowner) {
xattributes=new Hashtable();
xattributes.put("showlabel","yes");row.addCell(bodef.getAttributeRef("owner"),xattributes,"","");
} else row.addCell();
} 
} //endCreateSection
sec.render(pageContext,currObjectList,DOCLIST,DOC);%>
                  ...
                  <%
BOI=currObjectList.getObject();sec = DOC.getSection("s19909503");
if(sec != null)
  sec.setTitle("Propriedades do Modelo de Documento");
if(sec==null) { 
  sec = DOC.createSection("s19909503","");
  sec.p_name = "seccao_master_template";
  sec.p_id = "seccao_master_template";
  bodef=boDefHandler.getBoDefinition("Ebo_DocumentTemplate");  sec.setTitle("Propriedades do Modelo de Documento");
  docHTML_sectionRow row; 
...
sec.render(pageContext,currObjectList,DOCLIST,DOC);%>
                  ...
                  <%
BOI=currObjectList.getObject();sec = DOC.getSection("s14228788");
if(sec != null)
  sec.setTitle("Caracteristicas");
if(sec==null) { 
  sec = DOC.createSection("s14228788","");
  sec.p_name = "phisicalProperties";
  sec.p_id = "phisicalProperties";
  bodef=boDefHandler.getBoDefinition("Ebo_DocumentTemplate");  sec.setTitle("Caracteristicas");
  docHTML_sectionRow row; 
boolean canAccessfileName=BOI.getAttribute("fileName").canAccess();

if (canAccessfileName ) { 
row=sec.addRow();
if( canAccessfileName) {
xattributes=new Hashtable();
xattributes.put("showlabel","yes");row.addCell(bodef.getAttributeRef("fileName"),xattributes);
} else row.addCell();
} 
boolean canAccesslastModified=BOI.getAttribute("lastModified").canAccess();

if (canAccesslastModified ) { 
row=sec.addRow();
if( canAccesslastModified) {
xattributes=new Hashtable();
xattributes.put("showlabel","yes");row.addCell(bodef.getAttributeRef("lastModified"),xattributes);
} else row.addCell();
} 
boolean canAccessfileSize=BOI.getAttribute("fileSize").canAccess();

if (canAccessfileSize ) { 
row=sec.addRow();
if( canAccessfileSize) {
xattributes=new Hashtable();
xattributes.put("showlabel","yes");row.addCell(bodef.getAttributeRef("fileSize"),xattributes);
} else row.addCell();
} 
boolean canAccesssrcObj=BOI.getAttribute("srcObj").canAccess();

if (canAccesssrcObj ) { 
row=sec.addRow();
if( canAccesssrcObj) {
xattributes=new Hashtable();
xattributes.put("showlabel","yes");row.addCell(bodef.getAttributeRef("srcObj"),xattributes,"","");
} else row.addCell();
} 
} //endCreateSection
sec.render(pageContext,currObjectList,DOCLIST,DOC);%>

                  <%DOCLIST.cvui=cvui11421180;}%>
               </DIV>
               <%}%>
               <%if (DOC.hasCategoryRights("")){ %>
               <DIV id="<%=DOCLIST.cvui%>_tabbody_1" class="tab">
                  <%{ int cvui811304=DOCLIST.cvui;%>
                  <div width="100%" height="100%"><IFRAME id='inc_Ebo_DocumentTemplate__<%=BOI.bo_boui%>__dynbrige_boObject' xsrc='<%="__listcardid.jsp?docid="+IDX+"&method=list&list_frommethod=referencedBy%28%29&parent_attribute=dynbrige_boObject&parent_boui="+BOI.bo_boui%>' frameBorder=0 width='100%' scrolling=no height='100%'tabindex='<%=DOC.getTabindex(DOC.IFRAME, BOI.getName(), String.valueOf(BOI.bo_boui), "inc_boObject__" + String.valueOf(BOI.bo_boui) + "__dynbrige_boObject", DOCLIST)%>'></IFRAME></div>
                  <%DOCLIST.cvui=cvui811304;}%>
               </DIV>
               <%}%>
            </TD>
         </TR>
...
<%}%>
<FORM name='boFormSubmit' method='post' action='ebo_documenttemplate_generaledit.jsp'>
