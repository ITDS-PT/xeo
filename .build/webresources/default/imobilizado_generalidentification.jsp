...
if(bosession== null) {
    response.sendRedirect("login.jsp?returnPage=imobilizado_generalidentification.jsp");
    return;
}
if(boctx==null) {
...
<form class="objectForm" name="boForm" id="<%=IDX%>">
   ...
   <table bo_node="global">
      <tbody>
         <tr>
            <td><% if(BOI.getAttribute("id").hasRights()){%> <%=DOC.render(currObjectList,BOI.getAttribute("id"),docHTML.RENDER_LABEL)%><% } %></td>
            <td><% if(BOI.getAttribute("id").hasRights()){%> <%=DOC.render(currObjectList,BOI.getAttribute("id"),docHTML.RENDER_FIELD)%><% } %></td>
         </tr>
         <tr>
            <td colspan="2" class="fieldtext"><% if(BOI.getAttribute("description").hasRights()){%> <%=DOC.render(currObjectList,BOI.getAttribute("description"),docHTML.RENDER_FIELDTEXT)%><% } %></td>
         </tr>
      </tbody>
   </table>
   <%}%>
...
<%}%>
<FORM name='boFormSubmit' method='post' action='imobilizado_generalidentification.jsp'>
