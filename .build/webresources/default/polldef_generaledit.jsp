...
if(bosession== null) {
    response.sendRedirect("login.jsp?returnPage=polldef_generaledit.jsp");
    return;
}
if(boctx==null) {
...
if ((BOI==null && securityRights.hasRights(boctx,"PollDef")) || 
(BOI!=null && BOI.exists() && securityRights.hasRights(boctx,"PollDef") && securityOPL.canRead(BOI)) || 
(BOI!=null && !BOI.exists() && securityRights.hasRights(boctx,"PollDef",securityRights.ADD)) 
) {
if (BOI!=null && BOI.exists() && !BOI.isEnabledforRequest &&(!securityRights.hasRights(boctx,"PollDef",securityRights.WRITE) ||  !securityOPL.canWrite(BOI) && !BOI.isEnabledforRequest)) { 
  BOI.setDisabled(); }
                        ...
                        <TR>
                           <TD><%if (DOC.hasCategoryRights("processo")){ %>
                              <DIV id="<%=DOCLIST.cvui%>_tabbody_0" class="tab">
                                 <%{ int cvui22125271=DOCLIST.cvui;%>
                                 <%
BOI=currObjectList.getObject();sec = DOC.getSection("s9221799");
if(sec != null)
  sec.setTitle("Geral");
if(sec==null) { 
  sec = DOC.createSection("s9221799","");
  sec.p_name = "section_id";
  sec.p_id = "section_id";
  bodef=boDefHandler.getBoDefinition("PollDef");  sec.setTitle("Geral");
  sec.p_height="60px";
  docHTML_sectionRow row; 
boolean canAccessname=BOI.getAttribute("name").canAccess();
boolean canAccesscode=BOI.getAttribute("code").canAccess();

if (canAccessname || canAccesscode ) { 
row=sec.addRow();
if( canAccessname) {
...
if( canAccesscode) {
xattributes=new Hashtable();
xattributes.put("showlabel","yes");row.addCell(bodef.getAttributeRef("code"),xattributes);
} else row.addCell();
} 
} //endCreateSection
sec.render(pageContext,currObjectList,DOCLIST,DOC);%>

                                 <div width="100%" height="100%"><% if(BOI.getAttribute("options").hasRights()){%> <IFRAME id='inc_PollDef__<%=BOI.bo_boui%>__options' xsrc='<%="polloption_generallist.jsp?docid="+IDX+"&method=list&parent_attribute=options&parent_boui="+BOI.bo_boui %>' frameBorder=0 width='100%' scrolling=no height='100%' tabindex='<%=DOC.getTabindex(DOC.IFRAME, BOI.getName(), String.valueOf(BOI.bo_boui), "inc_PollDef__" + BOI.bo_boui + "__options", DOCLIST)%>'></IFRAME><% } %> </div>
                                 <%DOCLIST.cvui=cvui22125271;}%>
                              </DIV>
                              <%}%>
                           </TD>
                        </TR>
                  ...
                  <%}%>
                  <%DOCLIST.cvui=cvui8051026;}%>
               </DIV>
               <%}%>
               <%if (DOC.hasCategoryRights("")){ %>
               <DIV id="barea_<%=DOCLIST.cvui%>_1" class="area">
                  <%{ int cvui2858133=DOCLIST.cvui;%>
                  ...
                  <TABLE id="<%=DOCLIST.cvui%>" class="layout" cellspacing="0" cellpadding="0">
                     <TBODY>
                        ...
                        <TR>
                           <TD><%if (DOC.hasCategoryRights("processo")){ %>
                              <DIV id="<%=DOCLIST.cvui%>_tabbody_0" class="tab">
                                 <%{ int cvui20260978=DOCLIST.cvui;%>
                                 <%
BOI=currObjectList.getObject();sec = DOC.getSection("s15894599");
if(sec != null)
  sec.setTitle("Geral");
if(sec==null) { 
  sec = DOC.createSection("s15894599","");
  sec.p_name = "section_id";
  sec.p_id = "section_id";
  bodef=boDefHandler.getBoDefinition("PollDef");  sec.setTitle("Geral");
  sec.p_height="60px";
  docHTML_sectionRow row; 
boolean canAccessdefaultCond=BOI.getAttribute("defaultCond").canAccess();

if (canAccessdefaultCond ) { 
row=sec.addRow();
if( canAccessdefaultCond) {
xattributes=new Hashtable();
xattributes.put("showlabel","yes");row.addCell(bodef.getAttributeRef("defaultCond"),xattributes,"","");
} else row.addCell();
} 
} //endCreateSection
sec.render(pageContext,currObjectList,DOCLIST,DOC);%>

                                 <div width="100%" height="100%"><% if(BOI.getAttribute("conditions").hasRights()){%> <IFRAME id='inc_PollDef__<%=BOI.bo_boui%>__conditions' xsrc='<%="pollcondition_generallist.jsp?docid="+IDX+"&method=list&parent_attribute=conditions&parent_boui="+BOI.bo_boui %>' frameBorder=0 width='100%' scrolling=no height='100%' tabindex='<%=DOC.getTabindex(DOC.IFRAME, BOI.getName(), String.valueOf(BOI.bo_boui), "inc_PollDef__" + BOI.bo_boui + "__conditions", DOCLIST)%>'></IFRAME><% } %> </div>
                                 <%DOCLIST.cvui=cvui20260978;}%>
                              </DIV>
                              <%}%>
                           </TD>
                        </TR>
...
<%}%>
<FORM name='boFormSubmit' method='post' action='polldef_generaledit.jsp'>
