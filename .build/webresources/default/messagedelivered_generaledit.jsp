...
if(bosession== null) {
    response.sendRedirect("login.jsp?returnPage=messagedelivered_generaledit.jsp");
    return;
}
if(boctx==null) {
...
if ((BOI==null && securityRights.hasRights(boctx,"messageDelivered")) || 
(BOI!=null && BOI.exists() && securityRights.hasRights(boctx,"messageDelivered") && securityOPL.canRead(BOI)) || 
(BOI!=null && !BOI.exists() && securityRights.hasRights(boctx,"messageDelivered",securityRights.ADD)) 
) {
if (BOI!=null && BOI.exists() && !BOI.isEnabledforRequest &&(!securityRights.hasRights(boctx,"messageDelivered",securityRights.WRITE) ||  !securityOPL.canWrite(BOI) && !BOI.isEnabledforRequest)) { 
  BOI.setDisabled(); }
...
<form class="objectForm" name="boForm" id="<%=IDX%>">
   ...
   <%if(DOC.hasCategoryRights("settings")|| DOC.hasCategoryRights("documents")|| DOC.hasCategoryRights("attachedObjects")|| DOC.hasCategoryRights("")){%>
                  ...
                  <TBODY>
                     <TR>
                        <TD style="padding:0px" id="<%=DOCLIST.cvui%>_tabs" valign="bottom" noWrap="yes">
                           <%if (DOC.hasCategoryRights("settings")){ %><SPAN class="tab tabOn" id="<%=DOCLIST.cvui%>_tabheader_0" name="tab_settingsSystem" tabNumber="<%=DOCLIST.cvui%>" tabIndex="<%=DOC.getTabindex(DOC.PANEL, BOI==null?"tab_settingsSystem" : BOI.getName(), String.valueOf(BOI==null?0:BOI.bo_boui), String.valueOf(DOCLIST.cvui) + "_tabheader_0", DOCLIST)%>"><%=DOC.getCategoryLabel_for_TAB_Header("settings")%></SPAN><%}%><%if (DOC.hasCategoryRights("documents")){ %><SPAN class="tab" id="<%=DOCLIST.cvui%>_tabheader_1" tabNumber="<%=DOCLIST.cvui%>" tabIndex="<%=DOC.getTabindex(DOC.PANEL, BOI==null?"null" : BOI.getName(), String.valueOf(BOI==null?0:BOI.bo_boui), String.valueOf(DOCLIST.cvui) + "_tabheader_1", DOCLIST)%>"><%=DOC.getCategoryLabel_for_TAB_Header("documents")%></SPAN><%}%><%if (DOC.hasCategoryRights("attachedObjects")){ %><SPAN class="tab" id="<%=DOCLIST.cvui%>_tabheader_2" name="tab_attachedObjects" tabNumber="<%=DOCLIST.cvui%>" tabIndex="<%=DOC.getTabindex(DOC.PANEL, BOI==null?"tab_attachedObjects" : BOI.getName(), String.valueOf(BOI==null?0:BOI.bo_boui), String.valueOf(DOCLIST.cvui) + "_tabheader_2", DOCLIST)%>"><%=DOC.getCategoryLabel_for_TAB_Header("attachedObjects")%></SPAN><%}%><%if (BOI.getMode()==netgest.bo.runtime.boObject.MODE_EDIT_TEMPLATE ){ %><SPAN class="tab" id="<%=DOCLIST.cvui%>_tabheader_3" name="modelo" tabNumber="<%=DOCLIST.cvui%>" tabIndex="<%=DOC.getTabindex(DOC.PANEL, BOI==null?"modelo" : BOI.getName(), String.valueOf(BOI==null?0:BOI.bo_boui), String.valueOf(DOCLIST.cvui) + "_tabheader_3", DOCLIST)%>">Modelo</SPAN><%}%>
                        </TD>
                     </TR>
                  </TBODY>
               </TABLE>
         ...
         <TR>
            <TD><%if (DOC.hasCategoryRights("settings")){ %>
               <DIV id="<%=DOCLIST.cvui%>_tabbody_0" class="tab" style="overflow-y:auto">
                  <%{ int cvui30091091=DOCLIST.cvui;%>
                  <%
BOI=currObjectList.getObject();sec = DOC.getSection("s8572558");
if(sec != null)
  sec.setTitle("");
if(sec==null) { 
  sec = DOC.createSection("s8572558","");
  sec.p_name = "section_settingsRES";
  sec.p_id = "section_settingsRES";
sec.p_showLabel=false;
  bodef=boDefHandler.getBoDefinition("messageDelivered");  sec.setTitle("");
  sec.p_height="520px";
...
sec.render(pageContext,currObjectList,DOCLIST,DOC);%>
                  ...
                  <%DOCLIST.cvui=cvui30091091;}%>
               </DIV>
               <%}%>
               <%if (DOC.hasCategoryRights("documents")){ %>
               <DIV id="<%=DOCLIST.cvui%>_tabbody_1" class="tab" style="overflow-y:auto">
                  <%{ int cvui29807110=DOCLIST.cvui;%>
                  <div width="100%" height="100%"><% if(BOI.getAttribute("documents").hasRights()){%> <IFRAME id='inc_messageDelivered__<%=BOI.bo_boui%>__documents' xsrc='<%="ebo_document_generallist.jsp?docid="+IDX+"&method=list&parent_attribute=documents&parent_boui="+BOI.bo_boui %>' frameBorder=0 width='100%' scrolling=no height='100%' tabindex='<%=DOC.getTabindex(DOC.IFRAME, BOI.getName(), String.valueOf(BOI.bo_boui), "inc_messageDelivered__" + BOI.bo_boui + "__documents", DOCLIST)%>'></IFRAME><% } %> </div>
                  <%DOCLIST.cvui=cvui29807110;}%>
               </DIV>
               <%}%>
               <%if (DOC.hasCategoryRights("attachedObjects")){ %>
               <DIV id="<%=DOCLIST.cvui%>_tabbody_2" class="tab" style="overflow-y:auto">
                  <%{ int cvui430344=DOCLIST.cvui;%>
                  <div width="100%" height="100%"><% if(BOI.getAttribute("attachedObjects").hasRights()){%> <IFRAME id='inc_messageDelivered__<%=BOI.bo_boui%>__attachedObjects' xsrc='<%="messagedelivered_attachedobjects_generallist.jsp?docid="+IDX+"&method=list&parent_attribute=attachedObjects&parent_boui="+BOI.bo_boui %>' frameBorder=0 width='100%' scrolling=no height='100%' tabindex='<%=DOC.getTabindex(DOC.IFRAME, BOI.getName(), String.valueOf(BOI.bo_boui), "inc_messageDelivered__" + BOI.bo_boui + "__attachedObjects", DOCLIST)%>'></IFRAME><% } %> </div>
                  <%DOCLIST.cvui=cvui430344;}%>
               </DIV>
               <%}%>
...
<%}%>
<FORM name='boFormSubmit' method='post' action='messagedelivered_generaledit.jsp'>
