...
if(bosession== null) {
    response.sendRedirect("login.jsp?returnPage=ebo_stylesheet_generallist.jsp");
    return;
}
if(boctx==null) {
...
<script>var objLabel='Lista de  XML StyleSheet';</script>
...
<form class="objectForm" name="boForm" id="<%=IDX%>">
   ...
   <%
BOI=currObjectList.getObject();grid = DOC.getGRID("g2607168");
if(grid==null) { 
  bodef=boDefHandler.getBoDefinition("Ebo_styleSheet");  grid = DOC.createGRID("g2607168","std");
  grid.setTitle();  
...
}if ( ( BOI != null && BOI.getAttribute("document").hasRights() ) || securityRights.hasRights( DOC.getEboContext() , currObjectList.getBoDef().getName() ,"document", securityRights.READ )){
xattributes=new Hashtable();
xattributes.put("width","150");grid.addColAtr(bodef.getAttributeRef("document"),150,"","",xattributes);
}} //endCreategrid
...
<%}%>
<FORM name='boFormSubmit' method='post' action='ebo_stylesheet_generallist.jsp'>
