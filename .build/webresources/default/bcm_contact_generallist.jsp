...
if(bosession== null) {
    response.sendRedirect("login.jsp?returnPage=bcm_contact_generallist.jsp");
    return;
}
if(boctx==null) {
...
<form class="objectForm" name="boForm" id="<%=IDX%>">
   ...
   <%
BOI=currObjectList.getObject();grid = DOC.getGRID("g8083992");
if(grid==null) { 
  bodef=boDefHandler.getBoDefinition("BCM_Contact");  grid = DOC.createGRID("g8083992","std");
  grid.setTitle();  
if ( ( BOI != null && BOI.getAttribute("entity").hasRights() ) || securityRights.hasRights( DOC.getEboContext() , currObjectList.getBoDef().getName() ,"entity", securityRights.READ )){
xattributes=new Hashtable();
xattributes.put("width","150");grid.addColAtr(bodef.getAttributeRef("entity"),150,"","",xattributes);
}if ( ( BOI != null && BOI.getAttribute("contactName").hasRights() ) || securityRights.hasRights( DOC.getEboContext() , currObjectList.getBoDef().getName() ,"contactName", securityRights.READ )){
xattributes=new Hashtable();
xattributes.put("width","150");grid.addColAtr(bodef.getAttributeRef("contactName"),150,xattributes);
}if ( ( BOI != null && BOI.getAttribute("type").hasRights() ) || securityRights.hasRights( DOC.getEboContext() , currObjectList.getBoDef().getName() ,"type", securityRights.READ )){
xattributes=new Hashtable();
xattributes.put("width","100");grid.addColAtr(bodef.getAttributeRef("type"),100,xattributes);
}if ( ( BOI != null && BOI.getAttribute("cargo").hasRights() ) || securityRights.hasRights( DOC.getEboContext() , currObjectList.getBoDef().getName() ,"cargo", securityRights.READ )){
xattributes=new Hashtable();
xattributes.put("width","100");grid.addColAtr(bodef.getAttributeRef("cargo"),100,xattributes);
}} //endCreategrid
...
<%}%>
<FORM name='boFormSubmit' method='post' action='bcm_contact_generallist.jsp'>
