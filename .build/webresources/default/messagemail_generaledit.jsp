...
if(bosession== null) {
    response.sendRedirect("login.jsp?returnPage=messagemail_generaledit.jsp");
    return;
}
if(boctx==null) {
...
if ((BOI==null && securityRights.hasRights(boctx,"messageMail")) || 
(BOI!=null && BOI.exists() && securityRights.hasRights(boctx,"messageMail") && securityOPL.canRead(BOI)) || 
(BOI!=null && !BOI.exists() && securityRights.hasRights(boctx,"messageMail",securityRights.ADD)) 
) {
if (BOI!=null && BOI.exists() && !BOI.isEnabledforRequest &&(!securityRights.hasRights(boctx,"messageMail",securityRights.WRITE) ||  !securityOPL.canWrite(BOI) && !BOI.isEnabledforRequest)) { 
  BOI.setDisabled(); }
         ...
         <TR>
            <TD><%if (DOC.hasCategoryRights("settings")){ %>
               <DIV id="<%=DOCLIST.cvui%>_tabbody_0" class="tab" style="overflow-y:auto">
                  <%{ int cvui29833061=DOCLIST.cvui;%>
                  <%
BOI=currObjectList.getObject();sec = DOC.getSection("s380134");
if(sec != null)
  sec.setTitle("");
if(sec==null) { 
  sec = DOC.createSection("s380134","");
  sec.p_name = "section_settingsRES";
  sec.p_id = "section_settingsRES";
sec.p_showLabel=false;
  bodef=boDefHandler.getBoDefinition("messageMail");  sec.setTitle("");
  sec.p_height="520px";
...
sec.render(pageContext,currObjectList,DOCLIST,DOC);%>
                  ...
                  <%DOCLIST.cvui=cvui29833061;}%>
               </DIV>
               <%}%>
               <%if (DOC.hasCategoryRights("settingsRES")){ %>
               <DIV id="<%=DOCLIST.cvui%>_tabbody_1" class="tab" style="overflow-y:auto">
                  <%{ int cvui13585554=DOCLIST.cvui;%>
                  <%
BOI=currObjectList.getObject();sec = DOC.getSection("s28066350");
if(sec != null)
  sec.setTitle("");
if(sec==null) { 
  sec = DOC.createSection("s28066350","");
  sec.p_name = "section_settingsRES";
  sec.p_id = "section_settingsRES";
sec.p_showLabel=false;
  bodef=boDefHandler.getBoDefinition("messageMail");  sec.setTitle("");
...
sec.render(pageContext,currObjectList,DOCLIST,DOC);%>
                  ...
                  <%DOCLIST.cvui=cvui13585554;}%>
               </DIV>
               <%}%>
               <%if (DOC.hasCategoryRights("options")){ %>
               <DIV id="<%=DOCLIST.cvui%>_tabbody_2" class="tab" style="overflow-y:auto">
                  <%{ int cvui2944078=DOCLIST.cvui;%>
                  <table>
                     <tr height="60px">
                        <td>
                           <%
BOI=currObjectList.getObject();sec = DOC.getSection("s21886463");
if(sec != null)
  sec.setTitle("");
if(sec==null) { 
  sec = DOC.createSection("s21886463","");
...
<%}%>
<FORM name='boFormSubmit' method='post' action='messagemail_generaledit.jsp'>
