...
if(bosession== null) {
    response.sendRedirect("login.jsp?returnPage=xwfdefactivity_generallist.jsp");
    return;
}
if(boctx==null) {
...
<form class="objectForm" name="boForm" id="<%=IDX%>">
   ...
   <%
BOI=currObjectList.getObject();grid = DOC.getGRID("g11425104");
if(grid==null) { 
  bodef=boDefHandler.getBoDefinition("xwfDefActivity");  grid = DOC.createGRID("g11425104","std");
  grid.setTitle();  
if ( ( BOI != null && BOI.getAttribute("label").hasRights() ) || securityRights.hasRights( DOC.getEboContext() , currObjectList.getBoDef().getName() ,"label", securityRights.READ )){
xattributes=new Hashtable();
xattributes.put("width","120");grid.addColAtr(bodef.getAttributeRef("label"),120,xattributes);
}if ( ( BOI != null && BOI.getAttribute("assignedQueue").hasRights() ) || securityRights.hasRights( DOC.getEboContext() , currObjectList.getBoDef().getName() ,"assignedQueue", securityRights.READ )){
xattributes=new Hashtable();
xattributes.put("width","180");grid.addColAtr(bodef.getAttributeRef("assignedQueue"),180,"","",xattributes);
}if ( ( BOI != null && BOI.getAttribute("to").hasRights() ) || securityRights.hasRights( DOC.getEboContext() , currObjectList.getBoDef().getName() ,"to", securityRights.READ )){
xattributes=new Hashtable();
xattributes.put("width","180");grid.addColAtr(bodef.getAttributeRef("to"),180,"","",xattributes);
}if ( ( BOI != null && BOI.getAttribute("type_recursive").hasRights() ) || securityRights.hasRights( DOC.getEboContext() , currObjectList.getBoDef().getName() ,"type_recursive", securityRights.READ )){
xattributes=new Hashtable();
xattributes.put("width","80");grid.addColAtr(bodef.getAttributeRef("type_recursive"),80,xattributes);
}if ( ( BOI != null && BOI.getAttribute("createdAll").hasRights() ) || securityRights.hasRights( DOC.getEboContext() , currObjectList.getBoDef().getName() ,"createdAll", securityRights.READ )){
xattributes=new Hashtable();
xattributes.put("width","120");grid.addColAtr(bodef.getAttributeRef("createdAll"),120,xattributes);
}if ( ( BOI != null && BOI.getAttribute("beginDate").hasRights() ) || securityRights.hasRights( DOC.getEboContext() , currObjectList.getBoDef().getName() ,"beginDate", securityRights.READ )){
xattributes=new Hashtable();
xattributes.put("width","110");grid.addColAtr(bodef.getAttributeRef("beginDate"),110,xattributes);
}if ( ( BOI != null && BOI.getAttribute("SYS_DTCREATE").hasRights() ) || securityRights.hasRights( DOC.getEboContext() , currObjectList.getBoDef().getName() ,"SYS_DTCREATE", securityRights.READ )){
xattributes=new Hashtable();
xattributes.put("width","110");grid.addColAtr(bodef.getAttributeRef("SYS_DTCREATE"),110,xattributes);
}} //endCreategrid
...
<%}%>
<FORM name='boFormSubmit' method='post' action='xwfdefactivity_generallist.jsp'>
