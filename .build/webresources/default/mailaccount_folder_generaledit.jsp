...
if(bosession== null) {
    response.sendRedirect("login.jsp?returnPage=mailaccount_folder_generaledit.jsp");
    return;
}
if(boctx==null) {
...
if ((BOI==null && securityRights.hasRights(boctx,"mailAccount_folder")) || 
(BOI!=null && BOI.exists() && securityRights.hasRights(boctx,"mailAccount_folder") && securityOPL.canRead(BOI)) || 
(BOI!=null && !BOI.exists() && securityRights.hasRights(boctx,"mailAccount_folder",securityRights.ADD)) 
) {
if (BOI!=null && BOI.exists() && !BOI.isEnabledforRequest &&(!securityRights.hasRights(boctx,"mailAccount_folder",securityRights.WRITE) ||  !securityOPL.canWrite(BOI) && !BOI.isEnabledforRequest)) { 
  BOI.setDisabled(); }
         ...
         <TR>
            ...
            <TD id="Areas_<%=DOCLIST.cvui%>">
               ...
               <DIV id="barea_<%=DOCLIST.cvui%>_0" class="areaSEL">
                  <%{ int cvui15146873=DOCLIST.cvui;%>
                  <%DOCLIST.cvui=DOCLIST.getVui(IDX, BOI.getBoui(), "barea_" + DOCLIST.cvui + "_0");%>
                  <%if(DOC.hasCategoryRights("settings")|| DOC.hasCategoryRights("settings")){%>
                                 ...
                                 <TBODY>
                                    <TR>
                                       <TD style="padding:0px" id="<%=DOCLIST.cvui%>_tabs" valign="bottom" noWrap="yes">
                                          <%if (DOC.hasCategoryRights("settings")){ %><SPAN class="tab tabOn" id="<%=DOCLIST.cvui%>_tabheader_0" name="tab_settings" tabNumber="<%=DOCLIST.cvui%>" tabIndex="<%=DOC.getTabindex(DOC.PANEL, BOI==null?"tab_settings" : BOI.getName(), String.valueOf(BOI==null?0:BOI.bo_boui), String.valueOf(DOCLIST.cvui) + "_tabheader_0", DOCLIST)%>">Identificação da Pasta</SPAN><%}%><%if (DOC.hasCategoryRights("settings")){ %><SPAN class="tab" id="<%=DOCLIST.cvui%>_tabheader_1" name="tab_mailErrors" tabNumber="<%=DOCLIST.cvui%>" tabIndex="<%=DOC.getTabindex(DOC.PANEL, BOI==null?"tab_mailErrors" : BOI.getName(), String.valueOf(BOI==null?0:BOI.bo_boui), String.valueOf(DOCLIST.cvui) + "_tabheader_1", DOCLIST)%>">Email c/ erros</SPAN><%}%>
                                       </TD>
                                    </TR>
                                 </TBODY>
                              </TABLE>
                        ...
                        <TR>
                           <TD><%if (DOC.hasCategoryRights("settings")){ %>
                              <DIV id="<%=DOCLIST.cvui%>_tabbody_0" class="tab">
                                 <%{ int cvui29589496=DOCLIST.cvui;%>
                                 <%
BOI=currObjectList.getObject();sec = DOC.getSection("s434649");
if(sec != null)
  sec.setTitle("");
if(sec==null) { 
  sec = DOC.createSection("s434649","");
  sec.p_name = "section_folders";
  sec.p_id = "section_folders";
sec.p_showLabel=false;
  bodef=boDefHandler.getBoDefinition("mailAccount_folder");  sec.setTitle("");
  docHTML_sectionRow row; 
boolean canAccessfolder=BOI.getAttribute("folder").canAccess();

if (canAccessfolder ) { 
row=sec.addRow();
if( canAccessfolder) {
xattributes=new Hashtable();
xattributes.put("showlabel","yes");row.addCell(bodef.getAttributeRef("folder"),xattributes);
} else row.addCell();
} 
} //endCreateSection
sec.render(pageContext,currObjectList,DOCLIST,DOC);%>

                                 <%DOCLIST.cvui=cvui29589496;}%>
                              </DIV>
                              <%}%>
                              <%if (DOC.hasCategoryRights("settings")){ %>
                              <DIV id="<%=DOCLIST.cvui%>_tabbody_1" class="tab">
                                 <%{ int cvui23834002=DOCLIST.cvui;%>
                                 <div width="100%" height="100%"><% if(BOI.getAttribute("errors").hasRights()){%> <IFRAME id='inc_mailAccount_folder__<%=BOI.bo_boui%>__errors' xsrc='<%="mailerror_generallist.jsp?docid="+IDX+"&method=list&parent_attribute=errors&parent_boui="+BOI.bo_boui %>' frameBorder=0 width='100%' scrolling=no height='100%' tabindex='<%=DOC.getTabindex(DOC.IFRAME, BOI.getName(), String.valueOf(BOI.bo_boui), "inc_mailAccount_folder__" + BOI.bo_boui + "__errors", DOCLIST)%>'></IFRAME><% } %> </div>
                                 <%DOCLIST.cvui=cvui23834002;}%>
                              </DIV>
                              <%}%>
                           </TD>
                        </TR>
...
<%}%>
<FORM name='boFormSubmit' method='post' action='mailaccount_folder_generaledit.jsp'>
