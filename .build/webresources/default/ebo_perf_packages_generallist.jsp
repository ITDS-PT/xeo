...
if(bosession== null) {
    response.sendRedirect("login.jsp?returnPage=ebo_perf_packages_generallist.jsp");
    return;
}
if(boctx==null) {
...
<form class="objectForm" name="boForm" id="<%=IDX%>">
   <%
BOI=currObjectList.getObject();grid = DOC.getGRID("g28312251");
if(grid==null) { 
  bodef=boDefHandler.getBoDefinition("boObject");  boDefBridge bridgeDef=boDefHandler.getBoDefinition("Ebo_Perf").getAttributeRef("packages").getBridge();  grid = DOC.createGRID("g28312251","std",true);
  grid.setTitle();  
...
<%}%>
<FORM name='boFormSubmit' method='post' action='ebo_perf_packages_generallist.jsp'>
