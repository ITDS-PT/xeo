...
if(bosession== null) {
    response.sendRedirect("login.jsp?returnPage=imessagecontext_generallist.jsp");
    return;
}
if(boctx==null) {
...
<form class="objectForm" name="boForm" id="<%=IDX%>">
   <%if(DOC.hasCategoryRights("iMessageContextList")){ %>
   <%
BOI=currObjectList.getObject();grid = DOC.getGRID("g6985550");
if(grid==null) { 
  bodef=boDefHandler.getBoDefinition("iMessageContext");  grid = DOC.createGRID("g6985550","std");
  grid.setTitle();  
...
<%}%>
<FORM name='boFormSubmit' method='post' action='imessagecontext_generallist.jsp'>
