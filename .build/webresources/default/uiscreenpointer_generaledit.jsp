...
if(bosession== null) {
    response.sendRedirect("login.jsp?returnPage=uiscreenpointer_generaledit.jsp");
    return;
}
if(boctx==null) {
...
if ((BOI==null && securityRights.hasRights(boctx,"uiScreenPointer")) || 
(BOI!=null && BOI.exists() && securityRights.hasRights(boctx,"uiScreenPointer") && securityOPL.canRead(BOI)) || 
(BOI!=null && !BOI.exists() && securityRights.hasRights(boctx,"uiScreenPointer",securityRights.ADD)) 
) {
if (BOI!=null && BOI.exists() && !BOI.isEnabledforRequest &&(!securityRights.hasRights(boctx,"uiScreenPointer",securityRights.WRITE) ||  !securityOPL.canWrite(BOI) && !BOI.isEnabledforRequest)) { 
  BOI.setDisabled(); }
...
<form class="objectForm" name="boForm" id="<%=IDX%>">
   ...
   <%if(DOC.hasCategoryRights("global")|| DOC.hasCategoryRights("share")){%>
         ...
         <TR>
            <TD><%if (DOC.hasCategoryRights("global")){ %>
               <DIV id="<%=DOCLIST.cvui%>_tabbody_0" class="tab">
                  <%{ int cvui5331627=DOCLIST.cvui;%>
                  <%
BOI=currObjectList.getObject();sec = DOC.getSection("s18334458");
if(sec != null)
  sec.setTitle(DOC.getCategoryLabel("general","global"));
if(sec==null) { 
  sec = DOC.createSection("s18334458","");
  sec.p_name = "";
  sec.p_id = "";
sec.p_showLabel=false;
  bodef=boDefHandler.getBoDefinition("uiScreenPointer");  sec.setTitle(DOC.getCategoryLabel("general","global"));
...
row=sec.addRow();
row.addCell();
boolean canAccessformName=BOI.getAttribute("formName").canAccess();

if (canAccessformName ) { 
row=sec.addRow();
if( canAccessformName) {
xattributes=new Hashtable();
xattributes.put("showlabel","yes");row.addCell(bodef.getAttributeRef("formName"),xattributes);
} else row.addCell();
} 
boolean canAccessxeoQL=BOI.getAttribute("xeoQL").canAccess();

if (canAccessxeoQL ) { 
row=sec.addRow();
if( canAccessxeoQL) {
xattributes=new Hashtable();
xattributes.put("showlabel","yes");row.addCell(bodef.getAttributeRef("xeoQL"),xattributes);
} else row.addCell();
} 
} //endCreateSection
sec.render(pageContext,currObjectList,DOCLIST,DOC);%>

                  <%DOCLIST.cvui=cvui5331627;}%>
               </DIV>
               <%}%>
               <%if (DOC.hasCategoryRights("share")){ %>
               <DIV id="<%=DOCLIST.cvui%>_tabbody_1" class="tab">
                  <%{ int cvui16896410=DOCLIST.cvui;%>
                  <div width="100%" height="100%"><% if(BOI.getAttribute("share").hasRights()){%> <IFRAME id='inc_uiScreenPointer__<%=BOI.bo_boui%>__share' xsrc='<%="uiscreenpointer_share_generallist.jsp?docid="+IDX+"&method=list&parent_attribute=share&parent_boui="+BOI.bo_boui %>' frameBorder=0 width='100%' scrolling=no height='100%' tabindex='<%=DOC.getTabindex(DOC.IFRAME, BOI.getName(), String.valueOf(BOI.bo_boui), "inc_uiScreenPointer__" + BOI.bo_boui + "__share", DOCLIST)%>'></IFRAME><% } %> </div>
                  <%DOCLIST.cvui=cvui16896410;}%>
               </DIV>
               <%}%>
            </TD>
         </TR>
...
<%}%>
<FORM name='boFormSubmit' method='post' action='uiscreenpointer_generaledit.jsp'>
