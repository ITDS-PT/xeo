...
if(bosession== null) {
    response.sendRedirect("login.jsp?returnPage=ebo_daodepends_generaledit.jsp");
    return;
}
if(boctx==null) {
...
if ((BOI==null && securityRights.hasRights(boctx,"Ebo_DAOdepends")) || 
(BOI!=null && BOI.exists() && securityRights.hasRights(boctx,"Ebo_DAOdepends") && securityOPL.canRead(BOI)) || 
(BOI!=null && !BOI.exists() && securityRights.hasRights(boctx,"Ebo_DAOdepends",securityRights.ADD)) 
) {
if (BOI!=null && BOI.exists() && !BOI.isEnabledforRequest &&(!securityRights.hasRights(boctx,"Ebo_DAOdepends",securityRights.WRITE) ||  !securityOPL.canWrite(BOI) && !BOI.isEnabledforRequest)) { 
  BOI.setDisabled(); }
                                 ...
                                 <TBODY>
                                    <TR>
                                       <TD style="padding:0px" id="<%=DOCLIST.cvui%>_tabs" valign="bottom" noWrap="yes">
                                          <%if (DOC.hasCategoryRights("settings")){ %><SPAN class="tab tabOn" id="<%=DOCLIST.cvui%>_tabheader_0" tabNumber="<%=DOCLIST.cvui%>" tabIndex="<%=DOC.getTabindex(DOC.PANEL, BOI==null?"null" : BOI.getName(), String.valueOf(BOI==null?0:BOI.bo_boui), String.valueOf(DOCLIST.cvui) + "_tabheader_0", DOCLIST)%>"><%=DOC.getCategoryLabel_for_TAB_Header("settings")%></SPAN><%}%>
                                       </TD>
                                    </TR>
                                 </TBODY>
                              </TABLE>
                        ...
                        <TR>
                           <TD><%if (DOC.hasCategoryRights("settings")){ %>
                              <DIV id="<%=DOCLIST.cvui%>_tabbody_0" class="tab">
                                 <%{ int cvui15056182=DOCLIST.cvui;%>
                                 <%
BOI=currObjectList.getObject();sec = DOC.getSection("s15295824");
if(sec != null)
  sec.setTitle(DOC.getCategoryLabel("general","settings"));
if(sec==null) { 
  sec = DOC.createSection("s15295824","");
  sec.p_name = "";
  sec.p_id = "";
sec.p_showLabel=false;
  bodef=boDefHandler.getBoDefinition("Ebo_DAOdepends");  sec.setTitle(DOC.getCategoryLabel("general","settings"));
  sec.p_height="100%";
  docHTML_sectionRow row; 
boolean canAccessDAO_code=BOI.getAttribute("DAO_code").canAccess();

if (canAccessDAO_code ) { 
row=sec.addRow();
if( canAccessDAO_code) {
xattributes=new Hashtable();
xattributes.put("showlabel","yes");row.addCell(bodef.getAttributeRef("DAO_code"),xattributes);
} else row.addCell();
row.addCell();
} 
boolean canAccesstype=BOI.getAttribute("type").canAccess();
boolean canAccesslag=BOI.getAttribute("lag").canAccess();

if (canAccesstype || canAccesslag ) { 
row=sec.addRow();
if( canAccesstype) {
xattributes=new Hashtable();
...
if( canAccesslag) {
xattributes=new Hashtable();
xattributes.put("showlabel","yes");row.addCell(bodef.getAttributeRef("lag"),xattributes);
} else row.addCell();
} 
row=sec.addRow();
row.addCell();
...
<%}%>
<FORM name='boFormSubmit' method='post' action='ebo_daodepends_generaledit.jsp'>
