...
if(bosession== null) {
    response.sendRedirect("login.jsp?returnPage=mailerror_generaledit.jsp");
    return;
}
if(boctx==null) {
...
if ((BOI==null && securityRights.hasRights(boctx,"mailError")) || 
(BOI!=null && BOI.exists() && securityRights.hasRights(boctx,"mailError") && securityOPL.canRead(BOI)) || 
(BOI!=null && !BOI.exists() && securityRights.hasRights(boctx,"mailError",securityRights.ADD)) 
) {
if (BOI!=null && BOI.exists() && !BOI.isEnabledforRequest &&(!securityRights.hasRights(boctx,"mailError",securityRights.WRITE) ||  !securityOPL.canWrite(BOI) && !BOI.isEnabledforRequest)) { 
  BOI.setDisabled(); }
...
<form class="objectForm" name="boForm" id="<%=IDX%>">
   ...
   <%if(DOC.hasCategoryRights("mailerror")){%>
               ...
               <DIV id="barea_<%=DOCLIST.cvui%>_0" class="areaSEL">
                  ...
                  <TABLE id="<%=DOCLIST.cvui%>" class="layout" voui="panel" cellspacing="0" cellpadding="0">
                     <TBODY>
                        ...
                        <TR>
                           <TD><%if (DOC.hasCategoryRights("mailerror")){ %>
                              <DIV id="<%=DOCLIST.cvui%>_tabbody_0" class="tab">
                                 <%{ int cvui24726061=DOCLIST.cvui;%>
                                 <%
BOI=currObjectList.getObject();sec = DOC.getSection("s32833349");
if(sec != null)
  sec.setTitle(DOC.getCategoryLabel("general","mailerror"));
if(sec==null) { 
  sec = DOC.createSection("s32833349","");
  sec.p_name = "";
  sec.p_id = "";
sec.p_showLabel=false;
  bodef=boDefHandler.getBoDefinition("mailError");  sec.setTitle(DOC.getCategoryLabel("general","mailerror"));
  sec.p_height="100%";
  docHTML_sectionRow row; 
boolean canAccessmessageid=BOI.getAttribute("messageid").canAccess();

if (canAccessmessageid ) { 
row=sec.addRow();
if( canAccessmessageid) {
xattributes=new Hashtable();
xattributes.put("showlabel","yes");row.addCell(bodef.getAttributeRef("messageid"),xattributes);
} else row.addCell();
} 
boolean canAccesssubject=BOI.getAttribute("subject").canAccess();

if (canAccesssubject ) { 
row=sec.addRow();
if( canAccesssubject) {
xattributes=new Hashtable();
xattributes.put("showlabel","yes");row.addCell(bodef.getAttributeRef("subject"),xattributes);
} else row.addCell();
} 
boolean canAccesserror=BOI.getAttribute("error").canAccess();

if (canAccesserror ) { 
row=sec.addRow();
if( canAccesserror) {
xattributes=new Hashtable();
xattributes.put("showlabel","yes");xattributes.put("height","100%");row.addCell(bodef.getAttributeRef("error"),xattributes);
} else row.addCell();
} 
} //endCreateSection
sec.render(pageContext,currObjectList,DOCLIST,DOC);%>

                                 <%DOCLIST.cvui=cvui24726061;}%>
                              </DIV>
                              <%}%>
                           </TD>
                        </TR>
...
<%}%>
<FORM name='boFormSubmit' method='post' action='mailerror_generaledit.jsp'>
