...
if(bosession== null) {
    response.sendRedirect("login.jsp?returnPage=uiworkplace_generaledit.jsp");
    return;
}
if(boctx==null) {
...
if ((BOI==null && securityRights.hasRights(boctx,"uiWorkPlace")) || 
(BOI!=null && BOI.exists() && securityRights.hasRights(boctx,"uiWorkPlace") && securityOPL.canRead(BOI)) || 
(BOI!=null && !BOI.exists() && securityRights.hasRights(boctx,"uiWorkPlace",securityRights.ADD)) 
) {
if (BOI!=null && BOI.exists() && !BOI.isEnabledforRequest &&(!securityRights.hasRights(boctx,"uiWorkPlace",securityRights.WRITE) ||  !securityOPL.canWrite(BOI) && !BOI.isEnabledforRequest)) { 
  BOI.setDisabled(); }
...
<form class="objectForm" name="boForm" id="<%=IDX%>">
   ...
   <%if(DOC.hasCategoryRights("settings")|| DOC.hasCategoryRights("listbar")){%>
                  ...
                  <TBODY>
                     <TR>
                        <TD style="padding:0px" id="<%=DOCLIST.cvui%>_tabs" valign="bottom" noWrap="yes">
                           <%if (DOC.hasCategoryRights("settings")){ %><SPAN class="tab tabOn" id="<%=DOCLIST.cvui%>_tabheader_0" name="settings" tabNumber="<%=DOCLIST.cvui%>" tabIndex="<%=DOC.getTabindex(DOC.PANEL, BOI==null?"settings" : BOI.getName(), String.valueOf(BOI==null?0:BOI.bo_boui), String.valueOf(DOCLIST.cvui) + "_tabheader_0", DOCLIST)%>"><%=DOC.getCategoryLabel_for_TAB_Header("settings")%></SPAN><%}%><%if (DOC.hasCategoryRights("listbar")){ %><SPAN class="tab" id="<%=DOCLIST.cvui%>_tabheader_1" name="tab_Listbar" tabNumber="<%=DOCLIST.cvui%>" tabIndex="<%=DOC.getTabindex(DOC.PANEL, BOI==null?"tab_Listbar" : BOI.getName(), String.valueOf(BOI==null?0:BOI.bo_boui), String.valueOf(DOCLIST.cvui) + "_tabheader_1", DOCLIST)%>"><%=DOC.getCategoryLabel_for_TAB_Header("listbar")%></SPAN><%}%>
                        </TD>
                     </TR>
                  </TBODY>
               </TABLE>
         ...
         <TR>
            <TD><%if (DOC.hasCategoryRights("settings")){ %>
               <DIV id="<%=DOCLIST.cvui%>_tabbody_0" class="tab">
                  <%{ int cvui31932654=DOCLIST.cvui;%>
                  <%
BOI=currObjectList.getObject();sec = DOC.getSection("s22537447");
if(sec != null)
  sec.setTitle(DOC.getCategoryLabel("general","settings"));
if(sec==null) { 
  sec = DOC.createSection("s22537447","");
  sec.p_name = "";
  sec.p_id = "";
sec.p_showLabel=false;
  bodef=boDefHandler.getBoDefinition("uiWorkPlace");  sec.setTitle(DOC.getCategoryLabel("general","settings"));
...
boolean canAccessimg=BOI.getAttribute("img").canAccess();

if (canAccessimg ) { 
row=sec.addRow();
if( canAccessimg) {
xattributes=new Hashtable();
xattributes.put("showlabel","yes");row.addCell(bodef.getAttributeRef("img"),xattributes);
} else row.addCell();
} 
boolean canAccessprofile=BOI.getAttribute("profile").canAccess();

if (canAccessprofile ) { 
row=sec.addRow();
if( canAccessprofile) {
xattributes=new Hashtable();
xattributes.put("showlabel","yes");row.addCell(bodef.getAttributeRef("profile"),xattributes,"","");
} else row.addCell();
} 
boolean canAccesslargura=BOI.getAttribute("largura").canAccess();

if (canAccesslargura ) { 
row=sec.addRow();
if( canAccesslargura) {
xattributes=new Hashtable();
xattributes.put("showlabel","yes");row.addCell(bodef.getAttributeRef("largura"),xattributes);
} else row.addCell();
} 
} //endCreateSection
sec.render(pageContext,currObjectList,DOCLIST,DOC);%>

                  <%DOCLIST.cvui=cvui31932654;}%>
               </DIV>
               <%}%>
               <%if (DOC.hasCategoryRights("listbar")){ %>
               <DIV id="<%=DOCLIST.cvui%>_tabbody_1" class="tab">
                  <%{ int cvui24456094=DOCLIST.cvui;%>
                  <%
BOI=currObjectList.getObject();sec = DOC.getSection("s25344985");
if(sec != null)
  sec.setTitle(DOC.getCategoryLabel("general","listbar"));
if(sec==null) { 
  sec = DOC.createSection("s25344985","");
  sec.p_name = "";
  sec.p_id = "";
sec.p_showLabel=false;
  bodef=boDefHandler.getBoDefinition("uiWorkPlace");  sec.setTitle(DOC.getCategoryLabel("general","listbar"));
  docHTML_sectionRow row; 
boolean canAccesslistbar=BOI.getAttribute("listbar").canAccess();

if (canAccesslistbar ) { 
row=sec.addRow();
if( canAccesslistbar) {
xattributes=new Hashtable();
xattributes.put("height","100%");xattributes.put("showlabel","no");row.addCell(bodef.getAttributeRef("listbar"),xattributes,"","");
} else row.addCell();
} 
} //endCreateSection
sec.render(pageContext,currObjectList,DOCLIST,DOC);%>

                  <%DOCLIST.cvui=cvui24456094;}%>
               </DIV>
               <%}%>
            </TD>
         </TR>
...
<%}%>
<FORM name='boFormSubmit' method='post' action='uiworkplace_generaledit.jsp'>
