...
if(bosession== null) {
    response.sendRedirect("login.jsp?returnPage=ebo_filterquery_generallist.jsp");
    return;
}
if(boctx==null) {
...
<script>var objLabel='Lista de  Condição';</script>
...
<form class="objectForm" name="boForm" id="<%=IDX%>">
   ...
   <%
BOI=currObjectList.getObject();grid = DOC.getGRID("g22037543");
if(grid==null) { 
  bodef=boDefHandler.getBoDefinition("Ebo_FilterQuery");  grid = DOC.createGRID("g22037543","std");
  grid.setTitle();  
if ( ( BOI != null && BOI.getAttribute("active").hasRights() ) || securityRights.hasRights( DOC.getEboContext() , currObjectList.getBoDef().getName() ,"active", securityRights.READ )){
xattributes=new Hashtable();
xattributes.put("width","15");grid.addColAtr(bodef.getAttributeRef("active"),15,xattributes);
}if ( ( BOI != null && BOI.getAttribute("joinQuery").hasRights() ) || securityRights.hasRights( DOC.getEboContext() , currObjectList.getBoDef().getName() ,"joinQuery", securityRights.READ )){
xattributes=new Hashtable();
xattributes.put("width","15");grid.addColAtr(bodef.getAttributeRef("joinQuery"),15,xattributes);
}if ( ( BOI != null && BOI.getAttribute("attributeName").hasRights() ) || securityRights.hasRights( DOC.getEboContext() , currObjectList.getBoDef().getName() ,"attributeName", securityRights.READ )){
xattributes=new Hashtable();
xattributes.put("width","40");grid.addColAtr(bodef.getAttributeRef("attributeName"),40,xattributes);
}if ( ( BOI != null && BOI.getAttribute("operator").hasRights() ) || securityRights.hasRights( DOC.getEboContext() , currObjectList.getBoDef().getName() ,"operator", securityRights.READ )){
xattributes=new Hashtable();
xattributes.put("width","25");grid.addColAtr(bodef.getAttributeRef("operator"),25,xattributes);
}if ( ( BOI != null && BOI.getAttribute("value").hasRights() ) || securityRights.hasRights( DOC.getEboContext() , currObjectList.getBoDef().getName() ,"value", securityRights.READ )){
xattributes=new Hashtable();
xattributes.put("width","100");grid.addColAtr(bodef.getAttributeRef("value"),100,xattributes);
}if ( ( BOI != null && BOI.getAttribute("question").hasRights() ) || securityRights.hasRights( DOC.getEboContext() , currObjectList.getBoDef().getName() ,"question", securityRights.READ )){
xattributes=new Hashtable();
xattributes.put("width","100");grid.addColAtr(bodef.getAttributeRef("question"),100,xattributes);
}} //endCreategrid
...
<%}%>
<FORM name='boFormSubmit' method='post' action='ebo_filterquery_generallist.jsp'>
