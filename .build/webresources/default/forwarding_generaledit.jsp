...
if(bosession== null) {
    response.sendRedirect("login.jsp?returnPage=forwarding_generaledit.jsp");
    return;
}
if(boctx==null) {
...
if ((BOI==null && securityRights.hasRights(boctx,"forwarding")) || 
(BOI!=null && BOI.exists() && securityRights.hasRights(boctx,"forwarding") && securityOPL.canRead(BOI)) || 
(BOI!=null && !BOI.exists() && securityRights.hasRights(boctx,"forwarding",securityRights.ADD)) 
) {
if (BOI!=null && BOI.exists() && !BOI.isEnabledforRequest &&(!securityRights.hasRights(boctx,"forwarding",securityRights.WRITE) ||  !securityOPL.canWrite(BOI) && !BOI.isEnabledforRequest)) { 
  BOI.setDisabled(); }
                        ...
                        <TR>
                           <TD><%if (DOC.hasCategoryRights("global")){ %>
                              <DIV id="<%=DOCLIST.cvui%>_tabbody_0" class="tab">
                                 <%{ int cvui8598839=DOCLIST.cvui;%>
                                 <%
BOI=currObjectList.getObject();sec = DOC.getSection("s1764798");
if(sec != null)
  sec.setTitle(DOC.getCategoryLabel("general","global"));
if(sec==null) { 
  sec = DOC.createSection("s1764798","");
  sec.p_name = "";
  sec.p_id = "";
sec.p_showLabel=false;
  bodef=boDefHandler.getBoDefinition("forwarding");  sec.setTitle(DOC.getCategoryLabel("general","global"));
...
if (canAccessforwarding || canAccessdtdoc ) { 
row=sec.addRow();
...
<%}%>
<FORM name='boFormSubmit' method='post' action='forwarding_generaledit.jsp'>
