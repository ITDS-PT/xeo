...
if(bosession== null) {
    response.sendRedirect("login.jsp?returnPage=ebo_map_generalidentification.jsp");
    return;
}
if(boctx==null) {
...
<form class="objectForm" name="boForm" id="<%=IDX%>">
   ...
   <div bo_node="identification"><% if(BOI.getAttribute("id").hasRights()){%> <%=DOC.render(currObjectList,BOI.getAttribute("id"),docHTML.RENDER_FIELD)%><% } %> </div>
   <%}%>
</form>
<%}%>
<FORM name='boFormSubmit' method='post' action='ebo_map_generalidentification.jsp'>
