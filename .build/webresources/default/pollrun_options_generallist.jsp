...
if(bosession== null) {
    response.sendRedirect("login.jsp?returnPage=pollrun_options_generallist.jsp");
    return;
}
if(boctx==null) {
...
<script>var objLabel='Lista de  Votação';</script>
...
<form class="objectForm" name="boForm" id="<%=IDX%>">
   <%
BOI=currObjectList.getObject();grid = DOC.getGRID("g2079355");
if(grid==null) { 
  bodef=boDefHandler.getBoDefinition("PollOption");  boDefBridge bridgeDef=boDefHandler.getBoDefinition("PollRun").getAttributeRef("options").getBridge();  grid = DOC.createGRID("g2079355","std",true);
  grid.setTitle();  
if ( ( BOI != null && BOI.getAttribute("codeOption").hasRights() ) || securityRights.hasRights( DOC.getEboContext() , currObjectList.getBoDef().getName() ,"codeOption", securityRights.READ )){
xattributes=new Hashtable();
xattributes.put("width","200");xattributes.put("referenceLabel","PollOption.codeOption");grid.addColAtr(bodef.getAttributeRef("codeOption"),200,xattributes);
}if ( ( BOI != null && BOI.getAttribute("labelOption").hasRights() ) || securityRights.hasRights( DOC.getEboContext() , currObjectList.getBoDef().getName() ,"labelOption", securityRights.READ )){
xattributes=new Hashtable();
xattributes.put("width","200");xattributes.put("referenceLabel","PollOption.labelOption");grid.addColAtr(bodef.getAttributeRef("labelOption"),200,xattributes);
}xattributes=new Hashtable();
xattributes.put("width","100");grid.addColBridgeAtr(bridgeDef.getAttributeRef("votes"),100,xattributes);
xattributes=new Hashtable();
xattributes.put("width","100");grid.addColBridgeAtr(bridgeDef.getAttributeRef("percent"),100,xattributes);
} //endCreategrid
grid.render(pageContext,currObjectList,DOC, DOCLIST);%>

</form>
<%}%>
<FORM name='boFormSubmit' method='post' action='pollrun_options_generallist.jsp'>
