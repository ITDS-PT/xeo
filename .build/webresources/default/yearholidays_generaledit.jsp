...
if(bosession== null) {
    response.sendRedirect("login.jsp?returnPage=yearholidays_generaledit.jsp");
    return;
}
if(boctx==null) {
...
if ((BOI==null && securityRights.hasRights(boctx,"yearHolidays")) || 
(BOI!=null && BOI.exists() && securityRights.hasRights(boctx,"yearHolidays") && securityOPL.canRead(BOI)) || 
(BOI!=null && !BOI.exists() && securityRights.hasRights(boctx,"yearHolidays",securityRights.ADD)) 
) {
if (BOI!=null && BOI.exists() && !BOI.isEnabledforRequest &&(!securityRights.hasRights(boctx,"yearHolidays",securityRights.WRITE) ||  !securityOPL.canWrite(BOI) && !BOI.isEnabledforRequest)) { 
  BOI.setDisabled(); }
...
<form class="objectForm" name="boForm" id="<%=IDX%>">
   ...
   <%if(DOC.hasCategoryRights("year_holidays")){%>
               ...
               <DIV id="barea_<%=DOCLIST.cvui%>_0" class="areaSEL">
                  ...
                  <TABLE id="<%=DOCLIST.cvui%>" class="layout" voui="panel" cellspacing="0" cellpadding="0">
                     <TBODY>
                                 ...
                                 <TBODY>
                                    <TR>
                                       <TD style="padding:0px" id="<%=DOCLIST.cvui%>_tabs" valign="bottom" noWrap="yes">
                                          <%if (DOC.hasCategoryRights("year_holidays")){ %><SPAN class="tab tabOn" id="<%=DOCLIST.cvui%>_tabheader_0" tabNumber="<%=DOCLIST.cvui%>" tabIndex="<%=DOC.getTabindex(DOC.PANEL, BOI==null?"null" : BOI.getName(), String.valueOf(BOI==null?0:BOI.bo_boui), String.valueOf(DOCLIST.cvui) + "_tabheader_0", DOCLIST)%>"><%=DOC.getCategoryLabel_for_TAB_Header("year_holidays")%></SPAN><%}%>
                                       </TD>
                                    </TR>
                                 </TBODY>
                              </TABLE>
                        ...
                        <TR>
                           <TD><%if (DOC.hasCategoryRights("year_holidays")){ %>
                              <DIV id="<%=DOCLIST.cvui%>_tabbody_0" class="tab">
                                 <%{ int cvui10768317=DOCLIST.cvui;%>
                                 <table height="100%" width="100%">
                                    <tr height="50px">
                                       <td>
                                          <%
BOI=currObjectList.getObject();sec = DOC.getSection("s24054228");
if(sec != null)
  sec.setTitle("");
if(sec==null) { 
  sec = DOC.createSection("s24054228","");
  sec.p_name = "section_header";
  sec.p_id = "section_header";
sec.p_showLabel=false;
  bodef=boDefHandler.getBoDefinition("yearHolidays");  sec.setTitle("");
  docHTML_sectionRow row; 
boolean canAccessyearSelect=BOI.getAttribute("yearSelect").canAccess();

if (canAccessyearSelect ) { 
row=sec.addRow();
if( canAccessyearSelect) {
xattributes=new Hashtable();
xattributes.put("showlabel","yes");row.addCell(bodef.getAttributeRef("yearSelect"),xattributes);
} else row.addCell();
row.addCell();
} 
...
sec.render(pageContext,currObjectList,DOCLIST,DOC);%></td>
                                    ...
                                    <tr>
                                       <td>
                                          <div width="100%" height="100%"><% if(BOI.getAttribute("holidays").hasRights()){%> <IFRAME id='inc_yearHolidays__<%=BOI.bo_boui%>__holidays' xsrc='<%="holiday_generallist.jsp?docid="+IDX+"&method=list&parent_attribute=holidays&parent_boui="+BOI.bo_boui %>' frameBorder=0 width='100%' scrolling=no height='100%' tabindex='<%=DOC.getTabindex(DOC.IFRAME, BOI.getName(), String.valueOf(BOI.bo_boui), "inc_yearHolidays__" + BOI.bo_boui + "__holidays", DOCLIST)%>'></IFRAME><% } %> </div></td>
                                    </tr>
                                 </table>
                                 <%DOCLIST.cvui=cvui10768317;}%>
                              </DIV>
                              <%}%>
                           </TD>
                        </TR>
...
<%}%>
<FORM name='boFormSubmit' method='post' action='yearholidays_generaledit.jsp'>
