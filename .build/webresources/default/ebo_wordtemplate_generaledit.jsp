...
if(bosession== null) {
    response.sendRedirect("login.jsp?returnPage=ebo_wordtemplate_generaledit.jsp");
    return;
}
if(boctx==null) {
...
if ((BOI==null && securityRights.hasRights(boctx,"Ebo_WordTemplate")) || 
(BOI!=null && BOI.exists() && securityRights.hasRights(boctx,"Ebo_WordTemplate") && securityOPL.canRead(BOI)) || 
(BOI!=null && !BOI.exists() && securityRights.hasRights(boctx,"Ebo_WordTemplate",securityRights.ADD)) 
) {
if (BOI!=null && BOI.exists() && !BOI.isEnabledforRequest &&(!securityRights.hasRights(boctx,"Ebo_WordTemplate",securityRights.WRITE) ||  !securityOPL.canWrite(BOI) && !BOI.isEnabledforRequest)) { 
  BOI.setDisabled(); }
         ...
         <TR>
            <TD><%if (DOC.hasCategoryRights("global")){ %>
               <DIV id="<%=DOCLIST.cvui%>_tabbody_0" class="tab" style="overflow-y:auto">
                  <%{ int cvui9486627=DOCLIST.cvui;%>
                  <%
BOI=currObjectList.getObject();sec = DOC.getSection("s24638620");
if(sec != null)
  sec.setTitle(DOC.getCategoryLabel("general","global"));
if(sec==null) { 
  sec = DOC.createSection("s24638620","");
  sec.p_name = "seccao_master_document";
  sec.p_id = "seccao_master_document";
sec.p_showLabel=false;
  bodef=boDefHandler.getBoDefinition("Ebo_WordTemplate");  sec.setTitle(DOC.getCategoryLabel("general","global"));
...
sec.render(pageContext,currObjectList,DOCLIST,DOC);%>
                  ...
                  <%
BOI=currObjectList.getObject();sec = DOC.getSection("s6054876");
if(sec != null)
  sec.setTitle("Propriedades do Modelo de Documento");
if(sec==null) { 
  sec = DOC.createSection("s6054876","");
  sec.p_name = "seccao_master_template";
  sec.p_id = "seccao_master_template";
  bodef=boDefHandler.getBoDefinition("Ebo_WordTemplate");  sec.setTitle("Propriedades do Modelo de Documento");
  docHTML_sectionRow row; 
...
sec.render(pageContext,currObjectList,DOCLIST,DOC);%>
                  ...
                  <%
BOI=currObjectList.getObject();sec = DOC.getSection("s27271821");
if(sec != null)
  sec.setTitle("Caracteristicas");
if(sec==null) { 
  sec = DOC.createSection("s27271821","");
  sec.p_name = "phisicalProperties";
  sec.p_id = "phisicalProperties";
  bodef=boDefHandler.getBoDefinition("Ebo_WordTemplate");  sec.setTitle("Caracteristicas");
  docHTML_sectionRow row; 
...
<%}%>
<FORM name='boFormSubmit' method='post' action='ebo_wordtemplate_generaledit.jsp'>
