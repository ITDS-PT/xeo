...
if(bosession== null) {
    response.sendRedirect("login.jsp?returnPage=sch_workplace_generaledit.jsp");
    return;
}
if(boctx==null) {
...
if ((BOI==null && securityRights.hasRights(boctx,"Sch_workplace")) || 
(BOI!=null && BOI.exists() && securityRights.hasRights(boctx,"Sch_workplace") && securityOPL.canRead(BOI)) || 
(BOI!=null && !BOI.exists() && securityRights.hasRights(boctx,"Sch_workplace",securityRights.ADD)) 
) {
if (BOI!=null && BOI.exists() && !BOI.isEnabledforRequest &&(!securityRights.hasRights(boctx,"Sch_workplace",securityRights.WRITE) ||  !securityOPL.canWrite(BOI) && !BOI.isEnabledforRequest)) { 
  BOI.setDisabled(); }
         ...
         <TR>
            <TD><%if (DOC.hasCategoryRights("identification")){ %>
               <DIV id="<%=DOCLIST.cvui%>_tabbody_0" class="tab">
                  <%{ int cvui29877846=DOCLIST.cvui;%>
                  <table>
                     <tr height="1%">
                        <td>
                           <%
BOI=currObjectList.getObject();sec = DOC.getSection("s14888065");
if(sec != null)
  sec.setTitle("");
if(sec==null) { 
  sec = DOC.createSection("s14888065","");
  sec.p_name = "";
  sec.p_id = "";
sec.p_showLabel=false;
  bodef=boDefHandler.getBoDefinition("Sch_workplace");  sec.setTitle("");
...
if (canAccesscountry || canAccessstate ) { 
row=sec.addRow();
if( canAccesscountry) {
xattributes=new Hashtable();
...
boolean canAccesscity=BOI.getAttribute("city").canAccess();
boolean canAccesstimeZone=BOI.getAttribute("timeZone").canAccess();

if (canAccesscity || canAccesstimeZone ) { 
row=sec.addRow();
if( canAccesscity) {
xattributes=new Hashtable();
xattributes.put("showlabel","yes");row.addCell(bodef.getAttributeRef("city"),xattributes);
} else row.addCell();
if( canAccesstimeZone) {
xattributes=new Hashtable();
xattributes.put("showlabel","yes");row.addCell(bodef.getAttributeRef("timeZone"),xattributes);
} else row.addCell();
} 
boolean canAccessworkSchedule=BOI.getAttribute("workSchedule").canAccess();
boolean canAccesschristianHolidays=BOI.getAttribute("christianHolidays").canAccess();

if (canAccessworkSchedule || canAccesschristianHolidays ) { 
row=sec.addRow();
if( canAccessworkSchedule) {
xattributes=new Hashtable();
xattributes.put("showlabel","yes");row.addCell(bodef.getAttributeRef("workSchedule"),xattributes,"","");
} else row.addCell();
if( canAccesschristianHolidays) {
xattributes=new Hashtable();
xattributes.put("showlabel","yes");row.addCell(bodef.getAttributeRef("christianHolidays"),xattributes);
} else row.addCell();
} 
} //endCreateSection
sec.render(pageContext,currObjectList,DOCLIST,DOC);%></td>
                     ...
                     <tr height="99%">
                        <td>
                           <div height="100%" width="100%"><% if(BOI.getAttribute("fixedHolidays").hasRights()){%> <IFRAME id='inc_Sch_workplace__<%=BOI.bo_boui%>__fixedHolidays' xsrc='<%="holiday_generallist.jsp?docid="+IDX+"&method=list&parent_attribute=fixedHolidays&parent_boui="+BOI.bo_boui %>' frameBorder=0 width='100%' scrolling=no height='100%' tabindex='<%=DOC.getTabindex(DOC.IFRAME, BOI.getName(), String.valueOf(BOI.bo_boui), "inc_Sch_workplace__" + BOI.bo_boui + "__fixedHolidays", DOCLIST)%>'></IFRAME><% } %> </div></td>
                     </tr>
                  </table>
                  <%DOCLIST.cvui=cvui29877846;}%>
               </DIV>
               <%}%>
            </TD>
         </TR>
...
<%}%>
<FORM name='boFormSubmit' method='post' action='sch_workplace_generaledit.jsp'>
