...
if(bosession== null) {
    response.sendRedirect("login.jsp?returnPage=icontact_generallist.jsp");
    return;
}
if(boctx==null) {
...
<script>var objLabel='Lista de  Contacto';</script>
...
<form class="objectForm" name="boForm" id="<%=IDX%>">
   <%if(DOC.hasCategoryRights("iContactList")){ %>
   <%
BOI=currObjectList.getObject();grid = DOC.getGRID("g16984264");
if(grid==null) { 
  bodef=boDefHandler.getBoDefinition("iContact");  grid = DOC.createGRID("g16984264","std");
  grid.setTitle();  
if ( ( BOI != null && BOI.getAttribute("name").hasRights() ) || securityRights.hasRights( DOC.getEboContext() , currObjectList.getBoDef().getName() ,"name", securityRights.READ )){
xattributes=new Hashtable();
xattributes.put("width","100");grid.addColAtr(bodef.getAttributeRef("name"),100,xattributes);
}if ( ( BOI != null && BOI.getAttribute("email").hasRights() ) || securityRights.hasRights( DOC.getEboContext() , currObjectList.getBoDef().getName() ,"email", securityRights.READ )){
xattributes=new Hashtable();
xattributes.put("width","100");grid.addColAtr(bodef.getAttributeRef("email"),100,xattributes);
}if ( ( BOI != null && BOI.getAttribute("fax").hasRights() ) || securityRights.hasRights( DOC.getEboContext() , currObjectList.getBoDef().getName() ,"fax", securityRights.READ )){
xattributes=new Hashtable();
xattributes.put("width","100");grid.addColAtr(bodef.getAttributeRef("fax"),100,xattributes);
}if ( ( BOI != null && BOI.getAttribute("telemovel").hasRights() ) || securityRights.hasRights( DOC.getEboContext() , currObjectList.getBoDef().getName() ,"telemovel", securityRights.READ )){
xattributes=new Hashtable();
xattributes.put("width","100");grid.addColAtr(bodef.getAttributeRef("telemovel"),100,xattributes);
}if ( ( BOI != null && BOI.getAttribute("lastname").hasRights() ) || securityRights.hasRights( DOC.getEboContext() , currObjectList.getBoDef().getName() ,"lastname", securityRights.READ )){
xattributes=new Hashtable();
xattributes.put("width","100");grid.addColAtr(bodef.getAttributeRef("lastname"),100,xattributes);
}} //endCreategrid
...
<%}%>
<FORM name='boFormSubmit' method='post' action='icontact_generallist.jsp'>
