...
if(bosession== null) {
    response.sendRedirect("login.jsp?returnPage=daywork_generaledit.jsp");
    return;
}
if(boctx==null) {
...
if ((BOI==null && securityRights.hasRights(boctx,"daywork")) || 
(BOI!=null && BOI.exists() && securityRights.hasRights(boctx,"daywork") && securityOPL.canRead(BOI)) || 
(BOI!=null && !BOI.exists() && securityRights.hasRights(boctx,"daywork",securityRights.ADD)) 
) {
if (BOI!=null && BOI.exists() && !BOI.isEnabledforRequest &&(!securityRights.hasRights(boctx,"daywork",securityRights.WRITE) ||  !securityOPL.canWrite(BOI) && !BOI.isEnabledforRequest)) { 
  BOI.setDisabled(); }
         ...
         <TR>
            <TD><%if (DOC.hasCategoryRights("identification")){ %>
               <DIV id="<%=DOCLIST.cvui%>_tabbody_0" class="tab">
                  <%{ int cvui22357580=DOCLIST.cvui;%>
                  <table>
                     <tr height="1%">
                        <td>
                           <%
BOI=currObjectList.getObject();sec = DOC.getSection("s21330566");
if(sec != null)
  sec.setTitle("");
if(sec==null) { 
  sec = DOC.createSection("s21330566","");
  sec.p_name = "";
  sec.p_id = "";
sec.p_showLabel=false;
  bodef=boDefHandler.getBoDefinition("daywork");  sec.setTitle("");
...
sec.render(pageContext,currObjectList,DOCLIST,DOC);%></td>
                     ...
                     <tr height="99%">
                        <td>
                           <div height="100%" width="100%"><% if(BOI.getAttribute("periods").hasRights()){%> <IFRAME id='inc_daywork__<%=BOI.bo_boui%>__periods' xsrc='<%="dayperiod_generallist.jsp?docid="+IDX+"&method=list&parent_attribute=periods&parent_boui="+BOI.bo_boui %>' frameBorder=0 width='100%' scrolling=no height='100%' tabindex='<%=DOC.getTabindex(DOC.IFRAME, BOI.getName(), String.valueOf(BOI.bo_boui), "inc_daywork__" + BOI.bo_boui + "__periods", DOCLIST)%>'></IFRAME><% } %> </div></td>
                     </tr>
                  </table>
                  <%DOCLIST.cvui=cvui22357580;}%>
               </DIV>
               <%}%>
            </TD>
         </TR>
...
<%}%>
<FORM name='boFormSubmit' method='post' action='daywork_generaledit.jsp'>
