...
if(bosession== null) {
    response.sendRedirect("login.jsp?returnPage=ebo_login_generaledit.jsp");
    return;
}
if(boctx==null) {
...
if ((BOI==null && securityRights.hasRights(boctx,"Ebo_Login")) || 
(BOI!=null && BOI.exists() && securityRights.hasRights(boctx,"Ebo_Login") && securityOPL.canRead(BOI)) || 
(BOI!=null && !BOI.exists() && securityRights.hasRights(boctx,"Ebo_Login",securityRights.ADD)) 
) {
if (BOI!=null && BOI.exists() && !BOI.isEnabledforRequest &&(!securityRights.hasRights(boctx,"Ebo_Login",securityRights.WRITE) ||  !securityOPL.canWrite(BOI) && !BOI.isEnabledforRequest)) { 
  BOI.setDisabled(); }
         ...
         <TR>
            <TD><%if (DOC.hasCategoryRights("identification")){ %>
               <DIV id="<%=DOCLIST.cvui%>_tabbody_0" class="tab">
                  <%{ int cvui16622574=DOCLIST.cvui;%>
                  <%
BOI=currObjectList.getObject();sec = DOC.getSection("s1403142");
if(sec != null)
  sec.setTitle(DOC.getCategoryLabel("general","identification"));
if(sec==null) { 
  sec = DOC.createSection("s1403142","");
  sec.p_name = "";
  sec.p_id = "";
sec.p_showLabel=false;
  bodef=boDefHandler.getBoDefinition("Ebo_Login");  sec.setTitle(DOC.getCategoryLabel("general","identification"));
  docHTML_sectionRow row; 
boolean canAccessuser=BOI.getAttribute("user").canAccess();

if (canAccessuser ) { 
row=sec.addRow();
if( canAccessuser) {
xattributes=new Hashtable();
xattributes.put("showlabel","yes");row.addCell(bodef.getAttributeRef("user"),xattributes,"","");
} else row.addCell();
row.addCell();
} 
boolean canAccessdata=BOI.getAttribute("data").canAccess();

if (canAccessdata ) { 
row=sec.addRow();
if( canAccessdata) {
xattributes=new Hashtable();
xattributes.put("showlabel","yes");row.addCell(bodef.getAttributeRef("data"),xattributes);
} else row.addCell();
row.addCell();
} 
boolean canAccessmovementType=BOI.getAttribute("movementType").canAccess();

if (canAccessmovementType ) { 
row=sec.addRow();
if( canAccessmovementType) {
xattributes=new Hashtable();
xattributes.put("showlabel","yes");row.addCell(bodef.getAttributeRef("movementType"),xattributes);
} else row.addCell();
row.addCell();
} 
boolean canAccessrepository=BOI.getAttribute("repository").canAccess();

if (canAccessrepository ) { 
row=sec.addRow();
if( canAccessrepository) {
xattributes=new Hashtable();
xattributes.put("showlabel","yes");row.addCell(bodef.getAttributeRef("repository"),xattributes);
} else row.addCell();
row.addCell();
} 
boolean canAccessiProfile=BOI.getAttribute("iProfile").canAccess();

if (canAccessiProfile ) { 
row=sec.addRow();
if( canAccessiProfile) {
xattributes=new Hashtable();
xattributes.put("showlabel","yes");row.addCell(bodef.getAttributeRef("iProfile"),xattributes,"","");
} else row.addCell();
row.addCell();
} 
boolean canAccessremoteAddr=BOI.getAttribute("remoteAddr").canAccess();
boolean canAccessremoteHost=BOI.getAttribute("remoteHost").canAccess();

if (canAccessremoteAddr || canAccessremoteHost ) { 
row=sec.addRow();
if( canAccessremoteAddr) {
xattributes=new Hashtable();
xattributes.put("showlabel","yes");row.addCell(bodef.getAttributeRef("remoteAddr"),xattributes);
} else row.addCell();
if( canAccessremoteHost) {
xattributes=new Hashtable();
xattributes.put("showlabel","yes");row.addCell(bodef.getAttributeRef("remoteHost"),xattributes);
} else row.addCell();
} 
boolean canAccessremoteUser=BOI.getAttribute("remoteUser").canAccess();
boolean canAccessremoteSessionId=BOI.getAttribute("remoteSessionId").canAccess();

if (canAccessremoteUser || canAccessremoteSessionId ) { 
row=sec.addRow();
if( canAccessremoteUser) {
xattributes=new Hashtable();
xattributes.put("showlabel","yes");row.addCell(bodef.getAttributeRef("remoteUser"),xattributes);
} else row.addCell();
if( canAccessremoteSessionId) {
xattributes=new Hashtable();
xattributes.put("showlabel","yes");row.addCell(bodef.getAttributeRef("remoteSessionId"),xattributes);
} else row.addCell();
} 
} //endCreateSection
sec.render(pageContext,currObjectList,DOCLIST,DOC);%>

                  <%DOCLIST.cvui=cvui16622574;}%>
               </DIV>
               <%}%>
            </TD>
         </TR>
...
<%}%>
<FORM name='boFormSubmit' method='post' action='ebo_login_generaledit.jsp'>
