...
if(bosession== null) {
    response.sendRedirect("login.jsp?returnPage=ebo_daoconfig_generaledit.jsp");
    return;
}
if(boctx==null) {
...
if ((BOI==null && securityRights.hasRights(boctx,"Ebo_DAOConfig")) || 
(BOI!=null && BOI.exists() && securityRights.hasRights(boctx,"Ebo_DAOConfig") && securityOPL.canRead(BOI)) || 
(BOI!=null && !BOI.exists() && securityRights.hasRights(boctx,"Ebo_DAOConfig",securityRights.ADD)) 
) {
if (BOI!=null && BOI.exists() && !BOI.isEnabledforRequest &&(!securityRights.hasRights(boctx,"Ebo_DAOConfig",securityRights.WRITE) ||  !securityOPL.canWrite(BOI) && !BOI.isEnabledforRequest)) { 
  BOI.setDisabled(); }
...
<form class="objectForm" name="boForm" id="<%=IDX%>">
   <%
BOI=currObjectList.getObject();sec = DOC.getSection("s25716720");
if(sec != null)
  sec.setTitle("");
if(sec==null) { 
  sec = DOC.createSection("s25716720","");
  sec.p_name = "";
  sec.p_id = "";
sec.p_showLabel=false;
  bodef=boDefHandler.getBoDefinition("Ebo_DAOConfig");  sec.setTitle("");
...
sec.render(pageContext,currObjectList,DOCLIST,DOC);%>
   ...
   <%if(DOC.hasCategoryRights("extAtt_parameter")|| DOC.hasCategoryRights("linedepends")){%>
   <TABLE id="<%=DOCLIST.cvui%>" class="layout" cellspacing="0" cellpadding="0">
      <TBODY>
                  ...
                  <TBODY>
                     <TR>
                        <TD style="padding:0px" id="<%=DOCLIST.cvui%>_tabs" valign="bottom" noWrap="yes">
                           <%if (DOC.hasCategoryRights("extAtt_parameter")){ %><SPAN class="tab tabOn" id="<%=DOCLIST.cvui%>_tabheader_0" name="tab_extendAttribute" tabNumber="<%=DOCLIST.cvui%>" tabIndex="<%=DOC.getTabindex(DOC.PANEL, BOI==null?"tab_extendAttribute" : BOI.getName(), String.valueOf(BOI==null?0:BOI.bo_boui), String.valueOf(DOCLIST.cvui) + "_tabheader_0", DOCLIST)%>"><%=DOC.getCategoryLabel_for_TAB_Header("extAtt_parameter")%></SPAN><%}%><%if (DOC.hasCategoryRights("linedepends")){ %><SPAN class="tab" id="<%=DOCLIST.cvui%>_tabheader_1" tabNumber="<%=DOCLIST.cvui%>" tabIndex="<%=DOC.getTabindex(DOC.PANEL, BOI==null?"null" : BOI.getName(), String.valueOf(BOI==null?0:BOI.bo_boui), String.valueOf(DOCLIST.cvui) + "_tabheader_1", DOCLIST)%>"><%=DOC.getCategoryLabel_for_TAB_Header("linedepends")%></SPAN><%}%>
                        </TD>
                     </TR>
                  </TBODY>
               </TABLE>
         ...
         <TR>
            <TD><%if (DOC.hasCategoryRights("extAtt_parameter")){ %>
               <DIV id="<%=DOCLIST.cvui%>_tabbody_0" class="tab">
                  <%{ int cvui6277012=DOCLIST.cvui;%>
                  <div width="100%" height="100%"><% if(BOI.getAttribute("extAtt_parameter").hasRights()){%> <IFRAME id='inc_Ebo_DAOConfig__<%=BOI.bo_boui%>__extAtt_parameter' xsrc='<%="ebo_daoparameter_generallist.jsp?docid="+IDX+"&method=list&parent_attribute=extAtt_parameter&parent_boui="+BOI.bo_boui %>' frameBorder=0 width='100%' scrolling=no height='100%' tabindex='<%=DOC.getTabindex(DOC.IFRAME, BOI.getName(), String.valueOf(BOI.bo_boui), "inc_Ebo_DAOConfig__" + BOI.bo_boui + "__extAtt_parameter", DOCLIST)%>'></IFRAME><% } %> </div>
                  <%DOCLIST.cvui=cvui6277012;}%>
               </DIV>
               <%}%>
               <%if (DOC.hasCategoryRights("linedepends")){ %>
               <DIV id="<%=DOCLIST.cvui%>_tabbody_1" class="tab">
                  <%{ int cvui2619130=DOCLIST.cvui;%>
                  <div width="100%" height="100%"><% if(BOI.getAttribute("linedepends").hasRights()){%> <IFRAME id='inc_Ebo_DAOConfig__<%=BOI.bo_boui%>__linedepends' xsrc='<%="ebo_daodepends_generallist.jsp?docid="+IDX+"&method=list&parent_attribute=linedepends&parent_boui="+BOI.bo_boui %>' frameBorder=0 width='100%' scrolling=no height='100%' tabindex='<%=DOC.getTabindex(DOC.IFRAME, BOI.getName(), String.valueOf(BOI.bo_boui), "inc_Ebo_DAOConfig__" + BOI.bo_boui + "__linedepends", DOCLIST)%>'></IFRAME><% } %> </div>
                  <%DOCLIST.cvui=cvui2619130;}%>
               </DIV>
               <%}%>
            </TD>
         </TR>
...
<%}%>
<FORM name='boFormSubmit' method='post' action='ebo_daoconfig_generaledit.jsp'>
