...
if(bosession== null) {
    response.sendRedirect("login.jsp?returnPage=address_generalquickview.jsp");
    return;
}
if(boctx==null) {
...
<form class="objectForm" name="boForm" id="<%=IDX%>">
   <%
BOI=currObjectList.getObject();sec = DOC.getSection("s27831591");
if(sec != null)
  sec.setTitle(DOC.getCategoryLabel("general",""));
if(sec==null) { 
  sec = DOC.createSection("s27831591","");
  sec.p_name = "";
  sec.p_id = "";
...
boolean canAccessconcelho=BOI.getAttribute("concelho").canAccess();

if (canAccessconcelho ) { 
row=sec.addRow();
if( canAccessconcelho) {
xattributes=new Hashtable();
xattributes.put("showlabel","yes");row.addCell(bodef.getAttributeRef("concelho"),xattributes);
} else row.addCell();
} 
...
<%}%>
<FORM name='boFormSubmit' method='post' action='address_generalquickview.jsp'>
