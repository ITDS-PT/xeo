...
if(bosession== null) {
    response.sendRedirect("login.jsp?returnPage=deliverymessage_generaledit.jsp");
    return;
}
if(boctx==null) {
...
if ((BOI==null && securityRights.hasRights(boctx,"deliveryMessage")) || 
(BOI!=null && BOI.exists() && securityRights.hasRights(boctx,"deliveryMessage") && securityOPL.canRead(BOI)) || 
(BOI!=null && !BOI.exists() && securityRights.hasRights(boctx,"deliveryMessage",securityRights.ADD)) 
) {
if (BOI!=null && BOI.exists() && !BOI.isEnabledforRequest &&(!securityRights.hasRights(boctx,"deliveryMessage",securityRights.WRITE) ||  !securityOPL.canWrite(BOI) && !BOI.isEnabledforRequest)) { 
  BOI.setDisabled(); }
...
<form class="objectForm" name="boForm" id="<%=IDX%>">
   ...
   <%if(DOC.hasCategoryRights("identification")|| DOC.hasCategoryRights("referedObject")){%>
                  ...
                  <TBODY>
                     <TR>
                        <TD style="padding:0px" id="<%=DOCLIST.cvui%>_tabs" valign="bottom" noWrap="yes">
                           <%if (DOC.hasCategoryRights("identification")){ %><SPAN class="tab tabOn" id="<%=DOCLIST.cvui%>_tabheader_0" tabNumber="<%=DOCLIST.cvui%>" tabIndex="<%=DOC.getTabindex(DOC.PANEL, BOI==null?"null" : BOI.getName(), String.valueOf(BOI==null?0:BOI.bo_boui), String.valueOf(DOCLIST.cvui) + "_tabheader_0", DOCLIST)%>">Identificação</SPAN><%}%><%if (DOC.hasCategoryRights("referedObject")){ %><SPAN class="tab" id="<%=DOCLIST.cvui%>_tabheader_1" tabNumber="<%=DOCLIST.cvui%>" tabIndex="<%=DOC.getTabindex(DOC.PANEL, BOI==null?"null" : BOI.getName(), String.valueOf(BOI==null?0:BOI.bo_boui), String.valueOf(DOCLIST.cvui) + "_tabheader_1", DOCLIST)%>">Detalhe</SPAN><%}%>
                        </TD>
                     </TR>
                  </TBODY>
               </TABLE>
         ...
         <TR>
            <TD><%if (DOC.hasCategoryRights("identification")){ %>
               <DIV id="<%=DOCLIST.cvui%>_tabbody_0" class="tab">
                  <%{ int cvui15288340=DOCLIST.cvui;%>
                  <%
BOI=currObjectList.getObject();sec = DOC.getSection("s24859906");
if(sec != null)
  sec.setTitle("Contacto");
if(sec==null) { 
  sec = DOC.createSection("s24859906","");
  sec.p_name = "";
  sec.p_id = "";
sec.p_showLabel=false;
  bodef=boDefHandler.getBoDefinition("deliveryMessage");  sec.setTitle("Contacto");
  docHTML_sectionRow row; 
boolean canAccessac=BOI.getAttribute("ac").canAccess();

if (canAccessac ) { 
row=sec.addRow();
if( canAccessac) {
xattributes=new Hashtable();
xattributes.put("showlabel","yes");row.addCell(bodef.getAttributeRef("ac"),xattributes);
} else row.addCell();
} 
...
if (canAccesstelemovel || canAccessfax ) { 
row=sec.addRow();
...
boolean canAccessdate_sent_read_receipt=BOI.getAttribute("date_sent_read_receipt").canAccess();

if (canAccessdate_sent_read_receipt ) { 
row=sec.addRow();
if( canAccessdate_sent_read_receipt) {
xattributes=new Hashtable();
xattributes.put("showlabel","yes");row.addCell(bodef.getAttributeRef("date_sent_read_receipt"),xattributes);
} else row.addCell();
row.addCell();
} 
} //endCreateSection
sec.render(pageContext,currObjectList,DOCLIST,DOC);%>
                  ...
                  <%DOCLIST.cvui=cvui15288340;}%>
               </DIV>
               <%}%>
               <%if (DOC.hasCategoryRights("referedObject")){ %>
               <DIV id="<%=DOCLIST.cvui%>_tabbody_1" class="tab">
                  <%{ int cvui669616=DOCLIST.cvui;%>
                  <%
BOI=currObjectList.getObject();sec = DOC.getSection("s13517416");
if(sec != null)
  sec.setTitle("Objecto");
if(sec==null) { 
  sec = DOC.createSection("s13517416","");
  sec.p_name = "";
  sec.p_id = "";
sec.p_showLabel=false;
  bodef=boDefHandler.getBoDefinition("deliveryMessage");  sec.setTitle("Objecto");
  docHTML_sectionRow row; 
boolean canAccessrefObj=BOI.getAttribute("refObj").canAccess();

if (canAccessrefObj ) { 
row=sec.addRow();
if( canAccessrefObj) {
xattributes=new Hashtable();
xattributes.put("showlabel","yes");row.addCell(bodef.getAttributeRef("refObj"),xattributes,"","");
} else row.addCell();
} 
} //endCreateSection
sec.render(pageContext,currObjectList,DOCLIST,DOC);%>

                  <%DOCLIST.cvui=cvui669616;}%>
               </DIV>
               <%}%>
            </TD>
         </TR>
...
<%}%>
<FORM name='boFormSubmit' method='post' action='deliverymessage_generaledit.jsp'>
