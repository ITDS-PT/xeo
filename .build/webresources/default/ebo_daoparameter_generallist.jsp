...
if(bosession== null) {
    response.sendRedirect("login.jsp?returnPage=ebo_daoparameter_generallist.jsp");
    return;
}
if(boctx==null) {
...
<script>var objLabel='Lista de  DAO Config Object Line Parameters';</script>
...
<form class="objectForm" name="boForm" id="<%=IDX%>">
   ...
   <%
BOI=currObjectList.getObject();grid = DOC.getGRID("g31097783");
if(grid==null) { 
  bodef=boDefHandler.getBoDefinition("Ebo_DAOparameter");  grid = DOC.createGRID("g31097783","");
  grid.setTitle();  
xattributes=new Hashtable();
xattributes.put("width","");xattributes.put("method","getCardIDByExtendAttributeAlias");grid.addColSpecial("method",40,xattributes);
if ( ( BOI != null && BOI.getAttribute("inout").hasRights() ) || securityRights.hasRights( DOC.getEboContext() , currObjectList.getBoDef().getName() ,"inout", securityRights.READ )){
xattributes=new Hashtable();
xattributes.put("width","110");grid.addColAtr(bodef.getAttributeRef("inout"),110,xattributes);
}xattributes=new Hashtable();
xattributes.put("width","250");xattributes.put("method","getAliasChildLov");grid.addColSpecial("method",250,xattributes);
} //endCreategrid
...
<%}%>
<FORM name='boFormSubmit' method='post' action='ebo_daoparameter_generallist.jsp'>
