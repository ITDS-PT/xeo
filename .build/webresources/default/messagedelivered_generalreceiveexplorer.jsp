...
if(bosession== null) {
    response.sendRedirect("login.jsp?returnPage=messagedelivered_generalreceiveexplorer.jsp");
    return;
}
if(boctx==null) {
...
<form class="objectForm" name="boForm" id="<%=IDX%>">
   <div width="100%" height="100%"><IFRAME    src="__explorer.jsp?docid=<%=DOC.getDocIdx()%>&pageName=<%=pageContext.getPage().getClass().getName()%>&objectName=messageDelivered&form=receiveExplorer" id='explorer' frameBorder='0' width='100%' height='100%' scrolling='no' tabindex='0'></IFRAME></div><script> window.gridKey='24910770'</script>  

</form>
<%}%>
<FORM name='boFormSubmit' method='post' action='messagedelivered_generalreceiveexplorer.jsp'>
