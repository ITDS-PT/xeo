...
if(bosession== null) {
    response.sendRedirect("login.jsp?returnPage=ebo_alias_generallist.jsp");
    return;
}
if(boctx==null) {
...
<script>var objLabel='Lista de  Alias';</script>
...
<form class="objectForm" name="boForm" id="<%=IDX%>">
   <%
BOI=currObjectList.getObject();grid = DOC.getGRID("g3443960");
if(grid==null) { 
  bodef=boDefHandler.getBoDefinition("Ebo_Alias");  grid = DOC.createGRID("g3443960","std");
  grid.setTitle();  
if ( ( BOI != null && BOI.getAttribute("ui").hasRights() ) || securityRights.hasRights( DOC.getEboContext() , currObjectList.getBoDef().getName() ,"ui", securityRights.READ )){
xattributes=new Hashtable();
xattributes.put("width","100");grid.addColAtr(bodef.getAttributeRef("ui"),100,"","",xattributes);
}if ( ( BOI != null && BOI.getAttribute("alias").hasRights() ) || securityRights.hasRights( DOC.getEboContext() , currObjectList.getBoDef().getName() ,"alias", securityRights.READ )){
xattributes=new Hashtable();
xattributes.put("width","100");grid.addColAtr(bodef.getAttributeRef("alias"),100,xattributes);
}} //endCreategrid
grid.render(pageContext,currObjectList,DOC, DOCLIST);%>

</form>
<%}%>
<FORM name='boFormSubmit' method='post' action='ebo_alias_generallist.jsp'>
