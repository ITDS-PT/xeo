...
if(bosession== null) {
    response.sendRedirect("login.jsp?returnPage=ebo_documenttemplate_generallist.jsp");
    return;
}
if(boctx==null) {
...
<script>var objLabel='Lista de  Template Documento';</script>
...
<form class="objectForm" name="boForm" id="<%=IDX%>">
   ...
   <%
BOI=currObjectList.getObject();grid = DOC.getGRID("g12491144");
if(grid==null) { 
  bodef=boDefHandler.getBoDefinition("Ebo_DocumentTemplate");  grid = DOC.createGRID("g12491144","std");
  grid.setTitle();  
if ( ( BOI != null && BOI.getAttribute("fileName").hasRights() ) || securityRights.hasRights( DOC.getEboContext() , currObjectList.getBoDef().getName() ,"fileName", securityRights.READ )){
xattributes=new Hashtable();
xattributes.put("width","150");grid.addColAtr(bodef.getAttributeRef("fileName"),150,xattributes);
...
xattributes=new Hashtable();
xattributes.put("width","100");grid.addColAtr(bodef.getAttributeRef("object"),100,"","",xattributes);
}} //endCreategrid
grid.render(pageContext,currObjectList,DOC, DOCLIST);%>
   ...
   <%if(DOC.hasCategoryRights("global")){ %>
   <%
BOI=currObjectList.getObject();grid = DOC.getGRID("g22132289");
if(grid==null) { 
  bodef=boDefHandler.getBoDefinition("Ebo_DocumentTemplate");  grid = DOC.createGRID("g22132289","std");
  grid.setTitle();  
...
xattributes=new Hashtable();
xattributes.put("width","100");grid.addColAtr(bodef.getAttributeRef("description"),100,xattributes);
}if ( ( BOI != null && BOI.getAttribute("fileName").hasRights() ) || securityRights.hasRights( DOC.getEboContext() , currObjectList.getBoDef().getName() ,"fileName", securityRights.READ )){
xattributes=new Hashtable();
xattributes.put("width","150");grid.addColAtr(bodef.getAttributeRef("fileName"),150,xattributes);
}} //endCreategrid
...
<%}%>
<FORM name='boFormSubmit' method='post' action='ebo_documenttemplate_generallist.jsp'>
