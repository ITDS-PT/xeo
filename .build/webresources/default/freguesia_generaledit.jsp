...
if(bosession== null) {
    response.sendRedirect("login.jsp?returnPage=freguesia_generaledit.jsp");
    return;
}
if(boctx==null) {
...
if ((BOI==null && securityRights.hasRights(boctx,"freguesia")) || 
(BOI!=null && BOI.exists() && securityRights.hasRights(boctx,"freguesia") && securityOPL.canRead(BOI)) || 
(BOI!=null && !BOI.exists() && securityRights.hasRights(boctx,"freguesia",securityRights.ADD)) 
) {
if (BOI!=null && BOI.exists() && !BOI.isEnabledforRequest &&(!securityRights.hasRights(boctx,"freguesia",securityRights.WRITE) ||  !securityOPL.canWrite(BOI) && !BOI.isEnabledforRequest)) { 
  BOI.setDisabled(); }
                        ...
                        <TR>
                           <TD><%if (DOC.hasCategoryRights("settings")){ %>
                              <DIV id="<%=DOCLIST.cvui%>_tabbody_0" class="tab">
                                 <%{ int cvui27280055=DOCLIST.cvui;%>
                                 <%
BOI=currObjectList.getObject();sec = DOC.getSection("s8129164");
if(sec != null)
  sec.setTitle("");
if(sec==null) { 
  sec = DOC.createSection("s8129164","");
  sec.p_name = "section_settings";
  sec.p_id = "section_settings";
sec.p_showLabel=false;
  bodef=boDefHandler.getBoDefinition("freguesia");  sec.setTitle("");
...
<%}%>
<FORM name='boFormSubmit' method='post' action='freguesia_generaledit.jsp'>
