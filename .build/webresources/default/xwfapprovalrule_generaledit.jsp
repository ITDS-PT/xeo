...
if(bosession== null) {
    response.sendRedirect("login.jsp?returnPage=xwfapprovalrule_generaledit.jsp");
    return;
}
if(boctx==null) {
...
if ((BOI==null && securityRights.hasRights(boctx,"xwfApprovalRule")) || 
(BOI!=null && BOI.exists() && securityRights.hasRights(boctx,"xwfApprovalRule") && securityOPL.canRead(BOI)) || 
(BOI!=null && !BOI.exists() && securityRights.hasRights(boctx,"xwfApprovalRule",securityRights.ADD)) 
) {
if (BOI!=null && BOI.exists() && !BOI.isEnabledforRequest &&(!securityRights.hasRights(boctx,"xwfApprovalRule",securityRights.WRITE) ||  !securityOPL.canWrite(BOI) && !BOI.isEnabledforRequest)) { 
  BOI.setDisabled(); }
                        ...
                        <TR>
                           <TD><%if (DOC.hasCategoryRights("processo")){ %>
                              <DIV id="<%=DOCLIST.cvui%>_tabbody_0" class="tab">
                                 <%{ int cvui32407843=DOCLIST.cvui;%>
                                 <%
BOI=currObjectList.getObject();sec = DOC.getSection("s23078906");
if(sec != null)
  sec.setTitle("Geral");
if(sec==null) { 
  sec = DOC.createSection("s23078906","");
  sec.p_name = "section_id";
  sec.p_id = "section_id";
  bodef=boDefHandler.getBoDefinition("xwfApprovalRule");  sec.setTitle("Geral");
  sec.p_height="60px";
...
boolean canAccessmode=BOI.getAttribute("mode").canAccess();

if (canAccessmode ) { 
row=sec.addRow();
if( canAccessmode) {
xattributes=new Hashtable();
xattributes.put("showlabel","yes");row.addCell(bodef.getAttributeRef("mode"),xattributes);
} else row.addCell();
} 
boolean canAccesspollDefinition=BOI.getAttribute("pollDefinition").canAccess();

if (canAccesspollDefinition ) { 
row=sec.addRow();
if( canAccesspollDefinition) {
xattributes=new Hashtable();
xattributes.put("showlabel","yes");row.addCell(bodef.getAttributeRef("pollDefinition"),xattributes,"","");
} else row.addCell();
} 
} //endCreateSection
sec.render(pageContext,currObjectList,DOCLIST,DOC);%>

                                 <div width="100%" height="100%"><% if(BOI.getAttribute("roles").hasRights()){%> <IFRAME id='inc_xwfApprovalRule__<%=BOI.bo_boui%>__roles' xsrc='<%="icontact_generallist.jsp?docid="+IDX+"&method=list&parent_attribute=roles&parent_boui="+BOI.bo_boui %>' frameBorder=0 width='100%' scrolling=no height='100%' tabindex='<%=DOC.getTabindex(DOC.IFRAME, BOI.getName(), String.valueOf(BOI.bo_boui), "inc_xwfApprovalRule__" + BOI.bo_boui + "__roles", DOCLIST)%>'></IFRAME><% } %> </div>
                                 <%DOCLIST.cvui=cvui32407843;}%>
                              </DIV>
                              <%}%>
                           </TD>
                        </TR>
...
<%}%>
<FORM name='boFormSubmit' method='post' action='xwfapprovalrule_generaledit.jsp'>
