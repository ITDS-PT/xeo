...
if(bosession== null) {
    response.sendRedirect("login.jsp?returnPage=xwfdecision_generallist.jsp");
    return;
}
if(boctx==null) {
...
<form class="objectForm" name="boForm" id="<%=IDX%>">
   ...
   <%
BOI=currObjectList.getObject();grid = DOC.getGRID("g26815403");
if(grid==null) { 
  bodef=boDefHandler.getBoDefinition("xwfDecision");  grid = DOC.createGRID("g26815403","std");
  grid.setTitle();  
...
}if ( ( BOI != null && BOI.getAttribute("answer").hasRights() ) || securityRights.hasRights( DOC.getEboContext() , currObjectList.getBoDef().getName() ,"answer", securityRights.READ )){
xattributes=new Hashtable();
xattributes.put("width","150");grid.addColAtr(bodef.getAttributeRef("answer"),150,xattributes);
}} //endCreategrid
...
<%}%>
<FORM name='boFormSubmit' method='post' action='xwfdecision_generallist.jsp'>
