...
if(bosession== null) {
    response.sendRedirect("login.jsp?returnPage=ebo_document_generaledit.jsp");
    return;
}
if(boctx==null) {
...
if ((BOI==null && securityRights.hasRights(boctx,"Ebo_Document")) || 
(BOI!=null && BOI.exists() && securityRights.hasRights(boctx,"Ebo_Document") && securityOPL.canRead(BOI)) || 
(BOI!=null && !BOI.exists() && securityRights.hasRights(boctx,"Ebo_Document",securityRights.ADD)) 
) {
if (BOI!=null && BOI.exists() && !BOI.isEnabledforRequest &&(!securityRights.hasRights(boctx,"Ebo_Document",securityRights.WRITE) ||  !securityOPL.canWrite(BOI) && !BOI.isEnabledforRequest)) { 
  BOI.setDisabled(); }
         ...
         <TR>
            <TD><%if (DOC.hasCategoryRights("global")){ %>
               <DIV id="<%=DOCLIST.cvui%>_tabbody_0" class="tab" style="overflow-y:auto">
                  <%{ int cvui18041737=DOCLIST.cvui;%>
                  <%
BOI=currObjectList.getObject();sec = DOC.getSection("s29641842");
if(sec != null)
  sec.setTitle(DOC.getCategoryLabel("general","global"));
if(sec==null) { 
  sec = DOC.createSection("s29641842","");
  sec.p_name = "seccao_master_document";
  sec.p_id = "seccao_master_document";
sec.p_showLabel=false;
  bodef=boDefHandler.getBoDefinition("Ebo_Document");  sec.setTitle(DOC.getCategoryLabel("general","global"));
...
sec.render(pageContext,currObjectList,DOCLIST,DOC);%>
                  ...
                  <%
BOI=currObjectList.getObject();sec = DOC.getSection("s7789185");
if(sec != null)
  sec.setTitle("Caracteristicas");
if(sec==null) { 
  sec = DOC.createSection("s7789185","");
  sec.p_name = "phisicalProperties";
  sec.p_id = "phisicalProperties";
  bodef=boDefHandler.getBoDefinition("Ebo_Document");  sec.setTitle("Caracteristicas");
  docHTML_sectionRow row; 
...
<%}%>
<FORM name='boFormSubmit' method='post' action='ebo_document_generaledit.jsp'>
