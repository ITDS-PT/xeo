...
if(bosession== null) {
    response.sendRedirect("login.jsp?returnPage=mailaccount_folder_generallist.jsp");
    return;
}
if(boctx==null) {
...
<form class="objectForm" name="boForm" id="<%=IDX%>">
   ...
   <%
BOI=currObjectList.getObject();grid = DOC.getGRID("g31444608");
if(grid==null) { 
  bodef=boDefHandler.getBoDefinition("mailAccount_folder");  boDefBridge bridgeDef=boDefHandler.getBoDefinition("mailAccount").getAttributeRef("folder").getBridge();  grid = DOC.createGRID("g31444608","std",true);
  grid.setTitle();  
if ( ( BOI != null && BOI.getAttribute("folder").hasRights() ) || securityRights.hasRights( DOC.getEboContext() , currObjectList.getBoDef().getName() ,"folder", securityRights.READ )){
xattributes=new Hashtable();
xattributes.put("width","200");grid.addColAtr(bodef.getAttributeRef("folder"),200,"","",xattributes);
}} //endCreategrid
...
<%}%>
<FORM name='boFormSubmit' method='post' action='mailaccount_folder_generallist.jsp'>
