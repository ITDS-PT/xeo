...
if(bosession== null) {
    response.sendRedirect("login.jsp?returnPage=organization_generalidentification.jsp");
    return;
}
if(boctx==null) {
...
<script>var objLabel='Lista de  Empresa';</script>
...
<form class="objectForm" name="boForm" id="<%=IDX%>">
   ...
   <table bo_node="global">
      <tbody>
         <tr>
            <td><% if(BOI.getAttribute("name").hasRights()){%> <%=DOC.render(currObjectList,BOI.getAttribute("name"),docHTML.RENDER_LABEL)%><% } %></td>
            <td><% if(BOI.getAttribute("name").hasRights()){%> <%=DOC.render(currObjectList,BOI.getAttribute("name"),docHTML.RENDER_FIELD)%><% } %></td>
         </tr>
         <tr>
            <td><% if(BOI.getAttribute("description").hasRights()){%> <%=DOC.render(currObjectList,BOI.getAttribute("description"),docHTML.RENDER_LABEL)%><% } %></td>
            <td><% if(BOI.getAttribute("description").hasRights()){%> <%=DOC.render(currObjectList,BOI.getAttribute("description"),docHTML.RENDER_FIELD)%><% } %></td>
         </tr>
      </tbody>
   </table>
   <%}%>
...
<%}%>
<FORM name='boFormSubmit' method='post' action='organization_generalidentification.jsp'>
