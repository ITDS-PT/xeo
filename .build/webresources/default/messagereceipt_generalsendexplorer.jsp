...
if(bosession== null) {
    response.sendRedirect("login.jsp?returnPage=messagereceipt_generalsendexplorer.jsp");
    return;
}
if(boctx==null) {
...
<form class="objectForm" name="boForm" id="<%=IDX%>">
   <div width="100%" height="100%"><IFRAME    src="__explorer.jsp?docid=<%=DOC.getDocIdx()%>&pageName=<%=pageContext.getPage().getClass().getName()%>&objectName=messageReceipt&form=sendExplorer" id='explorer' frameBorder='0' width='100%' height='100%' scrolling='no' tabindex='0'></IFRAME></div><script> window.gridKey='27020186'</script>  

</form>
<%}%>
<FORM name='boFormSubmit' method='post' action='messagereceipt_generalsendexplorer.jsp'>
