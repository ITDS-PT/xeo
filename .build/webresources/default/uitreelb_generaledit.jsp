...
if(bosession== null) {
    response.sendRedirect("login.jsp?returnPage=uitreelb_generaledit.jsp");
    return;
}
if(boctx==null) {
...
if ((BOI==null && securityRights.hasRights(boctx,"uiTreeLB")) || 
(BOI!=null && BOI.exists() && securityRights.hasRights(boctx,"uiTreeLB") && securityOPL.canRead(BOI)) || 
(BOI!=null && !BOI.exists() && securityRights.hasRights(boctx,"uiTreeLB",securityRights.ADD)) 
) {
if (BOI!=null && BOI.exists() && !BOI.isEnabledforRequest &&(!securityRights.hasRights(boctx,"uiTreeLB",securityRights.WRITE) ||  !securityOPL.canWrite(BOI) && !BOI.isEnabledforRequest)) { 
  BOI.setDisabled(); }
...
<form class="objectForm" name="boForm" id="<%=IDX%>">
   ...
   <%if(DOC.hasCategoryRights("settings")|| DOC.hasCategoryRights("xmlTree")){%>
                  ...
                  <TBODY>
                     <TR>
                        <TD style="padding:0px" id="<%=DOCLIST.cvui%>_tabs" valign="bottom" noWrap="yes">
                           <%if (DOC.hasCategoryRights("settings")){ %><SPAN class="tab tabOn" id="<%=DOCLIST.cvui%>_tabheader_0" name="settings" tabNumber="<%=DOCLIST.cvui%>" tabIndex="<%=DOC.getTabindex(DOC.PANEL, BOI==null?"settings" : BOI.getName(), String.valueOf(BOI==null?0:BOI.bo_boui), String.valueOf(DOCLIST.cvui) + "_tabheader_0", DOCLIST)%>"><%=DOC.getCategoryLabel_for_TAB_Header("settings")%></SPAN><%}%><%if (DOC.hasCategoryRights("xmlTree")){ %><SPAN class="tab" id="<%=DOCLIST.cvui%>_tabheader_1" name="tree_HTML" tabNumber="<%=DOCLIST.cvui%>" tabIndex="<%=DOC.getTabindex(DOC.PANEL, BOI==null?"tree_HTML" : BOI.getName(), String.valueOf(BOI==null?0:BOI.bo_boui), String.valueOf(DOCLIST.cvui) + "_tabheader_1", DOCLIST)%>"><%=DOC.getCategoryLabel_for_TAB_Header("xmlTree")%></SPAN><%}%>
                        </TD>
                     </TR>
                  </TBODY>
               </TABLE>
         ...
         <TR>
            <TD><%if (DOC.hasCategoryRights("settings")){ %>
               <DIV id="<%=DOCLIST.cvui%>_tabbody_0" class="tab">
                  <%{ int cvui14534080=DOCLIST.cvui;%>
                  <%
BOI=currObjectList.getObject();sec = DOC.getSection("s31572670");
if(sec != null)
  sec.setTitle(DOC.getCategoryLabel("general","settings"));
if(sec==null) { 
  sec = DOC.createSection("s31572670","");
  sec.p_name = "";
  sec.p_id = "";
sec.p_showLabel=false;
  bodef=boDefHandler.getBoDefinition("uiTreeLB");  sec.setTitle(DOC.getCategoryLabel("general","settings"));
...
sec.render(pageContext,currObjectList,DOCLIST,DOC);%>
                  ...
                  <%DOCLIST.cvui=cvui14534080;}%>
               </DIV>
               <%}%>
               <%if (DOC.hasCategoryRights("xmlTree")){ %>
               <DIV id="<%=DOCLIST.cvui%>_tabbody_1" class="tab">
                  <%{ int cvui12093308=DOCLIST.cvui;%>
                  <table height="100%" width="100%">
                     <tr>
                        <td>
                           <%
String inc_Tree1246043466025 = "treeBoui=" + (String.valueOf(BOI.getBoui()))+"&designerMode=" + ("edit")+"&docid=" + (IDX)+"&docid=" + ( DOC.getDocIdx() ) ;
%>
<IFRAME  src="__uiTreeWPLC_Designer.jsp?<%=inc_Tree1246043466025%>" xsrc="__uiTreeWPLC_Designer.jsp?<%=inc_Tree1246043466025%>" id='inc_Tree' order='90' name='inc_Tree' frameBorder='0' width='100%' height='100%' scrolling='no' tabindex='<%=DOC.getTabindex(DOC.IFRAME, BOI==null? "inc_Tree" : BOI.getName(), String.valueOf(parent_boui==null?( BOI==null?0 : BOI.getBoui()  ): new Long(parent_boui).longValue()), "inc_Tree", DOCLIST)%>'></IFRAME></td>
                     </tr>
                  </table>
                  <%DOCLIST.cvui=cvui12093308;}%>
               </DIV>
               <%}%>
            </TD>
         </TR>
...
<%}%>
<FORM name='boFormSubmit' method='post' action='uitreelb_generaledit.jsp'>
