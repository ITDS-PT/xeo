...
if(bosession== null) {
    response.sendRedirect("login.jsp?returnPage=messagesystem_generaledit.jsp");
    return;
}
if(boctx==null) {
...
if ((BOI==null && securityRights.hasRights(boctx,"messageSystem")) || 
(BOI!=null && BOI.exists() && securityRights.hasRights(boctx,"messageSystem") && securityOPL.canRead(BOI)) || 
(BOI!=null && !BOI.exists() && securityRights.hasRights(boctx,"messageSystem",securityRights.ADD)) 
) {
if (BOI!=null && BOI.exists() && !BOI.isEnabledforRequest &&(!securityRights.hasRights(boctx,"messageSystem",securityRights.WRITE) ||  !securityOPL.canWrite(BOI) && !BOI.isEnabledforRequest)) { 
  BOI.setDisabled(); }
         ...
         <TR>
            <TD><%if (DOC.hasCategoryRights("settings")){ %>
               <DIV id="<%=DOCLIST.cvui%>_tabbody_0" class="tab" style="overflow-y:auto">
                  <%{ int cvui30143304=DOCLIST.cvui;%>
                  <%
BOI=currObjectList.getObject();sec = DOC.getSection("s13701480");
if(sec != null)
  sec.setTitle("");
if(sec==null) { 
  sec = DOC.createSection("s13701480","");
  sec.p_name = "section_settingsRES";
  sec.p_id = "section_settingsRES";
sec.p_showLabel=false;
  bodef=boDefHandler.getBoDefinition("messageSystem");  sec.setTitle("");
  sec.p_height="520px";
...
sec.render(pageContext,currObjectList,DOCLIST,DOC);%>
                  ...
                  <%DOCLIST.cvui=cvui30143304;}%>
               </DIV>
               <%}%>
               <%if (DOC.hasCategoryRights("documents")){ %>
               <DIV id="<%=DOCLIST.cvui%>_tabbody_1" class="tab" style="overflow-y:auto">
                  <%{ int cvui30263658=DOCLIST.cvui;%>
                  <div width="100%" height="100%"><% if(BOI.getAttribute("documents").hasRights()){%> <IFRAME id='inc_messageSystem__<%=BOI.bo_boui%>__documents' xsrc='<%="ebo_document_generallist.jsp?docid="+IDX+"&method=list&parent_attribute=documents&parent_boui="+BOI.bo_boui %>' frameBorder=0 width='100%' scrolling=no height='100%' tabindex='<%=DOC.getTabindex(DOC.IFRAME, BOI.getName(), String.valueOf(BOI.bo_boui), "inc_messageSystem__" + BOI.bo_boui + "__documents", DOCLIST)%>'></IFRAME><% } %> </div>
                  <%DOCLIST.cvui=cvui30263658;}%>
               </DIV>
               <%}%>
               <%if (DOC.hasCategoryRights("attachedObjects")){ %>
               <DIV id="<%=DOCLIST.cvui%>_tabbody_2" class="tab" style="overflow-y:auto">
                  <%{ int cvui23234692=DOCLIST.cvui;%>
                  <div width="100%" height="100%"><% if(BOI.getAttribute("attachedObjects").hasRights()){%> <IFRAME id='inc_messageSystem__<%=BOI.bo_boui%>__attachedObjects' xsrc='<%="messagesystem_attachedobjects_generallist.jsp?docid="+IDX+"&method=list&parent_attribute=attachedObjects&parent_boui="+BOI.bo_boui %>' frameBorder=0 width='100%' scrolling=no height='100%' tabindex='<%=DOC.getTabindex(DOC.IFRAME, BOI.getName(), String.valueOf(BOI.bo_boui), "inc_messageSystem__" + BOI.bo_boui + "__attachedObjects", DOCLIST)%>'></IFRAME><% } %> </div>
                  <%DOCLIST.cvui=cvui23234692;}%>
               </DIV>
               <%}%>
...
<%}%>
<FORM name='boFormSubmit' method='post' action='messagesystem_generaledit.jsp'>
