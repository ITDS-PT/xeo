...
if(bosession== null) {
    response.sendRedirect("login.jsp?returnPage=ebo_daodepends_generallist.jsp");
    return;
}
if(boctx==null) {
...
<script>var objLabel='Lista de  DAO Config Object Line Depends';</script>
...
<form class="objectForm" name="boForm" id="<%=IDX%>">
   ...
   <%
BOI=currObjectList.getObject();grid = DOC.getGRID("g16414212");
if(grid==null) { 
  bodef=boDefHandler.getBoDefinition("Ebo_DAOdepends");  grid = DOC.createGRID("g16414212","");
  grid.setTitle();  
xattributes=new Hashtable();
xattributes.put("width","");xattributes.put("method","getCardIDByDAOCode");grid.addColSpecial("method",40,xattributes);
if ( ( BOI != null && BOI.getAttribute("DAO_code").hasRights() ) || securityRights.hasRights( DOC.getEboContext() , currObjectList.getBoDef().getName() ,"DAO_code", securityRights.READ )){
xattributes=new Hashtable();
xattributes.put("width","50");grid.addColAtr(bodef.getAttributeRef("DAO_code"),50,xattributes);
...
}if ( ( BOI != null && BOI.getAttribute("lag").hasRights() ) || securityRights.hasRights( DOC.getEboContext() , currObjectList.getBoDef().getName() ,"lag", securityRights.READ )){
xattributes=new Hashtable();
xattributes.put("width","100");grid.addColAtr(bodef.getAttributeRef("lag"),100,xattributes);
}} //endCreategrid
...
<%}%>
<FORM name='boFormSubmit' method='post' action='ebo_daodepends_generallist.jsp'>
