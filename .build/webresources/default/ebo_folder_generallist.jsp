...
if(bosession== null) {
    response.sendRedirect("login.jsp?returnPage=ebo_folder_generallist.jsp");
    return;
}
if(boctx==null) {
...
<script>var objLabel='Lista de  Directório';</script>
...
<form class="objectForm" name="boForm" id="<%=IDX%>">
   ...
   <%
BOI=currObjectList.getObject();grid = DOC.getGRID("g7869649");
if(grid==null) { 
  bodef=boDefHandler.getBoDefinition("Ebo_Folder");  grid = DOC.createGRID("g7869649","std");
  grid.setTitle();  
...
}if ( ( BOI != null && BOI.getAttribute("size").hasRights() ) || securityRights.hasRights( DOC.getEboContext() , currObjectList.getBoDef().getName() ,"size", securityRights.READ )){
xattributes=new Hashtable();
xattributes.put("width","100");grid.addColAtr(bodef.getAttributeRef("size"),100,xattributes);
}} //endCreategrid
...
<%}%>
<FORM name='boFormSubmit' method='post' action='ebo_folder_generallist.jsp'>
