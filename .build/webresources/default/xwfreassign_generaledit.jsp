...
if(bosession== null) {
    response.sendRedirect("login.jsp?returnPage=xwfreassign_generaledit.jsp");
    return;
}
if(boctx==null) {
...
if ((BOI==null && securityRights.hasRights(boctx,"xwfReassign")) || 
(BOI!=null && BOI.exists() && securityRights.hasRights(boctx,"xwfReassign") && securityOPL.canRead(BOI)) || 
(BOI!=null && !BOI.exists() && securityRights.hasRights(boctx,"xwfReassign",securityRights.ADD)) 
) {
if (BOI!=null && BOI.exists() && !BOI.isEnabledforRequest &&(!securityRights.hasRights(boctx,"xwfReassign",securityRights.WRITE) ||  !securityOPL.canWrite(BOI) && !BOI.isEnabledforRequest)) { 
  BOI.setDisabled(); }
...
<form class="objectForm" name="boForm" id="<%=IDX%>">
   <%
BOI=currObjectList.getObject();sec = DOC.getSection("s1474220");
if(sec != null)
  sec.setTitle("");
if(sec==null) { 
  sec = DOC.createSection("s1474220","");
  sec.p_name = "xwfreassign";
  sec.p_id = "xwfreassign";
sec.p_showLabel=false;
  bodef=boDefHandler.getBoDefinition("xwfReassign");  sec.setTitle("");
  sec.p_height="100px";
...
<%}%>
<FORM name='boFormSubmit' method='post' action='xwfreassign_generaledit.jsp'>
