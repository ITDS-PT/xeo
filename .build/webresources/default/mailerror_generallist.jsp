...
if(bosession== null) {
    response.sendRedirect("login.jsp?returnPage=mailerror_generallist.jsp");
    return;
}
if(boctx==null) {
...
<script>var objLabel='Lista de  Email c/ erro';</script>
...
<form class="objectForm" name="boForm" id="<%=IDX%>">
   ...
   <%
BOI=currObjectList.getObject();grid = DOC.getGRID("g29697988");
if(grid==null) { 
  bodef=boDefHandler.getBoDefinition("mailError");  grid = DOC.createGRID("g29697988","std");
  grid.setTitle();  
if ( ( BOI != null && BOI.getAttribute("messageid").hasRights() ) || securityRights.hasRights( DOC.getEboContext() , currObjectList.getBoDef().getName() ,"messageid", securityRights.READ )){
xattributes=new Hashtable();
xattributes.put("width","250");grid.addColAtr(bodef.getAttributeRef("messageid"),250,xattributes);
}if ( ( BOI != null && BOI.getAttribute("subject").hasRights() ) || securityRights.hasRights( DOC.getEboContext() , currObjectList.getBoDef().getName() ,"subject", securityRights.READ )){
xattributes=new Hashtable();
xattributes.put("width","50");grid.addColAtr(bodef.getAttributeRef("subject"),50,xattributes);
}} //endCreategrid
...
<%}%>
<FORM name='boFormSubmit' method='post' action='mailerror_generallist.jsp'>
