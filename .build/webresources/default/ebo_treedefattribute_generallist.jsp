...
if(bosession== null) {
    response.sendRedirect("login.jsp?returnPage=ebo_treedefattribute_generallist.jsp");
    return;
}
if(boctx==null) {
...
<title>
nbo «Ebo_treeDefAttribute»</title>
...
<script>var objLabel='Lista de  Defenição de Atributo de Listagem';</script>
...
<form class="objectForm" name="boForm" id="<%=IDX%>">
   ...
   <%
BOI=currObjectList.getObject();grid = DOC.getGRID("g5930571");
if(grid==null) { 
  bodef=boDefHandler.getBoDefinition("Ebo_treeDefAttribute");  grid = DOC.createGRID("g5930571","std");
  grid.setTitle();  
...
xattributes.put("width","150");grid.addColAtr(bodef.getAttributeRef("name"),150,xattributes);
}if ( ( BOI != null && BOI.getAttribute("width").hasRights() ) || securityRights.hasRights( DOC.getEboContext() , currObjectList.getBoDef().getName() ,"width", securityRights.READ )){
xattributes=new Hashtable();
xattributes.put("width","100");grid.addColAtr(bodef.getAttributeRef("width"),100,xattributes);
}if ( ( BOI != null && BOI.getAttribute("order").hasRights() ) || securityRights.hasRights( DOC.getEboContext() , currObjectList.getBoDef().getName() ,"order", securityRights.READ )){
xattributes=new Hashtable();
xattributes.put("width","50");grid.addColAtr(bodef.getAttributeRef("order"),50,xattributes);
}if ( ( BOI != null && BOI.getAttribute("orderPos").hasRights() ) || securityRights.hasRights( DOC.getEboContext() , currObjectList.getBoDef().getName() ,"orderPos", securityRights.READ )){
xattributes=new Hashtable();
xattributes.put("width","50");grid.addColAtr(bodef.getAttributeRef("orderPos"),50,xattributes);
}} //endCreategrid
...
<%}%>
<FORM name='boFormSubmit' method='post' action='ebo_treedefattribute_generallist.jsp'>
