...
if(bosession== null) {
    response.sendRedirect("login.jsp?returnPage=xwfstarterconfig_generaledit.jsp");
    return;
}
if(boctx==null) {
...
if ((BOI==null && securityRights.hasRights(boctx,"xwfStarterConfig")) || 
(BOI!=null && BOI.exists() && securityRights.hasRights(boctx,"xwfStarterConfig") && securityOPL.canRead(BOI)) || 
(BOI!=null && !BOI.exists() && securityRights.hasRights(boctx,"xwfStarterConfig",securityRights.ADD)) 
) {
if (BOI!=null && BOI.exists() && !BOI.isEnabledforRequest &&(!securityRights.hasRights(boctx,"xwfStarterConfig",securityRights.WRITE) ||  !securityOPL.canWrite(BOI) && !BOI.isEnabledforRequest)) { 
  BOI.setDisabled(); }
                  ...
                  <TBODY>
                     <TR>
                        <TD style="padding:0px" id="<%=DOCLIST.cvui%>_tabs" valign="bottom" noWrap="yes">
                           <%if (DOC.hasCategoryRights("")){ %><SPAN class="tab tabOn" id="<%=DOCLIST.cvui%>_tabheader_0" name="tab_starter" tabNumber="<%=DOCLIST.cvui%>" tabIndex="<%=DOC.getTabindex(DOC.PANEL, BOI==null?"tab_starter" : BOI.getName(), String.valueOf(BOI==null?0:BOI.bo_boui), String.valueOf(DOCLIST.cvui) + "_tabheader_0", DOCLIST)%>">Definição de Arranque</SPAN><%}%>
                        </TD>
                     </TR>
                  </TBODY>
               </TABLE>
         ...
         <TR>
            <TD><%if (DOC.hasCategoryRights("")){ %>
               <DIV id="<%=DOCLIST.cvui%>_tabbody_0" class="tab" style="overflow-y:auto">
                  <%{ int cvui26863821=DOCLIST.cvui;%>
                  <table width="100%" height="100%" style="overflow-y:auto">
                     <tr height="100%">
                        <td>
                           <%
BOI=currObjectList.getObject();sec = DOC.getSection("s25362712");
if(sec != null)
  sec.setTitle("");
if(sec==null) { 
  sec = DOC.createSection("s25362712","");
  sec.p_name = "section_settings";
  sec.p_id = "section_settings";
sec.p_showLabel=false;
  bodef=boDefHandler.getBoDefinition("xwfStarterConfig");  sec.setTitle("");
  sec.p_height="100%";
...
boolean canAccessautomatic=BOI.getAttribute("automatic").canAccess();

if (canAccessautomatic ) { 
row=sec.addRow();
if( canAccessautomatic) {
xattributes=new Hashtable();
xattributes.put("showlabel","yes");row.addCell(bodef.getAttributeRef("automatic"),xattributes);
} else row.addCell();
row.addCell();
} 
} //endCreateSection
sec.render(pageContext,currObjectList,DOCLIST,DOC);%></td>
                     ...
                     <tr height="1%">
                        <td style="font-size:13px;color:#4C73AE;font-weight:bold">Texto explicativo e de ajuda à condição</td>
                     </tr>
                     <tr height="1%">
                        <td style="font-size:13px;color:#4C73AE;font-weight:bold">Objecto</td>
                     </tr>
                     <tr height="20%">
                        <td valign="top">
                           <%
BOI=currObjectList.getObject();sec = DOC.getSection("s29151611");
if(sec != null)
  sec.setTitle("");
if(sec==null) { 
  sec = DOC.createSection("s29151611","");
  sec.p_name = "section_settings";
  sec.p_id = "section_settings";
sec.p_showLabel=false;
  bodef=boDefHandler.getBoDefinition("xwfStarterConfig");  sec.setTitle("");
  docHTML_sectionRow row; 
boolean canAccesscondition=BOI.getAttribute("condition").canAccess();

if (canAccesscondition ) { 
row=sec.addRow();
if( canAccesscondition) {
xattributes=new Hashtable();
xattributes.put("showlabel","yes");xattributes.put("height","200px");row.addCell(bodef.getAttributeRef("condition"),xattributes);
} else row.addCell();
} 
} //endCreateSection
sec.render(pageContext,currObjectList,DOCLIST,DOC);%></td>
...
<%}%>
<FORM name='boFormSubmit' method='post' action='xwfstarterconfig_generaledit.jsp'>
