...
if(bosession== null) {
    response.sendRedirect("login.jsp?returnPage=ebo_treedef_generaledit.jsp");
    return;
}
if(boctx==null) {
...
if ((BOI==null && securityRights.hasRights(boctx,"Ebo_treeDef")) || 
(BOI!=null && BOI.exists() && securityRights.hasRights(boctx,"Ebo_treeDef") && securityOPL.canRead(BOI)) || 
(BOI!=null && !BOI.exists() && securityRights.hasRights(boctx,"Ebo_treeDef",securityRights.ADD)) 
) {
if (BOI!=null && BOI.exists() && !BOI.isEnabledforRequest &&(!securityRights.hasRights(boctx,"Ebo_treeDef",securityRights.WRITE) ||  !securityOPL.canWrite(BOI) && !BOI.isEnabledforRequest)) { 
  BOI.setDisabled(); }
...
<form class="objectForm" name="boForm" id="<%=IDX%>">
   ...
   <%if(DOC.hasCategoryRights("identification")|| DOC.hasCategoryRights("identification")){%>
      ...
      <TBODY>
         <TR>
            <TD class="leftbar"><SPAN class="leftbar" id="<%=DOCLIST.cvui%>" defaultArea="area_<%=DOCLIST.cvui%>_0">
                  ...
                  <%}%>
                  <%if (DOC.hasCategoryRights("identification")){ %>
                  <DIV class="lbItem" id="area_<%=DOCLIST.cvui%>_1" tabIndex="<%=DOC.getTabindex(DOC.AREA, BOI==null?"area_" + String.valueOf(DOCLIST.cvui) + "_1" : BOI.getName(), String.valueOf(BOI==null?0:BOI.bo_boui), "area_" + String.valueOf(DOCLIST.cvui) + "_1", DOCLIST)%>" relatedAreaId="barea_<%=DOCLIST.cvui%>_1" areaNumber="<%=DOCLIST.cvui%>" onclick="so('<%=DOCLIST.cvui%>');loadArea('barea_<%=DOCLIST.cvui%>_1');"><IMG src="resources/area.gif"/>Partilhado Com:</DIV>
                  <%}%>
               </SPAN></TD>
            <TD id="Areas_<%=DOCLIST.cvui%>">
               <%if (DOC.hasCategoryRights("identification")){ %>
               <DIV id="barea_<%=DOCLIST.cvui%>_0" class="areaSEL">
                  <%{ int cvui26138208=DOCLIST.cvui;%>
                  <%DOCLIST.cvui=DOCLIST.getVui(IDX, BOI.getBoui(), "barea_" + DOCLIST.cvui + "_0");%>
                  <%if(DOC.hasCategoryRights("geral")|| DOC.hasCategoryRights("colsDef")|| DOC.hasCategoryRights("groupsDef")){%>
                  <TABLE id="<%=DOCLIST.cvui%>" class="layout" voui="panel" cellspacing="0" cellpadding="0">
                     <TBODY>
                                 ...
                                 <TBODY>
                                    <TR>
                                       <TD style="padding:0px" id="<%=DOCLIST.cvui%>_tabs" valign="bottom" noWrap="yes">
                                          <%if (DOC.hasCategoryRights("geral")){ %><SPAN class="tab tabOn" id="<%=DOCLIST.cvui%>_tabheader_0" tabNumber="<%=DOCLIST.cvui%>" tabIndex="<%=DOC.getTabindex(DOC.PANEL, BOI==null?"null" : BOI.getName(), String.valueOf(BOI==null?0:BOI.bo_boui), String.valueOf(DOCLIST.cvui) + "_tabheader_0", DOCLIST)%>">Dados</SPAN><%}%><%if (DOC.hasCategoryRights("colsDef")){ %><SPAN class="tab" id="<%=DOCLIST.cvui%>_tabheader_1" tabNumber="<%=DOCLIST.cvui%>" tabIndex="<%=DOC.getTabindex(DOC.PANEL, BOI==null?"null" : BOI.getName(), String.valueOf(BOI==null?0:BOI.bo_boui), String.valueOf(DOCLIST.cvui) + "_tabheader_1", DOCLIST)%>">Colunas</SPAN><%}%><%if (DOC.hasCategoryRights("groupsDef")){ %><SPAN class="tab" id="<%=DOCLIST.cvui%>_tabheader_2" tabNumber="<%=DOCLIST.cvui%>" tabIndex="<%=DOC.getTabindex(DOC.PANEL, BOI==null?"null" : BOI.getName(), String.valueOf(BOI==null?0:BOI.bo_boui), String.valueOf(DOCLIST.cvui) + "_tabheader_2", DOCLIST)%>">Grupos</SPAN><%}%>
                                       </TD>
                                    </TR>
                                 </TBODY>
                              </TABLE>
                        ...
                        <TR>
                           <TD><%if (DOC.hasCategoryRights("geral")){ %>
                              <DIV id="<%=DOCLIST.cvui%>_tabbody_0" class="tab">
                                 <%{ int cvui10247245=DOCLIST.cvui;%>
                                 <%
BOI=currObjectList.getObject();sec = DOC.getSection("s24106498");
if(sec != null)
  sec.setTitle("Geral");
if(sec==null) { 
  sec = DOC.createSection("s24106498","");
  sec.p_name = "";
  sec.p_id = "";
sec.p_showLabel=false;
  bodef=boDefHandler.getBoDefinition("Ebo_treeDef");  sec.setTitle("Geral");
  sec.p_height="100%";
  sec.p_width="100%";
...
boolean canAccessviewer=BOI.getAttribute("viewer").canAccess();

if (canAccessviewer ) { 
row=sec.addRow();
if( canAccessviewer) {
xattributes=new Hashtable();
xattributes.put("showlabel","yes");row.addCell(bodef.getAttributeRef("viewer"),xattributes);
} else row.addCell();
} 
boolean canAccessexpName=BOI.getAttribute("expName").canAccess();

if (canAccessexpName ) { 
row=sec.addRow();
if( canAccessexpName) {
xattributes=new Hashtable();
xattributes.put("showlabel","yes");row.addCell(bodef.getAttributeRef("expName"),xattributes);
} else row.addCell();
} 
boolean canAccessobjectType=BOI.getAttribute("objectType").canAccess();

if (canAccessobjectType ) { 
row=sec.addRow();
if( canAccessobjectType) {
xattributes=new Hashtable();
xattributes.put("showlabel","yes");row.addCell(bodef.getAttributeRef("objectType"),xattributes);
} else row.addCell();
} 
boolean canAccessfilter=BOI.getAttribute("filter").canAccess();

if (canAccessfilter ) { 
row=sec.addRow();
if( canAccessfilter) {
xattributes=new Hashtable();
xattributes.put("showlabel","yes");row.addCell(bodef.getAttributeRef("filter"),xattributes);
} else row.addCell();
} 
boolean canAccessuserQuery=BOI.getAttribute("userQuery").canAccess();

if (canAccessuserQuery ) { 
row=sec.addRow();
if( canAccessuserQuery) {
xattributes=new Hashtable();
xattributes.put("showlabel","yes");xattributes.put("width","100%");xattributes.put("height","100%");row.addCell(bodef.getAttributeRef("userQuery"),xattributes);
} else row.addCell();
} 
} //endCreateSection
sec.render(pageContext,currObjectList,DOCLIST,DOC);%>

                                 <%DOCLIST.cvui=cvui10247245;}%>
                              </DIV>
                              <%}%>
                              <%if (DOC.hasCategoryRights("colsDef")){ %>
                              <DIV id="<%=DOCLIST.cvui%>_tabbody_1" class="tab">
                                 <%{ int cvui21702732=DOCLIST.cvui;%>
                                 <div><% if(BOI.getAttribute("cols").hasRights()){%> <IFRAME id='inc_Ebo_treeDef__<%=BOI.bo_boui%>__cols' xsrc='<%="ebo_treedefattribute_generallist.jsp?docid="+IDX+"&method=list&parent_attribute=cols&parent_boui="+BOI.bo_boui %>' frameBorder=0 width='100%' scrolling=no height='100%' tabindex='<%=DOC.getTabindex(DOC.IFRAME, BOI.getName(), String.valueOf(BOI.bo_boui), "inc_Ebo_treeDef__" + BOI.bo_boui + "__cols", DOCLIST)%>'></IFRAME><% } %> </div>
                                 <%DOCLIST.cvui=cvui21702732;}%>
                              </DIV>
                              <%}%>
                              <%if (DOC.hasCategoryRights("groupsDef")){ %>
                              <DIV id="<%=DOCLIST.cvui%>_tabbody_2" class="tab">
                                 <%{ int cvui21153659=DOCLIST.cvui;%>
                                 <div><% if(BOI.getAttribute("groups").hasRights()){%> <IFRAME id='inc_Ebo_treeDef__<%=BOI.bo_boui%>__groups' xsrc='<%="ebo_treedefattribute_generallist.jsp?docid="+IDX+"&method=list&parent_attribute=groups&parent_boui="+BOI.bo_boui %>' frameBorder=0 width='100%' scrolling=no height='100%' tabindex='<%=DOC.getTabindex(DOC.IFRAME, BOI.getName(), String.valueOf(BOI.bo_boui), "inc_Ebo_treeDef__" + BOI.bo_boui + "__groups", DOCLIST)%>'></IFRAME><% } %> </div>
                                 <%DOCLIST.cvui=cvui21153659;}%>
                              </DIV>
                              <%}%>
                           </TD>
                        </TR>
                  ...
                  <TABLE id="<%=DOCLIST.cvui%>" class="layout" voui="panel" cellspacing="0" cellpadding="0">
                     <TBODY>
                           ...
                           <TD>
                              <TABLE cellpadding="0" cellspacing="0" class="tabBar" id="<%=DOCLIST.cvui%>_body" onkeyup="so('<%=DOCLIST.cvui%>');onKeyUpTab_std(event)" onmouseover="so('<%=DOCLIST.cvui%>');onOverTab_std(event)" onmouseout="so('<%=DOCLIST.cvui%>');onOutTab_std(event)" ondragenter="so('<%=DOCLIST.cvui%>');ondragenterTab_std(event)" ondragover="so('<%=DOCLIST.cvui%>');ondragoverTab_std(event)" onclick="so('<%=DOCLIST.cvui%>');onClickTab_std(event)">
                                 <TBODY>
                                    <TR>
                                       <TD style="padding:0px" id="<%=DOCLIST.cvui%>_tabs" valign="bottom" noWrap="yes">
                                          <%if (DOC.hasCategoryRights("geral")){ %><SPAN class="tab tabOn" id="<%=DOCLIST.cvui%>_tabheader_0" tabNumber="<%=DOCLIST.cvui%>" tabIndex="<%=DOC.getTabindex(DOC.PANEL, BOI==null?"null" : BOI.getName(), String.valueOf(BOI==null?0:BOI.bo_boui), String.valueOf(DOCLIST.cvui) + "_tabheader_0", DOCLIST)%>">Partilha</SPAN><%}%>
                                       </TD>
                                    </TR>
                                 </TBODY>
                              </TABLE>
                        ...
                        <TR>
                           <TD><%if (DOC.hasCategoryRights("geral")){ %>
                              <DIV id="<%=DOCLIST.cvui%>_tabbody_0" class="tab">
                                 <%{ int cvui27276155=DOCLIST.cvui;%>
                                 <div><% if(BOI.getAttribute("share").hasRights()){%> <IFRAME id='inc_Ebo_treeDef__<%=BOI.bo_boui%>__share' xsrc='<%="ebo_treedef_share_generallist.jsp?docid="+IDX+"&method=list&parent_attribute=share&parent_boui="+BOI.bo_boui %>' frameBorder=0 width='100%' scrolling=no height='100%' tabindex='<%=DOC.getTabindex(DOC.IFRAME, BOI.getName(), String.valueOf(BOI.bo_boui), "inc_Ebo_treeDef__" + BOI.bo_boui + "__share", DOCLIST)%>'></IFRAME><% } %> </div>
                                 <%DOCLIST.cvui=cvui27276155;}%>
                              </DIV>
                              <%}%>
                           </TD>
                        </TR>
...
<%}%>
<FORM name='boFormSubmit' method='post' action='ebo_treedef_generaledit.jsp'>
