...
if(bosession== null) {
    response.sendRedirect("login.jsp?returnPage=ebo_template_generaledit.jsp");
    return;
}
if(boctx==null) {
...
if ((BOI==null && securityRights.hasRights(boctx,"Ebo_Template")) || 
(BOI!=null && BOI.exists() && securityRights.hasRights(boctx,"Ebo_Template") && securityOPL.canRead(BOI)) || 
(BOI!=null && !BOI.exists() && securityRights.hasRights(boctx,"Ebo_Template",securityRights.ADD)) 
) {
if (BOI!=null && BOI.exists() && !BOI.isEnabledforRequest &&(!securityRights.hasRights(boctx,"Ebo_Template",securityRights.WRITE) ||  !securityOPL.canWrite(BOI) && !BOI.isEnabledforRequest)) { 
  BOI.setDisabled(); }
...
<form class="objectForm" name="boForm" id="<%=IDX%>">
   ...
   <%if(DOC.hasCategoryRights("identification")|| DOC.hasCategoryRights("mappingAttributes")|| DOC.hasCategoryRights("share")|| DOC.hasCategoryRights("templateOwners")|| DOC.hasCategoryRights("advanced")){%>
                  ...
                  <TBODY>
                     <TR>
                        <TD style="padding:0px" id="<%=DOCLIST.cvui%>_tabs" valign="bottom" noWrap="yes">
                           <%if (DOC.hasCategoryRights("identification")){ %><SPAN class="tab tabOn" id="<%=DOCLIST.cvui%>_tabheader_0" tabNumber="<%=DOCLIST.cvui%>" tabIndex="<%=DOC.getTabindex(DOC.PANEL, BOI==null?"null" : BOI.getName(), String.valueOf(BOI==null?0:BOI.bo_boui), String.valueOf(DOCLIST.cvui) + "_tabheader_0", DOCLIST)%>"><%=DOC.getCategoryLabel_for_TAB_Header("identification")%></SPAN><%}%><%if (DOC.hasCategoryRights("mappingAttributes")){ %><SPAN class="tab" id="<%=DOCLIST.cvui%>_tabheader_1" tabNumber="<%=DOCLIST.cvui%>" tabIndex="<%=DOC.getTabindex(DOC.PANEL, BOI==null?"null" : BOI.getName(), String.valueOf(BOI==null?0:BOI.bo_boui), String.valueOf(DOCLIST.cvui) + "_tabheader_1", DOCLIST)%>"><%=DOC.getCategoryLabel_for_TAB_Header("mappingAttributes")%></SPAN><%}%><%if (DOC.hasCategoryRights("share")){ %><SPAN class="tab" id="<%=DOCLIST.cvui%>_tabheader_2" tabNumber="<%=DOCLIST.cvui%>" tabIndex="<%=DOC.getTabindex(DOC.PANEL, BOI==null?"null" : BOI.getName(), String.valueOf(BOI==null?0:BOI.bo_boui), String.valueOf(DOCLIST.cvui) + "_tabheader_2", DOCLIST)%>"><%=DOC.getCategoryLabel_for_TAB_Header("share")%></SPAN><%}%><%if (DOC.hasCategoryRights("templateOwners")){ %><SPAN class="tab" id="<%=DOCLIST.cvui%>_tabheader_3" tabNumber="<%=DOCLIST.cvui%>" tabIndex="<%=DOC.getTabindex(DOC.PANEL, BOI==null?"null" : BOI.getName(), String.valueOf(BOI==null?0:BOI.bo_boui), String.valueOf(DOCLIST.cvui) + "_tabheader_3", DOCLIST)%>"><%=DOC.getCategoryLabel_for_TAB_Header("templateOwners")%></SPAN><%}%><%if (DOC.hasCategoryRights("advanced")){ %><SPAN class="tab" id="<%=DOCLIST.cvui%>_tabheader_4" tabNumber="<%=DOCLIST.cvui%>" tabIndex="<%=DOC.getTabindex(DOC.PANEL, BOI==null?"null" : BOI.getName(), String.valueOf(BOI==null?0:BOI.bo_boui), String.valueOf(DOCLIST.cvui) + "_tabheader_4", DOCLIST)%>"><%=DOC.getCategoryLabel_for_TAB_Header("advanced")%></SPAN><%}%>
                        </TD>
                     </TR>
                  </TBODY>
               </TABLE>
         ...
         <TR>
            <TD><%if (DOC.hasCategoryRights("identification")){ %>
               <DIV id="<%=DOCLIST.cvui%>_tabbody_0" class="tab">
                  <%{ int cvui20658550=DOCLIST.cvui;%>
                  <%
BOI=currObjectList.getObject();sec = DOC.getSection("s20552612");
if(sec != null)
  sec.setTitle(DOC.getCategoryLabel("general","identification"));
if(sec==null) { 
  sec = DOC.createSection("s20552612","");
  sec.p_name = "";
  sec.p_id = "";
...
boolean canAccesskeyWords=BOI.getAttribute("keyWords").canAccess();

if (canAccesskeyWords ) { 
row=sec.addRow();
if( canAccesskeyWords) {
xattributes=new Hashtable();
xattributes.put("showlabel","yes");row.addCell(bodef.getAttributeRef("keyWords"),xattributes);
} else row.addCell();
} 
} //endCreateSection
sec.render(pageContext,currObjectList,DOCLIST,DOC);%>

                  <%DOCLIST.cvui=cvui20658550;}%>
               </DIV>
               <%}%>
               <%if (DOC.hasCategoryRights("mappingAttributes")){ %>
               <DIV id="<%=DOCLIST.cvui%>_tabbody_1" class="tab">
                  <%{ int cvui18427824=DOCLIST.cvui;%>
                  ...
                  <%DOCLIST.cvui=cvui18427824;}%>
               </DIV>
               <%}%>
               <%if (DOC.hasCategoryRights("share")){ %>
               <DIV id="<%=DOCLIST.cvui%>_tabbody_2" class="tab">
                  <%{ int cvui9042044=DOCLIST.cvui;%>
                  <div width="100%" height="100%"><% if(BOI.getAttribute("share").hasRights()){%> <IFRAME id='inc_Ebo_Template__<%=BOI.bo_boui%>__share' xsrc='<%="ebo_template_share_generallist.jsp?docid="+IDX+"&method=list&parent_attribute=share&parent_boui="+BOI.bo_boui %>' frameBorder=0 width='100%' scrolling=no height='100%' tabindex='<%=DOC.getTabindex(DOC.IFRAME, BOI.getName(), String.valueOf(BOI.bo_boui), "inc_Ebo_Template__" + BOI.bo_boui + "__share", DOCLIST)%>'></IFRAME><% } %> </div>
                  <%DOCLIST.cvui=cvui9042044;}%>
               </DIV>
               <%}%>
               <%if (DOC.hasCategoryRights("templateOwners")){ %>
               <DIV id="<%=DOCLIST.cvui%>_tabbody_3" class="tab">
                  <%{ int cvui1618141=DOCLIST.cvui;%>
                  <div width="100%" height="100%"><% if(BOI.getAttribute("templateOwners").hasRights()){%> <IFRAME id='inc_Ebo_Template__<%=BOI.bo_boui%>__templateOwners' xsrc='<%="ebo_template_templateowners_generallist.jsp?docid="+IDX+"&method=list&parent_attribute=templateOwners&parent_boui="+BOI.bo_boui %>' frameBorder=0 width='100%' scrolling=no height='100%' tabindex='<%=DOC.getTabindex(DOC.IFRAME, BOI.getName(), String.valueOf(BOI.bo_boui), "inc_Ebo_Template__" + BOI.bo_boui + "__templateOwners", DOCLIST)%>'></IFRAME><% } %> </div>
                  <%DOCLIST.cvui=cvui1618141;}%>
               </DIV>
               <%}%>
               <%if (DOC.hasCategoryRights("advanced")){ %>
               <DIV id="<%=DOCLIST.cvui%>_tabbody_4" class="tab">
                  <%{ int cvui17009556=DOCLIST.cvui;%>
                  <%
BOI=currObjectList.getObject();sec = DOC.getSection("s29404768");
if(sec != null)
  sec.setTitle(DOC.getCategoryLabel("general","advanced"));
if(sec==null) { 
  sec = DOC.createSection("s29404768","");
  sec.p_name = "";
  sec.p_id = "";
sec.p_showLabel=false;
  bodef=boDefHandler.getBoDefinition("Ebo_Template");  sec.setTitle(DOC.getCategoryLabel("general","advanced"));
  docHTML_sectionRow row; 
boolean canAccesswhendApply=BOI.getAttribute("whendApply").canAccess();

if (canAccesswhendApply ) { 
row=sec.addRow();
if( canAccesswhendApply) {
xattributes=new Hashtable();
xattributes.put("showlabel","yes");row.addCell(bodef.getAttributeRef("whendApply"),xattributes);
} else row.addCell();
} 
boolean canAccesswhendCreate=BOI.getAttribute("whendCreate").canAccess();

if (canAccesswhendCreate ) { 
row=sec.addRow();
if( canAccesswhendCreate) {
xattributes=new Hashtable();
xattributes.put("showlabel","yes");row.addCell(bodef.getAttributeRef("whendCreate"),xattributes);
} else row.addCell();
} 
boolean canAccesswhendUpdate=BOI.getAttribute("whendUpdate").canAccess();

if (canAccesswhendUpdate ) { 
row=sec.addRow();
if( canAccesswhendUpdate) {
xattributes=new Hashtable();
xattributes.put("showlabel","yes");row.addCell(bodef.getAttributeRef("whendUpdate"),xattributes);
} else row.addCell();
} 
boolean canAccesswhendDelete=BOI.getAttribute("whendDelete").canAccess();

if (canAccesswhendDelete ) { 
row=sec.addRow();
if( canAccesswhendDelete) {
xattributes=new Hashtable();
xattributes.put("showlabel","yes");row.addCell(bodef.getAttributeRef("whendDelete"),xattributes);
} else row.addCell();
} 
} //endCreateSection
sec.render(pageContext,currObjectList,DOCLIST,DOC);%>

                  <%DOCLIST.cvui=cvui17009556;}%>
               </DIV>
               <%}%>
            </TD>
         </TR>
...
<%}%>
<FORM name='boFormSubmit' method='post' action='ebo_template_generaledit.jsp'>
