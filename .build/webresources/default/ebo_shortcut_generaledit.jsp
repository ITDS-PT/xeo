...
if(bosession== null) {
    response.sendRedirect("login.jsp?returnPage=ebo_shortcut_generaledit.jsp");
    return;
}
if(boctx==null) {
...
if ((BOI==null && securityRights.hasRights(boctx,"Ebo_ShortCut")) || 
(BOI!=null && BOI.exists() && securityRights.hasRights(boctx,"Ebo_ShortCut") && securityOPL.canRead(BOI)) || 
(BOI!=null && !BOI.exists() && securityRights.hasRights(boctx,"Ebo_ShortCut",securityRights.ADD)) 
) {
if (BOI!=null && BOI.exists() && !BOI.isEnabledforRequest &&(!securityRights.hasRights(boctx,"Ebo_ShortCut",securityRights.WRITE) ||  !securityOPL.canWrite(BOI) && !BOI.isEnabledforRequest)) { 
  BOI.setDisabled(); }
         ...
         <TR>
            <TD><%if (DOC.hasCategoryRights("identification")){ %>
               <DIV id="<%=DOCLIST.cvui%>_tabbody_0" class="tab">
                  <%{ int cvui26092677=DOCLIST.cvui;%>
                  <%
BOI=currObjectList.getObject();sec = DOC.getSection("s16765966");
if(sec != null)
  sec.setTitle(DOC.getCategoryLabel("general","identification"));
if(sec==null) { 
  sec = DOC.createSection("s16765966","");
  sec.p_name = "";
  sec.p_id = "";
sec.p_showLabel=false;
  bodef=boDefHandler.getBoDefinition("Ebo_ShortCut");  sec.setTitle(DOC.getCategoryLabel("general","identification"));
  sec.p_height="100%";
...
boolean canAccessaction=BOI.getAttribute("action").canAccess();

if (canAccessaction ) { 
row=sec.addRow();
if( canAccessaction) {
xattributes=new Hashtable();
xattributes.put("showlabel","yes");xattributes.put("height","100%");row.addCell(bodef.getAttributeRef("action"),xattributes);
} else row.addCell();
} 
} //endCreateSection
sec.render(pageContext,currObjectList,DOCLIST,DOC);%>

                  <%DOCLIST.cvui=cvui26092677;}%>
               </DIV>
               <%}%>
            </TD>
         </TR>
...
<%}%>
<FORM name='boFormSubmit' method='post' action='ebo_shortcut_generaledit.jsp'>
