...
if(bosession== null) {
    response.sendRedirect("login.jsp?returnPage=horario_dia_generaledit.jsp");
    return;
}
if(boctx==null) {
...
if ((BOI==null && securityRights.hasRights(boctx,"horario_dia")) || 
(BOI!=null && BOI.exists() && securityRights.hasRights(boctx,"horario_dia") && securityOPL.canRead(BOI)) || 
(BOI!=null && !BOI.exists() && securityRights.hasRights(boctx,"horario_dia",securityRights.ADD)) 
) {
if (BOI!=null && BOI.exists() && !BOI.isEnabledforRequest &&(!securityRights.hasRights(boctx,"horario_dia",securityRights.WRITE) ||  !securityOPL.canWrite(BOI) && !BOI.isEnabledforRequest)) { 
  BOI.setDisabled(); }
         ...
         <TR>
            <TD><%if (DOC.hasCategoryRights("identification")){ %>
               <DIV id="<%=DOCLIST.cvui%>_tabbody_0" class="tab">
                  <%{ int cvui4603674=DOCLIST.cvui;%>
                  <table>
                     <tr height="1%">
                        <td>
                           <%
BOI=currObjectList.getObject();sec = DOC.getSection("s30995274");
if(sec != null)
  sec.setTitle("");
if(sec==null) { 
  sec = DOC.createSection("s30995274","");
  sec.p_name = "";
  sec.p_id = "";
sec.p_showLabel=false;
  bodef=boDefHandler.getBoDefinition("horario_dia");  sec.setTitle("");
...
sec.render(pageContext,currObjectList,DOCLIST,DOC);%></td>
                     ...
                     <tr height="99%">
                        <td>
                           <div height="100%" width="100%"><% if(BOI.getAttribute("periodos").hasRights()){%> <IFRAME id='inc_horario_dia__<%=BOI.bo_boui%>__periodos' xsrc='<%="horario_dia_periodo_generallist.jsp?docid="+IDX+"&method=list&parent_attribute=periodos&parent_boui="+BOI.bo_boui %>' frameBorder=0 width='100%' scrolling=no height='100%' tabindex='<%=DOC.getTabindex(DOC.IFRAME, BOI.getName(), String.valueOf(BOI.bo_boui), "inc_horario_dia__" + BOI.bo_boui + "__periodos", DOCLIST)%>'></IFRAME><% } %> </div></td>
                     </tr>
                  </table>
                  <%DOCLIST.cvui=cvui4603674;}%>
               </DIV>
               <%}%>
            </TD>
         </TR>
...
<%}%>
<FORM name='boFormSubmit' method='post' action='horario_dia_generaledit.jsp'>
