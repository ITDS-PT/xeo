...
if(bosession== null) {
    response.sendRedirect("login.jsp?returnPage=uiprofile_generaledit.jsp");
    return;
}
if(boctx==null) {
...
if ((BOI==null && securityRights.hasRights(boctx,"uiProfile")) || 
(BOI!=null && BOI.exists() && securityRights.hasRights(boctx,"uiProfile") && securityOPL.canRead(BOI)) || 
(BOI!=null && !BOI.exists() && securityRights.hasRights(boctx,"uiProfile",securityRights.ADD)) 
) {
if (BOI!=null && BOI.exists() && !BOI.isEnabledforRequest &&(!securityRights.hasRights(boctx,"uiProfile",securityRights.WRITE) ||  !securityOPL.canWrite(BOI) && !BOI.isEnabledforRequest)) { 
  BOI.setDisabled(); }
         ...
         <TR>
            <TD><%if (DOC.hasCategoryRights("settings")){ %>
               <DIV id="<%=DOCLIST.cvui%>_tabbody_0" class="tab">
                  <%{ int cvui14370290=DOCLIST.cvui;%>
                  <%
BOI=currObjectList.getObject();sec = DOC.getSection("s30228855");
if(sec != null)
  sec.setTitle(DOC.getCategoryLabel("general","settings"));
if(sec==null) { 
  sec = DOC.createSection("s30228855","");
  sec.p_name = "";
  sec.p_id = "";
sec.p_showLabel=false;
  bodef=boDefHandler.getBoDefinition("uiProfile");  sec.setTitle(DOC.getCategoryLabel("general","settings"));
...
<%}%>
<FORM name='boFormSubmit' method='post' action='uiprofile_generaledit.jsp'>
