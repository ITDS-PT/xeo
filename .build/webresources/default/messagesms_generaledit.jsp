...
if(bosession== null) {
    response.sendRedirect("login.jsp?returnPage=messagesms_generaledit.jsp");
    return;
}
if(boctx==null) {
...
if ((BOI==null && securityRights.hasRights(boctx,"messageSMS")) || 
(BOI!=null && BOI.exists() && securityRights.hasRights(boctx,"messageSMS") && securityOPL.canRead(BOI)) || 
(BOI!=null && !BOI.exists() && securityRights.hasRights(boctx,"messageSMS",securityRights.ADD)) 
) {
if (BOI!=null && BOI.exists() && !BOI.isEnabledforRequest &&(!securityRights.hasRights(boctx,"messageSMS",securityRights.WRITE) ||  !securityOPL.canWrite(BOI) && !BOI.isEnabledforRequest)) { 
  BOI.setDisabled(); }
         ...
         <TR>
            <TD><%if (DOC.hasCategoryRights("settings")){ %>
               <DIV id="<%=DOCLIST.cvui%>_tabbody_0" class="tab" style="overflow-y:auto">
                  <%{ int cvui367605=DOCLIST.cvui;%>
                  <%
BOI=currObjectList.getObject();sec = DOC.getSection("s4333304");
if(sec != null)
  sec.setTitle("");
if(sec==null) { 
  sec = DOC.createSection("s4333304","");
...
boolean canAccesstextSMS=BOI.getAttribute("textSMS").canAccess();

if (canAccesstextSMS ) { 
row=sec.addRow();
if( canAccesstextSMS) {
xattributes=new Hashtable();
xattributes.put("showlabel","no");xattributes.put("height","100%");row.addCell(bodef.getAttributeRef("textSMS"),xattributes);
} else row.addCell();
} 
...
sec.render(pageContext,currObjectList,DOCLIST,DOC);%>
                  ...
                  <%DOCLIST.cvui=cvui367605;}%>
               </DIV>
               <%}%>
               <%if (DOC.hasCategoryRights("settingsRES")){ %>
               <DIV id="<%=DOCLIST.cvui%>_tabbody_1" class="tab" style="overflow-y:auto">
                  <%{ int cvui16734476=DOCLIST.cvui;%>
                  <%
BOI=currObjectList.getObject();sec = DOC.getSection("s3101319");
if(sec != null)
  sec.setTitle("");
if(sec==null) { 
  sec = DOC.createSection("s3101319","");
  sec.p_name = "section_settingsRES";
  sec.p_id = "section_settingsRES";
sec.p_showLabel=false;
  bodef=boDefHandler.getBoDefinition("messageSMS");  sec.setTitle("");
...
sec.render(pageContext,currObjectList,DOCLIST,DOC);%>
                  ...
                  <%DOCLIST.cvui=cvui16734476;}%>
               </DIV>
               <%}%>
               <%if (DOC.hasCategoryRights("options")){ %>
               <DIV id="<%=DOCLIST.cvui%>_tabbody_2" class="tab" style="overflow-y:auto">
                  <%{ int cvui13835561=DOCLIST.cvui;%>
                  <table>
                     <tr height="60px">
                        <td>
                           <%
BOI=currObjectList.getObject();sec = DOC.getSection("s2064539");
if(sec != null)
  sec.setTitle("");
if(sec==null) { 
  sec = DOC.createSection("s2064539","");
...
<%}%>
<FORM name='boFormSubmit' method='post' action='messagesms_generaledit.jsp'>
