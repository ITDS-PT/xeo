...
var objLabel='<img align="absmiddle" hspace=3 src="templates/form/std/search16.gif">Procura Avançada de Objectos';var objDescription='';
var objectName="<%=objectName%>";
var filterIsSaved = false;
var _attributesXml ="<%=attributesStr%>"
	//			"<attributes>"+
	//			"<name type=\"char\"><label>Assunto</label></name>"+
	//			"<description  type=\"char\"><label>Descri袯</label></description>"+
	//			"<priority type=\"number\"><label>Prioridade</label><lov><item><description>Normal</description><value>1</value></item><item><description>Alta</description><value>2</value></item></lov></priority>"+
	//			"<assignedQueue type=\"object.boObject\" relation=\"1\" ><label>Assignado a</label><objects>Ebo_perf;workQueue;Ebo_Role;Ebo_Group</objects></assignedQueue>"+
	//			"<process type=\"object.process\" relation=\"1\"><label>Processo</label></process>"+
	//			"<process.name type=\"char\" ><label>Processo.Nome</label></process.name>"+
	//			"<numero type=\"number\" decimals=\"2\" ><label>Numero</label></numero>"+
	//			"<data type=\"date\" ><label>Data testes</label></data>"+
	//			"<datahora type=\"datetime\" ><label>Data Hora</label></datahora>"+
	//			"<workHistory type=\"object.duration\" relation=\"N\"><label>Histórico do trabalho</label></workHistory>"+
	//			"</attributes>";
<%
if ( oFilter != null ){    
  ...
  <tr>
        ...
        <tr>
          ...
          <td class="icMenu" noWrap>
            <span class="menuFlat" title="Guardar" onclick=oQB.save('<%= objectName %>');  tabIndex="0"><img class="mnuBtn" src="templates/menu/std/16_save.gif" WIDTH="16" HEIGHT="16">Guardar Filtro</span>
			...
			<span class="menuFlat" title="Aplicar filtro com as linhas seleccionadas" onclick="oQB.apply();" tabIndex="0"><img class="mnuBtn" src="templates/menu/std/16_filter.gif" WIDTH="16" HEIGHT="16">Aplicar Filtro</span>
            <span class="menuFlat" title="Remover filtro" onclick="oQB.apply('remove');" tabIndex="0"><img class="mnuBtn" src="templates/menu/std/16_removefilter.gif" WIDTH="16" HEIGHT="16">Remover Filtro</span>
		  </td>
		</tr></tbody></table>
  ...
  <tr>
    <td>
            <table style="align:absmiddle;width:100%;">
                <tr>
                <td colspan=3 align='left' style='color:#0000FF;font-size:10px'><i>NOTA :Antes de aplicar seleccione as linhas que deseja considerar no filtro</i></td>
                </tr>
		    </table>
	...
	<td valign=top height='20px'>
		...
		<table style='width:100%'>
			<tr>
                ...
                <span><b>Procurar em todos os objectos</b></span>&nbsp;<input class='rad' onclick='changeExclude();' <%=onlyObjects.length()==0?"checked":""%>  id='selectedAllObject' type=checkbox />
                <span id="sayExclude" style='display:<%=onlyObjects.length()==0?"none":""%>'><font color="red">Seleccione os objectos que deseja procurar</font></span>
				</td>
			</tr>
		<tr><td width='100%'>
					    ...
					    <td>
							<table id="g1642_body" onmouseover="so('g1642');onOver_GridHeader_std(event);" onmouseout="so('g1642');onOut_GridHeader_std(event);" onclick="so('g1642');onClick_GridHeader_std(event);" cellpading="2" cellspacing="0" style="height:25px" class="gh_std">
								...
								<tbody>
									...
									<td class="ghSep_std">&nbsp;</td>
									<td class="gh_std">Condição </td>
									<td class="ghSep_std">&nbsp;</td>
									<td class="gh_std">Valor</td>
									<td class="ghSep_std">&nbsp;</td>
									<td class="gh_std"></td>
                                    <td class="ghSep_std">&nbsp;</td>
									<td class="gh_std"></td>
	...
	<tr>
		<td style="" height='10px'>
		    <table style="border:0px solid #CCCCCC;align:absmiddle;width:100%;">
				<tr>
					<td><button onclick='oQB.newQuery()'>Acrescentar Linha</button></td>
					<td><button onclick='oQB.removeSelected()'>Limpar linhas seleccionadas </button></td>
...
<TBODY>
  ...
  <TR>
    ...
    <TD style="BORDER-TOP: #ffffff 1px solid" align=right><BUTTON id=butBegin 
      onclick=DLG_applychanges();>OK_</BUTTON>&nbsp;<BUTTON id=cmdDialogCancel 
...
<TBODY>
  ...
  <TR>
    <TD class=main colSpan=2>
      ...
      <TABLE id=tblQuestion height="100%" cellSpacing=0 cellPadding=0 
        width="100%"><TBODY>
        <TR height=20>
          <TD>Etiqueta/Pergunta a efectuar:</TD>
          <TD></TD>
        </TR>
        <TR>
          <TD width="100%">
        ...
        <TR height=20>
          <TD>Valor por omissão deste parâmetro:</TD>
          <TD></TD>
        </TR>
        <TR>
          <TD width="100%">
  ...
  <TR>
    ...
    <TD style="BORDER-TOP: #ffffff 0px solid" align=right><BUTTON id=butBegin 
      onclick=DLG_applyParamchanges();>OK</BUTTON>&nbsp;<BUTTON id=cmdDialogCancel 
      onclick=DLG_cancel();>Cancela</BUTTON></TD></TR></TBODY></TABLE>
